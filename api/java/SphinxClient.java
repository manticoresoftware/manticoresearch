...
public class SphinxClient
{
	...
	protected byte[] _GetResponse(Socket sock, int client_ver) throws SphinxException
	{
		...
		try {
			...
			// check version
			if (ver < client_ver) {
				this._warning = "searchd command v." + (ver >> 8) + "." + (ver & 0xff) + "older than client's v." + (client_ver >> 8) + "." + (client_ver & 0xff) + ", some options might not work";
			}
		} catch (IOException e) {
			if (len != 0) {
	...
	{
		if (myAssert(mode == SPH_MATCH_ALL || mode == SPH_MATCH_ANY || mode == SPH_MATCH_PHRASE || mode == SPH_MATCH_BOOLEAN || mode == SPH_MATCH_EXTENDED, "SetMatchMode: Invalid match mode")) {
			return;
		}
		this._mode = mode;
	}
	...
	public void SetSortMode(int mode, String sortby) throws SphinxException
	{
		...
		if (myAssert(mode == SPH_SORT_RELEVANCE || mode == SPH_SORT_ATTR_DESC || mode == SPH_SORT_ATTR_ASC || mode == SPH_SORT_TIME_SEGMENTS || mode == SPH_SORT_EXTENDED, "SetSortMode: Invalid sortby mode")) {
			return;
		}
		if (myAssert(mode == SPH_SORT_RELEVANCE || (sortby != null && sortby.length() > 0), "SetSortMode: Empty sortBy for non-SPH_SORT_RELEVANCE")) {
			return;
		}
	...
	{
		if (myAssert(latitudeAttrName != null && latitudeAttrName.length() > 0, "SetGeoAnchor: no latitudeAttrName attribute name")) {
			return;
		}
		if (myAssert(longitudeAttrName != null && longitudeAttrName.length() > 0, "SetGeoAnchor: no longitudeAttrName attribute name")) {
			return;
		}
	...
	public void SetGroupBy(String attribute, int func, String groupsort) throws SphinxException
	{
		...
		if (myAssert(func == SPH_GROUPBY_DAY || func == SPH_GROUPBY_WEEK || func == SPH_GROUPBY_MONTH || func == SPH_GROUPBY_YEAR || func == SPH_GROUPBY_ATTR || func == SPH_GROUPBY_ATTRPAIR, "SetGroupBy: invalid func")) {
			return;
		}
	...
	public SphinxResult Query(String query, String index) throws SphinxException
	{
		...
		if (myAssert(this._reqs == null || this._reqs.size() == 0, "Multiquery mode in da house. There are " + this._reqs.size() + " queries in queue. Please use AddQuery(...) instead")) {
			return null;
		}
	...
	public synchronized int AddQuery(String query, String index) throws SphinxException
	{
		...
		try {
			...
			out.writeInt(this._offset);
			out.writeInt(this._limit);
			out.writeInt(this._mode);
			out.writeInt(this._sort);
			...
			out.writeInt(0);
			out.writeInt(this._minId);
			out.writeInt(this._maxId);
			...
			// anchor point
			if (this._latitudeAttrName == null || this._latitudeAttrName.length() == 0 || this._longitudeAttrName == null || this._longitudeAttrName.length() == 0) {
				out.writeInt(0);
			} else {
				out.writeInt(1);
