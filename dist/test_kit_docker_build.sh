...
start_mysql_src="$repo_root/dist/start-mysql.sh"
...
docker exec manticore-test-kit bash -c \
	'echo "apt list before update" && apt list --installed|grep manticore && apt-get -y update && echo "apt list after update" && apt list --installed|grep manticore && apt-get -y install manticore-galera && apt-get -y remove manticore-repo manticore && rm /etc/apt/sources.list.d/manticoresearch.list && apt-get update -y && dpkg -i --force-confnew /build/*.deb && printf "#!/bin/sh\nexit 101\n" > /usr/sbin/policy-rc.d && chmod +x /usr/sbin/policy-rc.d && apt-get install -y libxml2 libcurl4 libonig5 libzip4 librdkafka1 curl neovim git apache2-utils iproute2 bash mysql-server mysql-client php-cli php-mysql php-curl php-xml && killall mysqld mysqld_safe 2>/dev/null || true && rm -f /usr/sbin/policy-rc.d && php_cmd="$(command -v php || true)" && php_real="$(readlink -f "$php_cmd" || true)" && mkdir -p /usr/local/bin && [ -n "$php_real" ] && [ -x "$php_real" ] && install -m 0755 "$php_real" /usr/local/bin/php-real || true && [ -x /usr/bin/manticore-executor-dev ] && ln -sf /usr/bin/manticore-executor-dev /usr/bin/php && apt-get clean -y'
...
docker cp "$start_mysql_src" manticore-test-kit:/usr/local/bin/start-mysql
docker exec manticore-test-kit chmod +x /usr/local/bin/start-mysql
