...
## Source Changes (This Branch)
This section ties the kill-dictionary behavior to the specific functions that implement each stage, so you can follow the flow end to end.

### 1) Configuration and Runtime Controls
These decide whether kill-stats are used and when they rebuild.
- `src/searchd.cpp::ConfigureSearchd()` reads `kill_dictionary` and `kill_dictionary_idle_timeout` from the `searchd` section.
- `src/searchd.cpp::HandleSetGlobal()` accepts `SET GLOBAL kill_dictionary` and `SET GLOBAL kill_dictionary_idle_timeout` to switch modes or timeouts at runtime.
- `src/searchd.cpp::HandleMysqlShowVariables()` exposes those values in `SHOW [GLOBAL] VARIABLES`.

### 2) Persistence on Disk (.spks)
Kill-stats are stored per disk chunk in a new file:
- `src/indexfiles.h` / `src/indexfiles.cpp` add `SPH_EXT_SPKS` so `.spks` is a recognized index file.
- `src/sphinx.cpp::CSphIndex_VLN::FlushKillStatsFile()` writes `.spks` when a chunk’s kill-stats are rebuilt. If the file is missing, it forces a rebuild to avoid stale state.
- `src/sphinx.cpp::CSphIndex_VLN::HasKillStatsDirty()` treats missing `.spks` as dirty so rebuilds happen automatically.

### 3) Rebuild Scheduling (idle mode)
Idle mode rebuilds kill-stats in background after writes go quiet.
- `src/sphinxrt.cpp::SetRtKillStatsIdleTimeout()` / `GetRtKillStatsIdleTimeout()` store the idle timeout in seconds.
- `src/sphinxrt.cpp::RtIndex_c::ConditionalKillStats()` triggers rebuilds after the idle timeout; it now runs multiple chunk rebuilds in parallel, and labels jobs as `KILLSTATS <chunk>` for `SHOW THREADS`.
  - The job counter is held by a shared `Coro::Waitable_T<int>` so the wait/notify object stays alive until all background jobs finish.

### 4) Observability for rebuild state
These expose whether chunks still need a rebuild.
- `src/sphinxrt.cpp::RtIndex_c::GetStatus()` increments `kill_dictionary_dirty_chunks` for each disk chunk that reports dirty kill-stats.
- `src/searchd.cpp::AddDiskIndexStatus()` adds `kill_dictionary_dirty_chunks` to `SHOW TABLE <rt> STATUS`.
...
- `src/sphinxrt.cpp::RtIndex_c::RtQwordSetupSegment()` recomputes `uDocs/uHits` for RAM segments with dead rows by walking doclists and skipping dead row IDs.
- If all docs for a word are dead, the function now returns `false` (treats the word as not found) to avoid asserting in `RtQwordSetup()`.
