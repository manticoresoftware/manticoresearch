...
当使用 [Update](../Data_creation_and_modification/Updating_documents/UPDATE.md) 实时修改文档属性时，更改首先写入属性的内存副本。这些更新发生在内存映射文件中，这意味着操作系统决定何时将更改写入磁盘。在 searchd 正常关闭（由 SIGTERM 信号触发）时，所有更改都会强制写入磁盘。

您还可以指示 searchd 定期将这些更改写回磁盘，以防止数据丢失。这些刷新的间隔由 `attr_flush_period` 决定，以秒为单位指定（或使用 [special_suffixes](../Server_settings/Special_suffixes.md)）。

默认值为 0，它会禁用定期刷新。但是，刷新仍然会在正常关机期间发生。
