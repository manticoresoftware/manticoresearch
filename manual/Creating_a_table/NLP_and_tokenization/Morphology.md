...
The Chinese language segmentation can be performed using [ICU](http://site.icu-project.org/) or [Jieba](https://github.com/yanyiwu/cppjieba). Both libraries provide more precise segmentation compared to n-grams, but are slightly slower. The [charset_table](../../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#charset_table) must include all Chinese characters, which can be done by using the "cont", "cjk" or "chinese" character sets. When "morphology=icu_chinese" or "morphology=jieba_chinese" are specified, the documents are first pre-processed by ICU/Jieba. Then, the result is processed by the tokenizer according to the charset_table, and finally, other morphology processors specified in the "morphology" option are applied. Only those parts of texts that contain Chinese are passed to ICU/Jieba for segmentation, while others can be modified by different means such as different morphologies or charset_table.
