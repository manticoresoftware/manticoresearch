...
This will generate a Source RPM (`.src.rpm` file) containing all the source code. The SRPM is pre-configured with Galera disabled to simplify building.
...
This will generate all the component RPMs that would normally be built:
- `manticore-server-core-*.rpm` - Core server daemon
- `manticore-server-*.rpm` - Server configuration and systemd service
- `manticore-tools-*.rpm` - Command-line tools (indexer, indextool, etc.)
- `manticore-common-*.rpm` - Common files and configuration
- `manticore-devel-*.rpm` - Development headers
...
**Option 1: Install cmake in the expected location**
```bash
# Create a symlink to your system cmake
sudo mkdir -p /cmake-3.31.2-linux-x86_64/bin
sudo ln -sf /usr/bin/cmake /cmake-3.31.2-linux-x86_64/bin/cmake
```

**Option 2: Extract and modify the spec file**
```bash
# Install the SRPM to get the spec file
rpm -ivh manticore-*.src.rpm

# Edit the spec file to use system cmake
sed -i 's|/cmake-3.31.2-linux-x86_64/bin/cmake|cmake|g' ~/rpmbuild/SPECS/manticore.spec

# Build from the modified spec
rpmbuild -ba ~/rpmbuild/SPECS/manticore.spec
```

**Option 3: Use rpmbuild with defines**
```bash
# Override the cmake path during build
rpmbuild --rebuild manticore-*.src.rpm --define "cmake_cmd cmake"
```

**Note**: Option 3 is generally **not applicable** for SRPMs generated by CPack because the cmake path is hardcoded in the spec file, not using a variable that can be overridden. Options 1 and 2 are more reliable.

#### Troubleshooting missing dependencies

If you encounter errors about missing Boost components or other dependencies, you may need to install additional development packages:

**For Boost-related errors:**
```bash
# Install Boost development packages
dnf install -y boost-devel

# If still having issues, install static libraries
dnf install -y boost-static

# Check what boost packages are available if needed
dnf list available | grep boost
```

**For other common missing dependencies:**
```bash
# Install additional build tools and libraries
dnf install -y git make flex bison gcc-c++ cmake
dnf install -y libcurl-devel openssl-devel zlib-devel
dnf install -y expat-devel mariadb-connector-c-devel postgresql-devel unixODBC-devel
dnf install -y systemd-devel

# Enable EPEL repository if needed (for additional packages)
dnf install -y epel-release
```

**If you need to enable Galera (disabled by default in SRPMs):**
```bash
# Extract the SRPM and modify the spec file to enable Galera
rpm -ivh manticore-*.src.rpm

# Change the cmake flag to enable Galera
sed -i 's/-DWITH_GALERA=0/-DWITH_GALERA=1/g' ~/rpmbuild/SPECS/manticore.spec

# You'll also need to install Galera development dependencies
dnf install -y galera-devel
...
You can also build the SRPM with custom options by extracting and modifying the spec file:

```bash
# Install the SRPM to get the spec file and sources
rpm -ivh manticore-*.src.rpm

# The spec file is now in ~/rpmbuild/SPECS/
# Sources are in ~/rpmbuild/SOURCES/

# Build with custom options
rpmbuild -ba ~/rpmbuild/SPECS/manticore.spec --define "custom_option value"
```

#### Using a custom build directory

If you want to use a different build directory:

```bash
# Create custom build directory structure
mkdir -p /tmp/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}

# Build with custom topdir
