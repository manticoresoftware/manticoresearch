...
- `preserve`: `0` or `1` (default: `0`). When set to `1`, preserves the original form of words in phrases even when not found in the database during fuzzy matching. When set to `0`, words not found with fuzzy matching are ignored when building matching for fuzziness. Particularly useful for short words
