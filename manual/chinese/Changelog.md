...
此版本新增了分块和多向量自动嵌入、安全的 UTF-8 标识符、可配置的嵌入输入限制、德语 sharp-s 规范化、更丰富的 `manticore-load` 基准测试，以及支持认证并感知云环境的备份；同时修复了 KNN、混合与分组搜索、表结构变更、批量写入、复制、binlog 恢复、兼容性和关闭行为中的问题。
...
* 🪲 [v29.3.11](https://github.com/manticoresoftware/manticoresearch/tree/29.3.11) [PR #4879](https://github.com/manticoresoftware/manticoresearch/pull/4879) 修复了 [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array) 属性上的 [LENGTH()](Functions/Arrays_and_conditions_functions.md#LENGTH%28%29) 返回内部存储 word 数而不是向量数量的问题；空数组返回 `0`，修正后的值会在选择、过滤和分组中一致使用。
