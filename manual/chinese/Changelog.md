...
此版本新增了分块和多向量自动嵌入、安全的 UTF-8 标识符、可配置的嵌入输入限制、德语 sharp-s 规范化、更丰富的 `manticore-load` 基准测试、支持认证且感知云环境的备份，并默认启用内存映射列式属性；同时修复了 KNN、混合和分组搜索、表结构变更、批量写入、复制、binlog 恢复、兼容性以及关闭行为中的问题。
...
### 新功能和改进
* 🆕 [v29.9.0](https://github.com/manticoresoftware/manticoresearch/tree/29.9.0) [PR #4898](https://github.com/manticoresoftware/manticoresearch/pull/4898) 将默认的 [access_columnar_attrs](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#Accessing-table-files) 模式从缓冲式 `file` 读取改为 `mmap`，使操作系统可以按需缓存列式属性数据，而无需预读。若要保留之前的行为，请显式设置 `access_columnar_attrs='file'`；现在 `ALTER TABLE` 也会使用已配置的访问模式重新打开被替换的列式存储。
* 🆕 [v29.8.0](https://github.com/manticoresoftware/manticoresearch/tree/29.8.0) [Backup Issue #134](https://github.com/manticoresoftware/manticoresearch-backup/issues/134) 将 [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) 更新到 1.11.0，使 [S3 备份和恢复](Securing_and_compacting_a_table/Backup_and_restore.md#S3-storage-support) 在未设置静态密钥时可使用 AWS SDK 凭证提供程序链，包括 IRSA、共享凭证、ECS 任务角色和 EC2 实例配置文件；并新增了对临时凭证 `AWS_SESSION_TOKEN` 的支持。
* 🆕 [v29.7.0](https://github.com/manticoresoftware/manticoresearch/tree/29.7.0) [Issue #4816](https://github.com/manticoresoftware/manticoresearch/issues/4816) 为本地自动嵌入列新增可选的 [MAX_INPUT_TOKENS](Searching/KNN.md#Auto-Embeddings-%28Recommended%29)，允许在生成嵌入前截断每段输入文本，以限制长文档推理时间并减少重试导致的内存耗尽。可在 `CREATE TABLE` 时设置，也可通过 `ALTER TABLE ... MODIFY COLUMN` 修改，且不会重新嵌入已有行；设置为 `0` 或省略时会保留模型自身的限制。
* 🆕 [v29.6.0](https://github.com/manticoresoftware/manticoresearch/tree/29.6.0) [manticore-load Issue #26](https://github.com/manticoresoftware/manticore-load/issues/26) 将 [manticore-load](https://github.com/manticoresoftware/manticore-load) 更新到 1.28.0，新增可选的 `--http` 模式，用于通过 Manticore 的 HTTP JSON API 进行基准测试：写入使用 `/bulk`，搜索使用 `/search`，HTTP 默认端口为 `9308`，`--table` 用于选择目标表。
* 🆕 [v29.5.0](https://github.com/manticoresoftware/manticoresearch/tree/29.5.0) [manticore-load PR #28](https://github.com/manticoresoftware/manticore-load/pull/28) 将 `manticore-load` 更新到 1.27.0，在实时进度中加入本地 `searchd` RSS，在最终报告中加入峰值 RSS、磁盘和 CPU 使用情况，并在多命令工作负载中对所有表进行汇总监控。
* 🆕 [v29.4.0](https://github.com/manticoresoftware/manticoresearch/tree/29.4.0) [MCL Issue #190](https://github.com/manticoresoftware/columnar/issues/190) 为自动嵌入新增[分块策略](Searching/KNN.md#Chunking-strategies)：`mean` 将长文档嵌入为一个平均向量，而 `fixed`、`recursive` 和 `sentence` 会在 `float_vector_array` 中存储多个可搜索向量。新的 `MAX_TOKENS`、`OVERLAP_TOKENS` 和 `MAX_CHUNKS` 选项用于控制分块大小、重叠和数量；现有的 `truncate` 行为仍为默认值。
* 🆕 [v29.3.12](https://github.com/manticoresoftware/manticoresearch/tree/29.3.12) [Issue #4827](https://github.com/manticoresoftware/manticoresearch/issues/4827) 新增可选的[德语 AOT sharp-s 规范化](Creating_a_table/NLP_and_tokenization/Supported_languages.md#German-AOT-morphology-and-sharp-s)，通过 `charset_table=non_cont,german` 搭配 `lemmatize_de_v2` 或 `lemmatize_de_v2_all` 使用，使 `ß`、`ẞ` 和 `ss` 在普通整词搜索中等价匹配，同时 `index_exact_words=1` 会在精确词查询中保留它们的区别。现有德语词形模式保持不变；将已有数据的表切换到 v2 需要重建或重放数据。
* 🆕 [v29.3.4](https://github.com/manticoresoftware/manticoresearch/tree/29.3.4) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) 将 [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) 更新到 1.10.4，使其 CLI 可通过 `--user` 和 `--password` 或 `--token` 等[认证选项](Securing_and_compacting_a_table/Backup_and_restore.md#Arguments)认证 HTTP 请求，并确保备份失败或中断时会解冻已选择的表。
* 🆕 [v29.3.0](https://github.com/manticoresoftware/manticoresearch/tree/29.3.0) [Issue #4770](https://github.com/manticoresoftware/manticoresearch/issues/4770) 在 RT、percolate、distributed、template 和 plain 表，SQL DDL 与表达式，全文字段选择器，以及推断出的源 schema 中新增一致的[安全 UTF-8 表名、字段名和属性名](Creating_a_table/Data_types.md#Table-and-field-name-syntax)，允许使用中文、西里尔文等本地化标识符，而无需重命名源 schema。
* 🆕 [v29.2.0](https://github.com/manticoresoftware/manticoresearch/tree/29.2.0) [Issue #4099](https://github.com/manticoresoftware/manticoresearch/issues/4099) 为实时表新增 [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array)，允许每个文档存储并搜索多个 KNN 向量，例如文本分块、产品图片或视频关键帧。
* 🆕 [v29.1.0](https://github.com/manticoresoftware/manticoresearch/tree/29.1.0) [manticore-load PR #24](https://github.com/manticoresoftware/manticore-load/pull/24) 将 `manticore-load` 更新到 1.26.0，新增 `--worker-init` 和 `--worker-finalize` SQL 钩子，用于围绕基准测试工作负载进行连接级初始化和有序并发收尾。

### 问题修复
* 🪲 [v29.8.4](https://github.com/manticoresoftware/manticoresearch/tree/29.8.4) [Issue #4889](https://github.com/manticoresoftware/manticoresearch/issues/4889) 修复了[混合搜索](Searching/Hybrid_search.md#Attribute-filters)会忽略针对 `weight()` 或由其派生的表达式和别名的过滤器的问题；依赖权重的过滤器现在会在融合后基于最终文本权重执行。此类过滤器不支持出现在 `OR` 过滤树中，现在会返回明确错误。
* 🪲 [v29.8.2](https://github.com/manticoresoftware/manticoresearch/tree/29.8.2) [Issue #4893](https://github.com/manticoresoftware/manticoresearch/issues/4893) 修复了有效的固定长度 gzip 压缩 [/bulk](Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md#Bulk-adding-documents) 请求在压缩请求体跨多次 socket 读取到达时返回 HTTP 400 的问题；socket 读取边界不再影响解压或写入。
* 🪲 [v29.8.1](https://github.com/manticoresoftware/manticoresearch/tree/29.8.1) [Issue #4884](https://github.com/manticoresoftware/manticoresearch/issues/4884) 修复了 Elasticsearch 兼容的 `/_bulk` 请求在条目失败时返回请求级 HTTP `409`，导致 Fluent Bit 等客户端重试已应用写入的问题；已处理的批次现在返回 HTTP `200`，保留成功条目，将重复的 `create` 操作报告为条目级 `409` `version_conflict_engine_exception` 错误，包括同一批次内的重复项，并在请求级拒绝格式错误的 action 元数据。
* 🪲 [v29.7.2](https://github.com/manticoresoftware/manticoresearch/tree/29.7.2) [Issue #4872](https://github.com/manticoresoftware/manticoresearch/issues/4872) 修复了 [ALTER TABLE](Updating_table_schema_and_settings.md#Updating-table-schema-in-RT-mode) 在带有[自动嵌入](Searching/KNN.md#Auto-Embeddings-%28Recommended%29)的 RT 表上留下过期属性和源字段元数据的问题，该问题会静默丢弃随后通过 `INSERT` 或 `REPLACE` 写入新增 MVA 列的值，并可能在之前的列被删除后导致嵌入错位。升级前丢失的值必须从源数据或备份中恢复。
* 🪲 [v29.7.1](https://github.com/manticoresoftware/manticoresearch/tree/29.7.1) [MCL PR #205](https://github.com/manticoresoftware/columnar/pull/205) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 14.2.1，修复了 `ALTER TABLE ... MODIFY COLUMN ... MAX_INPUT_TOKENS` 后自动嵌入模型可能仍留在内存中的问题，并防止不同的 `API_TIMEOUT` 与 `MAX_INPUT_TOKENS` 设置在模型缓存中冲突并错误复用同一模型。
* 🪲 [v29.5.1](https://github.com/manticoresoftware/manticoresearch/tree/29.5.1) [MCL PR #204](https://github.com/manticoresoftware/columnar/pull/204) 修复了 [KNN](Searching/KNN.md#KNN-vector-search) HNSW 索引构建偶尔会让已索引向量无法被 KNN 查询访问的问题；新构建的索引现在会在保存前修复断开的 0 层图节点。现有索引不会自动重写，如受影响应重建。
* 🪲 [v29.3.13](https://github.com/manticoresoftware/manticoresearch/tree/29.3.13) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) 修复了启用认证时 SQL [BACKUP](Securing_and_compacting_a_table/Backup_and_restore.md#General-syntax-of-BACKUP) 和 `manticore-backup` 失败的问题；备份操作员现在可使用用户名/密码或 bearer token 认证，并需要 `backup ON *` 加上每个所选表的 `read` 权限，或用于完整备份的不受限 `read ON *`。降级到不支持 `backup` 操作的版本前，请移除所有 `backup` 授权。
* 🪲 [v29.3.11](https://github.com/manticoresoftware/manticoresearch/tree/29.3.11) [PR #4879](https://github.com/manticoresoftware/manticoresearch/pull/4879) 修复了 [float_vector_array](Functions/Arrays_and_conditions_functions.md#LENGTH%28%29) 属性上的 [LENGTH()](Creating_a_table/Data_types.md#Float-vector-array) 返回内部存储 word 数而不是向量数量的问题；空数组返回 `0`，修正后的值会在选择、过滤和分组中一致使用。
* 🪲 [v29.3.10](https://github.com/manticoresoftware/manticoresearch/tree/29.3.10) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) 将 Buddy 更新到 4.4.3，以保留已认证的 SQL 用户并将 Buddy 内部 bearer token 传递给 manticore-backup 请求，为 `BACKUP` 提供依赖侧的认证路径；完整的 `auth=1` 支持，包括守护进程侧的 `backup` 权限检查，需要 v29.3.13。
* 🪲 [v29.3.9](https://github.com/manticoresoftware/manticoresearch/tree/29.3.9) [Issue #4639](https://github.com/manticoresoftware/manticoresearch/issues/4639) 修复了带 `GROUP BY` 的[混合搜索](Searching/Hybrid_search.md)返回空分桶或缺失分桶的问题，包括 MVA 分组；分组结果现在会保留所有分桶，并遵循最终的 `ORDER BY` 和 `WITHIN GROUP ORDER BY`。
* 🪲 [v29.3.8](https://github.com/manticoresoftware/manticoresearch/tree/29.3.8) [Issue #4774](https://github.com/manticoresoftware/manticoresearch/issues/4774) 修复了在使用 document-ID 查找索引时，对有符号表示为负数的[文档 ID](Creating_a_table/Data_types.md#Document-ID) 进行相等、`IN` 和范围过滤不返回任何行的问题；查找过滤现在遵循索引的无符号 ID 顺序。
* 🪲 [v29.3.7](https://github.com/manticoresoftware/manticoresearch/tree/29.3.7) [Issue #4856](https://github.com/manticoresoftware/manticoresearch/issues/4856) 修复了多分块 RT 表 [GROUP BY](Searching/Grouping.md) 查询在分组数超过 `max_matches` 时可能返回重复分组和错误拆分计数，或在按 `COUNT(DISTINCT ...)` 结果排序且选择列表包含表达式时导致 `searchd` 挂起的问题。
* 🪲 [v29.3.6](https://github.com/manticoresoftware/manticoresearch/tree/29.3.6) [Issue #4826](https://github.com/manticoresoftware/manticoresearch/issues/4826) 将 [Buddy](Installation/Manticore_Buddy.md#Manticore-Buddy) 更新到 4.4.2，修复了当内部 `SELECT ...; SHOW META` 响应包含大整数 ID 或看起来像数字的长字符串时，[Kafka 物化视图](Integration/Kafka.md#Materialized-view) worker 因 `Invalid JSON found` 而永久停止的问题；响应值现在会无损保留，写入会继续进行。
* 🪲 [v29.3.5](https://github.com/manticoresoftware/manticoresearch/tree/29.3.5) [Issue #4822](https://github.com/manticoresoftware/manticoresearch/issues/4822) 修复了对 distributed 表执行 JSON `/search` 查询时，如果按字符串属性排序并跨多个本地表聚合 JSON 字段，`searchd` 会间歇性崩溃的问题。
* 🪲 [v29.3.3](https://github.com/manticoresoftware/manticoresearch/tree/29.3.3) [Issue #4860](https://github.com/manticoresoftware/manticoresearch/issues/4860) 修复了 `DROP TABLE` 与向使用[自动嵌入](Searching/KNN.md#Auto-Embeddings-%28Recommended%29)的 RT 表进行中的 `/bulk` 插入发生竞争时，`searchd` 崩溃的问题；事务现在会针对仍然存在的表完成，或在表被删除或替换时安全失败。
* 🪲 [v29.3.2](https://github.com/manticoresoftware/manticoresearch/tree/29.3.2) [Issue #4791](https://github.com/manticoresoftware/manticoresearch/issues/4791) 修复了在带本地分片的 distributed 和 sharded 表上执行 HTTP [KNN 查询](Searching/KNN.md#KNN-vector-search)时，1-bit 量化触发重新评分可能导致协调节点崩溃或返回错误最近邻的问题；合并后的分片结果不再进行第二次重新评分。
* 🪲 [v29.3.1](https://github.com/manticoresoftware/manticoresearch/tree/29.3.1) [PR #4864](https://github.com/manticoresoftware/manticoresearch/pull/4864) 修复了多语句请求的第二条语句通过组合 `MATCH()`、`KNN()` 和 `OPTION fusion_method='rrf'` 执行[混合搜索](Searching/Hybrid_search.md)时，`searchd` 崩溃的问题。
* 🪲 [v29.2.9](https://github.com/manticoresoftware/manticoresearch/tree/29.2.9) [Issue #4837](https://github.com/manticoresoftware/manticoresearch/issues/4837) 修复了将当前格式 killlist 应用于带旧版 JSON 头的较旧 plain 表时，[indextool --apply-killlists](Miscellaneous_tools.md#indextool) 崩溃的问题；`indextool` 现在会在处理查找数据前读取表实际的磁盘格式版本。
* 🪲 [v29.2.8](https://github.com/manticoresoftware/manticoresearch/tree/29.2.8) [Issue #4854](https://github.com/manticoresoftware/manticoresearch/issues/4854) 通过在 ubertests 之外同时运行 GoogleTest、API 一致性、`indextool` 和其他由 CTest 管理的测试，恢复了完整的 CI 测试覆盖。
* 🪲 [v29.2.7](https://github.com/manticoresoftware/manticoresearch/tree/29.2.7) [Issue #4842](https://github.com/manticoresoftware/manticoresearch/issues/4842) 修复了使用远程自动嵌入的实时表执行 [ALTER TABLE ... RENAME](Updating_table_schema_and_settings.md#Renaming-a-real-time-table) 时因 `Invalid API key for remote model` 失败的问题；表复制现在会保留隐藏的嵌入 API 密钥，且不会在 `SHOW CREATE TABLE` 中暴露。
* 🪲 [v29.2.6](https://github.com/manticoresoftware/manticoresearch/tree/29.2.6) [PR #4861](https://github.com/manticoresoftware/manticoresearch/pull/4861) 修复了带额外过滤条件的查询中 [KNN 搜索](Searching/KNN.md#KNN-vector-search)的性能回退：当 HNSW 迭代器已排除没有向量的文档时，避免冗余的 `knn_dist` 预过滤，同时在暴力搜索、全文、分组和分面 KNN 路径上保持正确排除。
* 🪲 [v29.2.5](https://github.com/manticoresoftware/manticoresearch/tree/29.2.5) [Issue #4833](https://github.com/manticoresoftware/manticoresearch/issues/4833) 修复了带 UUID 文档 ID 的表上 SQL 查询返回空的已存储 `text` 字段的问题；现在在显式选择或通过 `SELECT *` 选择时，会正确取回已存储字段。
* 🪲 [v29.2.4](https://github.com/manticoresoftware/manticoresearch/tree/29.2.4) [Issue #4828](https://github.com/manticoresoftware/manticoresearch/issues/4828) 通过支持 Sequel Ace 5.3.1+ 的 `CAST(DATABASE() AS BINARY)` 和 `CAST(@@character_set_client AS BINARY)` 探测，恢复了兼容性，避免后续每个查询都失败。
* 🪲 [v29.2.3](https://github.com/manticoresoftware/manticoresearch/tree/29.2.3) [PR #4619](https://github.com/manticoresoftware/manticoresearch/pull/4619) 修复了节点重新加入后 sharded 表重新平衡期间，优雅执行 `searchd --stopwait` 会挂起的问题；正在进行的分片传输现在会在关闭期间停止，分离的 worker 会使用有界等待。
* 🪲 [v29.2.2](https://github.com/manticoresoftware/manticoresearch/tree/29.2.2) [Issue #4793](https://github.com/manticoresoftware/manticoresearch/issues/4793) 修复了 plain 表配置的 [path](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#path) 包含缺失父目录时，`indexer` 无法构建表的问题；现在只要最近的已有父目录可写，就会自动创建缺失的目录树。
* 🪲 [v29.2.1](https://github.com/manticoresoftware/manticoresearch/tree/29.2.1) [PR #4832](https://github.com/manticoresoftware/manticoresearch/pull/4832) 更正并补全了最近新增源文件中的版权和 GPL 许可证头。
* 🪲 [v29.1.1](https://github.com/manticoresoftware/manticoresearch/tree/29.1.1) [Issue #4808](https://github.com/manticoresoftware/manticoresearch/issues/4808) 修复了从带 v15 binlog 的版本升级时，在关闭期间可能崩溃或从表元数据中遗漏新刷新的 RT 磁盘分块的问题；兼容的恢复日志现在会重放，已完成的分块会在干净关闭前发布，不支持的恢复格式会安全失败。
* 🪲 [v29.0.6](https://github.com/manticoresoftware/manticoresearch/tree/29.0.6) [Issue #4811](https://github.com/manticoresoftware/manticoresearch/issues/4811) 改进了致命 binlog 重放诊断信息，使其指明匹配的 [--replay-flags](Starting_the_server/Manually.md#searchd-command-line-options) 恢复选项：缺失 binlog 文件对应 `ignore-open-errors`，事务重放失败对应 `ignore-trx-errors`。
* 🪲 [v29.0.5](https://github.com/manticoresoftware/manticoresearch/tree/29.0.5) [Issue #4784](https://github.com/manticoresoftware/manticoresearch/issues/4784) 修复了带否定条件的 JSON `/search` 查询丢失 [NEAR 和邻近操作符](Searching/Full_text_matching/Operators.md)距离的问题，该问题会导致有效的位置查询因零距离错误失败或行为不正确。
* 🪲 [v29.0.4](https://github.com/manticoresoftware/manticoresearch/tree/29.0.4) [Issue #4788](https://github.com/manticoresoftware/manticoresearch/issues/4788) 修复了按字符串属性排序的 [LEFT JOIN](Searching/Joining.md) 查询会在客户端结果中返回内部 `@int_attr_*` 辅助列的问题。
* 🪲 [v29.0.3](https://github.com/manticoresoftware/manticoresearch/tree/29.0.3) [PR #4790](https://github.com/manticoresoftware/manticoresearch/pull/4790) 修复了带有负数或不可能元素计数的格式错误二进制 API `SEARCH` 请求可能终止 `searchd` 的问题；无效计数现在会以协议错误拒绝，包括 `max_batch_queries=0` 的情况。
...
### 新功能与改进
* 🆕 [v28.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.0) [ PR #4707](https://github.com/manticoresoftware/manticoresearch/pull/4707) 通过在 `rescore` 阶段批量计算距离，提升了 [KNN search](Searching/KNN.md#KNN-vector-search) 性能，降低了重新计算并重新排序最终全精度距离时的开销。
* 🆕 [v28.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.0) [Issue #688](https://github.com/manticoresoftware/manticoresearch-buddy/issues/688) [PR #4690](https://github.com/manticoresoftware/manticoresearch/pull/4690) Buddy 已更新至 4.2.0 版本，并为 [CREATE CHAT MODEL](Searching/Conversational_search.md) 添加了 `custom_prompt` 支持，使[对话式搜索](Searching/Conversational_search.md)的回答可以遵循自定义指令，例如引用规则和回复风格。同时还修复了 [问题 #686](https://github.com/manticoresoftware/manticoresearch-buddy/issues/686)——Buddy 工作进程中的文件描述符（FD）泄漏问题。
* 🆕 [v28.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.2.0) [ PR #4596](https://github.com/manticoresoftware/manticoresearch/pull/4596) 在 [快速入门指南](Quick_start_guide.md) 中新增了 `curl`/`wget` 管道到 shell 的安装器，使你可以用一条启动命令安装、升级并选择 Manticore 版本。
* 🆕 [v28.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.0) [ Issue #683](https://github.com/manticoresoftware/manticoresearch-buddy/issues/683) [ PR #685](https://github.com/manticoresoftware/manticoresearch-buddy/pull/685) 将 Buddy 更新到 4.1.0，为 [conversational search](Searching/Conversational_search.md) 增加了 HTTP JSON 支持，因此除了 SQL `CALL CHAT` 之外，还可以通过 `/search` JSON API 发送聊天请求。
* 🆕 [v27.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.4.0) [ PR #169](https://github.com/manticoresoftware/columnar/pull/169) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.7.0，新增 [embeddings_threads](Server_settings/Searchd.md#embeddings_threads)，用于限制自动 embedding 插入、`ALTER TABLE ... REBUILD KNN` 以及文本转向量 KNN 查询所使用的 CPU 线程数。
* 🆕 [v27.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.3.0) [ PR #4624](https://github.com/manticoresoftware/manticoresearch/pull/4624) 为 [faceted search](Searching/Faceted_search.md) 增加了零计数 facet 桶支持，包括 SQL `ZEROES` 和 JSON `"zeroes": true`，因此 `max` 模式的 facets 即使当前筛选后的计数为 `0`，也可以保留可见桶。
* 🆕 [v27.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.2.0) [ PR #4608](https://github.com/manticoresoftware/manticoresearch/pull/4608) 新增 [CREATE TABLE ... profile='relevance'](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#profile)，并为 [ranker](Searching/Options.md#ranker) 和 [boolean_mode](Searching/Options.md#boolean_mode) 增加了按表保存的默认值，使表可以内置全文排序和隐式关键词组合行为，而无需在查询中重复指定选项。
...
* 🪲 [v28.3.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.4) [ Issue #4691](https://github.com/manticoresoftware/manticoresearch/issues/4691) [ PR #4696](https://github.com/manticoresoftware/manticoresearch/pull/4696) 修复了经过身份验证的 MySQL 协议启动兼容性，因此 Connector/J 和 PyMySQL 客户端可以完成 native-password 登录流程，且无害的会话 `SET` 语句在认证下也不再失败。
* 🪲 [v28.3.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.3) [ Issue #4641](https://github.com/manticoresoftware/manticoresearch/issues/4641) [ PR #4649](https://github.com/manticoresoftware/manticoresearch/pull/4649) 修复了 AOT 混合关键词处理，使其遵循已配置的 [blend_mode](Searching/Options.md#expand_blended)，从而在索引和关键词提取中一致地恢复去分隔符的变体。
* 🪲 [v28.3.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.2) [ PR #186](https://github.com/manticoresoftware/columnar/pull/186) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新为包含显式的 `DOCS_PER_CHUNK` 构建修复，防止在新的基于子模块的 columnar 工作流中出现捆绑 KNN 库构建/打开失败。
* 🪲 [v28.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.1) [ Issue #4432](https://github.com/manticoresoftware/manticoresearch/issues/4432) [ PR #4689](https://github.com/manticoresoftware/manticoresearch/pull/4689) 修复了当某个节点在集群中只剩自己时的复制集群启动问题，因此剩余的单节点可以正常恢复，而不是启动失败。
* 🪲 [v28.1.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.5) [ PR #4687](https://github.com/manticoresoftware/manticoresearch/pull/4687) 修复了分布式存储字段获取逻辑，在使用远端 `GETFIELD` 响应前先进行校验，避免接受格式错误的代理响应。
* 🪲 [v28.1.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.4) [ PR #4676](https://github.com/manticoresoftware/manticoresearch/pull/4676) 修复了迁移后的 MCL 子模块构建和打包流程，在守护进程端切换到子模块之后，恢复了对捆绑 columnar 库的正确 CI、包和依赖解析处理。
* 🪲 [v28.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.3) [ PR #4663](https://github.com/manticoresoftware/manticoresearch/pull/4663) 修复了多个内存泄漏和清理问题，涉及可选库加载、SSL 密钥处理、RT 字典清理、定时器线程关闭以及 facet-zeroes 聚合路径。
* 🪲 [v28.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.2) [ Issue #4672](https://github.com/manticoresoftware/manticoresearch/issues/4672) [ PR #4679](https://github.com/manticoresoftware/manticoresearch/pull/4679) 修复了在多 chunk RT 表的同一个 `/search` 请求中同时使用 [percentiles](Searching/Grouping.md) 聚合和 terms 聚合时的崩溃问题。
* 🪲 [v28.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.1) [ PR #4669](https://github.com/manticoresoftware/manticoresearch/pull/4669) 修复了 RT 自动优化，使 [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 不再会将表压缩到少于 2 个磁盘 chunk，除非显式请求了更低的阈值。
* 🪲 [v28.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.2) [ Issue #4632](https://github.com/manticoresoftware/manticoresearch/issues/4632) [ PR #4673](https://github.com/manticoresoftware/manticoresearch/pull/4673) 修复了带有远端存储字段的分布式查询：远端 `GETFIELD` 获取出错时会失败，而不是返回看似成功但存储字段值静默为空的结果行。
* 🪲 [v28.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.1) [ Issue #4615](https://github.com/manticoresoftware/manticoresearch/issues/4615) [ PR #4631](https://github.com/manticoresoftware/manticoresearch/pull/4631) 修复了较长的 SQL 解析错误，使其保留 UTF-8 字符边界，因此包含西里尔文和其他多字节文本的无效查询不再会产生截断或损坏的错误消息。

## 版本 27.1.5
**发布时间**: 2026年6月19日

本次发布带来了内置认证和授权、分片表、对话式搜索、更快的向量搜索、更好的分面和聚合，以及一长串针对 KNN、复制、协议兼容性和其他领域的修复。
...
* 🆕 [v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0) [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) 为 MySQL、HTTP/HTTPS、分布式远程代理以及复制相关操作新增了[认证与授权](Security/Authentication_and_authorization.md)，包括用户、bearer token、细粒度权限、引导命令，以及感知认证的集群用户处理。
* 🆕 [v26.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.0) [ PR #4606](https://github.com/manticoresoftware/manticoresearch/pull/4606) 将 Buddy 更新到 3.47.0，加入了与新分片功能相关的分片回滚、升级、测试和修复。
* 🆕 [v26.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.1.0) [ PR #4537](https://github.com/manticoresoftware/manticoresearch/pull/4537) 新增 [原生 `type='sharding'` 表](Creating_a_table/Creating_a_sharded_table/Creating_a_sharded_table.md)，支持分布式插入，并带来涵盖创建、路由、回滚、故障切换和运维行为的重大分片升级。
* 🆕 **26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) 重构了复制存储，因此传入的复制表现在使用标准的 [data_dir](Server_settings/Searchd.md#data_dir)/`<table>` 布局，而不再使用集群专用路径，使复制表存储与普通本地表布局保持一致。
* 🆕 [v25.15.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.15.0) [ PR #4590](https://github.com/manticoresoftware/manticoresearch/pull/4590) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.5.0，加入并行 HNSW 构建所需的库支持。
...
* 🆕 [v25.11.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.0) [ PR #4558](https://github.com/manticoresoftware/manticoresearch/pull/4558) 通过 [CREATE CHAT MODEL](Searching/Conversational_search.md) 和 [CALL CHAT](Searching/Conversational_search.md) 为 Manticore Search 增加了 [conversational search](Searching/Conversational_search.md)，因此现有的向量化表可以借助 KNN 检索、对话历史和 LLM 支持的响应来回答问题。
...
* 🆕 [v25.7.18](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.18) [ Issue #4461](https://github.com/manticoresoftware/manticoresearch/issues/4461) 为 [date_histogram](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29) 增加了 `time_zone` 和 `offset` 支持，使聚合查询可以按时区感知进行日历分桶。
* 🆕 [v25.7.16](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.16) [ PR #4503](https://github.com/manticoresoftware/manticoresearch/pull/4503) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.2.5，提升 ONNX embedding 性能。
* 🆕 [v25.7.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.0) [ Issue #3280](https://github.com/manticoresoftware/manticoresearch/issues/3280) 新增类 Elasticsearch 的统计聚合，包括 `percentiles`、`percentile_ranks` 和 `mad`，可直接在聚合查询中计算百分位分布和中位绝对偏差。
* 🆕 [v25.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.0) [ PR #4407](https://github.com/manticoresoftware/manticoresearch/pull/4407) 扩展了 `expr` 排名器中的 [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29)，允许其接受可选的第三个 `avgdl` 参数，从而让查询可以覆盖 BM25A 评分中使用的平均文档长度。
* 🆕 [v25.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.4.0) [ Issue #4060](https://github.com/manticoresoftware/manticoresearch/issues/4060) 新增 systemd 状态通知，并将打包的 Linux 服务切换为 `Type=notify`，改善启动、重载和关闭处理，避免长时间关闭工作被误判为卡住的守护进程。
* 🆕 [v25.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.3.0) [ PR #4326](https://github.com/manticoresoftware/manticoresearch/pull/4326) 新增对在 `JOIN` 查询中显式为左表列加前缀的支持，使使用表限定列名时的 JOIN 条件和过滤更一致。
* 🆕 [v25.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.0) [ PR #146](https://github.com/manticoresoftware/columnar/pull/146) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.2.2，新增本地 ONNX embedding 支持，并通过更快的 embedding 推理和 HNSW 距离计算提升向量搜索性能，在支持的环境下还包含 AVX-512 优化。
* 🆕 [v25.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.0) [ PR #4454](https://github.com/manticoresoftware/manticoresearch/pull/4454) 通过优化距离计算并在支持时加载 AVX-512 columnar 库提升了 KNN 性能，降低了向量搜索负载的开销。
...
* 🪲 [v26.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.3.1) 回滚了 `26.3.0` 中的 `zeroes` facet 功能，移除了误发布的 SQL/JSON 零计数 facet 分桶支持。
* 🪲 [v26.2.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.4) [ Issue #4320](https://github.com/manticoresoftware/manticoresearch/issues/4320) 修复了重评分后的 KNN 结果排序问题，因此在重评分距离相同的情况下，显式 `ORDER BY` 绑定规则会正确生效。
* 🪲 [v26.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.3) [ PR #4614](https://github.com/manticoresoftware/manticoresearch/pull/4614) 将 Buddy 更新到 3.47.2，修复了多个复制和节点重新加入时的崩溃路径，包括无效表索引处理和复制快照锁定。
* 🪲 [v26.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.2) [ Issue #4603](https://github.com/manticoresoftware/manticoresearch/issues/4603) 修复了在使用 columnar 属性的表上执行带 `GROUP BY` 的混合融合查询时的崩溃。
* 🪲 [v26.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.1) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) 将 Buddy 更新到 3.47.1，使基于 `doc_id` 的 HTTP KNN 搜索保留 `offset` 和 `max_matches`。
* 🪲 [v26.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.3) [ PR #4604](https://github.com/manticoresoftware/manticoresearch/pull/4604) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.5.2，纳入随附 columnar 库中的堆栈相关稳定性修复。
* 🪲 [v26.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.2) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) 修复了 KNN 搜索对 `max_matches` 的遵循问题，纠正了结果窗口处理，避免返回错误的固定命中数。
* 🪲 [v26.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.1) [ PR #4592](https://github.com/manticoresoftware/manticoresearch/pull/4592) 修复了调试构建中的堆栈大小设置，防止因协程和线程栈帧过大导致的内存破坏。
* 🪲 [v25.13.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.5) [ Issue #4489](https://github.com/manticoresoftware/manticoresearch/issues/4489) 修复了在构建带 columnar 属性的普通索引时，由 MVA/docstore 空缺引起的 indexer 崩溃。
* 🪲 [v25.13.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.4) [ PR #4583](https://github.com/manticoresoftware/manticoresearch/pull/4583) 修复了 `auto` 和 `max` 模式下 `facet_filter_mode` 的分桶状态行为，使已选、可用和不可用 facet 分桶与预期的分面搜索体验保持一致。
* 🪲 [v25.13.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.3) [ Issue #4578](https://github.com/manticoresoftware/manticoresearch/issues/4578) 通过重构动态表内部数据流修复了 RT `.@files` 在写入负载下的崩溃，避免内部文件列表查询期间出现同步相关故障。
* 🪲 [v25.13.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.2) [ Issue #660](https://github.com/manticoresoftware/manticoresearch-buddy/issues/660) 将 Buddy 更新到 3.46.3，修复了基于 `doc_id` 的 KNN 自命中处理，并恢复了 Logstash 7.17 模板探测兼容性。
* 🪲 [v25.13.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.1) [ Issue #168](https://github.com/manticoresoftware/columnar/issues/168) 修复了 Windows 上 AVX-512 模块加载问题，防止在不支持的系统上选中打包的 [MCL](https://github.com/manticoresoftware/columnar) AVX-512 DLL，从而导致守护进程或 indexer 崩溃。
* 🪲 [v25.12.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.12.1) [ PR #4563](https://github.com/manticoresoftware/manticoresearch/pull/4563) 修复了二进制 MySQL 协议支持，包括特殊列、空值和有类型的数值，使预处理/二进制协议查询重新正常工作。
* 🪲 [v25.11.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.2) [ PR #4566](https://github.com/manticoresoftware/manticoresearch/pull/4566) 通过规范化 `Content-Length` 头格式修复了 Fluent Bit 批量导入互操作性，防止成功的批量响应被回放成重复插入。
* 🪲 [v25.11.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.1) [ Issue #4560](https://github.com/manticoresoftware/manticoresearch/issues/4560) 将 Buddy 更新到 3.46.1，修复了 `information_schema.tables` 的有类型 `COUNT(*)` 处理，并恢复了预期的集成行为。
* 🪲 [v25.9.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.9.1) [ Issue #4464](https://github.com/manticoresoftware/manticoresearch/issues/4464) 修复了需要 Buddy 参与处理的预处理语句，包括模糊 `MATCH(?)` 查询，并更正了 Buddy 返回的 bool、multi 和 JSON 值的严格有类型结果处理。
* 🪲 [v25.8.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.5) [ PR #4554](https://github.com/manticoresoftware/manticoresearch/pull/4554) 修复了 AOT 语言包相关的打包和 CI，确保 Docker 镜像包含所有语言包，且词形还原器/AOT 测试覆盖不再被跳过。
* 🪲 [v25.8.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.4) [ PR #4553](https://github.com/manticoresoftware/manticoresearch/pull/4553) 修复了乌克兰语词形还原器的加载和原始形式生成，增强了新的内置乌克兰语形态学支持。
* 🪲 [v25.8.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.3) [ Issue #4490](https://github.com/manticoresoftware/manticoresearch/issues/4490) 修复了 `CALL KEYWORDS` 中阿拉伯语混合 token 的词干提取，防止某些包含阿拉伯语和数字的短输入报出通用的 `Something went wrong` 错误。
* 🪲 [v25.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.2) [ Issue #4509](https://github.com/manticoresoftware/manticoresearch/issues/4509) 修复了在使用较长 `data_dir` 路径创建集群时的复制崩溃，纠正了长目录名下的 cluster-binlog 处理。
* 🪲 [v25.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.1) [ PR #4535](https://github.com/manticoresoftware/manticoresearch/pull/4535) 修复了混合模式下的 `knn_dist` 输出，因此哨兵值 `FLT_MAX` 不再在 JSON 结果中被序列化为虚假的距离。
* 🪲 [v25.7.21](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.21) [ Issue #4525](https://github.com/manticoresoftware/manticoresearch/issues/4525) 通过正确释放重映射后的字符串属性修复了 scroll 处理中的内存泄漏。
* 🪲 [v25.7.20](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.20) [ Issue #4466](https://github.com/manticoresoftware/manticoresearch/issues/4466) 修复了当 JSON 数组聚合是请求中唯一的聚合时的处理问题，因此数组项会按各自分桶，而不是把整个数组作为每个 key 重复一遍。
* 🪲 [v25.7.19](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.19) [ PR #4533](https://github.com/manticoresoftware/manticoresearch/pull/4533) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 13.2.6，修复了在使用 1-bit 量化向量运行多线程 KNN 查询时可能出现的崩溃。
* 🪲 [v25.7.17](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.17) [ PR #4529](https://github.com/manticoresoftware/manticoresearch/pull/4529) 打包了若干小修复和清理工作，包括另一处 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 正确性修复，以及一些小的运行时/构建改进。
* 🪲 [v25.7.15](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.15) [ PR #4526](https://github.com/manticoresoftware/manticoresearch/pull/4526) 通过正确读取 columnar docid，修复了 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 在 columnar 模式下的 kill-list 处理，完成了针对 [MCL](https://github.com/manticoresoftware/columnar)/columnar 配置的近期 attach 修复。
* 🪲 [v25.7.14](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.14) [ Issue #4459](https://github.com/manticoresoftware/manticoresearch/issues/4459) 修复了聚合参数中 [DATE_HISTOGRAM()](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29) 的间隔解析，因此像 `1h`、`2m` 这样的 `fixed_interval` 值现在可以正常工作，而不会被当作原始秒数处理。
* 🪲 [v25.7.13](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.13) [ Issue #4522](https://github.com/manticoresoftware/manticoresearch/issues/4522) 修复了 JOIN 排序器/分组输出中多出的空字符，因此分组和分面 JOIN 值不再包含损坏的尾随 `\0` 字节。
* 🪲 [v25.7.12](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.12) [ Issue #4516](https://github.com/manticoresoftware/manticoresearch/issues/4516) 修复了 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 之后目标表的文档计数器，因此 `indexed_documents` 和相关元数据现在会反映附加后的实际行数。
* 🪲 [v25.7.11](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.11) [ Issue #4515](https://github.com/manticoresoftware/manticoresearch/issues/4515) 修复了 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 对已被杀死文档的处理，因此目标表中的重复 ID 在附加过程中不再被错误删除。
* 🪲 [v25.7.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.10) [ Issue #4514](https://github.com/manticoresoftware/manticoresearch/issues/4514) 修复了 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 之后源表的 `indexed_documents`，因此当一个磁盘块被移动到另一张表时，文档数现在会正确下降。
* 🪲 [v25.7.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.9) [ Issue #4513](https://github.com/manticoresoftware/manticoresearch/issues/4513) 修复了已附加索引的元数据，因此通过 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 创建的表会保留正确的 chunk 状态，重启后也不再损坏。
* 🪲 [v25.7.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.8) [ Issue #4512](https://github.com/manticoresoftware/manticoresearch/issues/4512) 修复了脏重启后启动时的 `indexed_documents` 元数据，因此当恢复已删除行时，RT 表不再报告膨胀的文档数。
* 🪲 [v25.7.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.7) [ Issue #159](https://github.com/manticoresoftware/columnar/issues/159) 修复了 embedding 库缺失或配置错误时的 KNN 启动诊断，因此 Manticore 在初始化期间不再报告误导性的 `(null)` 警告/错误。
* 🪲 [v25.7.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.6) [ Issue #4460](https://github.com/manticoresoftware/manticoresearch/issues/4460) 修复了 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 禁用二级索引时的复制处理，因此在复制环境中，该警告不再被升级为会破坏集群的错误。
* 🪲 [v25.7.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.5) [ Issue #4345](https://github.com/manticoresoftware/manticoresearch/issues/4345) 修复了二级索引无法生成迭代器时的 JSON 过滤回退，因此复合过滤树会保留预期过滤条件，而不是返回错误结果。
* 🪲 [v25.7.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.4) [ PR #662](https://github.com/manticoresoftware/manticoresearch-buddy/pull/662) 将 Buddy 更新到 3.44.5，包含一个 Composer 安全修复。
* 🪲 [v25.7.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.3) [ Issue #4404](https://github.com/manticoresoftware/manticoresearch/issues/4404) 修复了带 `scroll` 的 [hybrid search](Searching/Hybrid_search.md)，因此基于 scroll 的分页在使用 [hybrid search](Searching/Hybrid_search.md) 时现在可以正常工作，而不会失败或崩溃。
* 🪲 [v25.7.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.2) [ Issue #4491](https://github.com/manticoresoftware/manticoresearch/issues/4491) 修复了 RT chunk 重命名和 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 之后的二级索引状态，因此被 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 禁用的索引在守护进程重启后仍保持禁用，而不会被意外重新启用。
* 🪲 [v25.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.1) [ PR #4502](https://github.com/manticoresoftware/manticoresearch/pull/4502) 修复了在 [hybrid search](Searching/Hybrid_search.md) 中按 `knn_dist()` 和 `hybrid_score()` 过滤的问题，因此查询现在可以正确对向量距离和混合排名分数应用条件。
* 🪲 [v25.6.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.9) [ Issue #4473](https://github.com/manticoresoftware/manticoresearch/issues/4473) 通过在 `join.on` 中规范化表引用，修复了混合大小写表名的 HTTP JSON JOIN 解析，因此 JOIN 不再会因为表名包含大写字母而失败。
* 🪲 [v25.6.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.8) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) 修复了列式表在混合字段长度处理下剩余的 expr-ranker 崩溃，完成了在 [index_field_lengths](Functions/Searching_and_ranking_functions.md#BM25A%28%29)='1' 场景中对 [bm25a()](Functions/Searching_and_ranking_functions.md#BM25F%28%29)/[bm25f()](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 的修复。
* 🪲 [v25.6.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.7) [ Issue #4484](https://github.com/manticoresoftware/manticoresearch/issues/4484) 修复了 MySQL `COM_FIELD_LIST` 元数据路径中的模板表处理，防止客户端请求模板表字段元数据时崩溃。
* 🪲 [v25.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.6) [ Issue #4497](https://github.com/manticoresoftware/manticoresearch/issues/4497) 修复了模板表元数据路径上的崩溃，包括 `SHOW FIELDS` 和相关字段列表处理，这些路径此前可能导致守护进程崩溃。
* 🪲 [v25.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.5) [ Issue #2](https://github.com/manticoresoftware/telemetry-lib/issues/2) 将 Buddy 更新到 3.44.4，修复了 PHP 8.4 下的遥测传递，因此遥测快照现在可以成功发送，而不会在 HTTP POST 请求期间失败。
* 🪲 [v25.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.4) [ Issue #4469](https://github.com/manticoresoftware/manticoresearch/issues/4469) 修复了 [ALTER TABLE ... REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) 在 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 合并 RT 磁盘 chunk 之后的行为，因此 secondary-index 文件会与重命名后的优化 chunk 保持同步，不再报错 `secondary index file ... not loaded`。
* 🪲 [v25.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.3) [ Issue #4418](https://github.com/manticoresoftware/manticoresearch/issues/4418) 修复了 [hybrid search](Searching/Hybrid_search.md) 中的崩溃及相关 JOIN 处理问题，包括此前在结合全文检索和 KNN 结果时可能失败的 `LEFT JOIN` 查询。
* 🪲 [v25.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.2) [ PR #658](https://github.com/manticoresoftware/manticoresearch-buddy/pull/658) 将 Buddy 更新到 3.44.3，在 `binlog_path` 为空时修复了卷遥测，因此 `volume_size_bytes` 不再被错误地报告为 0。
* 🪲 [v25.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.1) [ Issue #4476](https://github.com/manticoresoftware/manticoresearch/issues/4476) 修复了 systemd 下的启动，因此当守护进程由 systemd 托管时不再需要 `pid_file`，因为 systemd 已经知道并控制进程 ID。
* 🪲 [v25.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.1) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) 修复了列式表在 [index_field_lengths](Functions/Searching_and_ranking_functions.md#BM25A%28%29)='1' 时使用 [bm25a()](Functions/Searching_and_ranking_functions.md#BM25F%28%29) 或 [bm25f()](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 触发的崩溃，因此自定义排序不再会从错误的存储中解引用字段长度属性。
* 🪲 [v25.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.0) [ Issue #4007](https://github.com/manticoresoftware/manticoresearch/issues/4007) 修复了 SphinxQL、HTTP 和 `JOIN` 查询中混合大小写表名处理不一致的问题，因此表名现在会统一规范化，而不是在使用大写字母时失败或表现不同。
* 🪲 [v25.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.1) [ Issue #4428](https://github.com/manticoresoftware/manticoresearch/issues/4428) 修复了 [hybrid search](Searching/Hybrid_search.md) 查询中使用 `GROUP BY` 时的崩溃，包括同时结合全文检索和 KNN 搜索的查询。
* 🪲 [v25.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.1) [ Issue #4455](https://github.com/manticoresoftware/manticoresearch/issues/4455) 回滚了最近将缺失的 `knn_embeddings` 库转为显式 KNN 初始化错误的改动，恢复了该可选 embedding 库缺失时的旧行为。
* 🪲 [v25.0.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.6) [ Issue #4437](https://github.com/manticoresoftware/manticoresearch/issues/4437) 修复了 `ALTER CLUSTER ... DROP` 之后的集群重新加入处理，因此在删除期间离线的节点不再保留过时的本地集群表元数据，避免该已删除表在重新加入后卡住且无法移除。
* 🪲 [v25.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.4) [ PR #4405](https://github.com/manticoresoftware/manticoresearch/pull/4405) 修复了针对词干化或其他方式归一化后的查询词的 BM25A 排名，因此诸如 `looking` 和 `look` 这样的等价查询现在会使用相同的局部/全局 IDF 统计，而不会产生不同的相关性分数。
* 🪲 [v25.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.3) [ PR #85](https://github.com/manticoresoftware/executor/pull/85) 将 Executor 更新到 1.4.3，通过使用预构建的、按架构区分的产物代替在镜像流水线中构建二进制文件，简化了其 Docker 打包流程。
* 🪲 [v25.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.2) [ Issue #4395](https://github.com/manticoresoftware/manticoresearch/issues/4395) 修复了在自动优化已经运行时执行 [OPTIMIZE TABLE ... OPTION sync=1, cutoff=N](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 的问题，因此手动 optimize 请求现在会等待其请求的 chunk 减少目标，而不会受制于另一个正在进行的 optimize 运行。
* 🪲 [v25.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.1) [ Issue #4398](https://github.com/manticoresoftware/manticoresearch/issues/4398) 修复了守护进程的查询时间统计，因此 HTTP `/search`、分布式更新查询以及从 master 路由到 agent 的 API 搜索请求都能正确报告全局计数器和各表耗时。

## 版本 25.0.0
**发布日期**：2026 年 3 月 30 日

本次发布带来了更大范围的打包重构、用于自动 embedding 模型的新 `API_URL` 和 `API_TIMEOUT` 选项、[hybrid search](Searching/Hybrid_search.md)、兼容 S3 的备份和恢复支持、借助 N 路合并和并行 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 提升的 RT 表维护速度、KNN 预过滤，以及 MySQL 兼容的预处理语句。

## 重大变更
* ⚠️ [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #123](https://github.com/manticoresoftware/columnar/pull/123) **Breaking change**: 将 **[MCL](https://github.com/manticoresoftware/columnar)** 更新到 13.0.0，新增 [API_URL](Searching/KNN.md#Creating-a-table-with-auto-embeddings) 和 [API_TIMEOUT](Searching/KNN.md#Creating-a-table-with-auto-embeddings)，用于 [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29)。如果你使用 [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29)，或者将 **[MCL](https://github.com/manticoresoftware/columnar)** 与 daemon 分开管理，这会影响到你，因为 Manticore 25.0.0 需要 **[MCL](https://github.com/manticoresoftware/columnar) 13.0.0**，并且与较旧版本的 [MCL](https://github.com/manticoresoftware/columnar) 不兼容；请将 daemon 和 [MCL](https://github.com/manticoresoftware/columnar) 一起升级，且只有在你同时恢复到匹配的旧版 [MCL](https://github.com/manticoresoftware/columnar) 时，降级才是安全的。
* ⚠️ [v24.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.0.0) [ Issue #4343](https://github.com/manticoresoftware/manticoresearch/issues/4343) **重大变更**：内部 **复制协议** 版本已升级，并新增了集群纪元跟踪功能，使离线节点在恢复时能够检测到集群表成员变更并使用 SST 而不是在 IST 恢复期间失败。此变更仅影响使用 **复制集群** 的用户：跨此变更的混合版本复制集群不兼容，因此请同时升级集群节点，降级仅在集群中未使用新复制协议时才安全。
* ⚠️ [v23.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.0) [ Issue #4364](https://github.com/manticoresoftware/manticoresearch/issues/4364) **破坏性变更**：引入 **索引格式版本 69**，以支持新的 bigram 分词选项，包括 [bigram_delimiter](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_delimiter) 模式和支持数字感知的 [bigram_index](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_index) 模式。如果你使用这些 bigram 选项，或者在重建索引后需要用较旧的 Manticore 版本打开重写后的索引，那么这会影响你：现有旧索引仍可读取，但以格式 69 重建或新写入的索引与较旧的 Manticore 版本不兼容，因此只有在这类索引被重写之前才能安全降级。
* ⚠️ [v22.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/22.0.0) [ Issue #4301](https://github.com/manticoresoftware/manticoresearch/issues/4301) **重大变更**：**RT 表的嵌入生成** 已从提交阶段移出，并作为向量数据进行复制。如果您使用带有 `model_name` 的 **RT 表** 的 **复制集群**，则会受到影响：它保留用户提供的向量，防止副本重新生成不同的嵌入，并在与复制一起使用时提高 **自动嵌入** 的性能，但跨此变更的混合版本复制集群不兼容。请同时升级集群节点；降级仅在集群中未使用新复制协议时才安全。
* ⚠️ [v21.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.0) [ PR #138](https://github.com/manticoresoftware/columnar/pull/138) **破坏性变更**：将 **[MCL](https://github.com/manticoresoftware/columnar)** 更新到 12.0.0，并提升所需的 **KNN API** 版本，以支持在 HNSW 遍历期间自适应的 [**KNN 早停**](Searching/KNN.md#Early-termination)。如果你将 **[MCL](https://github.com/manticoresoftware/columnar)** 独立于守护进程管理，或者运行依赖先前候选探索行为或边界结果选择的 **KNN 查询**，这会影响你：Manticore 21.0.0 需要带有新 KNN 接口的 **[MCL](https://github.com/manticoresoftware/columnar) 12.0.0**，并且借助该库，当找到足够候选项后 KNN 搜索可以更早停止，这可能改变被探索和返回的候选项。现有数据仍然兼容，无需特殊迁移，但升级守护进程和 [MCL](https://github.com/manticoresoftware/columnar) 时应同时进行；只有在你也恢复到匹配的旧版 [MCL](https://github.com/manticoresoftware/columnar) 时，降级才安全。
* ⚠️ [v19.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.0) [ Issue #4103](https://github.com/manticoresoftware/manticoresearch/issues/4103) **重大变更**：新增了 [**KNN 预过滤**](Searching/KNN.md#Filtering-strategies:-prefilter-vs.-postfilter)。如果您运行 **KNN 查询** 并同时使用属性过滤器，则会受到影响，因为现在可以在 KNN 搜索期间应用过滤器，而不仅仅是在候选选择之后。这会通过优先选择过滤后的最近邻来改变过滤 KNN 查询的结果选择行为；无需特殊迁移，但依赖于先前后过滤结果行为的查询在升级后可能会返回不同结果。由于此功能未引入索引或协议格式变更，因此可以降级。
* ⚠️ [v18.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.0) [ Issue #4261](https://github.com/manticoresoftware/manticoresearch/issues/4261) **破坏性变更**：引入 **索引格式版本 68**，用于修复 [hitless_words](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#hitless_words) 词典布局处理。如果你重建或新写入索引后又需要用较旧的 Manticore 版本打开重写后的索引，那么这会影响你：现有旧索引仍可读取，但任何以新格式重建或新写入的索引都与较旧的 Manticore 版本不兼容，因此只有在这类索引被重写之前才能安全降级。
...
## 新功能和改进
* 🆕 [v24.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.4.0) [ PR #4091](https://github.com/manticoresoftware/manticoresearch/pull/4091) 通过增加 N 路磁盘 chunk 合并和并行 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 作业改进了 RT 表压缩，缩短了合并大量磁盘 chunk 所需的时间，并暴露了新的 [merge_chunks_per_job](Server_settings/Searchd.md#merge_chunks_per_job) 和 [parallel_chunk_merges](Server_settings/Searchd.md#parallel_chunk_merges) 设置。
* 🆕 [v24.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.3.0) [ PR #133](https://github.com/manticoresoftware/manticoresearch-backup/pull/133) 将 Manticore Backup 更新到 1.10.0，添加了 S3 兼容的备份和恢复支持，包括 AWS S3、MinIO、Wasabi 和 Cloudflare R2。
* 🆕 [v24.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.0) [ Issue #2079](https://github.com/manticoresoftware/manticoresearch/issues/2079) 添加了 [混合搜索](Searching/Hybrid_search.md)（Reciprocal Rank Fusion，`fusion_method='rrf'`），允许 SQL 和 JSON 查询结合全文和 KNN 结果，使用 `hybrid_score()` 进行排序，并支持在单个融合结果集中包含多个 KNN 子查询。
* 🆕 [v24.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.0) [ PR #141](https://github.com/manticoresoftware/columnar/pull/141) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 12.4.1，新增对 GGUF 量化本地 embedding 模型、T5 编码器模型以及通过令牌认证下载的受限 Hugging Face 模型的支持。
* 🆕 [v23.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.2.0) [ PR #4372](https://github.com/manticoresoftware/manticoresearch-buddy/pull/648) 将 Buddy 更新到 3.44.0，添加了对涉及多个表的查询的模糊搜索支持。
* 🆕 [v23.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.1.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) 添加了新的 Manticore 捆绑包用于 deb 和 rpm，取代之前的元包以简化捆绑组件的安装和分发。
* 🆕 [v21.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.1.0) [ PR #4352](https://github.com/manticoresoftware/columnar/pull/140) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 12.1.0，提升了 KNN 早停性能，并增加了更高效的 64 位值编码。
* 🆕 [v19.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.0) [ PR #4306](https://github.com/manticoresoftware/manticoresearch/pull/4306) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 11.1.0，新增 codec 缓存支持。
...
* 🆕 [v19.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.3) [ Issue #4303](https://github.com/manticoresoftware/manticoresearch/issues/4303) 为 SQL 连接增加了独立的 [interactive_timeout](Server_settings/Setting_variables_online.md#SET) 和 [wait_timeout](Server_settings/Setting_variables_online.md#SET) 处理，因此交互式客户端和非交互式客户端可以像 MySQL 一样使用不同的空闲超时时间。
* 🆕 [v17.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.0) [ Issue #1124](https://github.com/manticoresoftware/manticoresearch/issues/1124) 添加了 MySQL 兼容的预处理语句支持，包括二进制协议的预处理/执行处理和绑定参数验证。

## 错误修复
* 🪲 [v24.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.3) [ Issue #4375](https://github.com/manticoresoftware/manticoresearch/issues/4375) 修复了当 `max_query_time` 中断使用 `NOTNEAR`、`MAYBE` 或否定操作符的查询时出现的误报全文匹配问题，因此超时搜索不再返回不满足查询条件的行。
* 🪲 [v24.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.2) [ Issue #4398](https://github.com/manticoresoftware/manticoresearch/issues/4398) 修复了查询时间统计，使查询日志、全局计数器和每个表的 `SHOW STATUS` 时间保持一致，Buddy 的内部请求不再扭曲用户面向的守护进程统计信息。
* 🪲 [v24.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.1) [ PR #653](https://github.com/manticoresoftware/manticoresearch-buddy/pull/653) 将 Buddy 更新到 3.44.1，修复了对空请求负载的默认处理不正确的问题。
* 🪲 [v24.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.3) [ PR #4396](https://github.com/manticoresoftware/manticoresearch/pull/4396) 修复了 macOS 上手动启动 `searchd` 的问题，Buddy 支持的 SQL 命令不再因加载较旧的系统 `libcurl` 而失败；运行时查找现在会自动优先使用 Homebrew curl，`MANTICORE_CURL_LIB` 仍可作为显式覆盖。
* 🪲 [v24.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.2) [ PR #4394](https://github.com/manticoresoftware/manticoresearch/pull/4394) 修复了 macOS 打包构建的问题，Buddy 支持的 SQL 命令不再因 `searchd` 链接至不兼容的 `libcurl` 而失败；该包现在优先使用 Homebrew curl，并支持 `MANTICORE_CURL_LIB` 运行时覆盖。
* 🪲 [v24.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.1) [ Issue #4388](https://github.com/manticoresoftware/manticoresearch/issues/4388) 修复了集群中事务包含重复文档 ID 时的复制问题，因此副本不再在供体正确删除重复项时丢失行。
* 🪲 [v24.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.0.1) [ Issue #4354](https://github.com/manticoresoftware/manticoresearch/issues/4354) 修复了 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 处理在变更属性时也必须禁用其二级索引的情况，避免了错误警告和不一致的二级索引状态。
* 🪲 [v23.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.2) [ PR #4370](https://github.com/manticoresoftware/columnar/issues/125) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 12.1.1，修复了文本包含无效 UTF-8 时自动 embedding 生成期间的崩溃，包括插入到 `html_strip=1` 表中的场景。
...
* 🪲 [v21.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.3) [ Issue #4315](https://github.com/manticoresoftware/manticoresearch/issues/4315) 修复了插入包含自动嵌入列的表时，MVA 值被保留而不是存储为空的问题。
...
* 🪲 [v21.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.1) [ PR #4349](https://github.com/manticoresoftware/manticoresearch/pull/4349) 修复了使用列存储的表在生成嵌入期间的崩溃。
* 🪲 [v19.2.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.5) [ PR #4333](https://github.com/manticoresoftware/executor/pull/81) 将 Executor 更新到 1.4.1，将 PHP 源下载从 php.net 切换到 GitHub 以避免受 CAPTCHA 保护的链接，并将捆绑的 PHP 更新到 8.4.18。
* 🪲 [v19.2.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.4) [ Issue #4314](https://github.com/manticoresoftware/manticoresearch/issues/4314) 修复了模型支持的 `float_vector` 列以支持显式的空 `FROM=''`，现在正确表示“使用所有文本/字符串字段”。
* 🪲 [v19.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.3) [ Issue #4315](https://github.com/manticoresoftware/manticoresearch/issues/4315) 修复了插入包含自动嵌入 `float_vector` 列的表时，MVA 值被保留而不是存储为空的问题。
* 🪲 [v19.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.2) [ Issue #4297](https://github.com/manticoresoftware/manticoresearch/issues/4297) 修复了 `IMPORT TABLE` 正确复制外部文件，包括 `hitless_words`，因此导入的表在导入后不再引用缺失的外部文件。
...
* 🪲 [v19.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.2) [ PR #4311](https://github.com/manticoresoftware/manticoresearch/issues/4307) 修复了 HTTP `/sql?mode=raw` 多语句请求中使用 `.@files` 后跟 `SHOW META` 的崩溃。
* 🪲 [v19.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.1) [ Issue #4103](https://github.com/manticoresoftware/manticoresearch/issues/4103) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 11.0.0，以支持 KNN 预过滤。
* 🪲 [v18.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.1) [ Issue #4293](https://github.com/manticoresoftware/manticoresearch/issues/4293) 修复了自动嵌入 `float_vector` 列的 `ALTER TABLE ... ADD COLUMN`，使 `model_name` 和 `FROM` 正确保留而不是被替换为 `knn_dims='0'`。
...
* 🪲 [v17.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.5) [ PR #4296](https://github.com/manticoresoftware/manticoresearch/pull/4296) 修复了全文 `AND` 节点的文档计数和成本估计，提高了对交集全文条件的查询规划器决策。
* 🪲 [v17.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.4) [ Issue #4274](https://github.com/manticoresoftware/manticoresearch/issues/4274) 修复了 `indextool --check` 无法打开磁盘块 `hitless_words` 文件的问题，通过相对于索引目录解析其路径来解决。
...
* 🪲 [v17.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.1) [ Issue #4207](https://github.com/manticoresoftware/manticoresearch/issues/4207) 添加了回归修复和测试覆盖率，以解决 RT 磁盘块保存期间可能导致文档删除和合并后出现重复行的竞态条件。
* 🪲 [v17.5.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.10) [ Issue #4207](https://github.com/manticoresoftware/columnar/issues/133) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 10.2.2，通过在本地模型使用周围增加并发保护，修复了在带本地 embedding 模型的表中插入数据时执行搜索会崩溃的问题。
* 🪲 [v17.5.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.9) [ PR #4247](https://github.com/manticoresoftware/manticoresearch/pull/4247) 修复了 RT 磁盘块保存期间的竞态条件，该竞态条件可能导致文档删除并在合并或保存后产生重复行。
* 🪲 [v17.5.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.8) [ PR #4241](https://github.com/manticoresoftware/manticoresearch/pull/4247) 修复了连接表中错误的后限制属性评估，包括在将 RAM 块刷新到磁盘后 `LEFT JOIN` 查询。
* 🪲 [v17.5.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.7) [ PR #4239](https://github.com/manticoresoftware/manticoresearch/pull/4239) 明确了日志和状态文档中查询时间和墙时间的定义。
...
* 🪲 [v17.5.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.5) [ Issue #858](https://github.com/manticoresoftware/manticoresearch/issues/858) 修复了 CJK 文本的 `HIGHLIGHT()` 和片段生成中在单词之间添加不必要的空格的问题；高亮摘录现在保留原始文本间距而不是分词间距。
* 🪲 [v17.5.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.4) [ Issue #1166](https://github.com/manticoresoftware/manticoresearch/issues/1166) 修复了在对带有 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)=1 的 RT 表执行 `TRUNCATE TABLE ... WITH RECONFIGURE` 之后进行 INSERT 时的崩溃，尤其是在字段与字符串属性混合时；现在会在重新配置期间正确设置字段长度属性。
* 🪲 [v17.5.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.3) 修复了手册中损坏的 Wordforms 文档内容。
* 🪲 [v17.5.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.2) [ Issue #3213](https://github.com/manticoresoftware/manticoresearch/issues/3213) 修复了在连接表的列别名构建的表达式上进行过滤的 JOIN 查询，例如 `SELECT t2.i al, (al*0.1) pct ... WHERE pct > 0`，这可能会因 `incoming-schema expression missing evaluator` 而失败；这些过滤器现在在连接后正确评估。
...
如果你遵循 [官方安装指南](https://manticoresearch.com/install/)，你无需担心这个问题。

❤️ 我们想感谢 [@pakud](https://github.com/pakud) 在 [PR #4075](https://github.com/manticoresoftware/manticoresearch/pull/4075) 上的工作。

## 突破性变更
* ⚠️ [v17.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.0.0) [ Issue #4120](https://github.com/manticoresoftware/manticoresearch/issues/4120) [MCL](https://github.com/manticoresoftware/columnar) 10.0.0：新增对 `DROP CACHE` 的支持。这更新了守护进程与 [MCL](https://github.com/manticoresoftware/columnar) 之间的接口。旧版 Manticore Search 无法加载新版 [MCL](https://github.com/manticoresoftware/columnar)。
* ⚠️ [v16.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/16.0.0) [ Issue #4019](https://github.com/manticoresoftware/manticoresearch/issues/4019) Percolate 查询的 JSON 响应现在将 hit 的 `_id` 和 `_score` 作为数字而不是字符串返回，与常规搜索一致；这对于依赖这些字段字符串类型的客户端来说是一个破坏性变更。

## 新功能和改进
* 🆕 [v17.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.0) [ PR #130](https://github.com/manticoresoftware/columnar/pull/130) [MCL](https://github.com/manticoresoftware/columnar) 更新到 10.2.0：修复了 QWEN 模型支持，并新增对更多模型的支持。
...
* 🆕 [v17.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.0) [ PR #4195](https://github.com/manticoresoftware/manticoresearch/pull/4195) 将 [MCL](https://github.com/manticoresoftware/columnar) 更新到 10.1.0；新增对 Qwen 本地 embedding 模型的支持。
* 🆕 [v17.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.1.0) [ Issue #3826](https://github.com/manticoresoftware/manticoresearch/issues/3826) Jieba 形态学实例现在在相同配置（模式、HMM 标志、用户字典路径）的表之间共享，当许多表使用 Jieba 时（例如许多空表不再导致约 20 GB 的内存使用），大大减少了内存使用。
* 🆕 [v17.0.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.0.7) [ Issue #2046](https://github.com/manticoresoftware/manticoresearch/issues/2046) 在 RT 模式下，停用词、词形变化、异常词和无命中词现在可以在 `CREATE TABLE` 中内联设置（分号分隔的值；词形变化/异常词使用 `>` 或 `=>` 表示对，`\` 用于转义），因此可以在不使用外部文件的情况下创建表；`SHOW CREATE TABLE` 返回这些内联值。
...
* 🆕 [v16.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/16.2.0) [ PR #4088](https://github.com/manticoresoftware/manticoresearch/pull/4088) 添加了 searchd 的 `--quiet` (`-q`) 标志以抑制启动输出（横幅和预缓存消息），仅打印错误；在循环或脚本中启动和停止 searchd 时很有用。
...
* 🪲 [v17.2.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.6) [ Issue #3203](https://github.com/manticoresoftware/manticoresearch/issues/3203) 修复了在表包含多个磁盘块时，[ALTER TABLE REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) 会失败并报 `failed to rename … .tmp.spjidx` 的问题。
...
* 🪲 [v17.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.3) [ Issue #4176](https://github.com/manticoresoftware/manticoresearch/issues/4176) 修复了表重命名会破坏使用外部 stopwords、wordforms 或 exceptions 的表：现在 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 会将这些文件迁移到新的基于 chunk 的格式并更新磁盘 chunk 头，因此在 `ALTER TABLE RENAME` 之后，daemon 重启时不再会报告缺少外部文件。
...
* 🪲 [v16.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/16.0.4) [ PR #4047](https://github.com/manticoresoftware/manticoresearch/pull/4047) 将 [MCL](https://github.com/manticoresoftware/columnar) 版本更新到 9.0.1。修复了 `block_size` 为 1 时的 KNN 浮点向量处理，在这种情况下禁用表压缩（因为 block size 1 不使用压缩）。
...
## 突破性变更
* ⚠️ [v15.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/15.0.0) [ PR #4003](https://github.com/manticoresoftware/manticoresearch/pull/4003) 将 [MCL](https://github.com/manticoresoftware/columnar) 要求更新到 9.0.0，带来未压缩浮点向量存储、KNN 向量的块大小变更以及非缓冲读取。此更新会改变数据格式。旧版 [MCL](https://github.com/manticoresoftware/columnar) 无法读取，但新版仍可正常读取你现有的 columnar 表。
...
* 🆕 [v14.7.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.7.0) [ Issue #3860](https://github.com/manticoresoftware/manticoresearch/issues/3860) 将 Buddy 要求更新到 3.40.1，其中包括自动补全的改进：将双字节分隔符字符标准化为空格，并过滤掉重复或格式错误的建议组合以提高建议质量。它还包括修复 Kafka 视图中的无效 JSON 错误以及修复自动补全，使得当 score map 中缺少某些键时，排序组合不再抛出错误。
* 🆕 [v14.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.6.0) [ Issue #615](https://github.com/manticoresoftware/manticoresearch-buddy/issues/615) 将 Manticore Buddy 要求更新到 3.39.1，其中包括修复 KNN 插件中的无效 JSON 错误，并允许 Buddy 处理器覆盖 HTTP `Content-Type`，因此 `/metrics` 现在返回 Prometheus 文本格式 (`text/plain; version=0.0.4`) 而不是 JSON，修复了抓取失败的问题。
* 🆕 [v14.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.4.0) [ PR #3942](https://github.com/manticoresoftware/manticoresearch/pull/3942) 将 Manticore Buddy 要求更新到 3.38.0，过滤掉零文档建议，改进 Ds\Map 的字符串键处理，将 Buddy 中的内存使用报告格式从 KB 更改为字节以提高精度，并增强性能、稳定性和可维护性。
* 🆕 [v14.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.5.0) [ Issue #3329](https://github.com/manticoresoftware/manticoresearch/issues/3329) 在记录查询请求时修剪 JSON 负载中的多余空格和换行符 —— 跳过前导/尾随空格以避免记录格式错误的 JSON。
* 🆕 [v14.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.3.0) [ PR #3932](https://github.com/manticoresoftware/manticoresearch/pull/3932) 改进了 `LOCK TABLES` / `UNLOCK TABLES` 的处理：写锁现在返回警告而不是错误，读锁在 `SHOW LOCKS` 中正确显示，整体锁逻辑行为一致。
* 🆕 [v14.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.2.0) [ Issue #3891](https://github.com/manticoresoftware/manticoresearch/issues/3891) 在 `JOIN ON` 子句中添加了任意过滤表达式的支持（不仅仅是等值比较），启用了类似 `... ON t1.id = t2.t1_id AND t2.value = 5` 的查询。
...
* 🪲 [v14.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.7.1) [ PR #3992](https://github.com/manticoresoftware/manticoresearch/pull/3992) 修复了通过 agent 处理的分布式表上 HTTP [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 和 DELETE 请求缺少错误消息的问题，因此操作失败时客户端现在可以正确收到错误。
...
* 🪲 [ Issue #3994](https://github.com/manticoresoftware/manticoresearch/issues/3994) 为Grafana版本12.3添加了测试。
* 🪲 [v14.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.0.1) [ Issue #3844](https://github.com/manticoresoftware/manticoresearch/issues/3844) 修复了使用`max(ft field)`导致的崩溃问题。
...
* 🪲 [v13.15.12](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.12) [ PR #3873](https://github.com/manticoresoftware/manticoresearch/pull/3873) 将buddy从3.36.0更新到3.36.1，并在EmulateElastic插件中添加了RT模式检查。
...
* 🪲 [v13.15.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.9) [ Issue #3783](https://github.com/manticoresoftware/manticoresearch/issues/3783) 修复了阻止在FreeBSD上原生编译的问题。
* 🪲 [v13.15.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.8) 修复了文档翻译。
* 🪲 [v13.15.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.7) [ PR #3868](https://github.com/manticoresoftware/manticoresearch/pull/3868) 将executor从1.3.5更新到1.3.6，并添加了对iconv扩展的支持。
* 🪲 [v13.15.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.6) 修复了与[issue 3817](https://github.com/manticoresoftware/manticoresearch/issues/3817)相关的模糊测试构建问题。
...
* 🪲 [v13.15.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.4) [ Issue #3686](https://github.com/manticoresoftware/manticoresearch/issues/3686) 修复了全文查询`"(abc|def)"`未按预期工作的问题。
* 🪲 [v13.15.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.3) [ Issue #3428](https://github.com/manticoresoftware/manticoresearch/issues/3428) 添加了使用`HAVING`的查询获取结果总数的功能。
...
* 🪲 [v13.15.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.15.1) [ PR #3848](https://github.com/manticoresoftware/manticoresearch/pull/3848) 修复了复制事务密钥生成和处理冲突事务的问题。
* 🪲 [v13.14.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.14.0) [ Issue #3806](https://github.com/manticoresoftware/manticoresearch/issues/3806) 修复了`CALL SUGGEST`不匹配三元组的问题。
* 🪲 [v13.13.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.8) [ PR #3839](https://github.com/manticoresoftware/manticoresearch/pull/3839) 将buddy从3.35.4更新到3.35.5以更正SQL查询中匹配连接的正则表达式。
* 🪲 [v13.13.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.7) [ Issue #3815](https://github.com/manticoresoftware/manticoresearch/issues/3815) 将buddy从3.35.3更新到3.35.4以修复REPLACE中负ID的问题。
* 🪲 [v13.13.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.6) [ PR #3830](https://github.com/manticoresoftware/manticoresearch/pull/3830) 将buddy从3.35.1更新到3.35.3。
...
* 🪲 [v13.13.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.1) [ Issue #3800](https://github.com/manticoresoftware/manticoresearch/issues/3800) 修复了在没有cjk/jieba支持时的构建问题。
...
### 新功能与改进
* 🆕 [v13.13.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.0) 新增对带 SI 块缓存的 [MCL](https://github.com/manticoresoftware/columnar) 8.1.0 的支持。
...
* 🪲 [v13.11.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.11.6) [ PR #3766](https://github.com/manticoresoftware/manticoresearch/pull/3766) 新增对 [MCL](https://github.com/manticoresoftware/columnar) 8.0.6 版本的支持。
...
* 🪲 [v13.11.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.11.2) [ Issue #3065](https://github.com/manticoresoftware/manticoresearch/issues/3065) 修复了在启用 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 的 columnar 表中插入数据时的崩溃。
...
* 🪲 [v13.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.5.1) [ Issue #3560](https://github.com/manticoresoftware/manticoresearch/issues/3560) 修复了 [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 在 KNN 数据下可能无限期挂起的问题。
...
* ⚠️ [v13.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.0.0) 更新了 KNN 库 API，以支持空的 [float_vector](https://manual.manticoresearch.com/Creating_a_table/Data_types#Float-vector) 值。此更新不会改变数据格式，但会提高 Manticore Search / [MCL](https://github.com/manticoresoftware/columnar) 的 API 版本。
* ⚠️ [v12.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/12.0.0) [ PR #3516](https://github.com/manticoresoftware/manticoresearch/pull/3516) 修复了 KNN 索引训练和构建期间源行 ID 与目标行 ID 不正确的 bug。此更新不会改变数据格式，但会提高 Manticore Search / [MCL](https://github.com/manticoresoftware/columnar) 的 API 版本。
...
* 🪲 [v10.2.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/10.2.7) 新增对 [MCL](https://github.com/manticoresoftware/columnar) 5.0.5 的支持，包括 embeddings 库文件名修复
...
* 🪲 [v9.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.2) 修复：再次更新 deps.txt，纳入 [MCL](https://github.com/manticoresoftware/columnar) 中与 embeddings 库相关的打包修复
* 🪲 [v9.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.1) 修复：更新 deps.txt，加入 [MCL](https://github.com/manticoresoftware/columnar) 和 embeddings 库的打包修复
...
* [Issue #3189](https://github.com/manticoresoftware/manticoresearch/issues/3189) 已修复：现在 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 语句可以正确遵循 `query_log_min_msec` 设置。
...
* [Issue #3054](https://github.com/manticoresoftware/manticoresearch/issues/3054) 修复了 [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) 命令的一个问题。
...
* ⚠️ BREAKING [Issue #2308](https://github.com/manticoresoftware/manticoresearch/issues/2308) 在 [ALTER CLUSTER ADD` 和 `DROP](Creating_a_cluster/Setting_up_replication/Adding_and_removing_a_table_from_a_replication_cluster.md#Adding-and-removing-a-table-from-a-replication-cluster) 中增加了对多表的支持。此更改也会影响复制协议。有关如何处理此更新，请参阅上一节。
* [Issue #2997](https://github.com/manticoresoftware/manticoresearch/issues/2997) 修复了Macos上的dlopen问题。
* [Commit 4954](https://github.com/manticoresoftware/manticoresearch/commit/4954b5de7341a29902a9b8fbb9a040f7942c77c4) 更改了带 KNN 索引表上 [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 的默认 cutoff，以提升搜索性能。
...
* [Issue #1818](https://github.com/manticoresoftware/manticoresearch/issues/1818) 当表被冻结时，[OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 现在会抛出错误。
...
* [Issue #2265](https://github.com/manticoresoftware/manticoresearch/issues/2265) 修复了在启用 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 时恢复 `mysqldump` 失败的问题。
...
* [Issue #1849](https://github.com/manticoresoftware/manticoresearch/issues/1849) 修复了与 [MCL](https://github.com/manticoresoftware/columnar) 一起使用时 indextool 在关闭阶段崩溃的问题。
...
### 与 [MCL](https://github.com/manticoresoftware/columnar) 相关（columnar、secondary、knn 库）
...
### 小改动
* [Buddy #153](https://github.com/manticoresoftware/manticoresearch-buddy/issues/153)：HTTP端点[/pq](Data_creation_and_modification/Adding_documents_to_a_table/Adding_rules_to_a_percolate_table.md?client=JSON#Adding-rules-to-a-percolate-table)现在作为`/json/pq` HTTP端点的别名。
...
* [Commit 3c84](https://github.com/manticoresoftware/manticoresearch/commit/3c84091f)：现在可以使用`SELECT`进行任意计算并显示`@@sysvars`。与之前不同，您不再仅限于一个计算。因此，类似`select user(), database(), @@version_comment, version(), 1+1 as a limit 10`的查询将返回所有列。请注意，可选的'limit'将始终被忽略。
* [Commit 6aca](https://github.com/manticoresoftware/manticoresearch/commit/6aca32f0)：实现了`CREATE DATABASE`的存根查询。
* [Commit 9dc1](https://github.com/manticoresoftware/manticoresearch/commit/9dc12334)：执行 `[ALTER TABLE table REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index)` 时，即使属性未更新，二级索引也会始终重建。
* [Commit 46ed](https://github.com/manticoresoftware/manticoresearch/commit/46edb089)：在使用CBO之前，现在会先识别使用预计算数据的排序器，以避免不必要的CBO计算。
* [Commit 102a](https://github.com/manticoresoftware/manticoresearch/commit/102ac604)：实现模拟并利用全文表达式堆栈以防止守护进程崩溃。
* [Commit 979f](https://github.com/manticoresoftware/manticoresearch/commit/979fa27c)：为不使用字符串/mvas/json属性的匹配添加了快速代码路径以进行匹配克隆。
* [Commit a073](https://github.com/manticoresoftware/manticoresearch/commit/a0735ffe)：添加了`SELECT DATABASE()`命令的支持。但是，它将始终返回`Manticore`。此添加对于与各种MySQL工具的集成至关重要。
...
* [Commit d70b](https://github.com/manticoresoftware/manticoresearch/commit/d70b0d58)：现在可以使用`SET`语句在运行时更改`thread_stack`。会话本地和守护进程范围的变体均可用。当前值可以在`show variables`输出中访问。
* [Commit d96e](https://github.com/manticoresoftware/manticoresearch/commit/d96ec6b9)：已将代码集成到CBO中，以更准确地估计在字符串属性上执行过滤器的复杂性。
...
* [Commit f3ae](https://github.com/manticoresoftware/manticoresearch/commit/f3ae8bea)：现在可以在`SHOW STATUS`命令中看到类似Linux上'uptime'的负载指标。
* [Commit f3cc](https://github.com/manticoresoftware/manticoresearch/commit/f3cc0971)：`DESC`和`SHOW CREATE TABLE`的字段和属性顺序现在与`SELECT * FROM`匹配。
...
* [Issue #271](https://github.com/manticoresoftware/manticoresearch/issues/271) "有时CALL SUGGEST不会建议单字母拼写错误的更正"：改进了[SUGGEST/QSUGGEST](Searching/Spell_correction.md#CALL-QSUGGEST,-CALL-SUGGEST)对短单词的行为：添加了`sentence`选项以显示整个句子
* [Issue #696](https://github.com/manticoresoftware/manticoresearch/issues/696) "当启用词干提取时，percolate索引无法通过精确短语查询正确搜索"：已修改percolate查询以处理精确术语修饰符，从而改进搜索功能。
* [Issue #829](https://github.com/manticoresoftware/manticoresearch/issues/829) "DATE FORMATTING方法"：添加了[date_format()](Functions/Date_and_time_functions.md#DATE_FORMAT())选择列表表达式，该表达式公开了`strftime()`函数。
* [Issue #961](https://github.com/manticoresoftware/manticoresearch/issues/961) "通过HTTP JSON API对桶进行排序"：在HTTP接口中，为每个聚合桶引入了可选的[sort属性](Searching/Faceted_search.md#HTTP-JSON)。
...
* [Issue #1070](https://github.com/manticoresoftware/manticoresearch/issues/1070) "CBO提示不支持多个属性"：启用了索引提示以处理多个属性。
* [Issue #1106](https://github.com/manticoresoftware/manticoresearch/issues/1106) "向HTTP搜索查询添加标签"：已向[HTTP PQ响应](Data_creation_and_modification/Adding_documents_to_a_table/Adding_rules_to_a_percolate_table.md?client=JSON#Adding-rules-to-a-percolate-table)添加了标签。
* [Issue #1301](https://github.com/manticoresoftware/manticoresearch/issues/1301) "buddy不应并行创建表"：解决了导致并行`CREATE TABLE`操作失败的问题。现在，一次只能运行一个`CREATE TABLE`操作。
* [Issue #1303](https://github.com/manticoresoftware/manticoresearch/issues/1303) "添加对列名中@的支持"。
* [Issue #1316](https://github.com/manticoresoftware/manticoresearch/issues/1316) "使用ps=1时，出租车数据集的查询速度很慢"：已改进CBO逻辑，并将默认直方图分辨率为8k，以提高对随机分布值属性的准确性。
* [Issue #1317](https://github.com/manticoresoftware/manticoresearch/issues/1317) "修复CBO与hn数据集上的全文搜索"：已实现增强逻辑以确定何时使用位图迭代器交集，何时使用优先队列。
...
* `/bulk`端点现在将空行作为[提交](Data_creation_and_modification/Transactions.md#BEGIN,-COMMIT,-and-ROLLBACK)命令处理。更多信息[此处](Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md?client=JSON#Bulk-adding-documents)。
* 已为[无效的索引提示](Searching/Options.md#Query-optimizer-hints)实现警告，提供更多信息透明度并允许错误缓解。
...
* ⚠️ [Issue #1057](https://github.com/manticoresoftware/manticoresearch/issues/1057)：[MCL](https://github.com/manticoresoftware/columnar) 库将不再在不支持 SSE 4.2 的系统上加载。
...
* [Commit 2757](https://github.com/manticoresoftware/columnar/commit/2757b99b) 如果磁盘块合并被中断，守护进程现在会清理与 [MCL](https://github.com/manticoresoftware/columnar) 相关的 tmp 文件。
...
* [Commit 06df](https://github.com/manticoresoftware/manticoresearch/commit/06dfdd2e668933495dd6cb52d5ba2e0aa9b26e58) 类似 `select attr, count(*) from plain_index` 这样的查询（无过滤）在你使用 [MCL](https://github.com/manticoresoftware/columnar) 时现在会更快。
...
* **⚠️ BREAKING CHANGE**: [MCL Issue #17](https://github.com/manticoresoftware/columnar/issues/17) [MCL](https://github.com/manticoresoftware/columnar): 为 columnar 扫描添加 SSE 代码。[MCL](https://github.com/manticoresoftware/columnar) 现在至少需要 SSE4.2。
...
* **⚠️ BREAKING CHANGE**: 包含 Manticore Columnar Library 的自定义 Homebrew formula。要安装 Manticore、[MCL](https://github.com/manticoresoftware/columnar) 以及其他必要组件，请使用以下命令 `brew install manticoresoftware/manticore/manticoresearch manticoresoftware/manticore/manticore-extra`。
...
* [MCL Issue #14](https://github.com/manticoresoftware/columnar/issues/14) [MCL](https://github.com/manticoresoftware/columnar)：当 ft 字段过多时，select 会崩溃
...
* [Issue #802](https://github.com/manticoresoftware/manticoresearch/issues/802) [MCL](https://github.com/manticoresoftware/columnar)：5.0.2 和 manticore-columnar-lib 1.15.4 中的 Indexer 崩溃
...
* [Issue #843](https://github.com/manticoresoftware/manticoresearch/issues/843) [MCL](https://github.com/manticoresoftware/columnar)：长文本字段问题
...
* [Issue #919](https://github.com/manticoresoftware/manticoresearch/issues/919) [MCL](https://github.com/manticoresoftware/columnar)：Manticore 在查询执行时崩溃，且其他进程在集群恢复期间崩溃。
...
* [Issue #982](https://github.com/manticoresoftware/manticoresearch/issues/982) [MCL](https://github.com/manticoresoftware/columnar)：来自二级索引的 5.0.3 崩溃
...
* [Issue #995](https://github.com/manticoresoftware/manticoresearch/issues/995) [MCL](https://github.com/manticoresoftware/columnar)：taxi1 中的 min(pickup_datetime) 崩溃
...
* [Issue #1000](https://github.com/manticoresoftware/manticoresearch/issues/1000) [MCL](https://github.com/manticoresoftware/columnar)：在未加载 columnar 库的情况下运行后，Columnar rt 索引损坏
...
* [Commit 7ec7](https://github.com/manticoresoftware/manticoresearch/commit/7ec76d4abf01b5fca6f10a0000fe64d5d8e5fa52) 由 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 引起的 RAM 耗尽问题
...
- **[Manticore Columnar Library](https://github.com/manticoresoftware/columnar) 的完整支持**。之前 Manticore Columnar Library 仅支持普通索引。现在支持：
  - 用于 `INSERT`、`REPLACE`、`DELETE`、[OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 的实时索引
  - 在复制中
  - 在 `ALTER` 中
  - 在 `indextool --check` 中
- **自动索引压缩**（[Issue #478](https://github.com/manticoresoftware/manticoresearch/issues/478)）。你终于不必再手动、或通过 crontask/其他自动化来调用 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 了。Manticore 现在会默认自动替你完成。你可以通过全局变量 [optimize_cutoff](Server_settings/Setting_variables_online.md) 设置默认压缩阈值。
...
- **实时索引中批量 INSERT 性能显著提升**。例如在 [Hetzner 的服务器 AX101](https://www.hetzner.com/dedicated-rootserver/ax101) 上，配备 SSD、128 GB 内存和 AMD 的 Ryzen™ 9 5950X（16*2 核心）**使用 3.6.0 版本每秒可插入 236K 文档**到具有模式 `name text, email string, description text, age int, active bit(1)` 的表中（默认 `rt_mem_limit`，批量大小 25000，16 个并发插入工作线程，总共插入 1600 万文档）。在 4.0.2 中，相同的并发/批量/数量可达到 **每秒 357K 文档**。
  ...
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 和 DELETE 会针对现有块执行，但对于可能正在发生的合并，写入会先收集起来，然后再应用到新的块上
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 会对每个块按顺序获取独占锁。合并在进入从块收集属性的阶段时会获取共享锁。因此同一时间只有一个操作（合并或更新）可以访问该块的属性。
  - 当合并进入需要属性的阶段时，它会设置一个特殊标志。[UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 完成后会检查该标志，如果已设置，就把整个更新存入一个特殊集合。最后，当合并结束时，它会将这些更新应用到新生成的磁盘块。
  - ALTER 通过独占锁运行
...
- [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 会自动执行。如果你不需要它，请确保在配置文件的 `searchd` 部分将 `auto_optimize=0` 设为 0
...
- [FORCE/IGNORE INDEX(id)](Data_creation_and_modification/Updating_documents/UPDATE.md#Updates-via-SQL) 用于 SELECT/[UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
- 合并磁盘块的块id现在是唯一的
...
* [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 会把磁盘块减少到若干个块（默认是 `2* 核心数`），而不是一个。最优块数可通过 [cutoff](Securing_and_compacting_a_table/Compacting_a_table.md#Number-of-optimized-disk-chunks) 选项控制。
...
* 过滤运算符 [in](Searching/Filters.md#Set-filters) 现在可通过 HTTP JSON 接口使用。
...
* [INSERT INTO PQ VALUES()](Data_creation_and_modification/Adding_documents_to_a_table/Adding_rules_to_a_percolate_table.md)（即不提供列列表）之前期望值恰好是 `(query, tags)`。现在已改为 `(id,query,tags,filters)`。如果你希望自动生成 id，可以将 id 设为 0。
* [allow_empty=0](Searching/Highlighting.md#allow_empty) 现在是通过 HTTP JSON 接口进行高亮时的新默认值。
...
* 改进的SSL支持
* 更新了非CJK内置字符集
* 在查询日志中禁用了 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)/DELETE 语句对 SELECT 的记录
* RHEL/CentOS 8软件包
...
* [Commit d0a7](https://github.com/manticoresoftware/manticoresearch/commit/d0a7c95906b9b5ba33c98267d54501cfe27508d6) 修复 #250 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 索引选项在 TSV 和 CSV 管道源中的问题
...
* 通过 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 更新文档 ID 已被禁用
...
* 可以使用 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 更新 String、JSON 和 MVA
* killlist在索引加载时应用
* killlist_target指令
* 多AND搜索加速
* 更好的平均性能和RAM使用
...
* ALTER CLUSTER 会发布 [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) 节点
* node_address指令
* 在SHOW STATUS中打印节点列表
...
* [Commit 4ffa](https://github.com/manticoresoftware/manticoresearch/commit/4ffa623e9d357a3b0b441615089e211f92f8de32) 修复了磁盘块上的总文档计数器受 [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) 命令影响并破坏权重计算的问题
...
* [Commit e386](https://github.com/manticoresoftware/manticoresearch/commit/e386d842b36195719e459b2db1fb20e402f9a51f) 修复了在启用 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) 的 RT 索引上执行 DELETE 时服务器崩溃的问题
...
* GROUP N BY 与 MVA 属性一起正确工作
* 黑洞代理在单独的线程上运行，以免影响主查询
* 在索引上实现引用计数，以避免轮换和高负载导致的阻塞
* 实现了 SHA1 哈希，尚未对外暴露
* 修复了在 FreeBSD、macOS 和 Alpine 上的编译问题

### 修复
* [提交 9897](https://github.com/manticoresoftware/manticore/commit/989752b) 块索引的过滤回归
...
* [提交 4a4b](https://github.com/manticoresoftware/manticore/commit/4a4bda5) 修复了多查询中使用池化属性时的崩溃
* [提交 3873](https://github.com/manticoresoftware/manticore/commit/3873bfb) 通过防止索引页面包含到核心文件中来修复核心文件大小减少的问题
* [提交 11e6](https://github.com/manticoresoftware/manticore/commit/11e6254) 修复了启动时指定无效代理时的 searchd 崩溃
...
* [提交 7239](https://github.com/manticoresoftware/manticore/commit/72395d9) 修复了带有错误查询的多查询时的服务器崩溃
* [提交 f353](https://github.com/manticoresoftware/manticore/commit/f353326) 修复了共享锁到独占锁的问题
* [提交 3754](https://github.com/manticoresoftware/manticore/commit/3754785) 修复了无索引查询时的服务器崩溃
* [提交 29f3](https://github.com/manticoresoftware/manticore/commit/29f360e) 修复了服务器死锁
