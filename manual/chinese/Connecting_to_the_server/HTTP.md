...
失败的查询（包括存在语法错误的查询）会返回 HTTP 状态 `500`，并在响应体的 `error` 字段中给出原因，因此应读取响应体，而不是把每个 `500` 都当作服务器故障。若要检查 HTTP 接口是否已启动，例如在启动脚本中，可以发送 `GET /`，它会返回 `200`。不带查询的 `GET /sql` 会返回 `400`，因此不能用作就绪检查。
