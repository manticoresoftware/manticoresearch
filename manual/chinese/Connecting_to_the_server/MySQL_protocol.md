...
3. **执行查询：** Manticore 使用预编译的计划和提供的参数值来执行该语句。
...
* `?`（问号）：表示单个参数（整数、浮点数或字符串）。字符串值会自动处理——例如，特殊字符如单引号会被转义，值会被引号括起来。布尔值可以作为字符串传递（`'true'`，`'false'`）。
* `?VEC?`：表示作为字符串提供的数字值列表（例如，`1,2,3` 或 `0.3,15E+3, 20 ,INF`）。仅允许数字、逗号和空格。验证后，值会严格按照提供的形式插入（无需额外转义或引号）。

## 向量参数（MVA/浮点向量）

在标准 Manticore SQL 语法中，[MVA](../Data_types.md#Multi-value-integer-%28MVA%29) 或 [浮点向量](Creating_a_table/Data_types.md#Float-vector) 以括号括起的值列表形式书写，例如 `(1, 2, 3)`。 
