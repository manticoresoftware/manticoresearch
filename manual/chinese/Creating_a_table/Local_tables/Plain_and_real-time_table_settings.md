...
此设置决定了 kill-list 将应用于哪个表。当前表中更新或删除的目标表中的匹配项将被抑制。在 `:kl` 模式中，要抑制的文档取自 [kill-list](../../Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Killlist_in_plain_tables.md)。在 `:id` 模式中，当前表中的所有文档 ID 在目标表中被抑制。如果两者都未指定，则两种模式均生效。[在此了解有关 kill-list 的更多信息](../../Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Killlist_in_plain_tables.md)

```ini
值：**未指定**（默认），target_table_name:kl，target_table_name:id，target_table_name。可指定多个值
```
...
```ini
此配置设置决定了哪些属性应存储在[列存储](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages)中，而不是行存储中。
您可以设置 `columnar_attrs = *` 来将所有支持的数据类型存储到列存储中。
```
...
默认情况下，所有存储在列存储中的字符串属性均存储预计算哈希。这些哈希用于分组和过滤。但它们占用额外空间，如果您不需要按该属性分组，可以通过禁用哈希生成来节省空间。
...
```sql
有关数据类型的更多信息，请参见[这里的更多内容](../../Creating_a_table/Data_types.md)。
```
...
| [text](../../Creating_a_table/Data_types.md#Text) | [rt_field](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_field)  | 选项：indexed，stored。默认：**both**。若仅保存文本但不索引，指定 "stored"。若仅索引文本不保存，指定 "indexed"。 | string |
...
| [string](../../Creating_a_table/Data_types.md#String) | [rt_attr_string](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_attr_string) | 字符串。选项 `indexed, attribute` 可同时使值全文索引且可筛选、排序和分组  |   |
...
| [bit(n)](../../Creating_a_table/Data_types.md#Integer) | [rt_attr_uint field_name:N](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_attr_uint) | N 表示要保存的最大位数  |   |
##### 通过 CREATE TABLE 创建实时表的示例
CREATE TABLE products (title text, price float) morphology='stem_en'

<!-- intro -->
这将创建一个 "products" 表，包含两个字段："title"（全文）和 "price"（浮点数），并将 "morphology" 设置为 "stem_en"。
<!-- request SQL -->
...
这将创建一个 "products" 表，包含三个字段：

```sql
* "title" 被索引，但不保存。
```
* "description" 被保存，但不索引。
* "author" 同时保存并索引。
...
```ini
该引擎设置更改表中所有属性的默认[属性存储](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages)。您也可以为每个属性[单独指定 engine](../../Creating_a_table/Data_types.md#How-to-switch-between-the-storages)。
有关如何为普通表启用列存储的信息，请参见 [columnar_attrs](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#columnar_attrs) 。
```

取值：

* columnar - 为所有表属性启用列存储，除 [json](../../Creating_a_table/Data_types.md#JSON) 之外

* **rowwise (默认)** - 不改变任何内容，使用传统的行存储。
## 其他设置
以下设置适用于实时表和普通表，无论是通过配置文件指定还是使用 `CREATE` 或 `ALTER` 命令在线设置。
...
在 seek+read 模式下，服务器使用 `pread` 系统调用读取文档列表和关键词位置，这些由 `*.spd` 和 `*.spp` 文件表示。服务器使用内部读取缓冲区来优化读取过程，缓冲区的大小可以通过选项 [read_buffer_docs](../../Server_settings/Searchd.md#read_buffer_docs) 和 [read_buffer_hits](../../Server_settings/Searchd.md#read_buffer_hits) 调整。还有一个选项 [preopen](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#preopen) 控制 Manticore 启动时如何打开文件。
在 mmap 访问模式下，搜索服务器使用 `mmap` 系统调用将表的文件映射到内存中，操作系统缓存文件内容。选项 [read_buffer_docs](../../Server_settings/Searchd.md#read_buffer_docs) 和 [read_buffer_hits](../../Server_settings/Searchd.md#read_buffer_hits) 对此模式下的对应文件无效。mmap 读取器还可以使用特权调用 `mlock` 锁定表的数据到内存，防止操作系统将缓存数据换出到磁盘。

要控制使用哪种访问模式，可以使用选项 **access_plain_attrs**、**access_blob_attrs**、**access_doclists**、**access_hitlists** 和 **access_dict**，其可选值如下：
...
| mlock | 服务器将表文件映射到内存中，然后执行 mlock() 系统调用，缓存文件内容并锁定在内存中以防止换出 |
| 设置 | 值 | 描述 |
| - | - | - |


| access_plain_attrs  | mmap, **mmap_preread** (默认), mlock | 控制如何读取 `*.spa`（普通属性）`*.spe`（跳跃列表）`*.spt`（查找表）`*.spm`（被杀文档） |
...
以下表格有助于选择所需模式：
| 表部分 | 保持在磁盘上 | 保持在内存中 | 启动时缓存到内存 | 锁定在内存中 |

| - | - | - | - | - |

| [行存储](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages)（非列存储）中的普通属性、跳跃列表、词表、查找表、被杀文档 | mmap | mmap | **mmap_preread**（默认） | mlock |
| 行存储字符串、多值属性 (MVA) 和 json 属性 | mmap | mmap | **mmap_preread**（默认） | mlock |
| [列存储](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages) 数值、字符串和多值属性 | 始终 | 仅通过操作系统 | 否 | 不支持 |
| 文档列表 | **file**（默认） | mmap | 否 | mlock |
| 命中列表 | **file**（默认） | mmap | 否 | mlock |
| 字典 | mmap | mmap | **mmap_preread**（默认） | mlock |
##### 建议如下：
* 若追求 **最快的搜索响应时间** 及充足的内存，使用 [行存储](../../Creating_a_table/Data_types.md#JSON) 属性并利用 `mlock` 将其锁定在内存中。同时，文档列表和命中列表也使用 mlock。

* 若优先考虑 **启动后不能接受性能降低**，并愿意接受较长的启动时间，使用选项 [--force-preread](../../Starting_the_server/Manually.md#searchd-command-line-options)。如果希望更快的 searchd 重启，保持默认的 `mmap_preread` 选项。

* 若想 **节约内存**，但仍保证所有属性能装入内存，则跳过 `mlock` 的使用。操作系统将根据频繁的磁盘读取决定哪些保留在内存中。
* 若行存储属性 **无法全部装入内存**，选择 [列存储属性](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages)。
* 若全文搜索 **性能不是重点** ，且希望节省内存，使用 `access_doclists/access_hitlists=file`。
默认模式权衡了：
* mmap，
...
* 对列存储属性的无预读寻址读取，
* 对文档列表和命中列表的无预读寻址读取。
这在大多数情况下提供了合理的搜索性能、内存利用率和更快的 searchd 重启。
### 其他性能相关设置
...
此设置为更新 blob 属性（如多值属性（MVA）、字符串和 JSON）预留额外空间。默认值为 128k。更新这些属性时，其长度可能会改变。若更新后的字符串比之前短，则覆盖 `*.spb` 文件中的旧数据；若更新后的字符串更长，则写入 `*.spb` 文件末尾。该文件为内存映射文件，根据操作系统内存映射文件的实现，调整大小可能较慢。为避免频繁调整，可以使用此设置在 .spb 文件末尾预留额外空间。
...
此设置控制文档存储所使用的块大小。默认值为16kb。当使用stored_fields或stored_only_fields存储原始文档文本时，文本存储在表内并被压缩以提高效率。为了优化小文档的磁盘访问和压缩率，这些文档会被连接成块。索引过程会收集文档，直到它们的总大小达到此选项指定的阈值。此时，该文档块会被压缩。可通过调整此选项来实现更好的压缩率（通过增加块大小）或更快的文档文本访问速度（通过减小块大小）。
...
此设置决定用于压缩存储在文档存储中的文档块的压缩类型。如果指定了stored_fields或stored_only_fields，文档存储会存储压缩后的文档块。‘lz4’ 提供快速的压缩和解压速度，而‘lz4hc’（高压缩）则以牺牲部分压缩速度为代价获得更好的压缩率。‘none’ 完全禁用压缩。
...
在文档存储中使用‘lz4hc’压缩时的压缩级别。通过调整压缩级别，可以在性能和压缩率之间找到合适的平衡。请注意，当使用‘lz4’压缩时，此选项不适用。
...
此设置表明searchd应在启动或轮换时打开所有表文件，并在运行期间保持打开状态。默认情况下，文件不会被预先打开。预先打开的表每个表需要几个文件描述符，但它们消除了每个查询调用open()的需要，并且能避免高负载下表轮换期间可能出现的竞争条件。然而，如果您提供许多表服务，为了节省文件描述符，按查询打开仍可能更为高效。
...
用于存储每个关键字文档列表的缓冲区大小。增加此值将在查询执行期间占用更多内存，但可能减少I/O时间。

```ini
值：size，默认 **256k**，最小值为8k。
```
...
用于存储每个关键字命中列表的缓冲区大小。增加此值将在查询执行期间占用更多内存，但可能减少I/O时间。
...
```ini
`inplace_enable` 选项在索引普通表时减少磁盘占用，同时略微降低索引速度（大约使用2倍更少的磁盘，但性能约为原始的90-95%）。
```

索引过程主要包含两个阶段。第一阶段，收集文档，处理并按关键词进行部分排序，中间结果写入临时文件（.tmp*）。第二阶段，文档完全排序并创建最终表文件。在生产环境中即时重建表大约需要峰值的3倍磁盘空间：首先用于中间临时文件，其次用于新构建的拷贝，再次为当前仍提供服务的旧表（中间数据大小与最终表相当）。对于大型数据集合而言，这可能过于占用磁盘，`inplace_enable` 选项可用于减少占用。启用后，它会重用临时文件，将最终数据写回它们，并在完成后重命名。但这可能需要额外的临时数据块迁移，因此会带来性能影响。

此指令对[searchd](../../Starting_the_server/Manually.md)无影响，仅对[indexer](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-tool)有效。
...
[in-place inversion](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#inplace_enable) 的微调选项。控制预分配的匹配列表间隙大小。可选，默认值为0。
...
inplace_reloc_factor 设置决定索引过程中使用的内存区域中重定位缓冲区的大小。默认值为0.1。
...
控制索引过程中就地写入使用的缓冲区大小。可选，默认值为0.1。

<!-- example inplace_write_factor -->

```ini
需要注意的是，此指令仅影响[indexer](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-tool)工具，而不影响[searchd](../../Starting_the_server/Manually.md)服务器。
```
