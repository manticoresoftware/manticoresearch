...
* [感知](../Creating_a_table/Local_tables/Percolate_table.md) 表
...
建议使用[截断表](../Emptying_a_table.md)来删除表中的所有文档，因为这是一种更快速的操作。


<!-- example delete 2 -->
在此示例中，我们删除所有匹配全文查询 `test document` 的文档，表名为 `test`：
...
* JSON 中的 `query` 包含一个全文搜索子句；其语法与[JSON/update](../Data_creation_and_modification/Updating_documents/UPDATE.md#Updates-via-HTTP-JSON) 中相同。
...
<!-- example delete 4 -->
这里，从名为 `test` 的表中删除与 `id` 匹配的文档：

注意，形如 `id=N` 或 `id IN (X,Y)` 的删除形式是最快的，因为它们删除文档时不执行搜索。
另外，响应中只包含第一个被删除文档的 id，位于相应的 `_id` 字段中。
...
例如，此处删除满足全文查询 `test document` 且属性 `mva1` 值大于 206 或 `mva1` 值为 100 或 103 的文档，表名为 `test`：
...
<!-- example delete 6 -->
这是一个在集群 `cluster` 的表 `test` 中删除文档的示例。请注意，必须提供集群名称属性以及表属性，才能从复制集群内的表中删除行：
...
您还可以使用 `/bulk` 端点在单个调用中执行多个删除操作。该端点只适用于 `Content-Type` 设置为 `application/x-ndjson` 的数据。数据应格式化为换行分隔的 JSON (NDJSON)。本质上，这意味着每行应包含一个完整的 JSON 语句，并以换行符 `\n` 结尾，并可能包含 `\r`。
