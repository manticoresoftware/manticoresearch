...
`REPLACE` 的工作原理类似于 [INSERT](../../Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md)，但在插入新文档之前，它会将具有相同 ID 的先前文档标记为已删除。

如果您正在寻找原地更新，请参阅 [此部分](../../Data_creation_and_modification/Updating_documents/UPDATE.md)。
...
* `/replace`：
  ...
  ```
  `/index` 是一个别名端点，工作原理相同。
* 类 Elasticsearch 的端点 `<table>/_doc/<id>`：
  ```
  ...
  ```
  > 注意：类 Elasticsearch 的替换需要 [Manticore Buddy](../Installation/Manticore_Buddy.md)。如果它不起作用，请确保已安装 Buddy。
* 部分替换：
  ```
  ...
  ```
  `<table name>` 可以是表名或 `cluster:table` 格式。如果需要，这允许跨特定集群进行更新。
...
docs = [     {"replace": {"table" : "products", "id" : 1, "doc" : {"title" : "document one"}}},     {"replace": {"table" : "products", "id" : 2, "doc" : {"title" : "文档二"}}} ]
...
``` python
indexApi = manticoresearch.IndexApi(client)
docs = [     {"replace": {"table" : "products", "id" : 1, "doc" : {"title" : "文档一"}}},     {"replace": {"table" : "products", "id" : 2, "doc" : {"title" : "文档二"}}} ]
...
``` javascript
docs = [
    {"replace": {"table" : "products", "id" : 1, "doc" : {"title" : "文档一"}}},
    {"replace": {"table" : "products", "id" : 2, "doc" : {"title" : "文档二"}}} ];
...
<!-- response javascript -->
```javascript
{"items":[{"replace":{"table":"products","_id":1,"created":false,"result":"更新","status":200}},{"replace":{"table":"products","_id":2,"created":false,"result":"更新","status":200}}],"errors":false}
...
``` java
body = "{"replace": {"index" : "products", "id" : 1, "doc" : {"title" : "文档一"}}}" +"
"+
    "{"replace": {"index" : "products", "id" : 2, "doc" : {"title" : "文档二"}}}"+"
" ;
indexApi.bulk(body);
```
...
```java
class BulkResponse {
    items: [{replace={_index=products, _id=1, created=false, result=更新, status=200}}, {replace={_index=products, _id=2, created=false, result=更新, status=200}}]
    error: null
...
``` clike
string body = "{"replace": {"index" : "products", "id" : 1, "doc" : {"title" : "文档一"}}}" +"
"+
    "{"replace": {"index" : "products", "id" : 2, "doc" : {"title" : "文档二"}}}"+"
" ;
indexApi.Bulk(body);
...
``` rust
string body = r#"{"replace": {"index" : "products", "id" : 1, "doc" : {"title" : "文档一"}}}
    {"replace": {"index" : "products", "id" : 2, "doc" : {"title" : "文档二"}}}
"#;
...
``` go
body := "{"replace": {"index": "test", "id": 1, "doc": {"content": "文本 11", "name": "文档 11", "cat": 1 }}}" + "
" +

"{"replace": {"index": "test", "id": 2, "doc": {"content": "文本 22", "name": "文档 22", "cat": 9 }}}" +"
";
