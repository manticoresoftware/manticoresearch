...
`UPDATE` 命令将指定表中现有文档的 [行级](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages) 属性值更改为新值。请注意，您不能更新全文字段或列属性的内容。如果有这样的需要，请使用 [REPLACE](../../Data_creation_and_modification/Updating_documents/REPLACE.md)。

属性更新支持 RT、PQ 和普通表。只要属性类型存储在 [行级存储](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages) 中，就可以进行更新。

请注意，文档 ID 不能被更新。

重要的是要意识到，更新属性会禁用其 [二级索引](../../Server_settings/Searchd.md#secondary_indexes)。如果维持二级索引连续性至关重要，请考虑完全或 [部分替换](../../Data_creation_and_modification/Updating_documents/REPLACE.md?client=REPLACE+SET)该文档。

阅读更多关于 `UPDATE` 与部分 `REPLACE` 的信息 [这里](../../Data_creation_and_modification/Updating_documents/REPLACE_vs_UPDATE.md#UPDATE-vs-partial-REPLACE)。
...
当向 32 位属性分配超范围的值时，它们将被修剪到其低 32 位而不会提示。例如，如果您尝试使用值 4294967297 更新 32 位无符号整数，则实际存储的值将为 1，因为 4294967297 的低 32 位 (0x100000001 以十六进制表示) 相当于 1 (0x00000001 以十六进制表示)。

<!-- example partial JSON update -->

`UPDATE` 可以用于对数字数据类型或数字数据类型数组执行部分 JSON 更新。只需确保不要使用浮动值更新整数值，因为这将被四舍五入。
...
更新其他数据类型或在JSON属性中更改属性类型需要完整的JSON更新。
...
当使用复制时，表名应以 `cluster_name:` 作为前缀（在 SQL 中），以便更新将传播到集群中的所有节点。通过 HTTP 的查询，应设置 `cluster` 属性。有关更多信息，请参阅 [设置复制](../../Creating_a_cluster/Setting_up_replication/Setting_up_replication.md)。
...
多值属性值集必须指定为用逗号分隔的列表，括在括号中。要从多值属性中删除所有值，只需将 `()` 分配给它。
...
`OPTION` 子句是一个特定于 Manticore 的扩展，它允许您控制一些每次更新的选项。 语法为：

```sql
OPTION <optionname>=<value> [ , ... ]
```

选项与 [SELECT](../../Searching/Full_text_matching/Basic_usage.md#SQL) 语句相同。 特别对于 `UPDATE` 语句，您可以使用以下选项：

*   'ignore_nonexistent_columns' - 如果设置为 **1**，则表示更新将悄无声息地忽略有关尝试更新当前表模式中不存在的列的任何警告。 默认值为 **0**。
*   'strict' - 此选项用于部分 JSON 属性更新。 默认情况下（strict=1），如果 `UPDATE` 查询尝试对非数字属性执行更新，则会导致错误。 使用 strict=0，如果更新多个属性并且某些属性不被允许，则 `UPDATE` 不会导致错误，并只在允许的属性上执行更改（其余的被忽略）。 如果没有 `SET` 的 `UPDATE` 更改是被允许的，尽管 strict=0，该命令仍然会导致错误。

### 查询优化器提示

在极少数情况下，Manticore 内置的查询分析器可能无法正确理解查询并确定是否应按 ID 使用表。 这可能导致查询性能不佳，例如 `UPDATE ... WHERE id = 123`。
有关如何强制优化器使用 docid 索引的信息，请参见 [查询优化器提示](../../Searching/Options.md#Query optimizer hints)。

## 通过 HTTP JSON 更新

使用 HTTP JSON 协议的更新通过 `/update` 端点执行。 语法与 [/insert 端点](../../Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md) 类似，但这次 `doc` 属性是必需的。

服务器将以 JSON 对象的形式响应，说明操作是否成功。
...
需要更新的文档 ID 可以直接使用 `id` 属性设置，如前面的示例所示，或者您可以通过查询更新文档，并将更新应用于所有匹配查询的文档：
...
查询语法与 [/search 端点](../../Searching/Full_text_matching/Basic_usage.md#HTTP-JSON) 中相同。 请注意，您不能同时指定 `id` 和 `query`。
...
FLUSH ATTRIBUTES 命令确保所有活动表中内存中的属性更新都刷新到磁盘。 它返回一个标记，用于标识自服务器启动以来实际执行的磁盘属性保存的数量。
...
<!-- example bulk update -->
您可以使用 `/bulk` 端点在单个调用中执行多个更新操作。此端点仅适用于 `Content-Type` 设置为 `application/x-ndjson` 的数据。数据应格式化为以换行符分隔的 JSON (NDJSON)。本质上，这意味着每一行应包含恰好一个 JSON 语句，并以换行符 `
` 结尾，并可能以 `` 结尾。
...
`/bulk` 端点支持插入、替换和删除。每个语句以动作类型开始（在这种情况下为 `update`）。以下是支持的操作列表：

* `insert`: 插入文档。语法与 [/insert 端点](../../Quick_start_guide.md#Add-documents) 相同。
* `create`: `insert` 的同义词
...
* `delete`: 删除文档。语法与 [/delete 端点](../../Data_creation_and_modification/Deleting_documents.md) 相同。
...
`attr_update_reserve` 是一个每个表的设置，用于确定为 blob 属性更新保留的空间。此设置是可选的，默认值为 128k。

当 blob 属性（MVAs、字符串、JSON）被更新时，它们的长度可能会改变。如果更新后的字符串（或 MVA，或 JSON）比旧字符串短，则会在 `.spb` 文件中覆盖旧字符串。然而，如果更新后的字符串更长，则更新会写入 `.spb` 文件的末尾。此文件是内存映射的，这意味着调整其大小可能是一个相当缓慢的过程，这取决于操作系统对内存映射文件的实现。

为了避免频繁的调整大小，您可以使用此选项指定要在 `.spb` 文件末尾保留的额外空间。
