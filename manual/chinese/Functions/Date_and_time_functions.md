...
注意，`CURTIME()`、`UTC_TIME()`、`UTC_TIMESTAMP()` 和 `TIMEDIFF()` 可以使用任意转换函数（例如 `BIGINT()`、`DOUBLE()` 等）提升为数值类型。
...
### DAY()
<!-- example DAY -->
根据当前时区，从时间戳参数中返回月份中的整数天数（范围 1..31）。
...
### DAYOFWEEK()
<!-- example DAYOFWEEK -->
根据当前时区，返回给定时间戳参数的星期整数索引（范围 1..7）。
注意，星期从周日开始。
...
### DAYOFYEAR()
<!-- example DAYOFYEAR -->
根据当前时区，返回给定时间戳参数的年度天数整数（范围 1..366）。
...
### YEARWEEK()
<!-- example YEARWEEK -->
根据当前时区，返回给定时间戳参数的当前周第一天的年份和日期代码整数（范围 1969001..2038366）。
...
### YEARMONTH()
<!-- example YEARMONTH -->
根据当前时区，从时间戳参数中返回年份和月份代码整数（范围 196912..203801）。
...
根据当前时区，返回年份、月份和日期代码整数（范围 19691231 到 20380119）。
