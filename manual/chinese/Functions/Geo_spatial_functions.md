...
* `haversine` 在半径为 **6,384 km** 的球体上计算大圆距离。
* `adaptive` 对经度差小于 13° 的点，近似计算 WGS84 椭球体上的距离；对距离更远的点，则在 6,371 km 半径的球体上使用 haversine 公式。

两者都不等同于许多规范和库使用的 6,371.0088 km 平均半径球体；在短距离下，结果与该模型的差异最多约为 0.3%，具体取决于纬度和方向。例如，赤道上 1 度经度的距离使用 `haversine` 为 111.4218 km，使用 `adaptive` 为 111.3207 km，而在 6,371.0088 km 半径球体上为 111.1951 km。若要匹配 6,371.0088 km 半径球体，请使用 `method=haversine`，并将结果乘以 `0.99796504` (6,371.0088 / 6,384)。
