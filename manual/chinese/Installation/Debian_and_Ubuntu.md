...
从 25.0.0 版本开始，`manticore` 是一个单一捆绑包。它已包含 Manticore Buddy、Manticore Columnar Library（列式存储、二级索引、KNN 搜索和自动嵌入）、backup、load 和 Galera，因此不要再与它一起安装其他 `manticore-*` 包。旧版说明中提到的 `sudo apt install manticore manticore-extra` 现在会因 `manticore : Breaks: manticore-extra` 而失败。
