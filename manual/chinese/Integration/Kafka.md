...
开始之前，您需要：
1. **定义源：** 指定 Manticore Search 将从中读取消息的 Kafka 主题。此设置包括代理主机、端口和主题名称等详细信息。
2. **设置目标表：** 选择一个 Manticore 实时表来存储传入的 Kafka 数据。
3. **创建物化视图：** 设置物化视图（`mv`）来处理从 Kafka 到 Manticore Search 目标表的数据转换和映射。在这里，您将定义字段映射、数据转换，以及传入数据流的任何过滤器或条件。
...
所有模式键不区分大小写，即 `Products`、`products` 和 `PrOdUcTs` 被视为相同。它们都会被转换为小写。

如果您的字段名不符合 Manticore Search 允许的[字段名语法](../Creating_a_table/Data_types.md#Field-name-syntax)（例如包含特殊字符或以数字开头），则必须定义模式映射。例如，`$keyName` 或 `123field` 在 JSON 中是有效键，但在 Manticore Search 中不是有效字段名。如果尝试使用无效字段名而不进行适当映射，Manticore 会返回错误，且源创建将失败。

针对这类情况，请使用以下模式语法将无效字段名映射为有效字段名：
...
| `partition_list`    | `int [, ...]`               | 读取的分区列表，[详细](../Integration/Kafka.md#Sharding-with-Kafka)                                                |
| `batch`             | `int`                       | 处理多少条消息后再继续。默认是 `100`；否则在超时后处理剩余消息                                                      |

### 目标表

<!-- example kafka_destination -->

目标表是一个普通的实时表，用于存储 Kafka 消息处理的结果。此表应定义为匹配传入数据的模式需求，并针对应用程序的查询性能需求进行优化。关于创建实时表的更多信息请参见[此处](../Creating_a_table/Local_tables/Real-time_table.md#Creating-a-real-time-table:)。
...
物化视图作为一个查询，将数据从 Kafka 源移动到目标表，允许您使用 Manticore Search 语法自定义该查询。确保 `select` 中的字段与源中的字段匹配。
...
数据以批处理方式从 Kafka 传输到 Manticore Search，每次运行后批处理被清空。对于跨批次的计算，如 AVG，请谨慎使用，因为分批处理可能导致计算结果不符合预期。
...
如果您移除 `source` 而不删除 MV，它会自动暂停。重新创建源后，需手动使用 `ALTER` 命令解除暂停。

目前，仅支持修改物化视图。要更改 `source` 参数，请删除然后重新创建源。
...
这种方法的主要优势之一是能够通过 Kafka 实现表的 `sharding`（分片）。
...
* 在此配置中，负载均衡需要手动管理。
* 默认情况下，Kafka 不采用轮询（round-robin）策略分发消息。
* 若要实现类似轮询的分发方式，发送数据时请确保 Kafka 生产者配置了：
  * `parse.key=true`
...
Kafka 在每个批次后或处理超时后提交偏移量。如果在物化视图查询期间进程意外停止，您可能会看到重复条目。为避免此问题，请在您的 schema 中包含一个 `id` 字段，允许 Manticore Search 防止表中的重复。

### 内部工作原理

- **工作进程初始化：** 配置源和物化视图后，Manticore Search 会设置专用工作进程来处理 Kafka 的数据摄取。
- **消息映射：** 消息根据源配置的 schema 进行映射，转换为结构化格式。
- **批量处理：** 消息被分组成批次以提高处理效率。批次大小可根据性能和延迟需求调整。
- **缓冲：** 映射后的数据批次存储在缓冲表中以便高效批量操作。
- **物化视图处理：** 视图逻辑应用于缓冲表中的数据，执行转换或筛选。
- **数据传输：** 处理后的数据传输到目标实时表。
- **清理：** 每批数据处理后清空缓冲表，确保为下批数据做好准备。
