...
<!-- example status -->
查看Manticore节点高级信息的最简单方法是在MySQL客户端中运行 `status`。它将显示各种方面的信息，例如：
* 当前版本
* 是否启用了SSL
* 当前TCP端口/Unix套接字
* 运行时间
...
`SHOW STATUS` 是一个SQL语句，显示各种有用的性能计数器。IO和CPU计数器仅在 `searchd` 使用 `--iostats` 和 `--cpustats` 开关启动时可用（或通过 `SET GLOBAL iostats/cpustats=1` 启用）。
...
支持可选的 `LIKE` 子句，允许您仅选择匹配特定模式的变量。模式语法遵循标准SQL通配符，`%` 表示任意数量的任意字符，`_` 表示单个字符。
...
`SHOW STATUS` 命令提供了Manticore中各种性能指标的详细报告，包括插入/替换、搜索和更新查询的查询时间统计。这些统计是在1、5和15分钟的滑动窗口内计算的，显示查询时间的平均值、最小值、最大值以及第95/99百分位数。

这些指标帮助跟踪特定时间间隔内的性能，使识别查询响应时间趋势和潜在瓶颈变得更容易。

`SHOW STATUS` 输出包含以下指标：
- `*_avg`：每种查询类型在过去1、5和15分钟的平均查询时间。
- `*_min`：每种查询类型记录到的最短查询时间。
- `*_max`：每种查询类型记录到的最长查询时间。
- `*_pct95`：95%查询完成所需时间。
- `*_pct99`：99%查询完成所需时间。

这些统计分别针对插入/替换（`insert_replace_stats_*`）、搜索（`search_stats_*`）和更新（`update_stats_*`）查询，提供不同操作性能的详细洞见。

如果在监控周期内没有执行查询，系统将显示 `N/A`。
...
`SHOW SETTINGS` 是一个SQL语句，显示配置文件中的当前设置。设置名称格式如下： `'config_section_name'.'setting_name'`

结果还包括两个额外值：
- `configuration_file` - 配置文件路径
- `worker_pid` - 正在运行的 `searchd` 实例的进程ID
...
`SHOW AGENT STATUS` 显示[远程代理](../Creating_a_table/Creating_a_distributed_table/Remote_tables.md#agent)或分布式表的统计信息。包括最后请求时长、最后响应时长、各种类型错误和成功次数等值。统计数据显示过去1、5和15个由 [ha_period_karma](../Server_settings/Searchd.md#ha_period_karma) 秒组成的时间间隔内每个代理的情况。
