...
`SHOW QUERIES` 返回关于所有正在运行的查询的信息。输出是一个具有以下结构的表：

- `id`：查询 ID，可用于 [KILL](../Node_info_and_management/KILL.md) 中终止查询
- `query`：查询语句或其部分内容
- `time`：命令执行耗时或查询执行的时间（在这种情况下，值将包含 `ago`）
- `protocol`：[连接协议](../Server_settings/Searchd.md#listen)，可能的值包括 `sphinx`、`mysql`、`http`、`ssl`、`compressed`、`replication`，或其组合（例如，`http,ssl` 或 `compressed,mysql`）
- `host`：客户端的 `ip:port`
...
如果想从线程本身的角度获取信息，请参阅 [SHOW THREADS](../Node_info_and_management/SHOW_THREADS.md)。
