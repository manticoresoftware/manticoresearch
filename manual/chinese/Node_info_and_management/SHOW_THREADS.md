...
`SHOW THREADS` 是一个 SQL 语句，用于显示关于所有线程及其当前活动的信息。

结果表包含以下列：

* `TID`: 内核分配给线程的 ID
* `Name`: 线程名称，也可在 `top`、`htop`、`ps` 和其他进程查看工具中看到
...
* `ConnID`: 连接 ID（从 0 开始）
* `This/prev job time`: 当线程忙时 - 当前作业运行了多长时间；当线程空闲时 - 上一个作业持续时间 + 后缀 `prev`
* `Jobs done`: 此线程完成的作业数量
* `Thread status`: `idling` 或 `working`
* `Info`: 关于查询的信息，如果查询针对分布式表或实时表，可能包括多个查询
...
```javascript
res = await utilsApi.sql('显示线程选项列=30');
```
...
```rust
utils_api.sql("显示线程选项列=30", Some(true)).await;
```
...
```go
apiClient.UtilsAPI.Sql(context.Background()).Body("显示线程选项列=30").Execute()
```
...
`SHOW THREADS` 是一个 SQL 语句，用于显示有关所有线程及其当前活动的信息。

结果表包含以下列：

* `TID`: 内核分配给线程的 ID
* `Name`: 线程名称，在 `top`、`htop`、`ps` 和其他进程查看工具中也可见
* `Proto`: 连接协议；可能的值包括 `sphinx`、`mysql`、`http`、`ssl`、`compressed`、`replication`，或组合（例如 `http,ssl` 或 `compressed,mysql`）
* `State`: 线程状态；可能的值为 `handshake`、`net_read`、`net_write`、`query`、`net_idle`
* `Connection from`: 客户端的 `ip:port`
* `ConnID`: 连接 ID（从 0 开始）
* `This/prev job time`: 当线程忙碌时 - 当前作业运行了多长时间；当线程空闲时 - 上一个作业持续时间 + 后缀 `prev`
* `Jobs done`: 此线程完成的作业数量
* `Thread status`: `idling` 或 `working`
* `Info`: 关于查询的信息，如果查询指向一个分布式表或实时表，可能包含多个查询
...
* 原始文本查询通过 Manticore SQL 接口执行
* 完整文本语法、注释和通过内部 Manticore 二进制协议运行的查询的数据大小（例如，从远程 Manticore 实例）
