...
* `tid` 和 `tid_saved`：表示保存表的状态。`tid` 随每次更改（事务）递增。`tid_saved` 显示在 `<table>.ram` 文件中 RAM 分块中保存状态的最大 `tid`。当数字不同时，说明某些更改仅存在于 RAM 中，并且（如果启用）也由 binlog 支持。执行 `FLUSH TABLE` 或计划定期刷新可以保存这些更改。刷新后，binlog 被清除，`tid_saved` 表示新的实际状态。
* `query_time_*`，`exact_query_time_*`：查询执行时间统计，针对最近 1 分钟、5 分钟、15 分钟和自服务器启动以来的总时间；数据封装为 JSON 对象，包括查询次数及最小值、最大值、平均值、95 和 99 百分位值。
* `found_rows_*`：查询找到的行数统计；针对最近 1 分钟、5 分钟、15 分钟和自服务器启动以来的总时间；数据封装为 JSON 对象，包括查询次数及最小值、最大值、平均值、95 和 99 百分位值。
* `command_*`：特定命令成功执行针对该表的总次数计数器。
* `search_stats_ms_*`：搜索查询执行时间（毫秒）统计。星号 * 表示时间窗口（例如 1min、5min、15min、total）。这些统计基于 1、5 和 15 分钟滑动窗口计算，显示查询时间的平均值、最小值、最大值和 95th/99th 百分位值。
* `insert_replace_stats_ms_*`：插入和替换查询执行时间（毫秒）统计。星号 * 表示时间窗口（例如 1min、5min、15min、total）。这些统计基于 1、5 和 15 分钟滑动窗口计算，显示查询时间的平均值、最小值、最大值和 95th/99th 百分位值。
* `update_stats_ms_*`：更新查询执行时间（毫秒）统计。星号 * 表示时间窗口（例如 1min、5min、15min、total）。这些统计基于 1、5 和 15 分钟滑动窗口计算，显示查询时间的平均值、最小值、最大值和 95th/99th 百分位值。
