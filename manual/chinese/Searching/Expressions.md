...
Manticore 通过 SQL 和 HTTP 支持任意算术表达式，结合属性值、内部属性（文档 ID 和相关性权重）、算术运算、多种内置函数以及用户定义函数。以下是完整的参考列表，便于快速访问。
...
标准算术运算符均可使用。涉及这些运算符的算术计算可以以三种不同模式执行：

1. 使用单精度 32 位 IEEE 754 浮点值（默认），
2. 使用带符号 32 位整数，
3. 使用带符号 64 位整数。

表达式解析器会自动切换到整数模式，如果没有运算产生浮点值。否则，使用默认的浮点模式。例如，a+b 如果两个参数都是 32 位整数，则用 32 位整数计算；如果两个参数都是整数但其中一个是 64 位，则用 64 位整数计算；否则使用浮点数计算。然而，`a/b` 或 `sqrt(a)` 总是以浮点数计算，因为这些操作结果非整数。若要避免，可使用 `IDIV(a,b)` 或 `a DIV b` 形式。此外，当参数为 32 位整数时，`a*b` 不会自动提升为 64 位。若需强制使用 64 位结果，可使用 [BIGINT()](../Functions/Type_casting_functions.md#BIGINT%28%29)，但请注意，如果存在非整数运算，BIGINT() 会被忽略。
...
比较运算符在条件为真时返回 1.0，否则返回 0.0。例如， `(a=b)+3` 当属性 `a` 等于属性 `b` 时结果为 4，否则为 3。不同于 MySQL，等值比较（即 `=` 和 `<>` 运算符）包含一个小的相等阈值（默认 1e-6）。如果两个比较值的差异在阈值内，则视为相等。

对于多值属性，`BETWEEN` 和 `IN` 运算符如果至少有一个值满足条件，则返回真（类似 [ANY()](../Functions/Arrays_and_conditions_functions.md#ANY%28%29)）。`IN` 运算符不支持 JSON 属性。`IS (NOT) NULL` 运算符仅支持 JSON 属性。
...
布尔运算符 (AND, OR, NOT) 按预期工作。它们是左结合的，优先级比其它运算符低。NOT 优先级高于 AND 和 OR，但仍低于任何其它运算符。AND 和 OR 具有相同优先级，复杂表达式中建议使用括号以避免混淆。
...
目前，只支持 `inline` 表达式。`inline` 属性的值（要计算的表达式）具有与 SQL 表达式相同的语法。
...
默认情况下，表达式值包含在结果集的 `_source` 数组中。如果源选择是选择性的（参见[源选择](../Searching/Search_results.md#Source-selection)），表达式名称可以添加到请求的 `_source` 参数中。注意，表达式名称必须为小写。

### expressions

<!-- example expressions -->

`expressions` 是 `script_fields` 的一种替代方案，语法更简单。示例请求添加了两个表达式，并将结果存储到 `add_all` 和 `title_len` 属性中。注意，表达式名称必须为小写。
