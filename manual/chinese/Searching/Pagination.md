...
Manticore Search 默认返回前 20 个匹配的文档。

#### SQL
在 SQL 中，可以使用 `LIMIT` 子句来浏览结果集。

`LIMIT` 可以接受一个数字作为返回集的大小（偏移量为零），或一对偏移量和大小值。

#### HTTP JSON
使用 HTTP JSON 时，节点 `offset` 和 `limit` 控制结果集的偏移量以及返回集的大小。或者，您可以使用一对 `size` 和 `from` 来替代。
...
默认情况下，Manticore Search 使用 1000 个最佳排名文档的结果集窗口，超过该值进行分页时，查询将结束于错误。

可以通过查询选项 [max_matches](../Searching/Options.md#max_matches) 调整此限制。

将 `max_matches` 提高到非常高的值仅在导航到此类点是必要时进行。较高的 `max_matches` 值需要更多内存，并可能增加查询响应时间。处理深层结果集的一种方法是将 `max_matches` 设置为偏移量和限制的总和。

将 `max_matches` 降低到 1000 以下的好处在于减少了查询使用的内存。它还可以减少查询时间，但在大多数情况下，可能不会有明显的收益。
...
滚动搜索选项提供了一种高效且可靠的方法来分页浏览大型结果集。与传统的基于偏移量的分页不同，滚动搜索提供了更好的深度分页性能，并提供了更简单的分页实现方式。
...
首先执行带有所需排序标准的初始查询。唯一的要求是 `id` 必须包含在 ORDER BY 子句中，以确保一致的分页。查询将返回您的结果和后续页面的滚动令牌。
...
**使用滚动的分页查询**
 ...
 要检索下一页结果，请在后续查询中将滚动令牌作为选项包含。当提供 `scroll` 选项时，指定排序标准是可选的。

```sql
SELECT ... [ORDER BY [... ,] id {ASC|DESC}] OPTION scroll='<base64 编码的滚动令牌>'[, ...];
```

这确保分页无缝继续，保持在初始查询中建立的排序上下文。
...
在初始请求中，在选项中指定 `"scroll": true` 和所需的排序标准。请注意，`id` 必须出现在 `sort` 数组中。响应将包含一个滚动令牌，可用于后续请求中的分页。
...
要继续分页，请在下一个请求的选项对象中包括从上一个响应中获得的滚动令牌。指定排序标准是可选的。
