...
<!-- example sql1 -->
当你通过 MySQL 协议运行 SQL 查询时，你会收到所请求的列作为结果，如果没有找到任何内容，则返回空结果集。
...
<!-- example sql2 -->
此外，你可以使用 [SHOW META](../Node_info_and_management/SHOW_META.md) 命令查看关于最新查询的额外元信息。
...
<!-- example sql4 -->
如果出现警告，结果集将包含警告标志，你可以使用 [SHOW WARNINGS](../Node_info_and_management/SHOW_WARNINGS.md) 来查看警告。
<!-- request SQL -->
```sql
...
* `took`：执行搜索所耗费的时间，单位毫秒
* `timed_out`：查询是否超时
* `hits`：搜索结果，包含以下属性：
  - `total`：匹配文档的总数
  - `hits`：包含匹配项的数组

查询结果还可以包含查询性能分析信息。详见 [查询性能分析](../Node_info_and_management/Profiling/Query_profile.md)。
...
默认情况下，所有属性都会在 `_source` 数组中返回。你可以在请求负载中使用 `_source` 属性来选择你想包含在结果集中的字段。示例：
...
你可以将想包含在查询结果中的属性指定为字符串（`"_source": "attr*"`）或者字符串数组（`"_source": [ "attr1", "attri*" ]"`）。每个条目可以是属性名或者通配符（支持 `*`、`%` 和 `?` 符号）。

你还可以使用 `includes` 和 `excludes` 属性显式指定你想包含和排除的属性：
...
空的 includes 列表表示“包含所有属性”，而空的 excludes 列表表示不排除任何属性。如果某个属性同时匹配 includes 和 excludes，则以 excludes 为准。
