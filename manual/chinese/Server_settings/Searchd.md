...
`hostname_lookup` 选项定义了续订主机名的策略。默认情况下，服务器启动时会缓存代理主机名的 IP 地址，以避免对 DNS 进行过多访问。然而，在某些情况下，IP 地址可能会动态变化（例如云托管），此时可能不希望缓存 IP 地址。将此选项设置为 `request` 会禁用缓存，并对每个查询都查询 DNS。也可以使用 [`FLUSH HOSTNAMES`](../Securing_and_compacting_a_table/Flushing_hostnames.md) 命令手动续订 IP 地址。
