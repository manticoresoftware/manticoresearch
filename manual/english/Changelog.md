...
* ⚠️ [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #123](https://github.com/manticoresoftware/columnar/pull/123) Breaking change: updated MCL to 13.0.0, adding `API_URL` and `API_TIMEOUT` for auto-embedding models. Manticore 25.0.0 requires MCL 13.0.0 and is not compatible with older MCL versions; upgrade the daemon and MCL together, and downgrade is only safe if you also restore a matching older MCL version.
...
* 🆕 [v23.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.1.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) Added a new Manticore bundle package for deb, rpm, and macOS, replacing the previous meta package to simplify installation and distribution of bundled components.
...
* ⚠️ [v23.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.0) [ Issue #4364](https://github.com/manticoresoftware/manticoresearch/issues/4364) Breaking change: index format version is bumped to 69 to support new bigram tokenization options, including `bigram_delimiter` modes and digit-aware `bigram_index` modes. Existing older indexes remain readable, but indexes rebuilt or newly written in format 69 are not compatible with older Manticore versions, so downgrade is only safe before such indexes are rewritten.
* ⚠️ [v22.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/22.0.0) [ Issue #4301](https://github.com/manticoresoftware/manticoresearch/issues/4301) Breaking change: embedding generation for RT tables is moved out of the commit phase and replicated as vector data, while the replication API is bumped to 0x10A. This preserves user-provided vectors in tables with `model_name` and prevents replicas from regenerating different embeddings, but mixed-version replication clusters are not compatible across this change. Upgrade clustered nodes together; downgrade is only safe before the new replication protocol is used in the cluster.
...
* 🆕 [v21.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.0) [ PR #4338](https://github.com/manticoresoftware/manticoresearch/pull/4338) Updated MCL to 12.0.0 to align with the KNN early termination changes introduced in 20.0.0. No additional migration steps are required beyond 20.0.0.
* ⚠️ [v20.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/20.0.0) [ PR #4335](https://github.com/manticoresoftware/manticoresearch/pull/4335) Breaking change: implemented KNN early termination. KNN search can now stop earlier once enough candidates are found, which changes query execution behavior and may affect which candidates are explored and returned in borderline cases. No special migration is required, and downgrade is possible since this change does not introduce a new index or protocol format.
...
* 🆕 [v19.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.3) [ Issue #4303](https://github.com/manticoresoftware/manticoresearch/issues/4303) Added separate `interactive_timeout` and `wait_timeout` handling for SQL connections, so interactive clients and non-interactive clients can use different idle timeouts like MySQL.
...
* ⚠️ [v18.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.0) [ Issue #4261](https://github.com/manticoresoftware/manticoresearch/issues/4261) Breaking change: index format version is bumped to 68 to fix `hitless_words` dictionary layout handling. Existing older indexes remain readable, but any indexes rebuilt or newly written in the new format are not compatible with older Manticore versions, so downgrade is only safe before such indexes are rewritten.
