...
* 🆕 [v26.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.1.0) [ PR #4537](https://github.com/manticoresoftware/manticoresearch/pull/4537) Added native `type='sharding'` tables with distributed inserts, plus major sharding upgrades covering creation, routing, rollback, failover, and operational behavior.
...
* 🆕 [v25.8.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.0) [ PR #4414](https://github.com/manticoresoftware/manticoresearch/pull/4414) Added a built-in Ukrainian lemmatizer, expanding native morphology support for Ukrainian text search.
...
## Breaking Changes
* ⚠️ **26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) **Breaking change**: incoming replicated tables are now stored under the normal [`data_dir`](Server_settings/Searchd.md#data_dir)/`<table>` layout instead of the cluster `path`. This affects you if you use **replication clusters** with a custom `path`: older versions could place received replicated table files under the cluster path, so after upgrading you may need to move or re-synchronize those tables into the standard `data_dir` layout. Downgrade is only safe before adopting the new replicated-table layout.
