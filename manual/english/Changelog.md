...
* 🆕 [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) Packaging change: `manticore` is now a bundle package for deb and rpm. It includes the daemon, tools, converter, development headers, ICU data, bundled dependency packages (`manticore-columnar-lib`, `manticore-backup`, `manticore-buddy`, `manticore-executor`, `manticore-load`, `manticore-galera`, and `manticore-tzdata` where applicable), plus built-in language packs for German, English, and Russian, along with Jieba support. The Ukrainian lemmatizer is not bundled; install it separately using the [Debian/Ubuntu](Installation/Debian_and_Ubuntu.md#Ukrainian-lemmatizer) or [RHEL/CentOS](Installation/RHEL_and_Centos.md#Ukrainian-lemmatizer) instructions.
