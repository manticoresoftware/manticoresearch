...
## Breaking Changes
* ⚠️ **[v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0)** [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) **Breaking change**: if you enable [authentication and authorization](Security/Authentication_and_authorization.md), anonymous client access no longer works, and distributed remote-agent / replication operations can fail unless participating daemons have matching stored authentication data for the relevant users. A successful `JOIN CLUSTER` also replaces the joining node's local auth data with the donor cluster's auth data. Existing table data remains compatible, but migration requires bootstrapping users, granting permissions, synchronizing auth data across nodes, and reviewing cluster-join procedures. Downgrade is only safe before enabling auth and before any cluster join has replaced local auth data.
