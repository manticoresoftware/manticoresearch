...
❤️ We’d like to thank [@tudorvasinca](https://github.com/tudorvasinca) for their work on [PR #4857](https://github.com/manticoresoftware/manticoresearch/pull/4857), [PR #4859](https://github.com/manticoresoftware/manticoresearch/pull/4859), and [PR #4873](https://github.com/manticoresoftware/manticoresearch/pull/4873).

### New Features and Improvements
* 🆕 [v29.9.0](https://github.com/manticoresoftware/manticoresearch/tree/29.9.0) [PR #4898](https://github.com/manticoresoftware/manticoresearch/pull/4898) Changed the default [access_columnar_attrs](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#Accessing-table-files) mode from buffered `file` reads to `mmap`, allowing the operating system to cache columnar attribute data on demand without prereading it. Set `access_columnar_attrs='file'` explicitly to retain the previous behavior; `ALTER TABLE` now also reopens replaced columnar storage using the configured access mode.
* 🆕 [v29.8.0](https://github.com/manticoresoftware/manticoresearch/tree/29.8.0) [Backup Issue #134](https://github.com/manticoresoftware/manticoresearch-backup/issues/134) Updated [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) to 1.11.0, allowing [S3 backups and restores](Securing_and_compacting_a_table/Backup_and_restore.md#S3-storage-support) to use the AWS SDK credential provider chain—including IRSA, shared credentials, ECS task roles, and EC2 instance profiles—when static keys are not set, and adding `AWS_SESSION_TOKEN` support for temporary credentials.
* 🆕 [v29.7.0](https://github.com/manticoresoftware/manticoresearch/tree/29.7.0) [Issue #4816](https://github.com/manticoresoftware/manticoresearch/issues/4816) Added opt-in [MAX_INPUT_TOKENS](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) for local auto-embedding columns, allowing each input text to be truncated before embedding to bound long-document inference time and reduce retry-driven memory exhaustion. It can be set during `CREATE TABLE` or changed with `ALTER TABLE ... MODIFY COLUMN` without re-embedding existing rows; `0` or omission preserves the model’s own limit.
* 🆕 [v29.6.0](https://github.com/manticoresoftware/manticoresearch/tree/29.6.0) [manticore-load Issue #26](https://github.com/manticoresoftware/manticore-load/issues/26) Updated [manticore-load](https://github.com/manticoresoftware/manticore-load) to 1.28.0, adding opt-in `--http` mode for benchmarking through Manticore’s HTTP JSON API: writes use `/bulk`, searches use `/search`, HTTP defaults to port `9308`, and `--table` selects the target table.
* 🆕 [v29.5.0](https://github.com/manticoresoftware/manticoresearch/tree/29.5.0) [manticore-load PR #28](https://github.com/manticoresoftware/manticore-load/pull/28) Updated `manticore-load` to 1.27.0, adding local `searchd` RSS to live progress, peak RSS, disk, and CPU usage to final reports, and aggregate monitoring across all tables in multi-command workloads.
* 🆕 [v29.4.0](https://github.com/manticoresoftware/manticoresearch/tree/29.4.0) [MCL Issue #190](https://github.com/manticoresoftware/columnar/issues/190) Added [chunking strategies](Searching/KNN.md#Chunking-strategies) for auto-embeddings: `mean` embeds a long document as one averaged vector, while `fixed`, `recursive`, and `sentence` store multiple searchable vectors in a `float_vector_array`. The new `MAX_TOKENS`, `OVERLAP_TOKENS`, and `MAX_CHUNKS` options control chunk size, overlap, and count; existing `truncate` behavior remains the default.
* 🆕 [v29.3.12](https://github.com/manticoresoftware/manticoresearch/tree/29.3.12) [Issue #4827](https://github.com/manticoresoftware/manticoresearch/issues/4827) Added opt-in [German AOT sharp-s normalization](Creating_a_table/NLP_and_tokenization/Supported_languages.md#German-AOT-morphology-and-sharp-s) using `charset_table=non_cont,german` with `lemmatize_de_v2` or `lemmatize_de_v2_all`, so `ß`, `ẞ`, and `ss` match equivalently in ordinary whole-word searches while `index_exact_words=1` preserves their distinction in exact-word queries. Existing German morphology modes remain unchanged; switching a populated table to v2 requires rebuilding or replaying it.
* 🆕 [v29.3.4](https://github.com/manticoresoftware/manticoresearch/tree/29.3.4) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Updated [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) to 1.10.4, allowing its CLI to authenticate HTTP requests with [authentication options](Securing_and_compacting_a_table/Backup_and_restore.md#Arguments) such as `--user` and `--password` or `--token`, and ensuring selected tables are unfrozen if a backup fails or is interrupted.
* 🆕 [v29.3.0](https://github.com/manticoresoftware/manticoresearch/tree/29.3.0) [Issue #4770](https://github.com/manticoresoftware/manticoresearch/issues/4770) Added consistent [safe UTF-8 table, field, and attribute names](Creating_a_table/Data_types.md#Table-and-field-name-syntax) across RT, percolate, distributed, template, and plain tables, SQL DDL and expressions, full-text field selectors, and inferred source schemas, allowing localized names such as Chinese and Cyrillic identifiers without renaming the source schema.
* 🆕 [v29.2.0](https://github.com/manticoresoftware/manticoresearch/tree/29.2.0) [Issue #4099](https://github.com/manticoresoftware/manticoresearch/issues/4099) Added [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array) for real-time tables, allowing each document to store and search several KNN vectors such as text chunks, product images, or video keyframes.
* 🆕 [v29.1.0](https://github.com/manticoresoftware/manticoresearch/tree/29.1.0) [manticore-load PR #24](https://github.com/manticoresoftware/manticore-load/pull/24) Updated `manticore-load` to 1.26.0, adding `--worker-init` and `--worker-finalize` SQL hooks for connection-scoped setup and ordered concurrent finalization around benchmark workloads.

### Bug Fixes
* 🪲 [v29.8.4](https://github.com/manticoresoftware/manticoresearch/tree/29.8.4) [Issue #4889](https://github.com/manticoresoftware/manticoresearch/issues/4889) Fixed [hybrid search](Searching/Hybrid_search.md#Attribute-filters) ignoring filters on `weight()` or expressions and aliases derived from it; weight-dependent filters now run after fusion against the final text weight. Such filters in `OR` filter trees are not supported and now return an explicit error.
* 🪲 [v29.8.2](https://github.com/manticoresoftware/manticoresearch/tree/29.8.2) [Issue #4893](https://github.com/manticoresoftware/manticoresearch/issues/4893) Fixed valid fixed-length gzip-compressed [/bulk](Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md#Bulk-adding-documents) requests failing with HTTP 400 when the compressed body arrived across multiple socket reads; socket-read boundaries no longer affect decompression or ingestion.
* 🪲 [v29.8.1](https://github.com/manticoresoftware/manticoresearch/tree/29.8.1) [Issue #4884](https://github.com/manticoresoftware/manticoresearch/issues/4884) Fixed Elasticsearch-compatible `/_bulk` requests returning request-level HTTP `409` for item failures, which caused clients such as Fluent Bit to retry already-applied writes; processed batches now return HTTP `200`, preserve successful items, report duplicate `create` operations as per-item `409` `version_conflict_engine_exception` errors, including duplicates within the same batch, and reject malformed action metadata at request level.
* 🪲 [v29.7.2](https://github.com/manticoresoftware/manticoresearch/tree/29.7.2) [Issue #4872](https://github.com/manticoresoftware/manticoresearch/issues/4872) Fixed [ALTER TABLE](Updating_table_schema_and_settings.md#Updating-table-schema-in-RT-mode) leaving stale attribute and source-field metadata on RT tables with [auto-embeddings](Searching/KNN.md#Auto-Embeddings-%28Recommended%29), which silently discarded values written by `INSERT` or `REPLACE` to subsequently added MVA columns and could misalign embeddings after earlier columns were dropped. Values lost before upgrading must be restored from their source or a backup.
* 🪲 [v29.7.1](https://github.com/manticoresoftware/manticoresearch/tree/29.7.1) [MCL PR #205](https://github.com/manticoresoftware/columnar/pull/205) Updated [MCL](https://github.com/manticoresoftware/columnar) to 14.2.1, fixing auto-embedding models potentially remaining in memory after `ALTER TABLE ... MODIFY COLUMN ... MAX_INPUT_TOKENS` and preventing different `API_TIMEOUT` and `MAX_INPUT_TOKENS` settings from colliding in the model cache and incorrectly reusing the same model.
* 🪲 [v29.5.1](https://github.com/manticoresoftware/manticoresearch/tree/29.5.1) [MCL PR #204](https://github.com/manticoresoftware/columnar/pull/204) Fixed [KNN](Searching/KNN.md#KNN-vector-search) HNSW index construction occasionally leaving indexed vectors unreachable by KNN queries; newly built indexes now repair disconnected level-0 graph nodes before being saved. Existing indexes are not rewritten automatically and should be rebuilt if affected.
* 🪲 [v29.3.13](https://github.com/manticoresoftware/manticoresearch/tree/29.3.13) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Fixed SQL [BACKUP](Securing_and_compacting_a_table/Backup_and_restore.md#General-syntax-of-BACKUP) and `manticore-backup` failing when authentication is enabled; backup operators can now authenticate with username/password or bearer tokens and require `backup ON *` plus `read` on each selected table, or unrestricted `read ON *` for full backups. Before downgrading to a version without the `backup` action, remove all `backup` grants.
* 🪲 [v29.3.11](https://github.com/manticoresoftware/manticoresearch/tree/29.3.11) [PR #4879](https://github.com/manticoresoftware/manticoresearch/pull/4879) Fixed [LENGTH()](Functions/Arrays_and_conditions_functions.md#LENGTH%28%29) on [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array) attributes returning the internal storage-word count instead of the number of vectors; empty arrays return `0`, and the corrected value is used consistently in selection, filtering, and grouping.
* 🪲 [v29.3.10](https://github.com/manticoresoftware/manticoresearch/tree/29.3.10) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Updated Buddy to 4.4.3 to preserve the authenticated SQL user and pass Buddy’s internal bearer token to manticore-backup requests, providing the dependency-side authentication path for `BACKUP`; complete `auth=1` support, including daemon-side `backup` permission checks, requires v29.3.13.
* 🪲 [v29.3.9](https://github.com/manticoresoftware/manticoresearch/tree/29.3.9) [Issue #4639](https://github.com/manticoresoftware/manticoresearch/issues/4639) Fixed [hybrid search](Searching/Hybrid_search.md) with `GROUP BY` returning empty or missing buckets, including MVA groups; grouped results now retain all buckets and honor final `ORDER BY` and `WITHIN GROUP ORDER BY`.
* 🪲 [v29.3.8](https://github.com/manticoresoftware/manticoresearch/tree/29.3.8) [Issue #4774](https://github.com/manticoresoftware/manticoresearch/issues/4774) Fixed equality, `IN`, and range filters on [document IDs](Creating_a_table/Data_types.md#Document-ID) whose signed representation is negative returning no rows when the document-ID lookup index was used; lookup filtering now follows the index’s unsigned ID order.
* 🪲 [v29.3.7](https://github.com/manticoresoftware/manticoresearch/tree/29.3.7) [Issue #4856](https://github.com/manticoresoftware/manticoresearch/issues/4856) Fixed multi-chunk RT-table [GROUP BY](Searching/Grouping.md) queries with more groups than `max_matches` potentially returning duplicate groups and incorrect split counts, or hanging `searchd` when `COUNT(DISTINCT ...)` was ordered by that count and the select list included an expression.
* 🪲 [v29.3.6](https://github.com/manticoresoftware/manticoresearch/tree/29.3.6) [Issue #4826](https://github.com/manticoresoftware/manticoresearch/issues/4826) Updated [Buddy](Installation/Manticore_Buddy.md#Manticore-Buddy) to 4.4.2, fixing [Kafka materialized-view](Integration/Kafka.md#Materialized-view) workers permanently stopping with `Invalid JSON found` when internal `SELECT ...; SHOW META` responses contained large integer IDs or long numeric-looking strings; response values are now preserved losslessly and ingestion continues.
* 🪲 [v29.3.5](https://github.com/manticoresoftware/manticoresearch/tree/29.3.5) [Issue #4822](https://github.com/manticoresoftware/manticoresearch/issues/4822) Fixed `searchd` intermittently crashing on JSON `/search` queries to distributed tables when sorting by a string attribute and aggregating a JSON field across multiple local tables.
* 🪲 [v29.3.3](https://github.com/manticoresoftware/manticoresearch/tree/29.3.3) [Issue #4860](https://github.com/manticoresoftware/manticoresearch/issues/4860) Fixed `searchd` crashing when `DROP TABLE` raced with an in-flight `/bulk` insert into an RT table using [auto-embeddings](Searching/KNN.md#Auto-Embeddings-%28Recommended%29); the transaction now completes against the live table or fails safely if the table is dropped or replaced.
* 🪲 [v29.3.2](https://github.com/manticoresoftware/manticoresearch/tree/29.3.2) [Issue #4791](https://github.com/manticoresoftware/manticoresearch/issues/4791) Fixed HTTP [KNN queries](Searching/KNN.md#KNN-vector-search) on distributed and sharded tables with a local shard potentially crashing the coordinator or returning the wrong nearest neighbor when 1-bit quantization triggered rescoring; merged shard results are no longer rescored a second time.
* 🪲 [v29.3.1](https://github.com/manticoresoftware/manticoresearch/tree/29.3.1) [PR #4864](https://github.com/manticoresoftware/manticoresearch/pull/4864) Fixed `searchd` crashing when a multi-statement request’s second statement performed [hybrid search](Searching/Hybrid_search.md) by combining `MATCH()`, `KNN()`, and `OPTION fusion_method='rrf'`.
* 🪲 [v29.2.9](https://github.com/manticoresoftware/manticoresearch/tree/29.2.9) [Issue #4837](https://github.com/manticoresoftware/manticoresearch/issues/4837) Fixed [indextool --apply-killlists](Miscellaneous_tools.md#indextool) crashing when a current-format killlist was applied to an older plain table with a legacy JSON header; `indextool` now reads the table’s actual on-disk format version before processing lookup data.
* 🪲 [v29.2.8](https://github.com/manticoresoftware/manticoresearch/tree/29.2.8) [Issue #4854](https://github.com/manticoresoftware/manticoresearch/issues/4854) Restored full CI test coverage by running GoogleTest, API consistency, `indextool`, and other CTest-managed tests alongside ubertests.
* 🪲 [v29.2.7](https://github.com/manticoresoftware/manticoresearch/tree/29.2.7) [Issue #4842](https://github.com/manticoresoftware/manticoresearch/issues/4842) Fixed [ALTER TABLE ... RENAME](Updating_table_schema_and_settings.md#Renaming-a-real-time-table) failing with `Invalid API key for remote model` for real-time tables using remote auto-embeddings; table copies now preserve hidden embedding API keys without exposing them in `SHOW CREATE TABLE`.
* 🪲 [v29.2.6](https://github.com/manticoresoftware/manticoresearch/tree/29.2.6) [PR #4861](https://github.com/manticoresoftware/manticoresearch/pull/4861) Fixed a [KNN search](Searching/KNN.md#KNN-vector-search) performance regression in queries with additional filters by avoiding a redundant `knn_dist` prefilter when the HNSW iterator already excludes documents without vectors, while preserving correct exclusion on brute-force, full-text, grouped, and faceted KNN paths.
* 🪲 [v29.2.5](https://github.com/manticoresoftware/manticoresearch/tree/29.2.5) [Issue #4833](https://github.com/manticoresoftware/manticoresearch/issues/4833) Fixed SQL queries on tables with UUID document IDs returning stored `text` fields as empty; stored fields are now retrieved correctly when selected explicitly or through `SELECT *`.
* 🪲 [v29.2.4](https://github.com/manticoresoftware/manticoresearch/tree/29.2.4) [Issue #4828](https://github.com/manticoresoftware/manticoresearch/issues/4828) Restored Sequel Ace 5.3.1+ compatibility by supporting its `CAST(DATABASE() AS BINARY)` and `CAST(@@character_set_client AS BINARY)` probes, preventing every subsequent query from failing.
* 🪲 [v29.2.3](https://github.com/manticoresoftware/manticoresearch/tree/29.2.3) [PR #4619](https://github.com/manticoresoftware/manticoresearch/pull/4619) Fixed graceful `searchd --stopwait` hanging while a sharded table was rebalanced after a node rejoined; in-progress shard transfer now stops during shutdown and detached workers use bounded waits.
* 🪲 [v29.2.2](https://github.com/manticoresoftware/manticoresearch/tree/29.2.2) [Issue #4793](https://github.com/manticoresoftware/manticoresearch/issues/4793) Fixed `indexer` failing to build plain tables when their configured [path](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#path) contained missing parent directories; it now creates the missing directory tree automatically when the nearest existing parent is writable.
* 🪲 [v29.2.1](https://github.com/manticoresoftware/manticoresearch/tree/29.2.1) [PR #4832](https://github.com/manticoresoftware/manticoresearch/pull/4832) Corrected and completed copyright and GPL license headers in recently added source files.
* 🪲 [v29.1.1](https://github.com/manticoresoftware/manticoresearch/tree/29.1.1) [Issue #4808](https://github.com/manticoresoftware/manticoresearch/issues/4808) Fixed upgrades from versions with v15 binlogs potentially crashing or omitting a newly flushed RT disk chunk from table metadata during shutdown; compatible recovery logs now replay, completed chunks are published before clean shutdown, and unsupported recovery formats fail safely.
* 🪲 [v29.0.6](https://github.com/manticoresoftware/manticoresearch/tree/29.0.6) [Issue #4811](https://github.com/manticoresoftware/manticoresearch/issues/4811) Improved fatal binlog replay diagnostics to name the matching [--replay-flags](Starting_the_server/Manually.md#searchd-command-line-options) recovery option: `ignore-open-errors` for missing binlog files and `ignore-trx-errors` for transaction replay failures.
* 🪲 [v29.0.5](https://github.com/manticoresoftware/manticoresearch/tree/29.0.5) [Issue #4784](https://github.com/manticoresoftware/manticoresearch/issues/4784) Fixed JSON `/search` queries with negation losing distances for [NEAR and proximity operators](Searching/Full_text_matching/Operators.md), which caused valid positional queries to fail with a zero-distance error or behave incorrectly.
* 🪲 [v29.0.4](https://github.com/manticoresoftware/manticoresearch/tree/29.0.4) [Issue #4788](https://github.com/manticoresoftware/manticoresearch/issues/4788) Fixed [LEFT JOIN](Searching/Joining.md) queries ordered by string attributes returning internal `@int_attr_*` helper columns in client results.
* 🪲 [v29.0.3](https://github.com/manticoresoftware/manticoresearch/tree/29.0.3) [PR #4790](https://github.com/manticoresoftware/manticoresearch/pull/4790) Fixed malformed binary API `SEARCH` requests with negative or impossible element counts potentially terminating `searchd`; invalid counts are now rejected with protocol errors, including when `max_batch_queries=0`.
...
### Bug Fixes
* 🪲 [v28.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.6) [Issue #4741](https://github.com/manticoresoftware/manticoresearch/issues/4741) Fixed `USER()` parsing after authentication support made `USER` a keyword, restoring MySQL client `status` command compatibility.
* 🪲 [v28.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.5) [Issue #4764](https://github.com/manticoresoftware/manticoresearch/issues/4764) Fixed [CALL SNIPPETS](Searching/Highlighting.md#CALL-SNIPPETS) taking seconds on complex boolean queries when `boolean_simplify` is enabled, and restored expected highlighting for affected queries.
* 🪲 [v28.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.4) [Issue #4737](https://github.com/manticoresoftware/manticoresearch/issues/4737) Fixed [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) parsing for qualified `system.<table>` names, allowing physical sharded tables to be optimized.
* 🪲 [v28.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.3) [Issue #138](https://github.com/manticoresoftware/manticoresearch-backup/issues/138) Updated [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) to 1.10.2, fixing successful backups leaving RT tables frozen and subsequent writes blocked indefinitely.
* 🪲 [v28.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.2) [Issue #4758](https://github.com/manticoresoftware/manticoresearch/issues/4758) Fixed obsolete RT external files being left behind after `ALTER`, `TRUNCATE`, chunk removal, or optimization, and prevented unrelated `ALTER TABLE` changes from losing Jieba tokenization settings; unsupported effective Jieba setting changes now fail explicitly.
* 🪲 [v28.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.1) [Issue #4750](https://github.com/manticoresoftware/manticoresearch/issues/4750) Fixed authenticated replication state transfer so `ALTER CLUSTER ... ADD` can add existing populated RT tables instead of failing with retries exhausted.
* 🪲 [v28.5.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.3) [Issue #4730](https://github.com/manticoresoftware/manticoresearch/issues/4730) Fixed long exact and phrase-like queries potentially corrupting memory or crashing `searchd` by accounting for their full term width in the query stack guard.
* 🪲 [v28.5.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.2) [PR #4729](https://github.com/manticoresoftware/manticoresearch/pull/4729) Improved `local_df` query processing for multi-chunk RT and distributed tables by deduplicating repeated keyword statistics, avoiding excessive pre-search work for large repeated `OR` and phrase queries.
* 🪲 [v28.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.1) [Issue #4726](https://github.com/manticoresoftware/manticoresearch/issues/4726) Fixed a SphinxQL connection-handling race that could spin workers at 100% CPU and leave `searchd` permanently unresponsive when clients connected during startup or aborted an early SSL upgrade.
* 🪲 [v28.4.11](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.11) [PR #4725](https://github.com/manticoresoftware/manticoresearch/pull/4725) Improved `indextool --check` to validate secondary-index structure and payloads, reporting corrupted `.spidx` files as check failures instead of crashing.
* 🪲 [v28.4.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.10) [PR #4735](https://github.com/manticoresoftware/manticoresearch/pull/4735) Fixed the Ruby API fixture's response parsing to preserve valid `"0"` chunks, matching the PHP client behavior.
* 🪲 [v28.4.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.9) [Issue #4595](https://github.com/manticoresoftware/manticoresearch/issues/4595) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.8.2, fixing a StreamVByte decoder over-read that could crash queries using secondary indexes, including `GROUP BY` on JSON fields.
* 🪲 [v28.4.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.8) [PR #4703](https://github.com/manticoresoftware/manticoresearch/pull/4703) Fixed PHP API response parsing when a valid response chunk ends with the string `"0"`, preventing false `failed to read searchd response` errors for affected packet sizes.
* 🪲 [v28.4.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.7) [PR #4721](https://github.com/manticoresoftware/manticoresearch/pull/4721) Fixed a concurrent `SHOW INDEX ... STATUS` race that could corrupt query-stat percentile calculations (`p95`/`p99`).
* 🪲 [v28.4.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.6) [Issue #4677](https://github.com/manticoresoftware/manticoresearch/issues/4677) Fixed document-ID `IN(...)` filters silently returning no rows for valid unsigned 64-bit IDs above `Long.MAX_VALUE`; invalid out-of-range document IDs now fail validation without restricting valid `multi64` values.
* 🪲 [v28.4.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.5) [Issue #4633](https://github.com/manticoresoftware/manticoresearch/issues/4633) Fixed a crash when a `LEFT JOIN` query uses multiple `FACET` clauses by preventing join multi-facet sorters from being cleaned up twice.
...
### New Features and Improvements
* 🆕 [v28.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.0) [ PR #4707](https://github.com/manticoresoftware/manticoresearch/pull/4707) Improved [KNN search](Searching/KNN.md#KNN-vector-search) performance by batching distance calculations during the `rescore` pass, reducing overhead when final full-precision distances are recomputed and re-sorted.
* 🆕 [v28.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.0) [ Issue #688](https://github.com/manticoresoftware/manticoresearch-buddy/issues/688) [ PR #4690](https://github.com/manticoresoftware/manticoresearch/pull/4690) Updated Buddy to 4.2.0, adding `custom_prompt` support for [CREATE CHAT MODEL](Searching/Conversational_search.md), so [conversational search](Searching/Conversational_search.md) answers can follow custom instructions such as citation rules and response style. It also fixes [issue #686](https://github.com/manticoresoftware/manticoresearch-buddy/issues/686) - Buddy worker FD leak.
* 🆕 [v28.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.2.0) [ PR #4596](https://github.com/manticoresoftware/manticoresearch/pull/4596) Added a `curl`/`wget` pipe-to-shell installer in the [quick start guide](Quick_start_guide.md), making it possible to install, upgrade, and select Manticore versions with a single bootstrap command.
* 🆕 [v28.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.0) [ Issue #683](https://github.com/manticoresoftware/manticoresearch-buddy/issues/683) [ PR #685](https://github.com/manticoresoftware/manticoresearch-buddy/pull/685) Updated Buddy to 4.1.0, adding HTTP JSON support for [conversational search](Searching/Conversational_search.md), so chat requests can be sent through the `/search` JSON API in addition to SQL `CALL CHAT`.
* 🆕 [v27.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.4.0) [ PR #169](https://github.com/manticoresoftware/columnar/pull/169) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.7.0, adding [embeddings_threads](Server_settings/Searchd.md#embeddings_threads) to cap CPU threads used for auto-embedding inserts, `ALTER TABLE ... REBUILD KNN`, and text-to-vector KNN queries.
* 🆕 [v27.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.3.0) [ PR #4624](https://github.com/manticoresoftware/manticoresearch/pull/4624) Added zero-count facet bucket support for [faceted search](Searching/Faceted_search.md), including SQL `ZEROES` and JSON `"zeroes": true`, so `max`-mode facets can keep visible buckets even when their current filtered count is `0`.
* 🆕 [v27.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.2.0) [ PR #4608](https://github.com/manticoresoftware/manticoresearch/pull/4608) Added [CREATE TABLE ... profile='relevance'](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#profile), plus stored per-table defaults for [ranker](Searching/Options.md#ranker) and [boolean_mode](Searching/Options.md#boolean_mode), making it possible to give tables built-in full-text ranking and implicit keyword-combining behavior without repeating query options.
...
* ⚠️ **[v28.4.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.1)** [ Issue #4720](https://github.com/manticoresoftware/manticoresearch/issues/4720) [ PR #4712](https://github.com/manticoresoftware/manticoresearch/pull/4712) **Breaking upgrade condition**: an authentication-enabled replication cluster whose `manticore.json` was saved before v28.4.1 has no stored replication user. After the upgrade, Manticore cannot restore that cluster automatically and leaves its saved metadata unchanged. Stop all cluster nodes and back up the data directory. On every node, remove only the affected cluster descriptor from `manticore.json`; keep the table entries and table directories. Start the nodes, recreate and join the cluster with the intended replication account, then attach the existing tables with `ALTER CLUSTER ... ADD`. Do not delete the table directories. Clusters saved by v28.4.1 and later include the replication user and are not affected.
...
* 🪲 [v28.3.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.7) [ Issue #4697](https://github.com/manticoresoftware/manticoresearch/issues/4697) [ PR #4698](https://github.com/manticoresoftware/manticoresearch/pull/4698) Fixed interrupted columnar/KNN merge cleanup so temporary component files under the tracked temp prefix are removed correctly, preventing orphaned `.tmp.spc.*` files from breaking later table rename, attach, or drop operations.
* 🪲 [v28.3.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.6) [ PR #4695](https://github.com/manticoresoftware/manticoresearch/pull/4695) Improved [DBeaver](Integration/DBeaver.md) compatibility by accepting simple single-table aliases in `SELECT` queries, so default DBeaver table-browsing queries no longer fail.
* 🪲 [v28.3.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.5) [ Issue #4682](https://github.com/manticoresoftware/manticoresearch/issues/4682) [ PR #4683](https://github.com/manticoresoftware/manticoresearch/pull/4683) Fixed the built-in Ukrainian lemmatizer so apostrophe words are normalized correctly, allowing forms like `здоров'ям` to match `здоров'я` under [lemmatize_uk_all](Creating_a_table/NLP_and_tokenization/Supported_languages.md).
* 🪲 [v28.3.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.4) [ Issue #4691](https://github.com/manticoresoftware/manticoresearch/issues/4691) [ PR #4696](https://github.com/manticoresoftware/manticoresearch/pull/4696) Fixed authenticated MySQL protocol startup compatibility so Connector/J and PyMySQL clients can complete native-password login flows and harmless session `SET` statements no longer fail under auth.
* 🪲 [v28.3.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.3) [ Issue #4641](https://github.com/manticoresoftware/manticoresearch/issues/4641) [ PR #4649](https://github.com/manticoresoftware/manticoresearch/pull/4649) Fixed AOT blended-keyword handling to honor the configured [blend_mode](Searching/Options.md#expand_blended), restoring separator-stripped variants consistently for indexing and keyword extraction.
* 🪲 [v28.3.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.2) [ PR #186](https://github.com/manticoresoftware/columnar/pull/186) Updated [MCL](https://github.com/manticoresoftware/columnar) to include an explicit `DOCS_PER_CHUNK` build fix, preventing bundled KNN library build/open failures in the new submodule-based columnar workflow.
* 🪲 [v28.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.1) [ Issue #4432](https://github.com/manticoresoftware/manticoresearch/issues/4432) [ PR #4689](https://github.com/manticoresoftware/manticoresearch/pull/4689) Fixed replication-cluster startup when a node was left alone in the cluster, so a remaining self-only node can recover cleanly instead of failing to start.
* 🪲 [v28.1.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.5) [ PR #4687](https://github.com/manticoresoftware/manticoresearch/pull/4687) Fixed distributed stored-field fetching to validate remote `GETFIELD` replies before using them, preventing malformed agent responses from being trusted.
* 🪲 [v28.1.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.4) [ PR #4676](https://github.com/manticoresoftware/manticoresearch/pull/4676) Fixed the migrated MCL submodule build and packaging workflow, restoring correct CI, package, and dependency-resolution handling for the bundled columnar library after the daemon-side submodule transition.
* 🪲 [v28.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.3) [ PR #4663](https://github.com/manticoresoftware/manticoresearch/pull/4663) Fixed several memory leaks and cleanup bugs across optional library loading, SSL key handling, RT dictionary cleanup, timer-thread shutdown, and facet-zeroes aggregation paths.
* 🪲 [v28.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.2) [ Issue #4672](https://github.com/manticoresoftware/manticoresearch/issues/4672) [ PR #4679](https://github.com/manticoresoftware/manticoresearch/pull/4679) Fixed a crash when [percentiles](Searching/Grouping.md) aggregations were used together with terms aggregations in the same `/search` request on multi-chunk RT tables.
* 🪲 [v28.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.1) [ PR #4669](https://github.com/manticoresoftware/manticoresearch/pull/4669) Fixed RT auto-optimization so [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) no longer compacts a table below 2 disk chunks unless that lower cutoff is explicitly requested.
* 🪲 [v28.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.2) [ Issue #4632](https://github.com/manticoresoftware/manticoresearch/issues/4632) [ PR #4673](https://github.com/manticoresoftware/manticoresearch/pull/4673) Fixed distributed queries with remote stored fields to fail on remote `GETFIELD` fetch errors instead of returning successful rows with silently empty stored-field values.
* 🪲 [v28.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.1) [ Issue #4615](https://github.com/manticoresoftware/manticoresearch/issues/4615) [ PR #4631](https://github.com/manticoresoftware/manticoresearch/pull/4631) Fixed long SQL parse errors to preserve UTF-8 character boundaries, so invalid queries containing Cyrillic and other multibyte text no longer produce truncated or corrupted error messages.
...
* 🆕 [v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0) [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) Added [authentication and authorization](Security/Authentication_and_authorization.md) for MySQL, HTTP/HTTPS, distributed remote agents, and replication-related operations, including users, bearer tokens, fine-grained permissions, bootstrap commands, and auth-aware cluster user handling.
* 🆕 [v26.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.0) [ PR #4606](https://github.com/manticoresoftware/manticoresearch/pull/4606) Updated Buddy to 3.47.0, adding the matching sharding rollback, upgrades, tests, and fixes needed for the new sharding functionality.
* 🆕 [v26.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.1.0) [ PR #4537](https://github.com/manticoresoftware/manticoresearch/pull/4537) Added [native "type='sharding'" tables](Creating_a_table/Creating_a_sharded_table/Creating_a_sharded_table.md) with distributed inserts, plus major sharding upgrades covering creation, routing, rollback, failover, and operational behavior.
* 🆕 **26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) Reworked replication storage so incoming replicated tables use the normal [data_dir](Server_settings/Searchd.md#data_dir)/`<table>` layout instead of cluster-specific paths, aligning replicated-table storage with ordinary local table layout.
* 🆕 [v25.15.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.15.0) [ PR #4590](https://github.com/manticoresoftware/manticoresearch/pull/4590) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.5.0, adding the library support needed for parallel HNSW build.
* 🆕 [v25.14.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.14.0) [ PR #4589](https://github.com/manticoresoftware/manticoresearch/pull/4589) Added multithreaded HNSW build support, speeding up KNN index construction during initial build, chunk merges, and `ALTER TABLE ... REBUILD KNN`.
* 🆕 [v25.13.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.0) [ PR #4475](https://github.com/manticoresoftware/manticoresearch/pull/4475) Added `searchd --check`, allowing configuration validation before startup without side effects.
* 🆕 [v25.12.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.12.0) [ Issue #4531](https://github.com/manticoresoftware/manticoresearch/issues/4531) Added `facet_filter_mode` for faceted search, making it possible to build richer e-commerce-style filters that can preserve and control alternative facet buckets under active filtering.
* 🆕 [v25.11.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.0) [ PR #4558](https://github.com/manticoresoftware/manticoresearch/pull/4558) Added [conversational search](Searching/Conversational_search.md) to Manticore Search through [CREATE CHAT MODEL](Searching/Conversational_search.md) and [CALL CHAT](Searching/Conversational_search.md), so existing vectorized tables can answer questions with KNN retrieval, conversation history, and LLM-backed responses.
* 🆕 [v25.10.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.10.0) [ PR #4555](https://github.com/manticoresoftware/manticoresearch/pull/4555) Updated Buddy to 3.45.0, adding support for OpenSearch Dashboards.
* 🆕 [v25.9.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.9.0) [ PR #4549](https://github.com/manticoresoftware/manticoresearch/pull/4549) Updated `manticore-load` to 1.25.0, adding multi-query support.
* 🆕 [v25.8.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.0) [ PR #4414](https://github.com/manticoresoftware/manticoresearch/pull/4414) Added a [built-in Ukrainian lemmatizer](Creating_a_table/NLP_and_tokenization/Morphology.md#morphology), expanding native morphology support for Ukrainian text search.
* 🆕 [v25.7.18](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.18) [ Issue #4461](https://github.com/manticoresoftware/manticoresearch/issues/4461) Added `time_zone` and `offset` support to [date_histogram](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29), making time-zone-aware calendar bucketing possible in aggregation queries.
* 🆕 [v25.7.16](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.16) [ PR #4503](https://github.com/manticoresoftware/manticoresearch/pull/4503) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.2.5, improving ONNX embeddings performance.
* 🆕 [v25.7.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.0) [ Issue #3280](https://github.com/manticoresoftware/manticoresearch/issues/3280) Added Elasticsearch-like statistical aggregations, including `percentiles`, `percentile_ranks`, and `mad`, making it possible to calculate percentile distributions and median absolute deviation directly in aggregation queries.
* 🆕 [v25.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.0) [ PR #4407](https://github.com/manticoresoftware/manticoresearch/pull/4407) Extended [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29) in the `expr` ranker to accept an optional third `avgdl` argument, allowing queries to override the average document length used in BM25A scoring.
* 🆕 [v25.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.4.0) [ Issue #4060](https://github.com/manticoresoftware/manticoresearch/issues/4060) Added systemd state notifications and switched the packaged Linux service to `Type=notify`, improving startup, reload, and shutdown handling and preventing long-running shutdown work from being treated as a stalled daemon.
* 🆕 [v25.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.3.0) [ PR #4326](https://github.com/manticoresoftware/manticoresearch/pull/4326) Added support for explicitly prefixing left-table columns in `JOIN` queries, making JOIN conditions and filters more consistent when table-qualified column names are used.
* 🆕 [v25.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.0) [ PR #146](https://github.com/manticoresoftware/columnar/pull/146) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.2.2, adding local ONNX embeddings support and improving vector-search performance with faster embeddings inference and HNSW distance calculations, including AVX-512 optimizations where supported.
* 🆕 [v25.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.0) [ PR #4454](https://github.com/manticoresoftware/manticoresearch/pull/4454) Improved KNN performance by optimizing distance calculation and adding AVX-512 columnar library loading when supported, reducing overhead in vector-search workloads.
* 🆕 [v25.0.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.5) [ Issue #1514](https://github.com/manticoresoftware/manticoresearch/issues/1514) Added [EXIT CLUSTER](Creating_a_cluster/Setting_up_replication/Managing_replication_nodes.md#EXIT-CLUSTER), allowing the current node to detach from a replication cluster online without restarting the daemon while keeping its local tables as regular local tables.

## Breaking Changes
* ⚠️ **[v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0)** [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) **Breaking change**: [authentication and authorization](Security/Authentication_and_authorization.md) is not enabled by default, but if you enable it, anonymous client access no longer works. Roll out the upgrade in stages: upgrade remote agents and replication peers first, then upgrade the masters that query or manage them, and enable auth only after the whole topology is on the new version. A new master can fail against an old agent because of protocol changes, while an old master can still read from a new agent only until auth is enabled. Participating daemons must also have matching stored authentication data for the relevant users, and a successful `JOIN CLUSTER` replaces the joining node's local auth data with the donor cluster's auth data. Existing table data remains compatible, but migration requires bootstrapping users, granting permissions, synchronizing auth data across nodes, and reviewing cluster-join procedures. Downgrade is only safe before enabling auth and before any cluster join has replaced local auth data.
* ⚠️ **v26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) **Breaking change**: replication clusters that relied on incoming replicated tables being stored under the cluster `path` no longer work the same way, because received tables now live under the standard [data_dir](Server_settings/Searchd.md#data_dir)/`<table>` layout. Existing table data remains usable, but if you use replication with a custom cluster `path`, migration may require moving or re-synchronizing replicated tables into the standard `data_dir` layout. Downgrade is only safe before adopting the new replicated-table layout.

### Bug Fixes
* 🪲 [v27.1.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.5) [ PR #4648](https://github.com/manticoresoftware/manticoresearch/pull/4648) Fixed a crash when fetching columnar `float_vector` attributes, preventing failures when vector columns are materialized from columnar tables.
* 🪲 [v27.1.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.4) [ Issue #4655](https://github.com/manticoresoftware/manticoresearch/issues/4655) [ PR #4657](https://github.com/manticoresoftware/manticoresearch/pull/4657) Fixed `ALTER TABLE ... RECONFIGURE` and `SHOW CREATE TABLE` for tables promoted from `dict='keywords'` to [dict=keywords_32k](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#dict), so the one-way dictionary upgrade now works and is reflected correctly in the table definition.
* 🪲 [v27.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.3) [ PR #4652](https://github.com/manticoresoftware/manticoresearch/pull/4652) Updated [Buddy](Installation/Manticore_Buddy.md#Manticore-Buddy) to 4.0.1, fixing Queue-plugin mutation permission handling under auth so read-only grants on `source/<name>` and `mva/<name>` no longer allow `DROP SOURCE` or `ALTER MATERIALIZED VIEW` side effects.
* 🪲 [v27.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.2) [ PR #4600](https://github.com/manticoresoftware/manticoresearch/pull/4600) Fixed `sql_attr_multi` query MVA collection for plain indexes built from multiple `source` blocks, so multi-source indexes no longer fail during indexing with a null error.
* 🪲 [v27.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.0) [ PR #4635](https://github.com/manticoresoftware/manticoresearch/pull/4635) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.6.1, preventing the embeddings library from crashing on emulated x86 CPUs.
* 🪲 [v26.3.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.3.2) [ PR #4620](https://github.com/manticoresoftware/manticoresearch/pull/4620) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.5.3, bringing in a quantizer type-mismatch fix from the bundled columnar library.
* 🪲 [v26.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.3.1) Reverted the `zeroes` facet feature from `26.3.0`, removing the mistakenly released SQL/JSON zero-count facet bucket support.
* 🪲 [v26.2.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.4) [ Issue #4320](https://github.com/manticoresoftware/manticoresearch/issues/4320) Fixed KNN result ordering after rescoring, so explicit `ORDER BY` tie-breakers are applied correctly when equal rescored distances occur.
* 🪲 [v26.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.3) [ PR #4614](https://github.com/manticoresoftware/manticoresearch/pull/4614) Updated Buddy to 3.47.2, fixing several replication and node-rejoin crash paths, including invalid table-index handling and replication snapshot locking.
* 🪲 [v26.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.2) [ Issue #4603](https://github.com/manticoresoftware/manticoresearch/issues/4603) Fixed a crash in hybrid fused queries with `GROUP BY` on tables that use columnar attributes.
* 🪲 [v26.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.1) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) Updated Buddy to 3.47.1, preserving `offset` and `max_matches` for HTTP KNN search by `doc_id`.
* 🪲 [v26.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.3) [ PR #4604](https://github.com/manticoresoftware/manticoresearch/pull/4604) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.5.2, bringing in stack-related stability fixes for the bundled columnar library.
* 🪲 [v26.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.2) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) Fixed KNN searches to honor `max_matches`, correcting result-window handling that could return an incorrect fixed number of hits.
* 🪲 [v26.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.1) [ PR #4592](https://github.com/manticoresoftware/manticoresearch/pull/4592) Fixed debug-build stack sizing to prevent memory corruption caused by oversized coroutine and thread stack frames.
* 🪲 [v25.13.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.5) [ Issue #4489](https://github.com/manticoresoftware/manticoresearch/issues/4489) Fixed an indexer crash caused by MVA/docstore gaps when building plain indexes with columnar attributes.
* 🪲 [v25.13.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.4) [ PR #4583](https://github.com/manticoresoftware/manticoresearch/pull/4583) Fixed `facet_filter_mode` bucket-status behavior in `auto` and `max` modes, aligning selected, available, and unavailable facet buckets with the intended faceted-search UX.
* 🪲 [v25.13.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.3) [ Issue #4578](https://github.com/manticoresoftware/manticoresearch/issues/4578) Fixed RT `.@files` crashes under write load by refactoring dynamic-table internal dataflow, preventing synchronization-related failures during internal file-list queries.
* 🪲 [v25.13.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.2) [ Issue #660](https://github.com/manticoresoftware/manticoresearch-buddy/issues/660) Updated Buddy to 3.46.3, fixing KNN-by-`doc_id` self-hit handling and restoring Logstash 7.17 template probe compatibility.
* 🪲 [v25.13.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.1) [ Issue #168](https://github.com/manticoresoftware/columnar/issues/168) Fixed Windows AVX-512 module loading, preventing packaged [MCL](https://github.com/manticoresoftware/columnar) AVX-512 DLLs from being selected on unsupported systems and crashing the daemon or indexer.
* 🪲 [v25.12.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.12.1) [ PR #4563](https://github.com/manticoresoftware/manticoresearch/pull/4563) Fixed binary MySQL protocol support, including special columns, nulls, and typed numeric values, so prepared/binary-protocol queries behave correctly again.
* 🪲 [v25.11.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.2) [ PR #4566](https://github.com/manticoresoftware/manticoresearch/pull/4566) Fixed Fluent Bit bulk-ingest interoperability by normalizing `Content-Length` header formatting, preventing successful bulk responses from being replayed as duplicate inserts.
* 🪲 [v25.11.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.1) [ Issue #4560](https://github.com/manticoresoftware/manticoresearch/issues/4560) Updated Buddy to 3.46.1, fixing typed `COUNT(*)` handling for `information_schema.tables` and restoring expected integration behavior.
* 🪲 [v25.9.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.9.1) [ Issue #4464](https://github.com/manticoresoftware/manticoresearch/issues/4464) Fixed prepared statements that need Buddy processing, including fuzzy `MATCH(?)` queries, and corrected strict typed result handling for Buddy-returned bool, multi, and JSON values.
* 🪲 [v25.8.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.5) [ PR #4554](https://github.com/manticoresoftware/manticoresearch/pull/4554) Fixed packaging and CI around AOT language packs, ensuring Docker images include all packs and lemmatizer/AOT test coverage is no longer skipped.
* 🪲 [v25.8.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.4) [ PR #4553](https://github.com/manticoresoftware/manticoresearch/pull/4553) Fixed Ukrainian lemmatizer loading and original-form generation, tightening the new built-in Ukrainian morphology support.
* 🪲 [v25.8.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.3) [ Issue #4490](https://github.com/manticoresoftware/manticoresearch/issues/4490) Fixed Arabic mixed-token stemming in `CALL KEYWORDS`, preventing certain short Arabic+numeric inputs from failing with a generic `Something went wrong` error.
* 🪲 [v25.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.2) [ Issue #4509](https://github.com/manticoresoftware/manticoresearch/issues/4509) Fixed a replication crash when creating a cluster with a long `data_dir` path, correcting cluster-binlog handling for long directory names.
* 🪲 [v25.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.1) [ PR #4535](https://github.com/manticoresoftware/manticoresearch/pull/4535) Fixed hybrid `knn_dist` output so sentinel `FLT_MAX` values are no longer serialized as bogus distances in JSON results.
* 🪲 [v25.7.21](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.21) [ Issue #4525](https://github.com/manticoresoftware/manticoresearch/issues/4525) Fixed a memory leak in scroll handling by correctly freeing remapped string attributes.
* 🪲 [v25.7.20](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.20) [ Issue #4466](https://github.com/manticoresoftware/manticoresearch/issues/4466) Fixed JSON array aggregation when it is the only aggregation in the request, so array items are bucketed individually instead of repeating the whole array as each key.
* 🪲 [v25.7.19](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.19) [ PR #4533](https://github.com/manticoresoftware/manticoresearch/pull/4533) Updated [MCL](https://github.com/manticoresoftware/columnar) to 13.2.6, fixing a crash that could occur when running multithreaded KNN queries with 1-bit quantized vectors.
* 🪲 [v25.7.17](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.17) [ PR #4529](https://github.com/manticoresoftware/manticoresearch/pull/4529) Bundled several minor fixes and cleanups, including another [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) correctness fix and small runtime/build improvements.
* 🪲 [v25.7.15](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.15) [ PR #4526](https://github.com/manticoresoftware/manticoresearch/pull/4526) Fixed [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) kill-list handling in columnar mode by reading columnar docids correctly, completing the recent attach fixes for [MCL](https://github.com/manticoresoftware/columnar)/columnar setups.
* 🪲 [v25.7.14](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.14) [ Issue #4459](https://github.com/manticoresoftware/manticoresearch/issues/4459) Fixed [DATE_HISTOGRAM()](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29) interval parsing in aggregation arguments, so `fixed_interval` values like `1h`, `2m`, and similar units now work correctly instead of behaving as raw seconds.
* 🪲 [v25.7.13](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.13) [ Issue #4522](https://github.com/manticoresoftware/manticoresearch/issues/4522) Fixed an extra null character in JOIN sorter/grouping output, so grouped and faceted JOIN values no longer contain corrupted trailing `\0` bytes.
* 🪲 [v25.7.12](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.12) [ Issue #4516](https://github.com/manticoresoftware/manticoresearch/issues/4516) Fixed document counters for the target table after [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), so `indexed_documents` and related metadata now reflect the actual number of rows after attach.
* 🪲 [v25.7.11](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.11) [ Issue #4515](https://github.com/manticoresoftware/manticoresearch/issues/4515) Fixed [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) handling of already-killed documents, so duplicate IDs in the target table are no longer removed incorrectly during attach.
* 🪲 [v25.7.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.10) [ Issue #4514](https://github.com/manticoresoftware/manticoresearch/issues/4514) Fixed `indexed_documents` for the source table after [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), so the document count now drops correctly when a disk chunk is moved to another table.
* 🪲 [v25.7.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.9) [ Issue #4513](https://github.com/manticoresoftware/manticoresearch/issues/4513) Fixed metadata for attached indexes, so tables created via [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) keep the correct chunk state and no longer become broken after restart.
* 🪲 [v25.7.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.8) [ Issue #4512](https://github.com/manticoresoftware/manticoresearch/issues/4512) Fixed `indexed_documents` metadata on startup after a dirty restart, so RT tables no longer report an inflated document count when deleted rows are recovered.
* 🪲 [v25.7.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.7) [ Issue #159](https://github.com/manticoresoftware/columnar/issues/159) Fixed KNN startup diagnostics when the embeddings library is missing or misconfigured, so Manticore no longer reports a misleading `(null)` warning/error during initialization.
* 🪲 [v25.7.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.6) [ Issue #4460](https://github.com/manticoresoftware/manticoresearch/issues/4460) Fixed replication handling when [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) disables secondary indexes, so the warning is no longer turned into a cluster-breaking error in replicated setups.
* 🪲 [v25.7.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.5) [ Issue #4345](https://github.com/manticoresoftware/manticoresearch/issues/4345) Fixed JSON filter fallback when secondary indexes do not produce iterators, so compound filter trees keep the intended filters instead of returning incorrect results.
* 🪲 [v25.7.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.4) [ PR #662](https://github.com/manticoresoftware/manticoresearch-buddy/pull/662) Updated Buddy to 3.44.5 to include a Composer security fix.
* 🪲 [v25.7.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.3) [ Issue #4404](https://github.com/manticoresoftware/manticoresearch/issues/4404) Fixed [hybrid search](Searching/Hybrid_search.md) with `scroll`, so scroll-based pagination now works correctly instead of failing or crashing when [hybrid search](Searching/Hybrid_search.md) is used.
* 🪲 [v25.7.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.2) [ Issue #4491](https://github.com/manticoresoftware/manticoresearch/issues/4491) Fixed secondary-index state after RT chunk rename and [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), so indexes disabled by [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) stay disabled across daemon restart instead of being unintentionally re-enabled.
* 🪲 [v25.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.1) [ PR #4502](https://github.com/manticoresoftware/manticoresearch/pull/4502) Fixed filtering by `knn_dist()` and `hybrid_score()` in [hybrid search](Searching/Hybrid_search.md), so queries can correctly apply conditions to vector distance and hybrid ranking scores.
* 🪲 [v25.6.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.9) [ Issue #4473](https://github.com/manticoresoftware/manticoresearch/issues/4473) Fixed HTTP JSON join parsing for mixed-case table names by canonicalizing table references in `join.on`, so joins no longer fail just because table names contain uppercase letters.
* 🪲 [v25.6.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.8) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) Fixed remaining expr-ranker crashes on columnar tables with mixed field-length handling, completing the [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29)/[bm25f()](Functions/Searching_and_ranking_functions.md#BM25F%28%29) fix for [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)='1' scenarios.
* 🪲 [v25.6.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.7) [ Issue #4484](https://github.com/manticoresoftware/manticoresearch/issues/4484) Fixed template-table handling in the MySQL `COM_FIELD_LIST` metadata path, preventing crashes when clients request field metadata for template tables.
* 🪲 [v25.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.6) [ Issue #4497](https://github.com/manticoresoftware/manticoresearch/issues/4497) Fixed metadata-path crashes for template tables, including `SHOW FIELDS` and related field-list handling that previously could crash the daemon.
* 🪲 [v25.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.5) [ Issue #2](https://github.com/manticoresoftware/telemetry-lib/issues/2) Updated Buddy to 3.44.4, fixing telemetry delivery under PHP 8.4 so telemetry snapshots are sent successfully instead of failing during the HTTP POST request.
* 🪲 [v25.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.4) [ Issue #4469](https://github.com/manticoresoftware/manticoresearch/issues/4469) Fixed [ALTER TABLE ... REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) after [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) merges RT disk chunks, so secondary-index files stay in sync with renamed optimized chunks and no longer fail with `secondary index file ... not loaded`.
* 🪲 [v25.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.3) [ Issue #4418](https://github.com/manticoresoftware/manticoresearch/issues/4418) Fixed crashes and related JOIN-handling issues in [hybrid search](Searching/Hybrid_search.md), including `LEFT JOIN` queries that previously could fail when full-text and KNN results were combined.
* 🪲 [v25.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.2) [ PR #658](https://github.com/manticoresoftware/manticoresearch-buddy/pull/658) Updated Buddy to 3.44.3, fixing volume telemetry when `binlog_path` is empty so `volume_size_bytes` is no longer incorrectly reported as zero.
* 🪲 [v25.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.1) [ Issue #4476](https://github.com/manticoresoftware/manticoresearch/issues/4476) Fixed startup under systemd so `pid_file` is no longer required when the daemon is managed by systemd, which already knows and controls the process ID.
* 🪲 [v25.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.1) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) Fixed a crash in expr rankers using [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29) or [bm25f()](Functions/Searching_and_ranking_functions.md#BM25F%28%29) on columnar tables with [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)='1', so custom ranking no longer dereferences field-length attributes from the wrong storage.
* 🪲 [v25.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.0) [ Issue #4007](https://github.com/manticoresoftware/manticoresearch/issues/4007) Fixed inconsistent handling of mixed-case table names across SphinxQL, HTTP, and `JOIN` queries, so table names are normalized consistently instead of failing or behaving differently when uppercase letters are used.
* 🪲 [v25.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.1) [ Issue #4428](https://github.com/manticoresoftware/manticoresearch/issues/4428) Fixed a crash when `GROUP BY` was used in [hybrid search](Searching/Hybrid_search.md) queries, including queries that combine full-text and KNN search.
* 🪲 [v25.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.1) [ Issue #4455](https://github.com/manticoresoftware/manticoresearch/issues/4455) Reverted the recent change that turned a missing `knn_embeddings` library into an explicit KNN initialization error, restoring the previous behavior for setups where that optional embeddings library is absent.
* 🪲 [v25.0.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.6) [ Issue #4437](https://github.com/manticoresoftware/manticoresearch/issues/4437) Fixed cluster rejoin handling after `ALTER CLUSTER ... DROP`, so a node that was offline during the drop no longer keeps stale local cluster-table metadata that leaves the dropped table stuck and impossible to remove after rejoining.
* 🪲 [v25.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.4) [ PR #4405](https://github.com/manticoresoftware/manticoresearch/pull/4405) Fixed BM25A ranking for stemmed or otherwise normalized query terms, so equivalent queries such as `looking` and `look` now use the same local/global IDF statistics instead of producing different relevance scores.
* 🪲 [v25.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.3) [ PR #85](https://github.com/manticoresoftware/executor/pull/85) Updated Executor to 1.4.3, simplifying its Docker packaging workflow by using pre-built architecture-specific artifacts instead of building binaries inside the image pipeline.
* 🪲 [v25.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.2) [ Issue #4395](https://github.com/manticoresoftware/manticoresearch/issues/4395) Fixed [OPTIMIZE TABLE ... OPTION sync=1, cutoff=N](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) when auto-optimize is already running, so the manual optimize request now waits for its requested chunk-reduction target instead of being tied to a different ongoing optimize run.
* 🪲 [v25.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.1) [ Issue #4398](https://github.com/manticoresoftware/manticoresearch/issues/4398) Fixed daemon query-time statistics so global counters and per-table timings are reported correctly for HTTP `/search`, distributed update queries, and API search requests routed from master to agent.

## Version 25.0.0
**Released**: March 30th 2026

This release brings a broader packaging overhaul, new `API_URL` and `API_TIMEOUT` options for auto-embedding models, [hybrid search](Searching/Hybrid_search.md), S3-compatible backup and restore support, faster RT tables maintenance with N-way merges and parallel [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), KNN prefiltering, and MySQL-compatible prepared statements.

## Breaking Changes
* ⚠️ [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #123](https://github.com/manticoresoftware/columnar/pull/123) **Breaking change**: updated **[MCL](https://github.com/manticoresoftware/columnar)** to 13.0.0, adding [API_URL](Searching/KNN.md#Creating-a-table-with-auto-embeddings) and [API_TIMEOUT](Searching/KNN.md#Creating-a-table-with-auto-embeddings) for [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29). This affects you if you use [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) or manage **[MCL](https://github.com/manticoresoftware/columnar)** separately from the daemon, because Manticore 25.0.0 requires **[MCL](https://github.com/manticoresoftware/columnar) 13.0.0** and is not compatible with older [MCL](https://github.com/manticoresoftware/columnar) versions; upgrade the daemon and [MCL](https://github.com/manticoresoftware/columnar) together, and downgrade is only safe if you also restore a matching older [MCL](https://github.com/manticoresoftware/columnar) version.
* ⚠️ [v24.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.0.0) [ Issue #4343](https://github.com/manticoresoftware/manticoresearch/issues/4343) **Breaking change**: the internal **replication protocol** version is bumped, and cluster epoch tracking is added so recovering nodes can detect when cluster table membership changed while they were offline and use SST instead of failing during IST recovery. This affects you only if you use **replication clusters**: mixed-version replication clusters are not compatible across this change, so upgrade clustered nodes together, and downgrade is only safe before the new replication protocol is used in the cluster.
* ⚠️ [v23.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.0) [ Issue #4364](https://github.com/manticoresoftware/manticoresearch/issues/4364) **Breaking change**: **index format version 69** is introduced to support new bigram tokenization options, including [bigram_delimiter](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_delimiter) modes and digit-aware [bigram_index](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_index) modes. This affects you if you use those bigram options or if you rebuild indexes and later need to open the rewritten indexes with an older Manticore version: existing older indexes remain readable, but indexes rebuilt or newly written in format 69 are not compatible with older Manticore versions, so downgrade is only safe before such indexes are rewritten.
* ⚠️ [v22.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/22.0.0) [ Issue #4301](https://github.com/manticoresoftware/manticoresearch/issues/4301) **Breaking change**: **embedding generation for RT tables** is moved out of the commit phase and replicated as vector data. This affects you if you use **replication clusters** with RT tables that have `model_name`: it preserves user-provided vectors, prevents replicas from regenerating different embeddings, and improves **auto-embeddings** performance when used with replication, but mixed-version replication clusters are not compatible across this change. Upgrade clustered nodes together; downgrade is only safe before the new replication protocol is used in the cluster.
* ⚠️ [v21.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.0) [ PR #138](https://github.com/manticoresoftware/columnar/pull/138) **Breaking change**: updated **[MCL](https://github.com/manticoresoftware/columnar)** to 12.0.0 and bumped the required **KNN API** to support adaptive [**KNN early termination**](Searching/KNN.md#Early-termination) during HNSW traversal. This affects you if you manage **[MCL](https://github.com/manticoresoftware/columnar)** separately from the daemon or run **KNN queries** that rely on the previous candidate exploration behavior or borderline result selection: Manticore 21.0.0 requires **[MCL](https://github.com/manticoresoftware/columnar) 12.0.0** with the newer KNN interface, and with that library KNN search can stop earlier once enough candidates are found, which may change which candidates are explored and returned. Existing data remains compatible and no special migration is required, but upgrade the daemon and [MCL](https://github.com/manticoresoftware/columnar) together; downgrade is only safe if you also restore a matching older [MCL](https://github.com/manticoresoftware/columnar) version.
* ⚠️ [v19.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.0) [ Issue #4103](https://github.com/manticoresoftware/manticoresearch/issues/4103) **Breaking change**: added [**KNN prefiltering**](Searching/KNN.md#Filtering-strategies:-prefilter-vs.-postfilter). This affects you if you run **KNN queries** together with attribute filters, because filters can now be applied during KNN search instead of only after candidate selection. That changes result-selection behavior for filtered KNN queries by prioritizing filtered nearest neighbors; no special migration is required, but queries relying on the previous post-filtered result behavior may return different results after upgrade. Downgrade is possible since no index or protocol format change is introduced by this feature.
* ⚠️ [v18.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.0) [ Issue #4261](https://github.com/manticoresoftware/manticoresearch/issues/4261) **Breaking change**: **index format version 68** is introduced to fix [hitless_words](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#hitless_words) dictionary layout handling. This affects you if you rebuild or newly write indexes and later need to open the rewritten indexes with an older Manticore version: existing older indexes remain readable, but any indexes rebuilt or newly written in the new format are not compatible with older Manticore versions, so downgrade is only safe before such indexes are rewritten.

### Packaging-related changes
* 🆕 [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) Packaging change: `manticore` is now a bundle package for deb and rpm. It includes the daemon, tools, converter, development headers, ICU data, bundled dependency packages (`manticore-columnar-lib`, `manticore-backup`, `manticore-buddy`, `manticore-executor`, `manticore-load`, `manticore-galera`, and `manticore-tzdata` where applicable), plus built-in language packs for German, English, and Russian, along with Jieba support. The Ukrainian lemmatizer is not bundled; install it separately. When upgrading from the previous package layout, normally you can just install the `manticore` package. If the old split packages cause conflicts, remove them with `apt remove 'manticore*'` or `yum remove 'manticore*'` and then install `manticore`. That cleanup does not remove your existing data or configuration.

## New Features and Improvements
* 🆕 [v24.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.4.0) [ PR #4091](https://github.com/manticoresoftware/manticoresearch/pull/4091) Improved RT table compaction by adding N-way disk chunk merges and parallel [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) jobs, reducing the time needed to merge many disk chunks and exposing the new [merge_chunks_per_job](Server_settings/Searchd.md#merge_chunks_per_job) and [parallel_chunk_merges](Server_settings/Searchd.md#parallel_chunk_merges) settings.
* 🆕 [v24.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.3.0) [ PR #133](https://github.com/manticoresoftware/manticoresearch-backup/pull/133) Updated Manticore Backup to 1.10.0, adding S3-compatible backup and restore support, including AWS S3, MinIO, Wasabi, and Cloudflare R2.
* 🆕 [v24.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.0) [ Issue #2079](https://github.com/manticoresoftware/manticoresearch/issues/2079) Added [hybrid search](Searching/Hybrid_search.md) with Reciprocal Rank Fusion (`fusion_method='rrf'`), allowing SQL and JSON queries to combine full-text and KNN results, rank them with `hybrid_score()`, and support multiple KNN subqueries in a single fused result set.
* 🆕 [v24.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.0) [ PR #141](https://github.com/manticoresoftware/columnar/pull/141) Updated [MCL](https://github.com/manticoresoftware/columnar) to 12.4.1, adding support for GGUF quantized local embedding models, T5 encoder models, and gated Hugging Face models via token-authenticated downloads.
* 🆕 [v23.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.2.0) [ PR #4372](https://github.com/manticoresoftware/manticoresearch-buddy/pull/648) Updated Buddy to 3.44.0, adding fuzzy search support for queries involving multiple tables.
* 🆕 [v23.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.1.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) Added a new Manticore bundle package for deb and rpm, replacing the previous meta package to simplify installation and distribution of bundled components.
* 🆕 [v21.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.1.0) [ PR #4352](https://github.com/manticoresoftware/columnar/pull/140) Updated [MCL](https://github.com/manticoresoftware/columnar) to 12.1.0, improving KNN early-termination performance and adding more efficient 64-bit value encoding.
* 🆕 [v19.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.0) [ PR #4306](https://github.com/manticoresoftware/manticoresearch/pull/4306) Updated [MCL](https://github.com/manticoresoftware/columnar) to 11.1.0, adding codec cache support.
* 🆕 [v19.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.1.0) [ PR #4271](https://github.com/manticoresoftware/manticoresearch-buddy/pull/644) Updated Buddy to 3.43.0, including improved metrics handling.
* 🆕 [v19.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.3) [ Issue #4303](https://github.com/manticoresoftware/manticoresearch/issues/4303) Added separate [interactive_timeout](Server_settings/Setting_variables_online.md#SET) and [wait_timeout](Server_settings/Setting_variables_online.md#SET) handling for SQL connections, so interactive clients and non-interactive clients can use different idle timeouts like MySQL.
* 🆕 [v17.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.0) [ Issue #1124](https://github.com/manticoresoftware/manticoresearch/issues/1124) Added MySQL-compatible prepared statements support, including binary protocol prepare/execute handling and bound parameter validation.

## Bug Fixes
* 🪲 [v24.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.3) [ Issue #4375](https://github.com/manticoresoftware/manticoresearch/issues/4375) Fixed false-positive full-text matches when `max_query_time` interrupts queries using operators such as `NOTNEAR`, `MAYBE`, or negation, so timed-out searches no longer return rows that do not actually satisfy the query.
* 🪲 [v24.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.2) [ Issue #4398](https://github.com/manticoresoftware/manticoresearch/issues/4398) Fixed query time statistics so query log, global counters, and per-table `SHOW STATUS` timings stay consistent, and Buddy’s internal requests no longer distort user-facing daemon stats.
* 🪲 [v24.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.2.1) [ PR #653](https://github.com/manticoresoftware/manticoresearch-buddy/pull/653) Updated Buddy to 3.44.1, fixing incorrect default handling of empty request payloads.
* 🪲 [v24.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.3) [ PR #4396](https://github.com/manticoresoftware/manticoresearch/pull/4396) Fixed manual `searchd` startup on macOS so Buddy-backed SQL commands no longer fail from loading an older system `libcurl`; runtime lookup now prefers Homebrew curl automatically, with `MANTICORE_CURL_LIB` still available as an explicit override.
* 🪲 [v24.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.2) [ PR #4394](https://github.com/manticoresoftware/manticoresearch/pull/4394) Fixed macOS packaged builds so Buddy-backed SQL commands no longer fail because `searchd` links against an incompatible `libcurl`; the package now prefers Homebrew curl and supports a `MANTICORE_CURL_LIB` runtime override.
* 🪲 [v24.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.1) [ Issue #4388](https://github.com/manticoresoftware/manticoresearch/issues/4388) Fixed replication in clusters when a transaction contains duplicate document IDs, so replicas no longer lose rows while the donor correctly removes duplicates.
* 🪲 [v24.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.0.1) [ Issue #4354](https://github.com/manticoresoftware/manticoresearch/issues/4354) Fixed [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) handling when a changed attribute must also disable its secondary index, preventing incorrect warnings and inconsistent secondary-index state.
* 🪲 [v23.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.2) [ PR #4370](https://github.com/manticoresoftware/columnar/issues/125) Updated [MCL](https://github.com/manticoresoftware/columnar) to 12.1.1, fixing crashes during auto-embedding generation when text contains invalid UTF-8, including inserts into tables with `html_strip=1`.
* 🪲 [v23.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.1) [ PR #4371](https://github.com/manticoresoftware/manticoresearch/pull/4371) Fixed a missing RT segments lock during embedding validation, preventing crashes and race conditions in embedding-related operations.
* 🪲 [v21.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.3) [ Issue #4315](https://github.com/manticoresoftware/manticoresearch/issues/4315) Fixed inserts into tables with auto-embedding columns so MVA values are preserved instead of being stored as empty.
* 🪲 [v21.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.2) [ PR #4277](https://github.com/manticoresoftware/manticoresearch-buddy/pull/649) Updated Buddy to 3.43.1, fixing autocomplete and fuzzy search handling for terms with numeric prefixes and wildcard asterisks.
* 🪲 [v21.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.1) [ PR #4349](https://github.com/manticoresoftware/manticoresearch/pull/4349) Fixed a crash during embedding generation for tables using columnar storage.
* 🪲 [v19.2.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.5) [ PR #4333](https://github.com/manticoresoftware/executor/pull/81) Updated Executor to 1.4.1, switching PHP source downloads from php.net to GitHub to avoid CAPTCHA-protected links, and bumping bundled PHP to 8.4.18.
* 🪲 [v19.2.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.4) [ Issue #4314](https://github.com/manticoresoftware/manticoresearch/issues/4314) Fixed model-backed `float_vector` columns to support an explicit empty `FROM=''`, which now correctly means “use all text/string fields.”.
* 🪲 [v19.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.3) [ Issue #4315](https://github.com/manticoresoftware/manticoresearch/issues/4315) Fixed inserts into tables with auto-embedding `float_vector` columns so MVA values are preserved instead of being stored as empty.
* 🪲 [v19.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.2) [ Issue #4297](https://github.com/manticoresoftware/manticoresearch/issues/4297) Fixed `IMPORT TABLE` to copy external files correctly, including `hitless_words`, so imported tables no longer reference missing external files after import.
* 🪲 [v19.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.1) [ Issue #4229](https://github.com/manticoresoftware/manticoresearch/issues/4229) Fixed `LEFT JOIN` results to emit proper MySQL `NULL` values instead of the string `NULL`, improving compatibility with native MySQL clients and drivers.
* 🪲 [v19.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.4) [ Issue #4308](https://github.com/manticoresoftware/manticoresearch/issues/4308) Fixed daemon crashes caused by filtering on stored/full-text fields such as `WHERE title='test'`; these queries now return an error instead of crashing.
* 🪲 [v19.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.2) [ PR #4311](https://github.com/manticoresoftware/manticoresearch/issues/4307) Fixed crashes in HTTP `/sql?mode=raw` multi-statement requests using `.@files` followed by `SHOW META`.
* 🪲 [v19.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.1) [ Issue #4103](https://github.com/manticoresoftware/manticoresearch/issues/4103) Updated [MCL](https://github.com/manticoresoftware/columnar) to 11.0.0 to support KNN prefiltering.
* 🪲 [v18.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.1) [ Issue #4293](https://github.com/manticoresoftware/manticoresearch/issues/4293) Fixed `ALTER TABLE ... ADD COLUMN` for auto-embedding `float_vector` columns so `model_name` and `FROM` are preserved correctly instead of being replaced with `knn_dims='0'`.
* 🪲 [v17.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.6) [ Issue #4264](https://github.com/manticoresoftware/manticoresearch/issues/4264) Fixed kqueue EOF handling on macOS to prevent spurious HTTP receive warnings such as `Resource temporarily unavailable` during request processing.
* 🪲 [v17.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.5) [ PR #4296](https://github.com/manticoresoftware/manticoresearch/pull/4296) Fixed document-count and cost estimation for full-text `AND` nodes, improving query planner decisions for intersecting full-text conditions.
* 🪲 [v17.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.4) [ Issue #4274](https://github.com/manticoresoftware/manticoresearch/issues/4274) Fixed `indextool --check` failing to open disk-chunk `hitless_words` files by resolving their paths relative to the index directory.
* 🪲 [v17.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.3) [ Issue #4284](https://github.com/manticoresoftware/manticoresearch/issues/4284) Fixed crashes on replica nodes when inserting into clustered tables with auto-embeddings by preserving the source text needed to generate embeddings during replicated commits.
* 🪲 [v17.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.2) [ Issue #4257](https://github.com/manticoresoftware/manticoresearch/issues/4257) Fixed a Valgrind-detected invalid read in prepared statement execution by ensuring the parser input buffer is terminated correctly.
* 🪲 [v17.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.6.1) [ Issue #4207](https://github.com/manticoresoftware/manticoresearch/issues/4207) Added a regression fix and test coverage for the RT disk chunk save race that could lose document kills and lead to duplicate rows after merges.
* 🪲 [v17.5.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.10) [ Issue #4207](https://github.com/manticoresoftware/columnar/issues/133) Updated [MCL](https://github.com/manticoresoftware/columnar) to 10.2.2, fixing crashes when running searches while data is being inserted into tables with local embedding models by adding a concurrency guard around local model usage.
* 🪲 [v17.5.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.9) [ PR #4247](https://github.com/manticoresoftware/manticoresearch/pull/4247) Fixed a race during RT disk chunk save that could lose document kills and produce duplicate rows after merges or saves.
* 🪲 [v17.5.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.8) [ PR #4241](https://github.com/manticoresoftware/manticoresearch/pull/4247) Fixed incorrect post-limit attribute evaluation in joined tables, including `LEFT JOIN` queries after flushing RAM chunks to disk.
* 🪲 [v17.5.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.7) [ PR #4239](https://github.com/manticoresoftware/manticoresearch/pull/4239) Clarified query-time and wall-time definitions in logging and status documentation.
* 🪲 [v17.5.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.6) [ PR #4245](https://github.com/manticoresoftware/manticoresearch/pull/4245) Fixed global `search_stats_ms_*` metrics overreporting search time by double-counting local execution time; these statistics now match the actual query execution time.
* 🪲 [v17.5.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.5) [ Issue #858](https://github.com/manticoresoftware/manticoresearch/issues/858) Fixed `HIGHLIGHT()` and snippet generation for CJK text adding unnecessary spaces between words; highlighted excerpts now preserve the original text spacing instead of tokenized spacing.
* 🪲 [v17.5.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.4) [ Issue #1166](https://github.com/manticoresoftware/manticoresearch/issues/1166) Fixed a crash on INSERT after `TRUNCATE TABLE ... WITH RECONFIGURE` for RT tables with [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)=1, particularly when fields and string attributes are mixed; field-length attributes are now set up correctly during reconfigure.
* 🪲 [v17.5.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.3) Fixed corrupted Wordforms documentation content in the manual.
* 🪲 [v17.5.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.2) [ Issue #3213](https://github.com/manticoresoftware/manticoresearch/issues/3213) Fixed JOIN queries filtering on expressions built from aliases of joined-table columns, such as `SELECT t2.i al, (al*0.1) pct ... WHERE pct > 0`, which could fail with `incoming-schema expression missing evaluator`; these filters now evaluate correctly after the join.
...
## Breaking Changes
* ⚠️ [v17.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.0.0) [ Issue #4120](https://github.com/manticoresoftware/manticoresearch/issues/4120) [MCL](https://github.com/manticoresoftware/columnar) 10.0.0: Added support for `DROP CACHE`. This updates the interface between the daemon and [MCL](https://github.com/manticoresoftware/columnar). Older Manticore Search versions don't suport the newer [MCL](https://github.com/manticoresoftware/columnar).
...
## New Features and Improvements
* 🆕 [v17.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.0) [ PR #130](https://github.com/manticoresoftware/columnar/pull/130) [MCL](https://github.com/manticoresoftware/columnar) updated to 10.2.0: QWEN model support fixed and support for additional models added.
...
* 🆕 [v17.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.0) [ PR #4195](https://github.com/manticoresoftware/manticoresearch/pull/4195) Updated [MCL](https://github.com/manticoresoftware/columnar) to 10.1.0; Added support for Qwen local embeddings models.
...
* 🪲 [v17.2.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.6) [ Issue #3203](https://github.com/manticoresoftware/manticoresearch/issues/3203) Fixed [ALTER TABLE REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) failing with `failed to rename … .tmp.spjidx` when the table had multiple disk chunks.
...
* 🪲 [v17.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.3) [ Issue #4176](https://github.com/manticoresoftware/manticoresearch/issues/4176) Fixed table rename breaking tables that use external stopwords, wordforms, or exceptions: [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) now migrates these files to the new chunk-based format and updates disk chunk headers, so after `ALTER TABLE RENAME` the daemon no longer reports missing external files on restart.
...
* 🪲 [v16.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/16.0.4) [ PR #4047](https://github.com/manticoresoftware/manticoresearch/pull/4047) Updated [MCL](https://github.com/manticoresoftware/columnar) version to 9.0.1. Fixed KNN float vector handling when block_size is 1 by disabling table compression in that case (compression is not used for block size 1).
...
## Breaking Changes
* ⚠️ [v15.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/15.0.0) [ PR #4003](https://github.com/manticoresoftware/manticoresearch/pull/4003) Updated [MCL](https://github.com/manticoresoftware/columnar) requirement to 9.0.0, bringing uncompressed float-vector storage, block-size change for KNN vectors, and non-buffered reads. This update changes the data format. Older [MCL](https://github.com/manticoresoftware/columnar) versions won’t be able to read it, but the new version can still read your existing columnar tables without any issues.
...
* 🪲 [v14.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.7.1) [ PR #3992](https://github.com/manticoresoftware/manticoresearch/pull/3992) Fixed missing error messages for HTTP [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) and DELETE requests on distributed tables handled via agents, so clients now correctly receive errors when operations fail.
...
### Bug Fixes
* 🪲 [ Issue #4299](https://github.com/manticoresoftware/manticoresearch/issues/4299) Added testing for Grafana version 12.4.
...
### New Features and Improvements
* 🆕 [v13.13.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.0) Added support for [MCL](https://github.com/manticoresoftware/columnar) 8.1.0 with SI block cache.
...
* 🪲 [v13.11.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.11.6) [ PR #3766](https://github.com/manticoresoftware/manticoresearch/pull/3766) Added support for [MCL](https://github.com/manticoresoftware/columnar) version 8.0.6.
...
* 🪲 [v13.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.5.1) [ Issue #3560](https://github.com/manticoresoftware/manticoresearch/issues/3560) Fixed a case where [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) could hang indefinitely with KNN data.
...
* 🪲 [v10.2.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/10.2.7)  Added support for [MCL](https://github.com/manticoresoftware/columnar) 5.0.5, including a fix for the embeddings library filename
...
* 🪲 [v9.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.2)  Fix: updated deps.txt again to include packaging fixes in [MCL](https://github.com/manticoresoftware/columnar) related to the embeddings library
* 🪲 [v9.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.1)  Fix: updated deps.txt with packaging fixes for [MCL](https://github.com/manticoresoftware/columnar) and the embeddings library
...
* [Issue #3189](https://github.com/manticoresoftware/manticoresearch/issues/3189) Fixed: [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) statements now correctly respect the `query_log_min_msec` setting.
...
* [Issue #3054](https://github.com/manticoresoftware/manticoresearch/issues/3054) Fixed an issue with the [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) command.
...
* [Commit 4954](https://github.com/manticoresoftware/manticoresearch/commit/4954b5de7341a29902a9b8fbb9a040f7942c77c4) Changed the default cutoff for [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) on tables with KNN indexes to improve search performance.
...
* [Issue #1818](https://github.com/manticoresoftware/manticoresearch/issues/1818) [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) now throws an error when the table is frozen.
...
* [Issue #2265](https://github.com/manticoresoftware/manticoresearch/issues/2265) Fixed failure to restore `mysqldump` with [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) enabled.
...
* [Issue #1849](https://github.com/manticoresoftware/manticoresearch/issues/1849) Fixed indextool crash on shutdown if used with [MCL](https://github.com/manticoresoftware/columnar).
...
### Related with [MCL](https://github.com/manticoresoftware/columnar) (columnar, secondary, knn libs)
...
* [Commit 9dc1](https://github.com/manticoresoftware/manticoresearch/commit/9dc12334): When executing `[ALTER TABLE table REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index)`, secondary indexes are now always rebuilt, even if attributes weren't updated.
...
* ⚠️ [Issue #1057](https://github.com/manticoresoftware/manticoresearch/issues/1057): [MCL](https://github.com/manticoresoftware/columnar) libraries will no longer load on systems that don't support SSE 4.2.
...
* **⚠️ BREAKING CHANGE**: Secondary indexes file format has been changed, so if you are using secondary indexes for searching and have `searchd.secondary_indexes = 1` in your configuration file, be aware that the new Manticore version **will skip loading the tables that have secondary indexes**. It's recommended to:
  ...
  - Run [ALTER TABLE `<table name>` REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) for each index to rebuild secondary indexes.

  If you are running a replication cluster, you'll need to run [ALTER TABLE `<table name>` REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) on all the nodes or follow [this instruction](Securing_and_compacting_a_table/Compacting_a_table.md#Optimizing-clustered-tables) with just change: run the [ALTER .. REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) instead of the [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE).
...
* [Commit 2757](https://github.com/manticoresoftware/columnar/commit/2757b99b) If a disk chunk merge is interrupted, the daemon now cleans up the [MCL](https://github.com/manticoresoftware/columnar)-related tmp files.
...
* [Commit 06df](https://github.com/manticoresoftware/manticoresearch/commit/06dfdd2e668933495dd6cb52d5ba2e0aa9b26e58) Queries like `select attr, count(*) from plain_index` (w/o filtering) are now faster in case you are using [MCL](https://github.com/manticoresoftware/columnar).
...
* **⚠️ BREAKING CHANGE**: [MCL Issue #17](https://github.com/manticoresoftware/columnar/issues/17) [MCL](https://github.com/manticoresoftware/columnar): add SSE code to columnar scan. [MCL](https://github.com/manticoresoftware/columnar) now requires at least SSE4.2.
...
* [MCL Issue #14](https://github.com/manticoresoftware/columnar/issues/14) [MCL](https://github.com/manticoresoftware/columnar): crash on select when too many ft fields
...
* [Issue #802](https://github.com/manticoresoftware/manticoresearch/issues/802) [MCL](https://github.com/manticoresoftware/columnar): Indexer crashes in 5.0.2 and manticore-columnar-lib 1.15.4
...
* [Issue #843](https://github.com/manticoresoftware/manticoresearch/issues/843) [MCL](https://github.com/manticoresoftware/columnar): Bug with long text field
...
* [Issue #919](https://github.com/manticoresoftware/manticoresearch/issues/919) [MCL](https://github.com/manticoresoftware/columnar): Manticore crashes on query execution and other crashed during cluster recovery.
...
* [Issue #982](https://github.com/manticoresoftware/manticoresearch/issues/982) [MCL](https://github.com/manticoresoftware/columnar): 5.0.3 crash from secondary index
...
* [Issue #995](https://github.com/manticoresoftware/manticoresearch/issues/995) [MCL](https://github.com/manticoresoftware/columnar): min(pickup_datetime) from taxi1 crashes
...
* [Issue #1000](https://github.com/manticoresoftware/manticoresearch/issues/1000) [MCL](https://github.com/manticoresoftware/columnar): Columnar rt index became damaged after run without columnar library
...
* ⚠️ If you are using old [php](https://github.com/manticoresoftware/manticoresearch/blob/main/api/sphinxapi.php), [python](https://github.com/manticoresoftware/manticoresearch/blob/main/api/sphinxapi.py) or [java](https://github.com/manticoresoftware/manticoresearch/tree/main/api/java) clients please follow the corresponding link and find an updated version. **The old versions are not fully compatible with Manticore 5.**
...
* [Commit 7ec7](https://github.com/manticoresoftware/manticoresearch/commit/7ec76d4abf01b5fca6f10a0000fe64d5d8e5fa52) RAM exhaustion issue caused by [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)s
...
- **Full support of [Manticore Columnar Library](https://github.com/manticoresoftware/columnar)**. Previously Manticore Columnar Library was supported only for plain indexes. Now it's supported:
  - in real-time indexes for `INSERT`, `REPLACE`, `DELETE`, [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE)
  - in replication
  - in `ALTER`
  - in `indextool --check`
- **Automatic indexes compaction** ([Issue #478](https://github.com/manticoresoftware/manticoresearch/issues/478)). Finally, you don't have to call [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) manually or via a crontask or other kind of automation. Manticore now does it for you automatically and by default. You can set default compaction threshold via [optimize_cutoff](Server_settings/Setting_variables_online.md) global variable.
...
- **Significantly faster bulk INSERT performance into a real-time index**. For example on [Hetzner's server AX101](https://www.hetzner.com/dedicated-rootserver/ax101) with SSD, 128 GB of RAM and AMD's Ryzen™ 9 5950X (16*2 cores) **with 3.6.0 you could get 236K docs per second** inserted into a table with schema `name text, email string, description text, age int, active bit(1)` (default `rt_mem_limit`, batch size 25000, 16 concurrent insert workers, 16 million docs inserted overall). In 4.0.2 the same concurrency/batch/count gives **357K docs per second**.
  ...
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)s and DELETEs are performed against existing chunks, but for the case of merging that may be happening the writes are collected and are then applied against the new chunks
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)s acquire an exclusive lock sequentially for every chunk. Merges acquire a shared lock when entering the stage of collecting attributes from the chunk. So at the same time only one (merge or update) operation has access to attributes of the chunk.
  - when merging gets to the phase, when it needs attributes it sets a special flag. When [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) finishes, it checks the flag, and if it's set, the whole update is stored in a special collection. Finally, when the merge finishes, it applies the updates set to the newborn disk chunk.
...
- [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) happens automatically. If you don't need it make sure to set `auto_optimize=0` in section `searchd` in the configuration file
...
- [FORCE/IGNORE INDEX(id)](Data_creation_and_modification/Updating_documents/UPDATE.md#Updates-via-SQL) for SELECT/[UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
* [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) reduces disk chunks to a number of chunks ( default is `2* No. of cores`) instead of a single one. The optimal number of chunks can be controlled by [cutoff](Securing_and_compacting_a_table/Compacting_a_table.md#Number-of-optimized-disk-chunks) option.
...
* disabled [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)/DELETE statements logging a SELECT in query log
...
* [Commit d0a7](https://github.com/manticoresoftware/manticoresearch/commit/d0a7c95906b9b5ba33c98267d54501cfe27508d6) fix \#250 [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) index option for TSV and CSV piped sources
...
* updating document ID with [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) is disabled
...
* String,JSON and MVAs can be updated using [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
* ALTER CLUSTER posts [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) nodes
...
* [Commit 4ffa](https://github.com/manticoresoftware/manticoresearch/commit/4ffa623e9d357a3b0b441615089e211f92f8de32) fixed total documents counter at disk chunk got affected by [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) command and breaks weight calculation
...
* [Commit e386](https://github.com/manticoresoftware/manticoresearch/commit/e386d842b36195719e459b2db1fb20e402f9a51f) fixed crash of server on DELETE to RT index with [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) enable
