...
ASCII letters in table names are converted to lowercase when created. Non-ASCII UTF-8 characters retain their original spelling and do not undergo Unicode case folding. See [table and field name syntax](Data_types.md#Table-and-field-name-syntax).
...
Table names are case-sensitive in this mode. Both table and field names can contain non-ASCII UTF-8 characters. Unicode case folding is not applied, so use the exact spelling when referring to them. See [table and field name syntax](Data_types.md#Table-and-field-name-syntax).
