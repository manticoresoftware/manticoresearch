...
The values must be separated by semicolons (`;`). Since word forms can contain `>` or `=>` as separators, and potentially other special characters, make sure to escape semicolons if they are part of the token itself (e.g. `\;`).
