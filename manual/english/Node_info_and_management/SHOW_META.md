...
* `total_found`:
  ...
  - When using `HAVING` with `GROUP BY`, `total_found` reflects the number of groups **after** the `HAVING` filter is applied (as of version 13.15.3). This enables proper pagination with `HAVING` clauses.
