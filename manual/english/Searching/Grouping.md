...
**Note:** Starting from version 13.15.3, `HAVING` correctly affects `total_found` in the [search query meta info](../Node_info_and_management/SHOW_META.md#SHOW-META). The `total_found` value now reflects the number of groups **after** the `HAVING` filter is applied, not before. This enables proper pagination when using `HAVING` clauses with `GROUP BY`.
