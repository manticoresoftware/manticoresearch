...
> NOTE: HNSW graph construction during RT chunk saves and during `OPTIMIZE TABLE` / auto-optimize chunk merges runs in parallel by default on multi-core hosts; the worker count is controlled by the [`knn_parallel_build`](../Server_settings/Searchd.md#knn_parallel_build) searchd setting (set it to `1` to force the serial path). This affects build-time performance only; query syntax and search-time settings are unchanged. Because parallel HNSW construction may insert vectors in a different order, the resulting graph may not be bit-identical to a serial build.
