...
* `ro = {1 | 0}` switches session to read-only mode or back. In `show variables` output the variable displayed with name `session_read_only`. If `searchd.read_only` is enabled, sessions start in read-only mode automatically and `SET ro=0` is rejected.
...
* `AUTO_OPTIMIZE = <non-negative integer>` Configures [auto_optimize](../Server_settings/Searchd.md#auto_optimize): `0` disables it, `1` enables it with the default threshold, and values greater than `1` multiply that threshold.
