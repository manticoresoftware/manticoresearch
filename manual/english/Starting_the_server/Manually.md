...
- `manticore_data/mysql.sock` as a MySQL-protocol Unix socket;
...
Standard MySQL-compatible tools can use the second socket:

```bash
mysql --socket="$PWD/manticore_data/mysql.sock"
```
...
`--local` cannot be combined with `--config`, and local mode is not supported on Windows. One or more `--listen` options can extend the two private Unix listeners. In local mode these options are additive rather than replacements, for example:

```bash
searchd --local --listen 127.0.0.1:19306:mysql41 --listen 127.0.0.1:19308:http
```
...
* `--local` (`-l` for short) starts [local configless mode](Starting_the_server/Manually.md#Local-configless-mode) in the current directory. It creates or reuses `./manticore_data`, adds private HTTP and MySQL Unix sockets, and writes `.manticore-local`. Repeated `--listen` options add listeners without replacing those defaults. It cannot be combined with `--config`, and `--status` is not supported in this mode.
...
*  `--port portnumber` (`-p` for short) is used to specify the port that Manticore should listen on to accept binary protocol requests, usually for debugging purposes. This will usually default to 9312, but sometimes you need to run it on a different port. Outside local mode, specifying it on the command line overrides listeners from the configuration file. In local mode, it explicitly adds a binary TCP listener while preserving the two default Unix sockets. The valid range is 0 to 65535, but ports numbered 1024 and below usually require a privileged account in order to run.
