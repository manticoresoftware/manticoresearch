...
* 🆕 [v26.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.1.0) [ PR #4537](https://github.com/manticoresoftware/manticoresearch/pull/4537) Добавлены [нативные таблицы `type='sharding'`](Creating_a_table/Creating_a_sharded_table/Creating_a_sharded_table.md) с распределёнными вставками, а также крупные улучшения шардинга, охватывающие создание, маршрутизацию, откат, отказоустойчивость и операционное поведение.
...
## Ломающие изменения
* ⚠️ **[v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0)** [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) **Ломающее изменение**: если включить [аутентификацию и авторизацию](Security/Authentication_and_authorization.md), анонимный доступ клиентов больше не работает, а операции с удалёнными агентами распределения и репликацией могут завершаться ошибкой, если у участвующих демонов нет совпадающих сохранённых данных аутентификации для нужных пользователей. Успешный `JOIN CLUSTER` также заменяет локальные данные аутентификации присоединяющегося узла данными кластера-донора. Существующие данные таблиц остаются совместимыми, но для миграции потребуется создать пользователей, выдать права, синхронизировать данные аутентификации между узлами и пересмотреть процедуры присоединения к кластеру. Даунгрейд безопасен только до включения auth и до того, как какой-либо `JOIN CLUSTER` заменит локальные данные аутентификации.
...
Этот релиз приносит более широкую переработку упаковки, новые параметры `API_URL` и `API_TIMEOUT` для моделей автоэмбеддингов, [гибридный поиск](Searching/Hybrid_search.md), поддержку резервного копирования и восстановления, совместимых с S3, более быстрое обслуживание RT-таблиц с N-way merge и параллельным [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), KNN-prefiltering и подготовленные выражения, совместимые с MySQL.

## Критические изменения
* ⚠️ [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #123](https://github.com/manticoresoftware/columnar/pull/123) **Ломающие изменения**: обновлен **[MCL](https://github.com/manticoresoftware/columnar)** до 13.0.0, добавлены [`API_URL`](Searching/KNN.md#Creating-a-table-with-auto-embeddings) и [`API_TIMEOUT`](Searching/KNN.md#Creating-a-table-with-auto-embeddings) для [**моделей автоэмбеддингов**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29). Это затрагивает вас, если вы используете [**модели автоэмбеддингов**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) или управляете **[MCL](https://github.com/manticoresoftware/columnar)** отдельно от демона, потому что Manticore 25.0.0 требует **[MCL](https://github.com/manticoresoftware/columnar) 13.0.0** и несовместим со старыми версиями [MCL](https://github.com/manticoresoftware/columnar); обновляйте демон и [MCL](https://github.com/manticoresoftware/columnar) вместе, а откат безопасен только если вы также вернете подходящую старую версию [MCL](https://github.com/manticoresoftware/columnar).
...
* ⚠️ [v18.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.0) [ Issue #4261](https://github.com/manticoresoftware/manticoresearch/issues/4261) **Ломающие изменения**: вводится **версия формата индекса 68** для исправления обработки layout словаря [`hitless_words`](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#hitless_words). Это затрагивает вас, если вы перестраиваете или заново записываете индексы, а затем нужно открыть переписанные индексы более старой версией Manticore: старые существующие индексы остаются читаемыми, но любые индексы, перестроенные или записанные заново в новом формате, несовместимы со старыми версиями Manticore, поэтому откат безопасен только до переписывания таких индексов.
...
## Новые возможности и улучшения
* 🆕 [v24.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.4.0) [ PR #4091](https://github.com/manticoresoftware/manticoresearch/pull/4091) Улучшена компактизация RT-таблиц за счет добавления N-way merge disk chunk и параллельных заданий [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), что сокращает время слияния большого числа disk chunk и открывает новые настройки [`merge_chunks_per_job`](Server_settings/Searchd.md#merge_chunks_per_job) и [`parallel_chunk_merges`](Server_settings/Searchd.md#parallel_chunk_merges).
...
* 🪲 [v17.5.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.4) [ Issue #1166](https://github.com/manticoresoftware/manticoresearch/issues/1166) Исправлен crash при INSERT после `TRUNCATE TABLE ... WITH RECONFIGURE` для RT-таблиц с [`index_field_lengths`](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)=1, особенно когда смешаны поля и строковые атрибуты; атрибуты длины полей теперь настраиваются корректно во время reconfigure.
...
* 🪲 [v17.2.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.6) [ Issue #3203](https://github.com/manticoresoftware/manticoresearch/issues/3203) Исправлено падение [`ALTER TABLE REBUILD SECONDARY`](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) с ошибкой `failed to rename … .tmp.spjidx`, когда у таблицы было несколько disk chunk.
...
❤️ Мы хотим от всей души поблагодарить [@ricardopintottrdata](https://github.com/ricardopintottrdata) за работу над [PR #3792](https://github.com/manticoresoftware/manticoresearch/pull/3792) и [PR #3828](https://github.com/manticoresoftware/manticoresearch/pull/3828) — за исправление проблем с итоговыми счетчиками в `HAVING` и ошибкой `filter with empty name` — а также [@jdelStrother](https://github.com/jdelStrother) за вклад в [PR #3819](https://github.com/manticoresoftware/manticoresearch/pull/3819), который улучшает обработку `ParseCJKSegmentation`, когда поддержка Jieba недоступна.
...
* [Issue #1818](https://github.com/manticoresoftware/manticoresearch/issues/1818) [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) теперь завершится с ошибкой, если таблица заморожена.
...
* **⚠️ СЛОМАННОЕ ИЗМЕНЕНИЕ**: Формат файла вторичных индексов был изменен, поэтому если вы используете вторичные индексы для поиска и у вас в конфигурационном файле указано `searchd.secondary_indexes = 1`, имейте в виду, что новая версия Manticore **пропустит загрузку таблиц, которые имеют вторичные индексы**. Рекомендуется:
  ...
  - Запустите [`ALTER TABLE <table name> REBUILD SECONDARY`](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) для каждого индекса, чтобы перестроить вторичные индексы.

  Если вы работаете в репликационном кластере, нужно запустить [`ALTER TABLE <table name> REBUILD SECONDARY`](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) на всех узлах или следовать [этой инструкции](Securing_and_compacting_a_table/Compacting_a_table.md#Optimizing-clustered-tables) с одним изменением: использовать [`ALTER .. REBUILD SECONDARY`](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) вместо [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE).
...
- **Полная поддержка [Manticore Columnar Library](https://github.com/manticoresoftware/columnar)**. Ранее Manticore Columnar Library поддерживалась только для обычных индексов. Теперь она поддерживается:
  - в real-time индексах для `INSERT`, `REPLACE`, `DELETE`, [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE)
  - в репликации
  - в `ALTER`
  - в `indextool --check`
...
- [`OPTIMIZE`](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) выполняется автоматически. Если вам это не нужно, убедитесь, что в разделе `searchd` конфигурационного файла задано `auto_optimize=0`
