...
## Версия 29.9.0
**Выпущено**: 11 сентября 2026 года

В этом выпуске добавлены фрагментированные и многовекторные автоматические эмбеддинги, безопасные UTF-8-идентификаторы, настраиваемые ограничения на входные данные для эмбеддингов, нормализация немецкой буквы ß, более подробное тестирование в `manticore-load`, резервные копии с аутентификацией и учетом облачной среды, а также memory-mapped columnar attributes по умолчанию. Кроме того, исправлены проблемы в KNN-поиске, гибридном и сгруппированном поиске, изменениях схемы, массовой загрузке, репликации, восстановлении binlog, совместимости и поведении при завершении работы.

❤️ Благодарим [@tudorvasinca](https://github.com/tudorvasinca) за работу над [PR #4857](https://github.com/manticoresoftware/manticoresearch/pull/4857), [PR #4859](https://github.com/manticoresoftware/manticoresearch/pull/4859) и [PR #4873](https://github.com/manticoresoftware/manticoresearch/pull/4873).

### Новые функции и улучшения
* 🆕 [v29.9.0](https://github.com/manticoresoftware/manticoresearch/tree/29.9.0) [PR #4898](https://github.com/manticoresoftware/manticoresearch/pull/4898) Режим [access_columnar_attrs](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#Accessing-table-files) по умолчанию изменен с буферизованного чтения `file` на `mmap`, что позволяет операционной системе кэшировать данные columnar attributes по требованию без предварительного чтения. Чтобы сохранить прежнее поведение, явно задайте `access_columnar_attrs='file'`; `ALTER TABLE` теперь также заново открывает замененное columnar storage с использованием настроенного режима доступа.
* 🆕 [v29.8.0](https://github.com/manticoresoftware/manticoresearch/tree/29.8.0) [Backup Issue #134](https://github.com/manticoresoftware/manticoresearch-backup/issues/134) Обновлен [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) до 1.11.0: теперь [резервное копирование и восстановление через S3](Securing_and_compacting_a_table/Backup_and_restore.md#S3-storage-support) могут использовать цепочку провайдеров учетных данных AWS SDK, включая IRSA, общие учетные данные, роли задач ECS и профили экземпляров EC2, если статические ключи не заданы. Также добавлена поддержка `AWS_SESSION_TOKEN` для временных учетных данных.
* 🆕 [v29.7.0](https://github.com/manticoresoftware/manticoresearch/tree/29.7.0) [Issue #4816](https://github.com/manticoresoftware/manticoresearch/issues/4816) Добавлен опциональный параметр [MAX_INPUT_TOKENS](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) для локальных колонок с автоэмбеддингами: он позволяет обрезать каждый входной текст перед построением эмбеддинга, чтобы ограничить время инференса для длинных документов и снизить риск исчерпания памяти из-за повторных попыток. Его можно задать при `CREATE TABLE` или изменить через `ALTER TABLE ... MODIFY COLUMN` без повторного построения эмбеддингов для существующих строк; `0` или отсутствие параметра сохраняет собственный лимит модели.
* 🆕 [v29.6.0](https://github.com/manticoresoftware/manticoresearch/tree/29.6.0) [manticore-load Issue #26](https://github.com/manticoresoftware/manticore-load/issues/26) Обновлен [manticore-load](https://github.com/manticoresoftware/manticore-load) до 1.28.0: добавлен опциональный режим `--http` для бенчмарков через HTTP JSON API Manticore. Записи выполняются через `/bulk`, поисковые запросы — через `/search`, HTTP по умолчанию использует порт `9308`, а `--table` выбирает целевую таблицу.
* 🆕 [v29.5.0](https://github.com/manticoresoftware/manticoresearch/tree/29.5.0) [manticore-load PR #28](https://github.com/manticoresoftware/manticore-load/pull/28) Обновлен `manticore-load` до 1.27.0: в текущий прогресс добавлен RSS локального `searchd`, в итоговые отчеты — пиковый RSS, использование диска и CPU, а также агрегированный мониторинг по всем таблицам в рабочих нагрузках с несколькими командами.
* 🆕 [v29.4.0](https://github.com/manticoresoftware/manticoresearch/tree/29.4.0) [MCL Issue #190](https://github.com/manticoresoftware/columnar/issues/190) Добавлены [стратегии фрагментации](Searching/KNN.md#Chunking-strategies) для автоэмбеддингов: `mean` представляет длинный документ одним усредненным вектором, а `fixed`, `recursive` и `sentence` сохраняют несколько доступных для поиска векторов в `float_vector_array`. Новые параметры `MAX_TOKENS`, `OVERLAP_TOKENS` и `MAX_CHUNKS` управляют размером фрагмента, перекрытием и количеством фрагментов; прежнее поведение `truncate` остается значением по умолчанию.
* 🆕 [v29.3.12](https://github.com/manticoresoftware/manticoresearch/tree/29.3.12) [Issue #4827](https://github.com/manticoresoftware/manticoresearch/issues/4827) Добавлена опциональная [нормализация немецкой sharp-s в AOT](Creating_a_table/NLP_and_tokenization/Supported_languages.md#German-AOT-morphology-and-sharp-s) с помощью `charset_table=non_cont,german` вместе с `lemmatize_de_v2` или `lemmatize_de_v2_all`: теперь `ß`, `ẞ` и `ss` сопоставляются как эквивалентные в обычном поиске по целым словам, а `index_exact_words=1` сохраняет различие между ними в запросах по точным словам. Существующие режимы немецкой морфологии не изменились; для переключения заполненной таблицы на v2 нужно перестроить или переиграть ее.
* 🆕 [v29.3.4](https://github.com/manticoresoftware/manticoresearch/tree/29.3.4) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Обновлен [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) до 1.10.4: теперь его CLI может аутентифицировать HTTP-запросы с помощью [параметров аутентификации](Securing_and_compacting_a_table/Backup_and_restore.md#Arguments), таких как `--user` и `--password` или `--token`, а выбранные таблицы гарантированно размораживаются, если резервное копирование завершается ошибкой или прерывается.
* 🆕 [v29.3.0](https://github.com/manticoresoftware/manticoresearch/tree/29.3.0) [Issue #4770](https://github.com/manticoresoftware/manticoresearch/issues/4770) Добавлена единая поддержка [безопасных UTF-8-имен таблиц, полей и атрибутов](Creating_a_table/Data_types.md#Table-and-field-name-syntax) для RT-, percolate-, распределенных, шаблонных и обычных таблиц, SQL DDL и выражений, селекторов полнотекстовых полей и автоматически определяемых схем источников. Это позволяет использовать локализованные имена, например китайские и кириллические идентификаторы, без переименования исходной схемы.
* 🆕 [v29.2.0](https://github.com/manticoresoftware/manticoresearch/tree/29.2.0) [Issue #4099](https://github.com/manticoresoftware/manticoresearch/issues/4099) Добавлен [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array) для таблиц реального времени: каждый документ теперь может хранить и искать несколько KNN-векторов, например фрагменты текста, изображения товаров или ключевые кадры видео.
* 🆕 [v29.1.0](https://github.com/manticoresoftware/manticoresearch/tree/29.1.0) [manticore-load PR #24](https://github.com/manticoresoftware/manticore-load/pull/24) Обновлен `manticore-load` до 1.26.0: добавлены SQL-хуки `--worker-init` и `--worker-finalize` для настройки в рамках соединения и упорядоченного параллельного завершения вокруг нагрузок бенчмарка.

### Исправления ошибок
* 🪲 [v29.8.4](https://github.com/manticoresoftware/manticoresearch/tree/29.8.4) [Issue #4889](https://github.com/manticoresoftware/manticoresearch/issues/4889) Исправлена ошибка, из-за которой [гибридный поиск](Searching/Hybrid_search.md#Attribute-filters) игнорировал фильтры по `weight()` или выражениям и алиасам, производным от него; фильтры, зависящие от веса, теперь выполняются после объединения по итоговому текстовому весу. Такие фильтры в деревьях фильтров `OR` не поддерживаются и теперь возвращают явную ошибку.
* 🪲 [v29.8.2](https://github.com/manticoresoftware/manticoresearch/tree/29.8.2) [Issue #4893](https://github.com/manticoresoftware/manticoresearch/issues/4893) Исправлена ошибка, из-за которой корректные запросы [/bulk](Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md#Bulk-adding-documents) с gzip-сжатием и фиксированной длиной завершались HTTP 400, если сжатое тело приходило за несколько чтений из сокета; границы чтения из сокета больше не влияют на распаковку и загрузку данных.
* 🪲 [v29.8.1](https://github.com/manticoresoftware/manticoresearch/tree/29.8.1) [Issue #4884](https://github.com/manticoresoftware/manticoresearch/issues/4884) Исправлены Elasticsearch-совместимые запросы `/_bulk`, которые возвращали HTTP `409` на уровне запроса при ошибках отдельных элементов, из-за чего клиенты вроде Fluent Bit повторяли уже примененные записи. Теперь обработанные пакеты возвращают HTTP `200`, сохраняют успешные элементы, сообщают о дублирующихся операциях `create` как об ошибках `409` `version_conflict_engine_exception` на уровне элемента, включая дубликаты внутри одного пакета, и отклоняют некорректные метаданные действий на уровне запроса.
* 🪲 [v29.7.2](https://github.com/manticoresoftware/manticoresearch/tree/29.7.2) [Issue #4872](https://github.com/manticoresoftware/manticoresearch/issues/4872) Исправлена проблема, при которой [ALTER TABLE](Updating_table_schema_and_settings.md#Updating-table-schema-in-RT-mode) оставлял устаревшие метаданные атрибутов и исходных полей в RT-таблицах с [автоэмбеддингами](Searching/KNN.md#Auto-Embeddings-%28Recommended%29). Из-за этого значения, записанные через `INSERT` или `REPLACE` в добавленные позднее MVA-колонки, незаметно отбрасывались, а эмбеддинги могли смещаться после удаления более ранних колонок. Значения, потерянные до обновления, нужно восстановить из источника или резервной копии.
* 🪲 [v29.7.1](https://github.com/manticoresoftware/manticoresearch/tree/29.7.1) [MCL PR #205](https://github.com/manticoresoftware/columnar/pull/205) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 14.2.1: исправлена ситуация, когда модели автоэмбеддингов могли оставаться в памяти после `ALTER TABLE ... MODIFY COLUMN ... MAX_INPUT_TOKENS`, а разные настройки `API_TIMEOUT` и `MAX_INPUT_TOKENS` больше не конфликтуют в кеше моделей и не приводят к ошибочному повторному использованию одной и той же модели.
* 🪲 [v29.5.1](https://github.com/manticoresoftware/manticoresearch/tree/29.5.1) [MCL PR #204](https://github.com/manticoresoftware/columnar/pull/204) Исправлена проблема, при которой построение HNSW-индекса [KNN](Searching/KNN.md#KNN-vector-search) иногда оставляло проиндексированные векторы недостижимыми для KNN-запросов. Новые индексы теперь восстанавливают несвязанные узлы графа уровня 0 перед сохранением. Существующие индексы не переписываются автоматически; если они затронуты, их следует перестроить.
* 🪲 [v29.3.13](https://github.com/manticoresoftware/manticoresearch/tree/29.3.13) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Исправлена ошибка, из-за которой SQL [BACKUP](Securing_and_compacting_a_table/Backup_and_restore.md#General-syntax-of-BACKUP) и `manticore-backup` не работали при включенной аутентификации. Теперь операторы резервного копирования могут проходить аутентификацию по имени пользователя и паролю или bearer-токенам; им требуется `backup ON *` плюс `read` для каждой выбранной таблицы либо неограниченный `read ON *` для полных резервных копий. Перед откатом на версию без действия `backup` удалите все права `backup`.
* 🪲 [v29.3.11](https://github.com/manticoresoftware/manticoresearch/tree/29.3.11) [PR #4879](https://github.com/manticoresoftware/manticoresearch/pull/4879) Исправлена функция [LENGTH()](Functions/Arrays_and_conditions_functions.md#LENGTH%28%29) для атрибутов [float_vector_array](Creating_a_table/Data_types.md#Float-vector-array): она возвращала число внутренних слов хранения вместо количества векторов. Пустые массивы возвращают `0`, а исправленное значение единообразно используется при выборке, фильтрации и группировке.
* 🪲 [v29.3.10](https://github.com/manticoresoftware/manticoresearch/tree/29.3.10) [Issue #4844](https://github.com/manticoresoftware/manticoresearch/issues/4844) Buddy обновлен до 4.4.3, чтобы сохранять аутентифицированного SQL-пользователя и передавать внутренний bearer-токен Buddy в запросы manticore-backup. Это обеспечивает путь аутентификации на стороне зависимости для `BACKUP`; полная поддержка `auth=1`, включая проверки права `backup` на стороне демона, требует v29.3.13.
* 🪲 [v29.3.9](https://github.com/manticoresoftware/manticoresearch/tree/29.3.9) [Issue #4639](https://github.com/manticoresoftware/manticoresearch/issues/4639) Исправлен [гибридный поиск](Searching/Hybrid_search.md) с `GROUP BY`, который возвращал пустые или отсутствующие группы, включая MVA-группы. Теперь сгруппированные результаты сохраняют все группы и соблюдают итоговый `ORDER BY` и `WITHIN GROUP ORDER BY`.
* 🪲 [v29.3.8](https://github.com/manticoresoftware/manticoresearch/tree/29.3.8) [Issue #4774](https://github.com/manticoresoftware/manticoresearch/issues/4774) Исправлены фильтры равенства, `IN` и диапазона по [ID документов](Creating_a_table/Data_types.md#Document-ID), у которых знаковое представление отрицательное: при использовании индекса поиска по ID документа они не возвращали строк. Теперь фильтрация через lookup следует беззнаковому порядку ID в индексе.
* 🪲 [v29.3.7](https://github.com/manticoresoftware/manticoresearch/tree/29.3.7) [Issue #4856](https://github.com/manticoresoftware/manticoresearch/issues/4856) Исправлены запросы [GROUP BY](Searching/Grouping.md) к RT-таблицам с несколькими фрагментами, где число групп превышало `max_matches`: они могли возвращать дублирующиеся группы и неверные счетчики по фрагментам либо приводить к зависанию `searchd`, когда `COUNT(DISTINCT ...)` сортировался по этому счетчику, а список выборки содержал выражение.
* 🪲 [v29.3.6](https://github.com/manticoresoftware/manticoresearch/tree/29.3.6) [Issue #4826](https://github.com/manticoresoftware/manticoresearch/issues/4826) Обновлен [Buddy](Installation/Manticore_Buddy.md#Manticore-Buddy) до 4.4.2: исправлена проблема, из-за которой воркеры [материализованных представлений Kafka](Integration/Kafka.md#Materialized-view) навсегда останавливались с `Invalid JSON found`, когда внутренние ответы `SELECT ...; SHOW META` содержали большие целочисленные ID или длинные строки, похожие на числа. Теперь значения ответов сохраняются без потерь, а загрузка данных продолжается.
* 🪲 [v29.3.5](https://github.com/manticoresoftware/manticoresearch/tree/29.3.5) [Issue #4822](https://github.com/manticoresoftware/manticoresearch/issues/4822) Исправлены периодические падения `searchd` на JSON-запросах `/search` к распределенным таблицам при сортировке по строковому атрибуту и агрегации JSON-поля по нескольким локальным таблицам.
* 🪲 [v29.3.3](https://github.com/manticoresoftware/manticoresearch/tree/29.3.3) [Issue #4860](https://github.com/manticoresoftware/manticoresearch/issues/4860) Исправлено падение `searchd`, когда `DROP TABLE` выполнялся одновременно с активной вставкой `/bulk` в RT-таблицу, использующую [автоэмбеддинги](Searching/KNN.md#Auto-Embeddings-%28Recommended%29). Теперь транзакция либо завершается для живой таблицы, либо безопасно завершается ошибкой, если таблица удалена или заменена.
* 🪲 [v29.3.2](https://github.com/manticoresoftware/manticoresearch/tree/29.3.2) [Issue #4791](https://github.com/manticoresoftware/manticoresearch/issues/4791) Исправлены HTTP-[KNN-запросы](Searching/KNN.md#KNN-vector-search) к распределенным и шардированным таблицам с локальным шардом, которые могли приводить к падению координатора или возвращать неверного ближайшего соседа, когда 1-битное квантование запускало повторное ранжирование. Объединенные результаты шардов больше не ранжируются повторно.
* 🪲 [v29.3.1](https://github.com/manticoresoftware/manticoresearch/tree/29.3.1) [PR #4864](https://github.com/manticoresoftware/manticoresearch/pull/4864) Исправлено падение `searchd`, когда во втором операторе многооператорного запроса выполнялся [гибридный поиск](Searching/Hybrid_search.md), объединяющий `MATCH()`, `KNN()` и `OPTION fusion_method='rrf'`.
* 🪲 [v29.2.9](https://github.com/manticoresoftware/manticoresearch/tree/29.2.9) [Issue #4837](https://github.com/manticoresoftware/manticoresearch/issues/4837) Исправлено падение [indextool --apply-killlists](Miscellaneous_tools.md#indextool), когда killlist текущего формата применялся к старой обычной таблице с устаревшим JSON-заголовком. Теперь `indextool` считывает фактическую версию формата таблицы на диске перед обработкой lookup-данных.
* 🪲 [v29.2.8](https://github.com/manticoresoftware/manticoresearch/tree/29.2.8) [Issue #4854](https://github.com/manticoresoftware/manticoresearch/issues/4854) Восстановлено полное покрытие CI-тестами: GoogleTest, проверки согласованности API, `indextool` и другие тесты под управлением CTest теперь запускаются вместе с ubertests.
* 🪲 [v29.2.7](https://github.com/manticoresoftware/manticoresearch/tree/29.2.7) [Issue #4842](https://github.com/manticoresoftware/manticoresearch/issues/4842) Исправлена ошибка [ALTER TABLE ... RENAME](Updating_table_schema_and_settings.md#Renaming-a-real-time-table) с `Invalid API key for remote model` для таблиц реального времени, использующих удаленные автоэмбеддинги. Теперь копии таблиц сохраняют скрытые API-ключи эмбеддингов, не раскрывая их в `SHOW CREATE TABLE`.
* 🪲 [v29.2.6](https://github.com/manticoresoftware/manticoresearch/tree/29.2.6) [PR #4861](https://github.com/manticoresoftware/manticoresearch/pull/4861) Исправлена регрессия производительности [KNN-поиска](Searching/KNN.md#KNN-vector-search) в запросах с дополнительными фильтрами: устранен лишний префильтр `knn_dist`, когда HNSW-итератор уже исключает документы без векторов. При этом сохранено корректное исключение на путях KNN с полным перебором, полнотекстовым поиском, группировкой и фасетами.
* 🪲 [v29.2.5](https://github.com/manticoresoftware/manticoresearch/tree/29.2.5) [Issue #4833](https://github.com/manticoresoftware/manticoresearch/issues/4833) Исправлены SQL-запросы к таблицам с UUID в качестве ID документов, которые возвращали сохраненные поля `text` пустыми. Теперь сохраненные поля корректно извлекаются при явном выборе или через `SELECT *`.
* 🪲 [v29.2.4](https://github.com/manticoresoftware/manticoresearch/tree/29.2.4) [Issue #4828](https://github.com/manticoresoftware/manticoresearch/issues/4828) Восстановлена совместимость с Sequel Ace 5.3.1+ за счет поддержки его проверок `CAST(DATABASE() AS BINARY)` и `CAST(@@character_set_client AS BINARY)`, из-за которых ранее все последующие запросы завершались ошибкой.
* 🪲 [v29.2.3](https://github.com/manticoresoftware/manticoresearch/tree/29.2.3) [PR #4619](https://github.com/manticoresoftware/manticoresearch/pull/4619) Исправлено зависание корректного `searchd --stopwait`, пока шардированная таблица перебалансировалась после повторного присоединения узла. Теперь текущая передача шарда останавливается при завершении работы, а отсоединенные воркеры используют ожидания с ограничением по времени.
* 🪲 [v29.2.2](https://github.com/manticoresoftware/manticoresearch/tree/29.2.2) [Issue #4793](https://github.com/manticoresoftware/manticoresearch/issues/4793) Исправлена ошибка, из-за которой `indexer` не мог построить обычные таблицы, если настроенный [path](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#path) содержал отсутствующие родительские каталоги. Теперь он автоматически создает недостающее дерево каталогов, если ближайший существующий родитель доступен для записи.
* 🪲 [v29.2.1](https://github.com/manticoresoftware/manticoresearch/tree/29.2.1) [PR #4832](https://github.com/manticoresoftware/manticoresearch/pull/4832) Исправлены и дополнены заголовки с авторскими правами и лицензией GPL в недавно добавленных исходных файлах.
* 🪲 [v29.1.1](https://github.com/manticoresoftware/manticoresearch/tree/29.1.1) [Issue #4808](https://github.com/manticoresoftware/manticoresearch/issues/4808) Исправлены обновления с версий с binlog v15, которые могли приводить к падению или отсутствию нового сброшенного на диск RT-чанка в метаданных таблицы при завершении работы. Совместимые журналы восстановления теперь переигрываются, завершенные чанки публикуются перед чистым завершением, а неподдерживаемые форматы восстановления безопасно завершаются ошибкой.
* 🪲 [v29.0.6](https://github.com/manticoresoftware/manticoresearch/tree/29.0.6) [Issue #4811](https://github.com/manticoresoftware/manticoresearch/issues/4811) Улучшена диагностика фатальных ошибок при переигрывании binlog: теперь в ней указывается соответствующий параметр восстановления [--replay-flags](Starting_the_server/Manually.md#searchd-command-line-options): `ignore-open-errors` для отсутствующих файлов binlog и `ignore-trx-errors` для ошибок переигрывания транзакций.
* 🪲 [v29.0.5](https://github.com/manticoresoftware/manticoresearch/tree/29.0.5) [Issue #4784](https://github.com/manticoresoftware/manticoresearch/issues/4784) Исправлены JSON-запросы `/search` с отрицанием, которые теряли расстояния для [операторов NEAR и proximity](Searching/Full_text_matching/Operators.md), из-за чего корректные позиционные запросы завершались ошибкой нулевого расстояния или работали неверно.
* 🪲 [v29.0.4](https://github.com/manticoresoftware/manticoresearch/tree/29.0.4) [Issue #4788](https://github.com/manticoresoftware/manticoresearch/issues/4788) Исправлены запросы [LEFT JOIN](Searching/Joining.md) с сортировкой по строковым атрибутам, которые возвращали внутренние вспомогательные колонки `@int_attr_*` в результатах клиента.
* 🪲 [v29.0.3](https://github.com/manticoresoftware/manticoresearch/tree/29.0.3) [PR #4790](https://github.com/manticoresoftware/manticoresearch/pull/4790) Исправлены некорректные запросы `SEARCH` через бинарный API с отрицательным или невозможным количеством элементов, которые могли завершить `searchd`; теперь недопустимые счетчики отклоняются с ошибками протокола, в том числе при `max_batch_queries=0`.
...
### Исправления ошибок
* 🪲 [v28.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.6) [Issue #4741](https://github.com/manticoresoftware/manticoresearch/issues/4741) Исправлен разбор `USER()` после того, как поддержка аутентификации сделала `USER` ключевым словом, что восстановило совместимость с командой `status` клиента MySQL.
* 🪲 [v28.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.5) [Issue #4764](https://github.com/manticoresoftware/manticoresearch/issues/4764) Исправлена работа [CALL SNIPPETS](Searching/Highlighting.md#CALL-SNIPPETS), которая занимала секунды на сложных булевых запросах при включенном `boolean_simplify`, и восстановлена ожидаемая подсветка для затронутых запросов.
* 🪲 [v28.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.4) [Issue #4737](https://github.com/manticoresoftware/manticoresearch/issues/4737) Исправлен разбор [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) для полных имен `system.<table>`, что позволяет оптимизировать физически шардинговые таблицы.
* 🪲 [v28.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.3) [Issue #138](https://github.com/manticoresoftware/manticoresearch-backup/issues/138) Обновлен [manticore-backup](https://github.com/manticoresoftware/manticoresearch-backup) до версии 1.10.2, что устраняет проблему, при которой успешные бэкапы оставляли RT-таблицы замороженными и блокировали последующие записи на неопределенное время.
* 🪲 [v28.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.2) [Issue #4758](https://github.com/manticoresoftware/manticoresearch/issues/4758) Исправлено оставление устаревших внешних файлов RT после `ALTER`, `TRUNCATE`, удаления чанков или оптимизации, а также предотвращена потеря настроек токенизации Jieba при несвязанных изменениях `ALTER TABLE`; неподдерживаемые фактические изменения настройки Jieba теперь завершаются явной ошибкой.
* 🪲 [v28.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.6.1) [Issue #4750](https://github.com/manticoresoftware/manticoresearch/issues/4750) Исправлена передача состояния аутентифицированной репликации, чтобы `ALTER CLUSTER ... ADD` мог добавлять уже заполненные RT-таблицы вместо ошибки с исчерпанием повторных попыток.
* 🪲 [v28.5.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.3) [Issue #4730](https://github.com/manticoresoftware/manticoresearch/issues/4730) Исправлена потенциальная порча памяти или аварийное завершение `searchd` на длинных точных и похожих на фразы запросах за счет учета их полной ширины термов в защите стека запросов.
* 🪲 [v28.5.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.2) [PR #4729](https://github.com/manticoresoftware/manticoresearch/pull/4729) Улучшена обработка запросов `local_df` для многочанковых RT- и распределенных таблиц за счет дедупликации повторяющейся статистики по ключевым словам, что позволяет избежать избыточной предварительной работы перед поиском для больших повторяющихся запросов `OR` и фразовых запросов.
* 🪲 [v28.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.5.1) [Issue #4726](https://github.com/manticoresoftware/manticoresearch/issues/4726) Исправлена гонка при обработке соединений SphinxQL, из-за которой workers могли уходить в 100% CPU, а `searchd` мог навсегда перестать отвечать, если клиенты подключались во время запуска или прерывали раннее обновление SSL.
* 🪲 [v28.4.11](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.11) [PR #4725](https://github.com/manticoresoftware/manticoresearch/pull/4725) Улучшен `indextool --check`: теперь он проверяет структуру и payload вторичных индексов и сообщает о поврежденных файлах `.spidx` как об ошибках проверки, а не завершает работу аварийно.
* 🪲 [v28.4.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.10) [PR #4735](https://github.com/manticoresoftware/manticoresearch/pull/4735) Исправлен разбор ответов в тестовом наборе Ruby API, чтобы сохранять корректные чанки `"0"`, как это делает PHP-клиент.
* 🪲 [v28.4.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.9) [Issue #4595](https://github.com/manticoresoftware/manticoresearch/issues/4595) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до версии 13.8.2, что устраняет выход за пределы чтения в декодере StreamVByte, который мог вызывать падение запросов с использованием вторичных индексов, включая `GROUP BY` по JSON-полям.
* 🪲 [v28.4.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.8) [PR #4703](https://github.com/manticoresoftware/manticoresearch/pull/4703) Исправлен разбор ответов PHP API, когда корректный фрагмент ответа заканчивается строкой `"0"`, что предотвращает ложные ошибки `failed to read searchd response` для затронутых размеров пакетов.
* 🪲 [v28.4.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.7) [PR #4721](https://github.com/manticoresoftware/manticoresearch/pull/4721) Исправлена гонка при одновременном выполнении `SHOW INDEX ... STATUS`, которая могла повреждать вычисление процентилей статистики запросов (`p95`/`p99`).
* 🪲 [v28.4.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.6) [Issue #4677](https://github.com/manticoresoftware/manticoresearch/issues/4677) Исправлены фильтры `IN(...)` по document ID, которые молча возвращали пустой результат для корректных unsigned 64-bit ID больше `Long.MAX_VALUE`; некорректные document ID вне диапазона теперь завершаются ошибкой проверки без ограничения корректных значений `multi64`.
* 🪲 [v28.4.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.5) [Issue #4633](https://github.com/manticoresoftware/manticoresearch/issues/4633) Исправлено падение при запросе `LEFT JOIN` с несколькими `FACET`-клаузами за счет того, что сортировщики множественных фасетов для join больше не очищаются дважды.
...
### Новые возможности и улучшения
* 🆕 [v28.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.0) [ PR #4707](https://github.com/manticoresoftware/manticoresearch/pull/4707) Улучшена производительность [KNN search](Searching/KNN.md#KNN-vector-search) за счет пакетной обработки вычислений расстояний на этапе `rescore`, что снижает накладные расходы при повторном вычислении и повторной сортировке финальных расстояний с полной точностью.
* 🆕 [v28.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.0) [Issue #688](https://github.com/manticoresoftware/manticoresearch-buddy/issues/688) [PR #4690](https://github.com/manticoresoftware/manticoresearch/pull/4690) Buddy обновлён до версии 4.2.0. Добавлена поддержка `custom_prompt` для [CREATE CHAT MODEL](Searching/Conversational_search.md), благодаря чему ответы [разговорного поиска](Searching/Conversational_search.md) могут следовать пользовательским инструкциям, например правилам цитирования и заданному стилю ответа. Также исправлена [ошибка #686](https://github.com/manticoresoftware/manticoresearch-buddy/issues/686) — утечка файловых дескрипторов (FD) в воркере Buddy.
* 🆕 [v28.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.2.0) [ PR #4596](https://github.com/manticoresoftware/manticoresearch/pull/4596) В [quick start guide](Quick_start_guide.md) добавлен установщик `curl`/`wget` с передачей в shell, что позволяет устанавливать, обновлять и выбирать версии Manticore одной командой bootstrap.
* 🆕 [v28.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.0) [ Issue #683](https://github.com/manticoresoftware/manticoresearch-buddy/issues/683) [ PR #685](https://github.com/manticoresoftware/manticoresearch-buddy/pull/685) Buddy обновлен до 4.1.0, добавлена поддержка HTTP JSON для [conversational search](Searching/Conversational_search.md), поэтому запросы к чату можно отправлять через JSON API `/search` в дополнение к SQL `CALL CHAT`.
* 🆕 [v27.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.4.0) [ PR #169](https://github.com/manticoresoftware/columnar/pull/169) [MCL](https://github.com/manticoresoftware/columnar) обновлен до 13.7.0, добавлена настройка [embeddings_threads](Server_settings/Searchd.md#embeddings_threads) для ограничения числа CPU-потоков, используемых при автогенерации эмбеддингов для вставок, `ALTER TABLE ... REBUILD KNN` и текстово-векторных KNN-запросов.
* 🆕 [v27.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.3.0) [ PR #4624](https://github.com/manticoresoftware/manticoresearch/pull/4624) Добавлена поддержка корзин фасетов с нулевым количеством для [faceted search](Searching/Faceted_search.md), включая SQL `ZEROES` и JSON `"zeroes": true`, чтобы фасеты в режиме `max` могли сохранять видимые корзины даже тогда, когда их текущий отфильтрованный счет равен `0`.
* 🆕 [v27.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.2.0) [ PR #4608](https://github.com/manticoresoftware/manticoresearch/pull/4608) Добавлено [CREATE TABLE ... profile='relevance'](Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#profile), а также сохранены значения по умолчанию на уровне таблицы для [ranker](Searching/Options.md#ranker) и [boolean_mode](Searching/Options.md#boolean_mode), что позволяет задавать таблицам встроенное ранжирование полнотекстового поиска и неявное поведение объединения ключевых слов без повторения параметров запроса.
...
* ⚠️ **[v28.4.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.4.1)** [ Issue #4720](https://github.com/manticoresoftware/manticoresearch/issues/4720) [ PR #4712](https://github.com/manticoresoftware/manticoresearch/pull/4712) **Условие для ломающего изменения**: в кластере репликации с включенной аутентификацией, чьё `manticore.json` было сохранено до v28.4.1, не хранится пользователь репликации. После обновления Manticore не может автоматически восстановить такой кластер и оставляет сохранённые метаданные без изменений. Остановите все узлы кластера и сделайте резервную копию каталога данных. На каждом узле удалите из `manticore.json` только затронутое описание кластера; оставьте записи таблиц и каталоги таблиц. Запустите узлы, заново создайте кластер и присоедините к нему узлы с нужной учётной записью репликации, затем подключите существующие таблицы с помощью `ALTER CLUSTER ... ADD`. Не удаляйте каталоги таблиц. Кластеры, сохранённые в v28.4.1 и новее, содержат пользователя репликации и этой проблемой не затронуты.
...
* 🪲 [v28.3.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.7) [ Issue #4697](https://github.com/manticoresoftware/manticoresearch/issues/4697) [ PR #4698](https://github.com/manticoresoftware/manticoresearch/pull/4698) Исправлена очистка после прерванного слияния columnar/KNN, чтобы временные файлы компонентов в отслеживаемом префиксе temp удалялись корректно, предотвращая появление осиротевших файлов `.tmp.spc.*`, которые могли ломать последующие операции переименования, подключения или удаления таблиц.
* 🪲 [v28.3.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.6) [ PR #4695](https://github.com/manticoresoftware/manticoresearch/pull/4695) Улучшена совместимость с [DBeaver](Integration/DBeaver.md) за счет поддержки простых псевдонимов одной таблицы в запросах `SELECT`, поэтому стандартные запросы DBeaver для просмотра таблиц больше не завершаются ошибкой.
* 🪲 [v28.3.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.5) [ Issue #4682](https://github.com/manticoresoftware/manticoresearch/issues/4682) [ PR #4683](https://github.com/manticoresoftware/manticoresearch/pull/4683) Исправлен встроенный украинский лемматизатор, чтобы слова с апострофом нормализовались корректно, благодаря чему формы вроде `здоров'ям` совпадают с `здоров'я` при [lemmatize_uk_all](Creating_a_table/NLP_and_tokenization/Supported_languages.md).
* 🪲 [v28.3.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.4) [ Issue #4691](https://github.com/manticoresoftware/manticoresearch/issues/4691) [ PR #4696](https://github.com/manticoresoftware/manticoresearch/pull/4696) Исправлена совместимость запуска MySQL-протокола под аутентификацией, чтобы клиенты Connector/J и PyMySQL могли завершать вход по native password, а безвредные `SET`-операторы сеанса больше не завершались ошибкой при auth.
* 🪲 [v28.3.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.3) [ Issue #4641](https://github.com/manticoresoftware/manticoresearch/issues/4641) [ PR #4649](https://github.com/manticoresoftware/manticoresearch/pull/4649) Исправлена обработка смешанных ключевых слов AOT, чтобы учитывать настроенный [blend_mode](Searching/Options.md#expand_blended), последовательно восстанавливая варианты без разделителей для индексации и извлечения ключевых слов.
* 🪲 [v28.3.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.2) [ PR #186](https://github.com/manticoresoftware/columnar/pull/186) [MCL](https://github.com/manticoresoftware/columnar) обновлен с явным исправлением сборки `DOCS_PER_CHUNK`, что предотвращает ошибки сборки и открытия встроенной библиотеки KNN в новом workflow на основе submodule для columnar.
* 🪲 [v28.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.3.1) [ Issue #4432](https://github.com/manticoresoftware/manticoresearch/issues/4432) [ PR #4689](https://github.com/manticoresoftware/manticoresearch/pull/4689) Исправлен запуск репликационного кластера, когда в кластере оставался только один узел, поэтому оставшийся единственный узел может корректно восстановиться вместо того, чтобы не запускаться.
* 🪲 [v28.1.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.5) [ PR #4687](https://github.com/manticoresoftware/manticoresearch/pull/4687) Исправлено получение хранимых полей в распределенных запросах: теперь удаленные ответы `GETFIELD` проверяются перед использованием, что предотвращает доверие к некорректным ответам агентов.
* 🪲 [v28.1.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.4) [ PR #4676](https://github.com/manticoresoftware/manticoresearch/pull/4676) Исправлен перенесенный workflow сборки и упаковки submodule MCL, восстановлена корректная обработка CI, пакетов и разрешения зависимостей для встроенной библиотеки columnar после перехода на submodule со стороны демона.
* 🪲 [v28.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.3) [ PR #4663](https://github.com/manticoresoftware/manticoresearch/pull/4663) Исправлено несколько утечек памяти и ошибок очистки в обработке загрузки необязательных библиотек, SSL-ключей, очистке словаря RT, завершении timer-thread и путях агрегации facet-zeroes.
* 🪲 [v28.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.2) [ Issue #4672](https://github.com/manticoresoftware/manticoresearch/issues/4672) [ PR #4679](https://github.com/manticoresoftware/manticoresearch/pull/4679) Исправлен сбой при использовании агрегатов [percentiles](Searching/Grouping.md) вместе с термовыми агрегатами в одном запросе `/search` на RT-таблицах с несколькими чанками.
* 🪲 [v28.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.1.1) [ PR #4669](https://github.com/manticoresoftware/manticoresearch/pull/4669) Исправлена автооптимизация RT, чтобы [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) больше не сжимал таблицу ниже 2 дисковых чанков, если такой нижний порог не запрошен явно.
* 🪲 [v28.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.2) [ Issue #4632](https://github.com/manticoresoftware/manticoresearch/issues/4632) [ PR #4673](https://github.com/manticoresoftware/manticoresearch/pull/4673) Исправлено поведение распределенных запросов с удаленными хранимыми полями: теперь они завершаются ошибкой при сбое выборки удаленного `GETFIELD`, вместо того чтобы возвращать успешные строки с молча пустыми значениями хранимых полей.
* 🪲 [v28.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/28.0.1) [ Issue #4615](https://github.com/manticoresoftware/manticoresearch/issues/4615) [ PR #4631](https://github.com/manticoresoftware/manticoresearch/pull/4631) Исправлены длинные ошибки SQL-парсинга: теперь они сохраняют границы UTF-8-символов, поэтому некорректные запросы с кириллицей и другим многобайтовым текстом больше не приводят к обрезанным или поврежденным сообщениям об ошибке.
...
* 🆕 [v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0) [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) Добавлены [аутентификация и авторизация](Security/Authentication_and_authorization.md) для MySQL, HTTP/HTTPS, удалённых агентов распределения и операций, связанных с репликацией, включая пользователей, bearer-токены, тонкие права, команды bootstrap и обработку пользователей кластера с учётом аутентификации.
* 🆕 [v26.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.0) [ PR #4606](https://github.com/manticoresoftware/manticoresearch/pull/4606) Обновлен Buddy до 3.47.0, добавлен соответствующий откат шардинга, а также доработки, тесты и исправления, необходимые для новой функциональности шардинга.
* 🆕 [v26.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.1.0) [ PR #4537](https://github.com/manticoresoftware/manticoresearch/pull/4537) Добавлены [нативные таблицы `type='sharding'`](Creating_a_table/Creating_a_sharded_table/Creating_a_sharded_table.md) с распределенными вставками, а также крупные улучшения шардирования, охватывающие создание, маршрутизацию, откат, failover и эксплуатационное поведение.
* 🆕 **26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) Переработано хранилище репликации, чтобы входящие реплицируемые таблицы использовали обычную схему [data_dir](Server_settings/Searchd.md#data_dir)/`<table>` вместо путей, специфичных для кластера, что выравнивает хранение реплицируемых таблиц с обычным локальным размещением таблиц.
* 🆕 [v25.15.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.15.0) [ PR #4590](https://github.com/manticoresoftware/manticoresearch/pull/4590) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.5.0, добавлена поддержка библиотеки, необходимая для параллельной сборки HNSW.
...
* 🆕 [v25.11.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.0) [ PR #4558](https://github.com/manticoresoftware/manticoresearch/pull/4558) В Manticore Search добавлен [conversational search](Searching/Conversational_search.md) через [CREATE CHAT MODEL](Searching/Conversational_search.md) и [CALL CHAT](Searching/Conversational_search.md), поэтому существующие векторизованные таблицы могут отвечать на вопросы с помощью KNN-поиска, истории диалога и ответов на базе LLM.
...
* 🆕 [v25.7.18](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.18) [ Issue #4461](https://github.com/manticoresoftware/manticoresearch/issues/4461) Добавлена поддержка `time_zone` и `offset` в [date_histogram](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29), что делает возможным календарное группирование с учётом часового пояса в агрегирующих запросах.
* 🆕 [v25.7.16](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.16) [ PR #4503](https://github.com/manticoresoftware/manticoresearch/pull/4503) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.2.5, улучшена производительность ONNX-эмбеддингов.
* 🆕 [v25.7.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.0) [ Issue #3280](https://github.com/manticoresoftware/manticoresearch/issues/3280) Добавлены статистические агрегации в стиле Elasticsearch, включая `percentiles`, `percentile_ranks` и `mad`, что позволяет рассчитывать распределения перцентилей и медианное абсолютное отклонение прямо в агрегатных запросах.
* 🆕 [v25.6.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.0) [ PR #4407](https://github.com/manticoresoftware/manticoresearch/pull/4407) [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29) в ранкере `expr` теперь принимает необязательный третий аргумент `avgdl`, позволяя запросам переопределять среднюю длину документа, используемую в оценке BM25A.
* 🆕 [v25.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.4.0) [ Issue #4060](https://github.com/manticoresoftware/manticoresearch/issues/4060) Добавлены уведомления состояния systemd, а пакетный сервис Linux переведен на `Type=notify`, что улучшает обработку запуска, перезагрузки и остановки, а также предотвращает ошибочное восприятие долгой остановки как зависшего демона.
* 🆕 [v25.3.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.3.0) [ PR #4326](https://github.com/manticoresoftware/manticoresearch/pull/4326) Добавлена явная поддержка префиксации колонок левой таблицы в запросах `JOIN`, что делает условия и фильтры JOIN более согласованными при использовании имен колонок с указанием таблицы.
* 🆕 [v25.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.0) [ PR #146](https://github.com/manticoresoftware/columnar/pull/146) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.2.2, добавлена локальная поддержка ONNX-эмбеддингов и улучшена производительность векторного поиска за счет более быстрого вывода эмбеддингов и расчетов расстояний HNSW, включая оптимизации AVX-512 там, где они поддерживаются.
* 🆕 [v25.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.0) [ PR #4454](https://github.com/manticoresoftware/manticoresearch/pull/4454) Улучшена производительность KNN за счет оптимизации вычисления расстояний и загрузки колонарной библиотеки AVX-512 там, где это поддерживается, что снижает накладные расходы в нагрузках векторного поиска.
* 🆕 [v25.0.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.5) [ Issue #1514](https://github.com/manticoresoftware/manticoresearch/issues/1514) Добавлена команда [EXIT CLUSTER](Creating_a_cluster/Setting_up_replication/Managing_replication_nodes.md#EXIT-CLUSTER), позволяющая текущему узлу онлайн отсоединиться от репликационного кластера без перезапуска демона, сохранив его локальные таблицы как обычные локальные таблицы.

## Ломающие изменения
* ⚠️ **[v27.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.0.0)** [ Issue #2833](https://github.com/manticoresoftware/manticoresearch/issues/2833) [ PR #3648](https://github.com/manticoresoftware/manticoresearch/pull/3648) **Ломающие изменения**: [аутентификация и авторизация](Security/Authentication_and_authorization.md) по умолчанию не включены, но если вы их включите, анонимный доступ клиентов перестанет работать. Обновление разворачивайте поэтапно: сначала обновите удаленные агенты и узлы репликации, затем обновите мастера, которые их запрашивают или ими управляют, и включайте auth только после того, как вся топология перейдет на новую версию. Новый мастер может не пройти согласование со старым агентом из-за изменений протокола, тогда как старый мастер сможет читать данные с нового агента только до включения auth. На участвующих демонах также должны совпадать сохраненные данные аутентификации для соответствующих пользователей, а успешный `JOIN CLUSTER` заменяет локальные данные auth присоединяющегося узла данными auth кластера-донора. Существующие данные таблиц остаются совместимыми, но миграция требует начальной настройки пользователей, выдачи прав, синхронизации данных auth между узлами и пересмотра процедур присоединения к кластеру. Понижение версии безопасно только до включения auth и до того, как какое-либо присоединение к кластеру заменит локальные данные auth.
* ⚠️ **v26.0.0** [ Issue #4431](https://github.com/manticoresoftware/manticoresearch/issues/4431) [ PR #4598](https://github.com/manticoresoftware/manticoresearch/pull/4598) **Ломающее изменение**: кластеры репликации, которые полагались на хранение входящих реплицируемых таблиц под кластерным `path`, больше не работают так же, потому что полученные таблицы теперь находятся по стандартной схеме [data_dir](Server_settings/Searchd.md#data_dir)/`<table>`. Существующие данные таблиц по-прежнему пригодны к использованию, но если вы используете репликацию с собственным кластерным `path`, для миграции может потребоваться переместить или заново синхронизировать реплицируемые таблицы в стандартную схему `data_dir`. Даунгрейд безопасен только до перехода на новую схему реплицируемых таблиц.

### Исправления ошибок
* 🪲 [v27.1.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.5) [ PR #4648](https://github.com/manticoresoftware/manticoresearch/pull/4648) Исправлен сбой при чтении столбцовых атрибутов `float_vector`, что предотвращает ошибки при материализации векторных столбцов из столбцовых таблиц.
* 🪲 [v27.1.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.4) [ Issue #4655](https://github.com/manticoresoftware/manticoresearch/issues/4655) [ PR #4657](https://github.com/manticoresoftware/manticoresearch/pull/4657) Исправлены `ALTER TABLE ... RECONFIGURE` и `SHOW CREATE TABLE` для таблиц, переведенных с `dict='keywords'` на [dict=keywords_32k](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#dict), поэтому однонаправленное обновление словаря теперь работает и корректно отражается в определении таблицы.
* 🪲 [v27.1.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.3) [ PR #4652](https://github.com/manticoresoftware/manticoresearch/pull/4652) Обновлен [Buddy](Installation/Manticore_Buddy.md#Manticore-Buddy) до 4.0.1, что исправляет обработку прав на изменения через Queue-плагин при аутентификации: теперь права только на чтение для `source/<name>` и `mva/<name>` больше не позволяют побочные эффекты вроде `DROP SOURCE` или `ALTER MATERIALIZED VIEW`.
* 🪲 [v27.1.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.2) [ PR #4600](https://github.com/manticoresoftware/manticoresearch/pull/4600) Исправлен сбор MVA из запроса `sql_attr_multi` для обычных индексов, собранных из нескольких блоков `source`, поэтому многоисточниковые индексы больше не падают при индексировании с ошибкой null.
* 🪲 [v27.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/27.1.0) [ PR #4635](https://github.com/manticoresoftware/manticoresearch/pull/4635) Обновлён [MCL](https://github.com/manticoresoftware/columnar) до 13.6.1, что предотвращает падение библиотеки эмбеддингов на эмулируемых процессорах x86.
* 🪲 [v26.3.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.3.2) [ PR #4620](https://github.com/manticoresoftware/manticoresearch/pull/4620) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.5.3, добавлено исправление несоответствия типов в quantizer из поставляемой колонарной библиотеки.
* 🪲 [v26.3.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.3.1) Откатан фасет `zeroes` из `26.3.0`, удалена ошибочно выпущенная поддержка SQL/JSON-ведра с нулевым количеством.
* 🪲 [v26.2.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.4) [ Issue #4320](https://github.com/manticoresoftware/manticoresearch/issues/4320) Исправлен порядок результатов KNN после rescoring, теперь явные тай-брейки `ORDER BY` применяются корректно при равных расстояниях после пересчета.
* 🪲 [v26.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.3) [ PR #4614](https://github.com/manticoresoftware/manticoresearch/pull/4614) Обновлен Buddy до 3.47.2, исправлено несколько путей падения при репликации и повторном присоединении узла, включая некорректную обработку индекса таблицы и блокировку снимка репликации.
* 🪲 [v26.2.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.2) [ Issue #4603](https://github.com/manticoresoftware/manticoresearch/issues/4603) Исправлен crash в гибридных fused-запросах с `GROUP BY` на таблицах, использующих columnar-атрибуты.
* 🪲 [v26.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.2.1) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) Обновлен Buddy до 3.47.1, сохранение `offset` и `max_matches` для HTTP KNN-поиска по `doc_id` теперь работает правильно.
* 🪲 [v26.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.3) [ PR #4604](https://github.com/manticoresoftware/manticoresearch/pull/4604) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.5.2, добавлены исправления стабильности, связанные со стеком, для поставляемой колонарной библиотеки.
* 🪲 [v26.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.2) [ Issue #4534](https://github.com/manticoresoftware/manticoresearch/issues/4534) Исправлен учет `max_matches` в KNN-поиске, скорректирована обработка окна результатов, из-за которой могло возвращаться неверное фиксированное число хитов.
* 🪲 [v26.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/26.0.1) [ PR #4592](https://github.com/manticoresoftware/manticoresearch/pull/4592) Исправлен размер стека в debug-сборках, чтобы предотвратить повреждение памяти из-за слишком больших кадров стека у coroutine и thread.
* 🪲 [v25.13.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.5) [ Issue #4489](https://github.com/manticoresoftware/manticoresearch/issues/4489) Исправлен crash indexer, вызванный пропусками MVA/docstore при построении обычных индексов с columnar-атрибутами.
* 🪲 [v25.13.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.4) [ PR #4583](https://github.com/manticoresoftware/manticoresearch/pull/4583) Исправлено поведение статуса корзин `facet_filter_mode` в режимах `auto` и `max`, чтобы выбранные, доступные и недоступные фасетные корзины соответствовали ожидаемому UX фасетного поиска.
* 🪲 [v25.13.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.3) [ Issue #4578](https://github.com/manticoresoftware/manticoresearch/issues/4578) Исправлены падения RT `.@files` под нагрузкой на запись за счет переработки внутреннего потока данных динамических таблиц, что предотвращает сбои, связанные с синхронизацией, во время внутренних запросов к списку файлов.
* 🪲 [v25.13.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.2) [ Issue #660](https://github.com/manticoresoftware/manticoresearch-buddy/issues/660) Обновлен Buddy до 3.46.3, исправлена обработка self-hit для KNN по `doc_id` и восстановлена совместимость проверки шаблона Logstash 7.17.
* 🪲 [v25.13.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.13.1) [ Issue #168](https://github.com/manticoresoftware/columnar/issues/168) Исправлена загрузка модуля AVX-512 в Windows, что предотвращает выбор поставляемых DLL [MCL](https://github.com/manticoresoftware/columnar) AVX-512 на неподдерживаемых системах и падение демона или indexer.
* 🪲 [v25.12.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.12.1) [ PR #4563](https://github.com/manticoresoftware/manticoresearch/pull/4563) Исправлена поддержка двоичного протокола MySQL, включая специальные колонки, `null` и типизированные числовые значения, так что подготовленные запросы и запросы по binary protocol снова работают корректно.
* 🪲 [v25.11.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.2) [ PR #4566](https://github.com/manticoresoftware/manticoresearch/pull/4566) Исправлена совместимость массовой загрузки Fluent Bit за счет нормализации формата заголовка `Content-Length`, чтобы успешные ответы bulk не воспроизводились как дублирующие вставки.
* 🪲 [v25.11.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.11.1) [ Issue #4560](https://github.com/manticoresoftware/manticoresearch/issues/4560) Обновлен Buddy до 3.46.1, исправлена обработка типизированного `COUNT(*)` для `information_schema.tables` и восстановлено ожидаемое поведение интеграции.
* 🪲 [v25.9.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.9.1) [ Issue #4464](https://github.com/manticoresoftware/manticoresearch/issues/4464) Исправлены подготовленные выражения, которым требуется обработка Buddy, включая нечеткие запросы `MATCH(?)`, а также скорректирована строгая обработка типизированных результатов для `bool`, `multi` и JSON-значений, возвращаемых Buddy.
* 🪲 [v25.8.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.5) [ PR #4554](https://github.com/manticoresoftware/manticoresearch/pull/4554) Исправлены упаковка и CI для языковых пакетов AOT, чтобы Docker-образы включали все пакеты, а покрытие тестами lemmatizer/AOT больше не пропускалось.
* 🪲 [v25.8.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.4) [ PR #4553](https://github.com/manticoresoftware/manticoresearch/pull/4553) Исправлены загрузка украинского лемматизатора и генерация исходной формы, что усилило новую встроенную поддержку украинской морфологии.
* 🪲 [v25.8.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.3) [ Issue #4490](https://github.com/manticoresoftware/manticoresearch/issues/4490) Исправлено стеммирование смешанных токенов на арабском в `CALL KEYWORDS`, чтобы некоторые короткие арабско-числовые входные данные больше не завершались общей ошибкой `Something went wrong`.
* 🪲 [v25.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.2) [ Issue #4509](https://github.com/manticoresoftware/manticoresearch/issues/4509) Исправлен crash репликации при создании кластера с длинным путем `data_dir`, скорректирована обработка cluster-binlog для длинных имен каталогов.
* 🪲 [v25.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.8.1) [ PR #4535](https://github.com/manticoresoftware/manticoresearch/pull/4535) Исправлен вывод гибридного `knn_dist`, чтобы sentinel `FLT_MAX` больше не сериализовался как ложное расстояние в JSON-результатах.
* 🪲 [v25.7.21](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.21) [ Issue #4525](https://github.com/manticoresoftware/manticoresearch/issues/4525) Исправлена утечка памяти в обработке scroll за счет корректного освобождения переназначенных строковых атрибутов.
* 🪲 [v25.7.20](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.20) [ Issue #4466](https://github.com/manticoresoftware/manticoresearch/issues/4466) Исправлена агрегация JSON-массива, когда она является единственной агрегацией в запросе: теперь элементы массива попадают в отдельные корзины, а не повторяется весь массив для каждого ключа.
* 🪲 [v25.7.19](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.19) [ PR #4533](https://github.com/manticoresoftware/manticoresearch/pull/4533) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 13.2.6, исправлен crash, который мог возникать при выполнении многопоточных KNN-запросов с 1-битно квантизированными векторами.
* 🪲 [v25.7.17](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.17) [ PR #4529](https://github.com/manticoresoftware/manticoresearch/pull/4529) Включено несколько мелких исправлений и очисток, включая ещё одно исправление корректности [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) и небольшие улучшения runtime/build.
* 🪲 [v25.7.15](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.15) [ PR #4526](https://github.com/manticoresoftware/manticoresearch/pull/4526) Исправлена обработка kill-list в режиме columnar для [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) за счет корректного чтения docid columnar, что завершает недавние исправления attach для конфигураций [MCL](https://github.com/manticoresoftware/columnar)/columnar.
* 🪲 [v25.7.14](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.14) [ Issue #4459](https://github.com/manticoresoftware/manticoresearch/issues/4459) Исправлен разбор интервалов [DATE_HISTOGRAM()](Functions/Date_and_time_functions.md#DATE_HISTOGRAM%28%29) в аргументах агрегации, поэтому значения `fixed_interval` вроде `1h`, `2m` и подобных единиц теперь работают корректно вместо того, чтобы обрабатываться как сырые секунды.
* 🪲 [v25.7.13](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.13) [ Issue #4522](https://github.com/manticoresoftware/manticoresearch/issues/4522) Исправлен лишний нулевой символ в выводе sorter/grouping для JOIN, поэтому сгруппированные и фасетированные значения JOIN больше не содержат поврежденные завершающие байты `\0`.
* 🪲 [v25.7.12](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.12) [ Issue #4516](https://github.com/manticoresoftware/manticoresearch/issues/4516) Исправлены счётчики документов для целевой таблицы после [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), так что `indexed_documents` и связанные метаданные теперь отражают фактическое число строк после attach.
* 🪲 [v25.7.11](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.11) [ Issue #4515](https://github.com/manticoresoftware/manticoresearch/issues/4515) Исправлена обработка уже убитых документов в [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), поэтому дублирующиеся ID в целевой таблице больше не удаляются некорректно во время attach.
* 🪲 [v25.7.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.10) [ Issue #4514](https://github.com/manticoresoftware/manticoresearch/issues/4514) Исправлен `indexed_documents` для исходной таблицы после [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), так что число документов теперь корректно уменьшается, когда disk chunk переносится в другую таблицу.
* 🪲 [v25.7.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.9) [ Issue #4513](https://github.com/manticoresoftware/manticoresearch/issues/4513) Исправлены метаданные для присоединённых индексов, поэтому таблицы, созданные через [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md), сохраняют корректное состояние chunk и больше не ломаются после перезапуска.
* 🪲 [v25.7.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.8) [ Issue #4512](https://github.com/manticoresoftware/manticoresearch/issues/4512) Исправлены метаданные `indexed_documents` при запуске после грязного перезапуска, поэтому RT-таблицы больше не сообщают завышенное число документов при восстановлении удаленных строк.
* 🪲 [v25.7.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.7) [ Issue #159](https://github.com/manticoresoftware/columnar/issues/159) Исправлены диагностические сообщения KNN при запуске, когда embeddings-библиотека отсутствует или настроена неверно, чтобы Manticore больше не показывал вводящее в заблуждение предупреждение/ошибку `(null)` во время инициализации.
* 🪲 [v25.7.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.6) [ Issue #4460](https://github.com/manticoresoftware/manticoresearch/issues/4460) Исправлена обработка репликации, когда [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) отключает вторичные индексы, поэтому предупреждение больше не превращается в ошибку, ломающую кластер, в реплицируемых конфигурациях.
* 🪲 [v25.7.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.5) [ Issue #4345](https://github.com/manticoresoftware/manticoresearch/issues/4345) Исправлен fallback JSON-фильтра, когда вторичные индексы не создают итераторы, чтобы деревья составных фильтров сохраняли нужные фильтры вместо возврата неверных результатов.
* 🪲 [v25.7.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.4) [ PR #662](https://github.com/manticoresoftware/manticoresearch-buddy/pull/662) Обновлен Buddy до 3.44.5, включено исправление безопасности Composer.
* 🪲 [v25.7.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.3) [ Issue #4404](https://github.com/manticoresoftware/manticoresearch/issues/4404) Исправлен [гибридный поиск](Searching/Hybrid_search.md) с `scroll`, поэтому пагинация на основе scroll теперь работает корректно, а не падает и не ломается при использовании [гибридного поиска](Searching/Hybrid_search.md).
* 🪲 [v25.7.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.2) [ Issue #4491](https://github.com/manticoresoftware/manticoresearch/issues/4491) Исправлено состояние вторичного индекса после переименования RT chunk и [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), поэтому индексы, отключённые [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md), остаются отключёнными после перезапуска демона вместо того, чтобы непреднамеренно включаться снова.
* 🪲 [v25.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.7.1) [ PR #4502](https://github.com/manticoresoftware/manticoresearch/pull/4502) Исправлена фильтрация по `knn_dist()` и `hybrid_score()` в [гибридном поиске](Searching/Hybrid_search.md), чтобы запросы могли корректно применять условия к расстоянию до вектора и оценкам гибридного ранжирования.
* 🪲 [v25.6.9](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.9) [ Issue #4473](https://github.com/manticoresoftware/manticoresearch/issues/4473) Исправлен разбор HTTP JSON JOIN для имен таблиц в смешанном регистре за счет приведения ссылок на таблицы в `join.on` к каноническому виду, поэтому JOIN больше не ломается только из-за заглавных букв в именах таблиц.
* 🪲 [v25.6.8](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.8) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) Исправлены оставшиеся падения expr-ranker на columnar-таблицах со смешанной обработкой длины полей, завершив исправление [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29)/[bm25f()](Functions/Searching_and_ranking_functions.md#BM25F%28%29) для сценариев [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)='1'.
* 🪲 [v25.6.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.7) [ Issue #4484](https://github.com/manticoresoftware/manticoresearch/issues/4484) Исправлена обработка template-таблиц в пути метаданных MySQL `COM_FIELD_LIST`, чтобы клиенты не вызывали падения при запросе метаданных полей для template-таблиц.
* 🪲 [v25.6.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.6) [ Issue #4497](https://github.com/manticoresoftware/manticoresearch/issues/4497) Исправлены падения в пути метаданных для template-таблиц, включая `SHOW FIELDS` и связанную обработку списков полей, которые раньше могли приводить к падению демона.
* 🪲 [v25.6.5](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.5) [ Issue #2](https://github.com/manticoresoftware/telemetry-lib/issues/2) Обновлен Buddy до 3.44.4, исправлена доставка telemetry в PHP 8.4, чтобы снимки telemetry успешно отправлялись, а не падали во время HTTP POST-запроса.
* 🪲 [v25.6.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.4) [ Issue #4469](https://github.com/manticoresoftware/manticoresearch/issues/4469) Исправлен [ALTER TABLE ... REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) после того, как [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) объединяет RT disk chunks, поэтому файлы вторичного индекса остаются синхронизированными с переименованными оптимизированными chunks и больше не завершаются ошибкой `secondary index file ... not loaded`.
* 🪲 [v25.6.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.3) [ Issue #4418](https://github.com/manticoresoftware/manticoresearch/issues/4418) Исправлены падения и связанные проблемы обработки JOIN в [гибридном поиске](Searching/Hybrid_search.md), включая запросы `LEFT JOIN`, которые раньше могли завершаться ошибкой при объединении результатов полнотекстового и KNN-поиска.
* 🪲 [v25.6.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.2) [ PR #658](https://github.com/manticoresoftware/manticoresearch-buddy/pull/658) Обновлен Buddy до 3.44.3, исправлена телеметрия объема, когда `binlog_path` пуст, чтобы `volume_size_bytes` больше не сообщался ошибочно как ноль.
* 🪲 [v25.6.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.6.1) [ Issue #4476](https://github.com/manticoresoftware/manticoresearch/issues/4476) Исправлен запуск под systemd: `pid_file` больше не требуется, когда демоном управляет systemd, который уже знает и контролирует PID процесса.
* 🪲 [v25.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.1) [ Issue #4444](https://github.com/manticoresoftware/manticoresearch/issues/4444) Исправлено падение expr-ranker при использовании [bm25a()](Functions/Searching_and_ranking_functions.md#BM25A%28%29) или [bm25f()](Functions/Searching_and_ranking_functions.md#BM25F%28%29) на columnar-таблицах с [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)='1', так что пользовательское ранжирование больше не разыменовывает атрибуты длины поля из неправильного хранилища.
* 🪲 [v25.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.5.0) [ Issue #4007](https://github.com/manticoresoftware/manticoresearch/issues/4007) Исправлена непоследовательная обработка имен таблиц в смешанном регистре в SphinxQL, HTTP и запросах `JOIN`, чтобы имена таблиц нормализовались одинаково вместо сбоев или разного поведения при использовании заглавных букв.
* 🪲 [v25.2.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.2.1) [ Issue #4428](https://github.com/manticoresoftware/manticoresearch/issues/4428) Исправлен crash при использовании `GROUP BY` в запросах [гибридного поиска](Searching/Hybrid_search.md), включая запросы, объединяющие полнотекстовый и KNN-поиск.
* 🪲 [v25.1.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.1.1) [ Issue #4455](https://github.com/manticoresoftware/manticoresearch/issues/4455) Откатено недавнее изменение, которое превращало отсутствие библиотеки `knn_embeddings` в явную ошибку инициализации KNN, восстановлено прежнее поведение для конфигураций, где эта необязательная embeddings-библиотека отсутствует.
* 🪲 [v25.0.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.6) [ Issue #4437](https://github.com/manticoresoftware/manticoresearch/issues/4437) Исправлена обработка повторного присоединения к кластеру после `ALTER CLUSTER ... DROP`, чтобы узел, который был офлайн во время удаления, больше не сохранял устаревшие локальные метаданные кластерной таблицы, из-за которых удаленная таблица застревала и становилась невозможной для удаления после повторного присоединения.
* 🪲 [v25.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.4) [ PR #4405](https://github.com/manticoresoftware/manticoresearch/pull/4405) Исправлен рейтинг BM25A для стеммированных или иным образом нормализованных терминов запроса, так что эквивалентные запросы вроде `looking` и `look` теперь используют одни и те же локальные/глобальные статистики IDF вместо разных оценок релевантности.
* 🪲 [v25.0.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.3) [ PR #85](https://github.com/manticoresoftware/executor/pull/85) Обновлен Executor до 1.4.3, упрощен его Docker-пайплайн упаковки за счет использования заранее собранных артефактов для конкретных архитектур вместо сборки бинарников внутри image pipeline.
* 🪲 [v25.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.2) [ Issue #4395](https://github.com/manticoresoftware/manticoresearch/issues/4395) Исправлен [OPTIMIZE TABLE ... OPTION sync=1, cutoff=N](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), когда автооптимизация уже выполняется, поэтому ручной запрос на оптимизацию теперь ждёт достижения запрошенной цели по сокращению чанков, а не привязывается к другой уже идущей оптимизации.
* 🪲 [v25.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.1) [ Issue #4398](https://github.com/manticoresoftware/manticoresearch/issues/4398) Исправлена статистика времени выполнения запросов в демоне: глобальные счетчики и время по таблицам теперь корректно отображаются для HTTP `/search`, распределенных запросов обновления и API-запросов поиска, перенаправленных с master на agent.

## Версия 25.0.0
**Выпуск**: 30 марта 2026 года

Этот выпуск приносит более масштабную переработку упаковки, новые параметры `API_URL` и `API_TIMEOUT` для моделей автогенерации эмбеддингов, [hybrid search](Searching/Hybrid_search.md), поддержку резервного копирования и восстановления, совместимых с S3, более быстрое обслуживание RT-таблиц с N-way merge и параллельным [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), предварительную фильтрацию KNN и подготовленные выражения, совместимые с MySQL.

## Критические изменения
* ⚠️ [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #123](https://github.com/manticoresoftware/columnar/pull/123) **Ломающее изменение**: **[MCL](https://github.com/manticoresoftware/columnar)** обновлен до 13.0.0, добавлены [API_URL](Searching/KNN.md#Creating-a-table-with-auto-embeddings) и [API_TIMEOUT](Searching/KNN.md#Creating-a-table-with-auto-embeddings) для [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29). Это затрагивает вас, если вы используете [**auto-embedding models**](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) или управляете **[MCL](https://github.com/manticoresoftware/columnar)** отдельно от демона, потому что Manticore 25.0.0 требует **[MCL](https://github.com/manticoresoftware/columnar) 13.0.0** и несовместим со старыми версиями [MCL](https://github.com/manticoresoftware/columnar); обновляйте демон и [MCL](https://github.com/manticoresoftware/columnar) вместе, а понижение версии безопасно только если вы также восстановите соответствующую более старую версию [MCL](https://github.com/manticoresoftware/columnar).
...
* ⚠️ [v23.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.0) [ Issue #4364](https://github.com/manticoresoftware/manticoresearch/issues/4364) **Ломающее изменение**: вводится **версия формата индекса 69** для поддержки новых вариантов биграммной токенизации, включая режимы [bigram_delimiter](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_delimiter) и чувствительные к цифрам режимы [bigram_index](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#bigram_index). Это затрагивает вас, если вы используете эти биграммные параметры или если вы пересобираете индексы и позже нужно открыть пересобранные индексы более старой версией Manticore: существующие старые индексы остаются читаемыми, но индексы, пересобранные или записанные заново в формате 69, несовместимы со старыми версиями Manticore, поэтому понижение версии безопасно только до такой пересборки индексов.
...
* ⚠️ [v21.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.0.0) [ PR #138](https://github.com/manticoresoftware/columnar/pull/138) **Ломающие изменения**: обновлен **[MCL](https://github.com/manticoresoftware/columnar)** до 12.0.0 и повышена требуемая **KNN API** для поддержки адаптивного [**раннего завершения KNN**](Searching/KNN.md#Early-termination) во время обхода HNSW. Это затрагивает вас, если вы управляете **[MCL](https://github.com/manticoresoftware/columnar)** отдельно от демона или запускаете **KNN-запросы**, которые зависят от прежнего поведения поиска кандидатов или граничного выбора результатов: Manticore 21.0.0 требует **[MCL](https://github.com/manticoresoftware/columnar) 12.0.0** с новым KNN-интерфейсом, и с этой библиотекой KNN-поиск может завершаться раньше, когда найдено достаточно кандидатов, что может изменить то, какие кандидаты исследуются и возвращаются. Существующие данные остаются совместимыми, дополнительная миграция не требуется, но обновляйте демон и [MCL](https://github.com/manticoresoftware/columnar) вместе; откат безопасен только если вы также вернете подходящую старую версию [MCL](https://github.com/manticoresoftware/columnar).
...
* ⚠️ [v18.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/18.0.0) [ Issue #4261](https://github.com/manticoresoftware/manticoresearch/issues/4261) **Ломающее изменение**: вводится **версия формата индекса 68** для исправления обработки структуры словаря [hitless_words](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#hitless_words). Это затрагивает вас, если вы пересобираете или заново записываете индексы и позже нужно открыть пересобранные индексы более старой версией Manticore: существующие старые индексы остаются читаемыми, но любые индексы, пересобранные или записанные заново в новом формате, несовместимы со старыми версиями Manticore, поэтому понижение версии безопасно только до такой пересборки индексов.

### Изменения, связанные с упаковкой
* 🆕 [v25.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/25.0.0) [ PR #4357](https://github.com/manticoresoftware/manticoresearch/pull/4357) Изменение упаковки: `manticore` теперь является пакетом-сборкой для deb и rpm. Он включает демон, инструменты, конвертер, заголовки для разработки, данные ICU, входящие в комплект пакеты зависимостей (`manticore-columnar-lib`, `manticore-backup`, `manticore-buddy`, `manticore-executor`, `manticore-load`, `manticore-galera` и `manticore-tzdata`, где применимо), а также встроенные языковые пакеты для немецкого, английского и русского языков вместе с поддержкой Jieba. Украинский лемматизатор не входит в комплект; установите его отдельно. При обновлении с предыдущей структуры пакетов обычно можно просто установить пакет `manticore`. Если старые раздельные пакеты вызывают конфликты, удалите их с помощью `apt remove 'manticore*'` или `yum remove 'manticore*'`, а затем установите `manticore`. Эта очистка не удалит ваши существующие данные или конфигурацию.

## Новые возможности и улучшения
* 🆕 [v24.4.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.4.0) [ PR #4091](https://github.com/manticoresoftware/manticoresearch/pull/4091) Улучшена компакция RT-таблиц за счет N-way-слияния дисковых чанков и параллельных задач [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE), что сокращает время, необходимое для слияния большого числа дисковых чанков, и добавляет новые настройки [merge_chunks_per_job](Server_settings/Searchd.md#merge_chunks_per_job) и [parallel_chunk_merges](Server_settings/Searchd.md#parallel_chunk_merges).
...
* 🆕 [v24.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.1.0) [ PR #141](https://github.com/manticoresoftware/columnar/pull/141) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 12.4.1, добавлена поддержка локальных моделей эмбеддингов GGUF с квантизацией, моделей энкодера T5 и gated-моделей Hugging Face через загрузку с token-authentication.
...
* 🆕 [v21.1.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/21.1.0) [ PR #4352](https://github.com/manticoresoftware/columnar/pull/140) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 12.1.0, улучшена производительность раннего завершения KNN и добавлено более эффективное кодирование 64-битных значений.
* 🆕 [v19.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.2.0) [ PR #4306](https://github.com/manticoresoftware/manticoresearch/pull/4306) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 11.1.0, добавлена поддержка кеша кодеков.
...
* 🪲 [v24.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/24.0.1) [ Issue #4354](https://github.com/manticoresoftware/manticoresearch/issues/4354) Исправлена обработка [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md), когда измененный атрибут также должен отключить свой вторичный индекс, что предотвращает неверные предупреждения и несогласованное состояние вторичного индекса.
* 🪲 [v23.0.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/23.0.2) [ PR #4370](https://github.com/manticoresoftware/columnar/issues/125) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 12.1.1, исправлены падения при генерации автоэмбеддингов, когда текст содержит невалидный UTF-8, включая вставки в таблицы с `html_strip=1`.
...
* 🪲 [v19.0.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/19.0.1) [ Issue #4103](https://github.com/manticoresoftware/manticoresearch/issues/4103) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 11.0.0 для поддержки KNN-prefiltering.
...
* 🪲 [v17.5.10](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.10) [ Issue #4207](https://github.com/manticoresoftware/columnar/issues/133) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 10.2.2, исправлены падения при выполнении поиска во время вставки данных в таблицы с локальными моделями эмбеддингов за счет добавления защиты от гонок вокруг использования локальной модели.
...
* 🪲 [v17.5.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.4) [ Issue #1166](https://github.com/manticoresoftware/manticoresearch/issues/1166) Исправлен сбой при INSERT после `TRUNCATE TABLE ... WITH RECONFIGURE` для RT-таблиц с [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)=1, особенно когда поля и строковые атрибуты смешаны; атрибуты длины полей теперь корректно настраиваются во время reconfigure.
...
## Критические изменения
* ⚠️ [v17.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.0.0) [ Issue #4120](https://github.com/manticoresoftware/manticoresearch/issues/4120) [MCL](https://github.com/manticoresoftware/columnar) 10.0.0: добавлена поддержка `DROP CACHE`. Это обновляет интерфейс между демоном и [MCL](https://github.com/manticoresoftware/columnar). Старые версии Manticore Search больше не поддерживают новую версию [MCL](https://github.com/manticoresoftware/columnar).
...
## Новые функции и улучшения
* 🆕 [v17.5.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.5.0) [ PR #130](https://github.com/manticoresoftware/columnar/pull/130) [MCL](https://github.com/manticoresoftware/columnar) обновлен до 10.2.0: исправлена поддержка модели QWEN и добавлена поддержка дополнительных моделей.
...
* 🆕 [v17.2.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.0) [ PR #4195](https://github.com/manticoresoftware/manticoresearch/pull/4195) Обновлен [MCL](https://github.com/manticoresoftware/columnar) до 10.1.0; добавлена поддержка локальных моделей эмбеддингов Qwen.
...
* 🪲 [v17.2.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.6) [ Issue #3203](https://github.com/manticoresoftware/manticoresearch/issues/3203) Исправлен сбой [ALTER TABLE REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index) с ошибкой `failed to rename … .tmp.spjidx`, когда в таблице было несколько дисковых чанков.
...
* 🪲 [v17.2.3](https://github.com/manticoresoftware/manticoresearch/releases/tag/17.2.3) [ Issue #4176](https://github.com/manticoresoftware/manticoresearch/issues/4176) Исправлено нарушение работы переименования таблицы для таблиц, использующих внешние stopwords, wordforms или exceptions: [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) теперь переносит эти файлы в новый чанковый формат и обновляет заголовки дисковых чанков, поэтому после `ALTER TABLE RENAME` демон больше не сообщает об отсутствующих внешних файлах при перезапуске.
...
* 🪲 [v16.0.4](https://github.com/manticoresoftware/manticoresearch/releases/tag/16.0.4) [ PR #4047](https://github.com/manticoresoftware/manticoresearch/pull/4047) Обновлена версия [MCL](https://github.com/manticoresoftware/columnar) до 9.0.1. Исправлена обработка float-векторов KNN при `block_size` = 1 за счет отключения сжатия таблицы в этом случае (при размере блока 1 сжатие не используется).
...
## Критические изменения
* ⚠️ [v15.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/15.0.0) [ PR #4003](https://github.com/manticoresoftware/manticoresearch/pull/4003) Обновлено требование к [MCL](https://github.com/manticoresoftware/columnar) до 9.0.0, что принесло несжатое хранение float-векторов, изменение размера блока для KNN-векторов и небуферизованное чтение. Это обновление меняет формат данных. Старые версии [MCL](https://github.com/manticoresoftware/columnar) не смогут его читать, но новая версия по-прежнему без проблем читает ваши существующие columnar-таблицы.
...
* 🪲 [v14.7.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/14.7.1) [ PR #3992](https://github.com/manticoresoftware/manticoresearch/pull/3992) Исправлено отсутствие сообщений об ошибках для HTTP [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) и DELETE-запросов к распределенным таблицам, обрабатываемых через agent, так что клиенты теперь корректно получают ошибки при сбоях операций.
...
### Новые функции и улучшения
* 🆕 [v13.13.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.13.0) Добавлена поддержка [MCL](https://github.com/manticoresoftware/columnar) 8.1.0 с SI block cache.
...
* 🪲 [v13.11.6](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.11.6) [ PR #3766](https://github.com/manticoresoftware/manticoresearch/pull/3766) Добавлена поддержка версии [MCL](https://github.com/manticoresoftware/columnar) 8.0.6.
...
* 🪲 [v13.11.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.11.2) [ Issue #3065](https://github.com/manticoresoftware/manticoresearch/issues/3065) Исправлен crash при вставке данных в columnar-таблицу с включенным [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths).
...
* 🪲 [v13.5.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/13.5.1) [ Issue #3560](https://github.com/manticoresoftware/manticoresearch/issues/3560) Исправлен случай, когда [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) мог зависать бесконечно на данных KNN.
...
* ⚠️ [v12.0.0](https://github.com/manticoresoftware/manticoresearch/releases/tag/12.0.0) [ PR #3516](https://github.com/manticoresoftware/manticoresearch/pull/3516) Исправлена ошибка с некорректными ID исходной и целевой строк при обучении и построении KNN-индекса. Это обновление не меняет формат данных, но увеличивает версию API Manticore Search / [MCL](https://github.com/manticoresoftware/columnar).
...
* 🪲 [v10.2.7](https://github.com/manticoresoftware/manticoresearch/releases/tag/10.2.7)  Добавлена поддержка [MCL](https://github.com/manticoresoftware/columnar) 5.0.5, включая исправление имени файла embeddings-библиотеки
...
* 🪲 [v9.8.2](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.2)  Исправление: снова обновлен `deps.txt`, чтобы включить исправления упаковки в [MCL](https://github.com/manticoresoftware/columnar), связанные с embeddings-библиотекой
* 🪲 [v9.8.1](https://github.com/manticoresoftware/manticoresearch/releases/tag/9.8.1)  Исправление: `deps.txt` обновлен с исправлениями упаковки для [MCL](https://github.com/manticoresoftware/columnar) и embeddings-библиотеки
...
* [Issue #3189](https://github.com/manticoresoftware/manticoresearch/issues/3189) Исправлено: операторы [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) теперь корректно учитывают настройку `query_log_min_msec`.
...
* [Issue #3054](https://github.com/manticoresoftware/manticoresearch/issues/3054) Исправлена проблема с командой [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md).
...
* ⚠️ BREAKING [Issue #2308](https://github.com/manticoresoftware/manticoresearch/issues/2308) Добавлена поддержка нескольких таблиц в [ALTER CLUSTER ADD` и `DROP](Creating_a_cluster/Setting_up_replication/Adding_and_removing_a_table_from_a_replication_cluster.md#Adding-and-removing-a-table-from-a-replication-cluster). Это изменение также затрагивает протокол репликации. См. предыдущий раздел для рекомендаций по обработке этого обновления.
* [Issue #2997](https://github.com/manticoresoftware/manticoresearch/issues/2997) Исправлена проблема с dlopen на Macos.
* [Commit 4954](https://github.com/manticoresoftware/manticoresearch/commit/4954b5de7341a29902a9b8fbb9a040f7942c77c4) Изменен cutoff по умолчанию для [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) на таблицах с KNN-индексами, чтобы улучшить производительность поиска.
...
* [Issue #1818](https://github.com/manticoresoftware/manticoresearch/issues/1818) [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) теперь возвращает ошибку, когда таблица заморожена.
...
* [Issue #2265](https://github.com/manticoresoftware/manticoresearch/issues/2265) Исправлен сбой восстановления `mysqldump` при включенном [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths).
...
* [Issue #1849](https://github.com/manticoresoftware/manticoresearch/issues/1849) Исправлен crash indextool при завершении работы, если он использовался вместе с [MCL](https://github.com/manticoresoftware/columnar).
...
### Связано с [MCL](https://github.com/manticoresoftware/columnar) (columnar, secondary, knn libs)
...
### [MCL](https://github.com/manticoresoftware/columnar)
...
* [Commit 9dc1](https://github.com/manticoresoftware/manticoresearch/commit/9dc12334): При выполнении `[ALTER TABLE table REBUILD SECONDARY](Updating_table_schema_and_settings.md#Rebuilding-a-secondary-index)` вторичные индексы теперь всегда перестраиваются, даже если атрибуты не были обновлены.
...
* ⚠️ [Issue #1057](https://github.com/manticoresoftware/manticoresearch/issues/1057): библиотеки [MCL](https://github.com/manticoresoftware/columnar) больше не будут загружаться на системах без поддержки SSE 4.2.
...
* Поскольку мы недавно добавили поддержку вторичных индексов, ситуация стала запутанной, так как "индекс" может относиться к вторичному индексу, полнотекстовому индексу или обычному/реальному `индексу`. Чтобы уменьшить путаницу, мы переименовываем последний в "таблицу". Следующие SQL/командные строки подвержены этому изменению. Их старые версии устарели, но все еще функциональны:
  ...
  - `OPTIMIZE INDEX` => [OPTIMIZE TABLE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE),
  - [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) `plain TO RTINDEX rt` => [ATTACH TABLE](Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Attaching_one_table_to_another.md) `plain TO TABLE rt`,
...
* [Commit 2757](https://github.com/manticoresoftware/columnar/commit/2757b99b) Если слияние disk chunk прерывается, демон теперь очищает временные файлы, связанные с [MCL](https://github.com/manticoresoftware/columnar).
...
* [Commit 06df](https://github.com/manticoresoftware/manticoresearch/commit/06dfdd2e668933495dd6cb52d5ba2e0aa9b26e58) Запросы вроде `select attr, count(*) from plain_index` (без фильтрации) теперь выполняются быстрее, если вы используете [MCL](https://github.com/manticoresoftware/columnar).
...
* **⚠️ ЛОМАЮЩЕЕ ИЗМЕНЕНИЕ**: [MCL Issue #17](https://github.com/manticoresoftware/columnar/issues/17) [MCL](https://github.com/manticoresoftware/columnar): добавлен SSE-код в columnar scan. [MCL](https://github.com/manticoresoftware/columnar) теперь требует как минимум SSE4.2.
...
* **⚠️ ЛОМАЮЩЕЕ ИЗМЕНЕНИЕ**: Пользовательские формулы Homebrew, включая формулу для Manticore Columnar Library. Чтобы установить Manticore, [MCL](https://github.com/manticoresoftware/columnar) и другие необходимые компоненты, используйте следующую команду `brew install manticoresoftware/manticore/manticoresearch manticoresoftware/manticore/manticore-extra`.
...
* [MCL Issue #14](https://github.com/manticoresoftware/columnar/issues/14) [MCL](https://github.com/manticoresoftware/columnar): crash при select, когда слишком много ft-полей
...
* [Issue #802](https://github.com/manticoresoftware/manticoresearch/issues/802) [MCL](https://github.com/manticoresoftware/columnar): indexer падает в 5.0.2 и manticore-columnar-lib 1.15.4
...
* [Issue #843](https://github.com/manticoresoftware/manticoresearch/issues/843) [MCL](https://github.com/manticoresoftware/columnar): ошибка с длинным текстовым полем
...
* [Issue #919](https://github.com/manticoresoftware/manticoresearch/issues/919) [MCL](https://github.com/manticoresoftware/columnar): Manticore падает при выполнении запроса и другие падения во время восстановления кластера.
...
* [Issue #982](https://github.com/manticoresoftware/manticoresearch/issues/982) [MCL](https://github.com/manticoresoftware/columnar): crash в 5.0.3 из-за вторичного индекса
...
* [Issue #995](https://github.com/manticoresoftware/manticoresearch/issues/995) [MCL](https://github.com/manticoresoftware/columnar): min(pickup_datetime) из taxi1 приводит к crash
...
* [Issue #1000](https://github.com/manticoresoftware/manticoresearch/issues/1000) [MCL](https://github.com/manticoresoftware/columnar): Columnar rt index оказался поврежден после запуска без columnar-библиотеки
...
* **⚠️ КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ**: Неявный [cutoff](Searching/Options.md#cutoff). Теперь Manticore не тратит время и ресурсы на обработку данных, которые не нужны в возвращаемом наборе результатов. Обратная сторона в том, что это влияет на `total_found` в [SHOW META](Node_info_and_management/SHOW_META.md#SHOW-META) и `hits.total` в JSON-выводе [hits.total](Searching/Full_text_matching/Basic_usage.md#HTTP-JSON). Теперь это значение корректно только когда вы видите `total_relation: eq`, тогда как `total_relation: gte` означает, что фактическое число документов, совпадающих с запросом, больше значения `total_found`, которое вы получили. Чтобы сохранить прежнее поведение, можно использовать опцию поиска `cutoff=0`, которая всегда делает `total_relation` равным `eq`.
...
* [Commit 7ec7](https://github.com/manticoresoftware/manticoresearch/commit/7ec76d4abf01b5fca6f10a0000fe64d5d8e5fa52) Проблема с исчерпанием RAM, вызванная [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
- **Полная поддержка [Manticore Columnar Library](https://github.com/manticoresoftware/columnar)**. Ранее Manticore Columnar Library поддерживалась только для обычных индексов. Теперь она поддерживается:
  - в реальных индексах для `INSERT`, `REPLACE`, `DELETE`, [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE)
  - в репликации
  - в `ALTER`
  - в `indextool --check`
- **Автоматическая компактизация индексов** ([Issue #478](https://github.com/manticoresoftware/manticoresearch/issues/478)). Теперь вам больше не нужно вызывать [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) вручную, через crontask или другой механизм автоматизации. Manticore теперь делает это автоматически и по умолчанию. Порог компактизации по умолчанию можно задать через глобальную переменную [optimize_cutoff](Server_settings/Setting_variables_online.md).
...
- **Значительно более высокая производительность массовых вставок в индекс реального времени**. Например, на [сервере AX101 от Hetzner](https://www.hetzner.com/dedicated-rootserver/ax101) с SSD, 128 ГБ ОЗУ и AMD Ryzen™ 9 5950X (16*2 ядра) **с 3.6.0 вы могли вставить 236K документов в секунду** в таблицу со схемой `name text, email string, description text, age int, active bit(1)` (по умолчанию `rt_mem_limit`, размер пакета 25000, 16 параллельных рабочих процессов вставки, всего вставлено 16 миллионов документов). В 4.0.2 та же степень параллелизма/размер пакета/количество дает **357K документов в секунду**.
  ...
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) и DELETE выполняются по существующим чанкам, но если в это время идет слияние, записи собираются и затем применяются к новым чанкам
  - [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) последовательно захватывают эксклюзивную блокировку для каждого чанка. Слияния захватывают shared lock при переходе к стадии сбора атрибутов из чанка. Поэтому одновременно доступ к атрибутам чанка имеет только одна операция (merge или update).
  - когда merge доходит до фазы, где ему нужны атрибуты, он устанавливает специальный флаг. Когда [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) завершается, он проверяет флаг, и если он установлен, все обновление сохраняется в специальной коллекции. Наконец, когда merge завершается, он применяет набор обновлений к новорожденному disk chunk.
...
- [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) выполняется автоматически. Если он вам не нужен, убедитесь, что в секции `searchd` в конфигурационном файле задано `auto_optimize=0`
...
- [FORCE/IGNORE INDEX(id)](Data_creation_and_modification/Updating_documents/UPDATE.md#Updates-via-SQL) для SELECT/[UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
* [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) уменьшает число disk chunk до некоторого количества (по умолчанию `2* No. of cores`) вместо одного. Оптимальное число chunk можно контролировать параметром [cutoff](Securing_and_compacting_a_table/Compacting_a_table.md#Number-of-optimized-disk-chunks).
...
* [INSERT INTO PQ VALUES()](Data_creation_and_modification/Adding_documents_to_a_table/Adding_rules_to_a_percolate_table.md) (то есть без указания списка столбцов) ранее ожидал ровно значения `(query, tags)`. Теперь это изменено на `(id,query,tags,filters)`. Значение `id` можно задать равным 0, если вы хотите, чтобы оно генерировалось автоматически.
...
30. [cd512c7d ](https://github.com/manticoresoftware/manticoresearch/commit/cd512c7d ) Потерян лог падения при аварии на чанке диска RT
...
* отключено логирование операторов [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)/DELETE как SELECT в query log
...
* [Commit d0a7](https://github.com/manticoresoftware/manticoresearch/commit/d0a7c95906b9b5ba33c98267d54501cfe27508d6) исправление \#250 для опции индекса [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths) для TSV и CSV piped sources
...
* изменение document ID с помощью [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md) отключено
...
* String,JSON и MVA можно обновлять с помощью [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
* ALTER CLUSTER публикует узлы [UPDATE](Data_creation_and_modification/Updating_documents/UPDATE.md)
...
* [Commit 4ffa](https://github.com/manticoresoftware/manticoresearch/commit/4ffa623e9d357a3b0b441615089e211f92f8de32) исправлен счетчик total documents на disk chunk, который затрагивался командой [OPTIMIZE](Securing_and_compacting_a_table/Compacting_a_table.md#OPTIMIZE-TABLE) и ломал расчет веса
...
* [Commit e386](https://github.com/manticoresoftware/manticoresearch/commit/e386d842b36195719e459b2db1fb20e402f9a51f) исправлен crash сервера при DELETE в RT index с включенным [index_field_lengths](Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#index_field_lengths)
