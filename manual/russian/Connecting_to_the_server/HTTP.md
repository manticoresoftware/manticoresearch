...
Если включены [аутентификация и авторизация](../Security/Authentication_and_authorization.md), HTTP/HTTPS-клиенты должны отправлять либо заголовок Basic-аутентификации, либо bearer-токен:
...
Создайте bearer-токен для аутентифицированного HTTP-пользователя или обновите его с помощью `POST /token`:

```bash
curl -u admin:StrongPass#2026 -X POST http://127.0.0.1:9308/token -d "{}"
```

Эндпоинт один раз возвращает необработанный токен. Храните его безопасно. `SHOW TOKEN` показывает сохраненный хэш токена, а не исходный bearer-токен. Чтобы заменить bearer-токен, обновите его через `POST /token` или SQL-команду `TOKEN`.

Используйте HTTPS при передаче учетных данных или bearer-токенов по сети.

Имена HTTP-заголовков и названия схем `Basic` и `Bearer` не чувствительны к регистру. Имена пользователей чувствительны к регистру и должны совпадать точно.

При отсутствии или некорректности учетных данных возвращается `401 Unauthorized`. При наличии корректных учетных данных, но отсутствии нужного разрешения возвращается `403 Forbidden`.
...
Неудачный запрос, в том числе с синтаксической ошибкой, возвращается с HTTP-статусом `500` и причиной в поле `error` тела ответа, поэтому читайте тело ответа, а не считайте каждый `500` сбоем сервера. Чтобы проверить, что HTTP-интерфейс запущен, например в стартовом скрипте, отправьте `GET /`: он вернет `200`. `GET /sql` без запроса возвращает `400`, поэтому это не проверка готовности.
...
Чтобы выполнять несколько запросов через SQL по одному соединению с `curl`, нужно объединить команды с помощью
`--next`:
