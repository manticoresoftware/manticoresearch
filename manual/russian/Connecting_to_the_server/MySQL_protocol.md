...
Manticore Search поддерживает серверные [подготавливаемые выражения](#prepared-statements) через протокол MySQL. Клиентские подготавливаемые выражения также могут использоваться. Важно отметить, что Manticore реализует типы данных многозначное значение (MVA) и `float_vector`, у которых нет эквивалентов в MySQL или библиотеках, реализующих подготовленные выражения. В этих случаях значения должны быть сформированы в сыром запросе как список чисел, разделенных запятыми.
...
В стандартном SQL-синтаксисе Manticore [MVA](../Data_types.md#Multi-value-integer-%28MVA%29) или [векторы с плавающей точкой](Creating_a_table/Data_types.md#Float-vector) записываются как список значений в круглых скобках, например `(1, 2, 3)`.
