...
Manticore Search поддерживает серверные [подготовленные выражения](../Connecting_to_the_server/MySQL_protocol.md#prepared-statements) по протоколу MySQL. Также могут использоваться клиентские подготовленные выражения. Важно отметить, что Manticore реализует типы данных multi-value (MVA) и `float_vector`, которые не имеют эквивалентов в MySQL или библиотеках, реализующих подготовленные выражения. В этих случаях значения должны быть созданы в исходном запросе в виде списка чисел, разделенных запятыми.
