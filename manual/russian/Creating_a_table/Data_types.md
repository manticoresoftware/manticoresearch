...
Идентификатор документа — обязательный атрибут, который должен быть уникальным. Идентификаторы документов поддерживают беззнаковые 64-битные значения. Явно заданный идентификатор документа не может быть равен нулю; отрицательные идентификаторы документов не допускаются. Идентификаторы документов всегда включены, даже если это не указано, и их нельзя обновлять.

При объявлении таблицы вы можете включить `id` в схему `CREATE TABLE`, но независимо от используемого типа данных он будет вести себя так, как описано выше. В интерфейсе MySQL/SQL идентификатор отображается как знаковый 64-битный `bigint`, поэтому большие беззнаковые значения ID там могут выглядеть как отрицательные числа.
...
Автогенерация ID зависит от типа таблицы. Таблицы RT и PQ могут генерировать ID, когда `id` опущен в запросе `insert` или `replace`, либо когда в качестве значения ID используется `0`. Обычные таблицы, построенные из внешних источников, не поддерживают автоматическую генерацию ID; их исходные данные должны содержать явные, уникальные, ненулевые беззнаковые 64-битные идентификаторы документов.

При работе с идентификаторами документов важно понимать, что беззнаковые 64-битные значения обрабатываются по-разному в зависимости от интерфейса:
