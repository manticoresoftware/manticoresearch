...
Атрибуты векторных чисел с плавающей запятой позволяют сохранять списки чисел с плавающей запятой переменной длины, преимущественно используемые в приложениях машинного обучения и поиске по сходству. Этот тип отличается от [мультизначимых атрибутов](../Creating_a_table/Data_types.md#Multi-value-integer-%28MVA%29) (MVA) рядом важных особенностей:
...
Атрибуты векторных чисел с плавающей запятой позволяют сохранять списки чисел с плавающей запятой переменной длины, преимущественно используемые в приложениях машинного обучения и поиске по сходству.

### Использование и ограничения
- В настоящий момент поддерживаются только в таблицах реального времени
- Могут использоваться только в поисках KNN (k-ближайших соседей)
- Не поддерживаются в простых таблицах или других функциях/выражениях
...
- Векторы с плавающей запятой нельзя использовать в обычных фильтрах или сортировках
...
Более подробную информацию о настройке векторов с плавающей запятой и их использовании в поисках см. в разделе [KNN search](../Searching/KNN.md).

### Автоматические встраивания (рекомендуется)

Самый удобный способ работы с векторами с плавающей запятой — использование **автоматических встраиваний**. Эта функция автоматически генерирует векторы из ваших текстовых данных с применением моделей машинного обучения, устраняя необходимость вручную вычислять и вставлять векторы.

#### Преимущества автоматических встраиваний
- **Упрощенный рабочий процесс**: Просто вставляйте текст, встраивания генерируются автоматически
- **Нет ручных вычислений векторов**: Не нужно запускать отдельные модели встраивания
- **Последовательные встраивания**: Одна и та же модель обеспечивает согласованное представление векторов
...
- **Гибкий выбор полей**: Контроль над полями, используемыми для генерации встраиваний
...
- **Sentence Transformers**: Любая [подходящая BERT-модель Hugging Face](https://huggingface.co/sentence-transformers/models) (например, `sentence-transformers/all-MiniLM-L6-v2`) — API-ключ не требуется. Manticore загружает модель при создании таблицы.
...
- **Несколько полей**: `FROM='title,description'` — используются и объединяются поля title и description
- **Все текстовые поля**: `FROM=''` (пусто) — используются все поля `text` (поле полнотекстового поиска) и `string` (строковый атрибут) в таблице
...
При использовании автоматических встраиваний **не указывайте поле вектора** в операторах INSERT. Встраивания автоматически генерируются из указанных текстовых полей:
...
## Мультизначимое целочисленное значение (MVA)

<!-- example for creating MVA32 -->

Мультизначимые атрибуты позволяют хранить списки 32-битных беззнаковых целых чисел переменной длины. Это может быть полезно для хранения отношений один ко многим числовых значений, таких как теги, категории продуктов и свойства.
...
Поддерживается фильтрация и агрегация, но не сортировка. Фильтрация может выполняться с использованием условия, при котором должен пройти хотя бы один элемент (с помощью [ANY()](../Functions/Arrays_and_conditions_functions.md#ANY%28%29)) или все элементы ([ALL()](../Functions/Arrays_and_conditions_functions.md#ALL%28%29)).
...
Можно извлекать информацию, такую как [наименьший](../Functions/Mathematical_functions.md#LEAST%28%29) или [наибольший](../Functions/Mathematical_functions.md#GREATEST%28%29) элемент, а также длину списка. Пример показывает сортировку по наименьшему элементу многозначного атрибута.
...
При группировке по многозначному атрибуту документ будет участвовать в таком количестве групп, сколько различных значений связано с этим документом. Например, если коллекция содержит ровно один документ с многозначным атрибутом 'product_codes', значения которого равны 5, 7 и 11, группировка по 'product_codes' создаст 3 группы с `COUNT(*)`, равным 1, и значениями ключей `GROUPBY()` 5, 7 и 11 соответственно. Также обратите внимание, что группировка по многозначным атрибутам может привести к дублированию документов в результирующем наборе, поскольку каждый документ может участвовать во многих группах.
...
Порядок чисел, вставленных в качестве значений многозначных атрибутов, не сохраняется. Значения хранятся внутренне как отсортированный набор.
...
## Многозначный целочисленный тип большого размера

<!-- example for creating MVA64 -->

Тип данных, позволяющий хранить списки переменной длины из 64-битных знаковых целых чисел. Обладает такой же функциональностью, как и многозначный целочисленный тип.
...
При использовании столбцового хранилища вы можете указывать следующие свойства для атрибутов.

<!-- example fast_fetch -->
### fast_fetch

По умолчанию хранилище Manticore Columnar хранит все атрибуты в столбцовом виде, а также в специальном docstore построчно. Это позволяет быстро выполнять запросы типа `SELECT * FROM ...`, особенно при извлечении большого количества записей за один раз. Однако, если вы уверены, что это не нужно или хотите сэкономить место на диске, вы можете отключить это, указав `fast_fetch='0'` при создании таблицы или (если вы определяете таблицу в конфиге) используя `columnar_no_fast_fetch`, как показано в следующем примере.
