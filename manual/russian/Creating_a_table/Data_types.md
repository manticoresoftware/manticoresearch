...
Имена таблиц в режимах RT и Plain ограничены 200 байтами после кодирования в UTF-8. Каждый ASCII-символ занимает один байт, а символ не из ASCII — два, три или четыре байта. Поэтому имя может содержать до 200 ASCII-символов, но меньше символов не из ASCII — точное число зависит от используемых символов. Для полных имен таблиц `system.` префикс учитывается в этом лимите. Ограничение также применяется при загрузке существующих определений таблиц, поэтому перед обновлением переименуйте все более длинные таблицы.
