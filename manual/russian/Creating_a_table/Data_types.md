...
Атрибуты с вектором с плавающей запятой позволяют хранить списки переменной длины из чисел с плавающей точкой, которые преимущественно используются в приложениях машинного обучения и поисках по сходству. Этот тип отличается от [мультизначных атрибутов](../Creating_a_table/Data_types.md#Multi-value-integer-%28MVA%29) (MVA) по нескольким важным параметрам:
...
Атрибуты с вектором с плавающей запятой позволяют хранить списки переменной длины из чисел с плавающей точкой, которые преимущественно используются в приложениях машинного обучения и поисках по сходству.

### Использование и ограничения
- В настоящее время поддерживается только в таблицах реального времени
- Может использоваться только в KNN (поисках k-ближайших соседей)
...
Для более подробной информации о настройке векторов с плавающей запятой и использовании их в поисках смотрите [KNN search](../Searching/KNN.md).

### Автоматические встраивания (рекомендуется)

Самый удобный способ работы с векторами с плавающей точкой — использовать **автоматические встраивания**. Эта функция автоматически генерирует встраивания из ваших текстовых данных с помощью моделей машинного обучения, устраняя необходимость вручную вычислять и вставлять векторы.

#### Преимущества автоматических встраиваний
- **Упрощенный рабочий процесс**: просто вставляете текст, встраивания генерируются автоматически
- **Нет необходимости в ручных вычислениях векторов**: не требуется запускать отдельные модели для встраивания
- **Согласованные встраивания**: одна и та же модель обеспечивает согласованное представление векторов
...
При создании таблицы с автоматическими встраиваниями укажите следующие дополнительные параметры:
- `MODEL_NAME`: модель встраивания для автоматической генерации векторов
- `FROM`: какие поля использовать для генерации встраивания (пустая строка означает все текстовые/строковые поля)

**Поддерживаемые модели встраивания:**
- **Sentence Transformers**: любая [подходящая модель BERT на Hugging Face](https://huggingface.co/sentence-transformers/models) (например, `sentence-transformers/all-MiniLM-L6-v2`) — ключ API не требуется. Manticore загружает модель при создании таблицы.
- **OpenAI**: модели встраивания OpenAI, такие как `openai/text-embedding-ada-002` — требуется параметр `API_KEY='<OPENAI_API_KEY>'`
- **Voyage**: модели встраивания Voyage AI — требуется параметр `API_KEY='<VOYAGE_API_KEY>'`
- **Jina**: модели встраивания Jina AI — требуется параметр `API_KEY='<JINA_API_KEY>'`
...
Использование всех текстовых полей для встраивания (параметр FROM пуст)
```sql
...
Параметр `FROM` управляет выбором полей для генерации встраивания:

- **Определенные поля**: `FROM='title'` — используется только поле title
- **Несколько полей**: `FROM='title,description'` — title и description конкатенируются и используются вместе
- **Все текстовые поля**: `FROM=''` (пусто) — используются все поля `text` (полное текстовое поле) и `string` (строковый атрибут) таблицы
- **Пустые векторы**: Вы можете вставлять пустые векторы с помощью `()`, чтобы исключить документы из векторного поиска

#### Вставка данных с автоматическими встраиваниями

При использовании автоматических встраиваний **не указывайте поле вектора** в ваших операторах INSERT. Встраивания автоматически генерируются из указанных текстовых полей:
...
В качестве альтернативы можно работать с вручную вычисленными векторами с плавающей точкой.
...
## Целое число с множественным значением (MVA)
