...
- Использовать `LENGTH(float_vector_attr)` в выражениях для получения размерности вектора (количество компонентов)
...
- **Пустые векторы (NULL):** Вы можете вставить или заменить (REPLACE) с помощью `()` или `NULL` для векторного столбца; это значение сохраняется как пустое и отображается как `NULL` в результатах. Документы с пустыми векторами исключаются из KNN-поиска. `NULL` в INSERT принимается, чтобы дампы mysqldump (которые записывают NULL для пустых столбцов) могли быть восстановлены без ошибок.
