...
```ini
Этот параметр определяет таблицу(ы), к которой будет применяться список удаления (kill-list). Совпадения в целевой таблице, которые обновляются или удаляются в текущей таблице, будут подавлены. В режиме `:kl` документы для подавления берутся из [kill-list](../../Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Killlist_in_plain_tables.md). В режиме `:id` все идентификаторы документов из текущей таблицы подавляются в целевой. Если ни один режим не указан, будут применяться оба режима. [Подробнее о списках удаления здесь](../../Data_creation_and_modification/Adding_data_from_external_storages/Adding_data_to_tables/Killlist_in_plain_tables.md)
```

Значение: **не указано** (по умолчанию), target_table_name:kl, target_table_name:id, target_table_name. Возможно указание нескольких значений
...
Можно указать `columnar_attrs = *`, чтобы хранить все поддерживаемые типы данных в колоночном хранилище.
...
```ini
По умолчанию все строковые атрибуты, хранящиеся в колоночном хранилище, сохраняют предварительно вычисленные хеши. Эти хеши используются для группировки и фильтрации. Однако они занимают дополнительное место, и если группировка по этому атрибуту не требуется, можно сэкономить место, отключив генерацию хеша.
```
...
| - | - | - | - |
| [text](../../Creating_a_table/Data_types.md#Text) | [rt_field](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_field)  | Опции: indexed, stored. По умолчанию: **оба**. Чтобы сохранить текст сохранённым, но индексированным, укажите только "stored". Чтобы сохранить только индекс, укажите только "indexed". | string |
...
| [multi](../../Creating_a_table/Data_types.md#Multi-value-integer-%28MVA%29) | [rt_attr_multi](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_attr_multi)   | мульти-целое | mva |
| [multi64](../../Creating_a_table/Data_types.md#Multi-value-big-integer) | [rt_attr_multi_64](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_attr_multi_64) | мульти-большое целое  | mva64 |
...
| [string](../../Creating_a_table/Data_types.md#String) | [rt_attr_string](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#rt_attr_string) | строка. Опция `indexed, attribute` позволяет сделать значение полнотекстово индексируемым и фильтруемым, сортируемым и группируемым одновременно  |   |
...
```
* "title" индексировано, но не сохранено.
* "description" сохранено, но не индексировано.
* "author" сохранено и индексировано одновременно.
...
Параметр engine изменяет хранилище атрибутов [по умолчанию](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages) для всех атрибутов в таблице. Вы также можете указать `engine` [отдельно для каждого атрибута](../../Creating_a_table/Data_types.md#How-to-switch-between-the-storages).
```

Для информации о том, как включить колоночное хранилище для простой таблицы, см. [columnar_attrs](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#columnar_attrs) .
...
Следующие настройки применимы как для таблиц в режиме реального времени, так и для обычных таблиц, независимо от того, указаны они в файле конфигурации или установлены онлайн с помощью команды `CREATE` или `ALTER`.
...
| - | - |
| file | сервер читает файлы таблицы с диска с помощью seek+read с использованием внутренних буферов при доступе к файлу |
| mmap | сервер отображает файлы таблицы в память, и ОС кэширует их содержимое при доступе к файлу |
| mmap_preread | сервер отображает файлы таблицы в память, и фоновый поток однократно читает их для прогрева кэша |
...
| - | - | - |
| access_plain_attrs  | mmap, **mmap_preread** (по умолчанию), mlock | контролирует способ чтения файлов `*.spa` (обычные атрибуты), `*.spe` (skip листы), `*.spt` (lookups), `*.spm` (killed docs) |
| access_blob_attrs   | mmap, **mmap_preread** (по умолчанию), mlock  | контролирует способ чтения файлов `*.spb` (blob атрибуты) (строки, MVA и JSON атрибуты) |
| access_doclists   | **file** (по умолчанию), mmap, mlock  | контролирует способ чтения данных `*.spd` (списки документов) |
| access_hitlists   | **file** (по умолчанию), mmap, mlock  | контролирует способ чтения данных `*.spp` (списки попаданий) |
| access_dict   | mmap, **mmap_preread** (по умолчанию), mlock  | контролирует способ чтения `*.spi` (словарь) |
Ниже приведена таблица, которая поможет вам выбрать желаемый режим:

| часть таблицы |	хранить на диске |	хранить в памяти |	кэшировать в памяти при запуске сервера | заблокировать в памяти |

| - | - | - | - | - |
| обычные атрибуты в [строчном](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages) (не колонный) хранилище, skip-листы, списки слов, lookups, killed docs | 	mmap | mmap |	**mmap_preread** (по умолчанию) | mlock |
| строковые атрибуты, многозначные атрибуты (MVA) и JSON атрибуты в строчном формате | mmap | mmap | **mmap_preread** (по умолчанию) | mlock |
...
* Для **максимально быстрого времени ответа на поиск** при большом объёме доступной памяти, используйте [строчные](../../Creating_a_table/Data_types.md#JSON) атрибуты и блокируйте их в памяти с помощью `mlock`. Дополнительно используйте mlock для списков документов и списков попаданий.

* Если вы отдаёте предпочтение, чтобы **производительность не снижалась после запуска**, и готовы принять более длительное время запуска, используйте опцию [--force-preread](../../Starting_the_server/Manually.md#searchd-command-line-options). Если желаете более быстрый рестарт searchd, придерживайтесь опции по умолчанию `mmap_preread`.
* Если вы хотите **экономить память**, при этом оставляя достаточный объём памяти для всех атрибутов, избегайте использования `mlock`. Операционная система будет определять, что должно храниться в памяти, основываясь на частых обращениях к диску.
* Если строчные атрибуты **не помещаются в память**, выбирайте [колоночные атрибуты](../../Creating_a_table/Data_types.md#Row-wise-and-columnar-attribute-storages).
...
* mmap,
* предварительное чтение неклоночных атрибутов,
* seek+read клоночных атрибутов без предварительного чтения,
* seek+read списков документов и списков попаданий без предварительного чтения.
Это обеспечивает достойную производительность поиска, оптимальное использование памяти и более быструю перезагрузку searchd в большинстве сценариев.
...
```ini
Этот параметр контролирует размер блоков, используемых для хранения документов. Значение по умолчанию — 16 кб. Когда исходный текст документа хранится с использованием stored_fields или stored_only_fields, он сохраняется внутри таблицы и сжимается для повышения эффективности. Чтобы оптимизировать доступ к диску и коэффициенты сжатия для маленьких документов, эти документы объединяются в блоки. Процесс индексирования собирает документы до тех пор, пока их общий размер не достигнет порога, указанного в этом параметре. В этот момент блок документов сжимается. Этот параметр может быть настроен для достижения лучшего коэффициента сжатия (увеличив размер блока) или более быстрого доступа к тексту документа (уменьшив размер блока).
```
...
```ini
Этот параметр определяет тип сжатия, используемый для сжатия блоков документов, хранящихся в хранилище документов. Если указаны stored_fields или stored_only_fields, хранилище документов сохраняет сжатые блоки документов. 'lz4' обеспечивает быструю скорость сжатия и распаковки, тогда как 'lz4hc' (высокое сжатие) жертвует некоторой скоростью сжатия ради лучшего коэффициента сжатия. 'none' полностью отключает сжатие.
```
...
```ini
Уровень сжатия, используемый при применении сжатия 'lz4hc' в хранилище документов. Настройка уровня сжатия позволяет найти правильный баланс между производительностью и коэффициентом сжатия при использовании сжатия 'lz4hc'. Обратите внимание, что этот параметр не применяется при использовании сжатия 'lz4'.
```
...
```ini
Этот параметр указывает, что searchd должен открыть все файлы таблиц при запуске или при ротации и держать их открытыми во время работы. По умолчанию файлы не открываются заранее. Таблицы, открытые заранее, требуют несколько дескрипторов файлов на таблицу, но они устраняют необходимость в вызовах open() на каждый запрос и защищены от гонок, которые могут возникать при ротации таблиц под высокой нагрузкой. Однако, если вы обслуживаете много таблиц, может быть более эффективно открывать их по запросу для экономии дескрипторов файлов.
```
...
```ini
Размер буфера для хранения списка документов по ключевому слову. Увеличение этого значения приведет к большему использованию памяти при выполнении запросов, но может уменьшить время ввода-вывода.
```
...
```ini
Размер буфера для хранения списка попаданий по ключевому слову. Увеличение этого значения приведет к большему использованию памяти при выполнении запросов, но может уменьшить время ввода-вывода.
```
...
```ini
Включает инверсирование таблицы на месте. Опционально, по умолчанию 0 (используются отдельные временные файлы).
```

Опция `inplace_enable` уменьшает дисковый след при индексировании plain таблиц, слегка замедляя индексирование (используется примерно в 2 раза меньше диска, но производительность составляет около 90-95% от исходной).

Процесс индексирования состоит из двух основных этапов. На первом этапе документы собираются, обрабатываются и частично сортируются по ключевым словам, а промежуточные результаты записываются во временные файлы (.tmp*). На втором этапе документы полностью сортируются и создаются финальные файлы таблицы. Перестроение производственной таблицы "на лету" требует примерно в 3 раза больше дискового пространства в пиковой нагрузке: сначала для промежуточных временных файлов, затем для вновь созданной копии, и третье — для старой таблицы, которая в это время обслуживает производственные запросы. (Промежуточные данные сопоставимы по размеру с финальной таблицей.) Это может быть слишком большой дисковый след для больших коллекций данных, и опция `inplace_enable` может быть использована для его уменьшения. При включении выполняется повторное использование временных файлов, выводятся финальные данные обратно в них и они переименовываются по завершении. Однако это может потребовать дополнительного перемещения временных чанков данных, откуда и происходит влияние на производительность.

Директива не влияет на [searchd](../../Starting_the_server/Manually.md), она влияет только на [indexer](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-tool).
...
```ini
Опция тонкой настройки [инверсирования на месте](../../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#inplace_enable). Управляет размером предварительно выделенного зазора в списке попаданий. Опционально, по умолчанию 0.
```

Директива влияет только на инструмент [searchd](../../Starting_the_server/Manually.md) и не оказывает влияния на [indexer](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-tool).
...
```ini
Параметр inplace_reloc_factor определяет размер буфера для переноса (relocation buffer) в памяти, используемой во время индексирования. Значение по умолчанию 0.1.
```

Опция необязательна и влияет только на инструмент [indexer](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-tool), не на сервер [searchd](../../Starting_the_server/Manually.md).
...
```ini
Управляет размером буфера, используемого для записи на месте во время индексирования. Опционально, по умолчанию 0.1.
```
