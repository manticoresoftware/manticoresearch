...
- `model_name`: модель эмбеддингов, которую нужно использовать (например, `"Xenova/all-MiniLM-L6-v2"` для быстрого пути ONNX — посмотрите [модели ONNX на Hugging Face](https://huggingface.co/Xenova/models?pipeline_tag=feature-extraction&search=minilm); `"sentence-transformers/all-MiniLM-L6-v2"` также поддерживается; `"openai/text-embedding-ada-002"` для OpenAI). Если параметр указан, `dims` нужно опустить, так как модель автоматически определяет размерность. Таблицу сравнения скорости и практические рекомендации см. в разделе [Выбор локальной модели эмбеддингов](../../Searching/KNN.md#Choosing-a-local-embedding-model).
