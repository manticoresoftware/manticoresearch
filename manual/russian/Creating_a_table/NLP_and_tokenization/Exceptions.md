...
В приведённом выше примере запрос `ms windows` *не* будет соответствовать документу с текстом `MS Windows`. Запрос будет интерпретирован как запрос для двух ключевых слов, `ms` и `windows`. Отображение для `MS Windows` — это одно ключевое слово `ms windows` с пробелом посередине. С другой стороны, `standartenfuhrer` найдет документы с содержимым `Standarten Fuhrer` или `Standarten Fuehrer` (написанными именно так, с заглавными буквами), или любой вариант написания самого ключевого слова, например, `staNdarTenfUhreR`. (Однако оно не найдет `standarten fuhrer`: этот текст не соответствует ни одному из перечисленных исключений из-за чувствительности к регистру и индексируется как два отдельных ключевых слова.)

Пробелы в списке токенов-источников имеют значение, но их количество — нет. Любое количество пробелов в списке источников будет соответствовать любому другому количеству пробелов в индексируемом документе или запросе. Например, токен-источник `AT & T` будет соответствовать тексту `AT & T`, независимо от количества пробелов как в части источника, так и в индексированном тексте. Такой текст, следовательно, будет проиндексирован как специальное ключевое слово `AT&T`, благодаря самой первой записи из примера.

Исключения также позволяют захватывать специальные символы (которые являются исключениями из общих правил `charset_table`; отсюда и название). Предположим, вы в целом не хотите рассматривать `+` как допустимый символ, но всё же хотите иметь возможность искать некоторые исключения из этого правила, такие как `C++`. Приведённый выше пример сделает именно это, полностью независимо от того, какие символы есть в таблице, а каких нет.
...
Значения должны быть разделены точкой с запятой (`;`). Поскольку исключения могут содержать `>` или `=>` как разделители, а также потенциально другие специальные символы, убедитесь, что экранируете точки с запятой, если они являются частью самого токена (например, `\;`).
