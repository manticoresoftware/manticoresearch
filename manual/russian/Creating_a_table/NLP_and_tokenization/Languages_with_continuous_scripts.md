# Китайские, японские и корейские (CJK) и тайские языки

Manticore предоставляет встроенную поддержку для индексирования языков с [непрерывными сценариями](https://en.wikipedia.org/wiki/Scriptio_continua) (т.е. языков, которые не используют пробелы или другие знаки между словами или предложениями). Это позволяет обрабатывать тексты на этих языках двумя различными способами:

<!-- example cont 1 -->
1. Точная сегментация с использованием библиотеки [ICU](https://icu.unicode.org/). В данный момент поддерживается только китайский.
...
<!-- example cont 2 -->
2. Точная сегментация с использованием библиотеки [Jieba](https://github.com/fxsjy/jieba). Как и ICU, она в данный момент поддерживает только китайский.
...
<!-- example cont 3 -->
3. Базовая поддержка с использованием опций N-грамм [ngram_len](../../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#ngram_len) и [ngram_chars](../../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#ngram_chars)
Для каждого языка, использующего непрерывный сценарий, существуют отдельные таблицы символов (`chinese`, `korean`, `japanese`, `thai`), которые можно использовать. В качестве альтернативы вы можете использовать общую таблицу символов `cont`, чтобы поддерживать все CJK и тайские языки одновременно, или таблицу символов `cjk`, чтобы включить только все CJK языки.
