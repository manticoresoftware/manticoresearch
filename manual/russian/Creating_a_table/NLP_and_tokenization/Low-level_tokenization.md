...
<!-- example dict -->
Тип используемого словарного индекса определяется одним из трех известных значений: `keywords`, `keywords_32k` или `crc`. Параметр необязателен, значение по умолчанию — `keywords`.

`dict=keywords` — словарный индекс слов по умолчанию. Он хранит ключевые слова в индексе и выполняет расширение шаблонов по wildcard во время поиска.

`dict=keywords_32k` — опциональный словарный индекс слов для токенов длиной до 32 КиБ. Он поддерживает нормализованные токены длиной до 32768 байт как в обычных, так и в RT-таблицах. Лимит измеряется в байтах после нормализации токена, а не в символах. Токены, превышающие этот предел, пропускаются с предупреждением вместо того, чтобы индексироваться как обрезанные термины. При включенных обычных настройках exact, prefix или infix поддерживаются точный поиск, поиск по префиксу и поиск по подстроке.

`keywords_32k` предназначен для длинных машинно сгенерированных значений, таких как хэши, сгенерированные ID, идентификаторы сообщений и длинные токены, похожие на адреса электронной почты. Длинные токены индексируются как исходные и не проходят стемминг или лемматизацию; короткие обычные токены по-прежнему могут использовать настроенную морфологию. `CALL SUGGEST` и `CALL QSUGGEST`, таблицы percolate, обработка фрагментов и подсветки для 32 КиБ и `indextool --dumpdict` пока не поддерживают `dict=keywords_32k`.

Использование режима словарного индекса, такого как `dict=keywords` или `dict=keywords_32k`, может значительно снизить нагрузку на индексирование и включить поиск подстрок по большим коллекциям. Этот режим можно использовать как для обычных, так и для RT-таблиц.
