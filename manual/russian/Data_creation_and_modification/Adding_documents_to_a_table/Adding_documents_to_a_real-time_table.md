...
При загрузке больших пакетов в локальную real-time таблицу Manticore Search может записывать строки `INSERT` напрямую в disk chunk и публиковать его при фиксации транзакции. Это позволяет не строить пакет сначала в RAM chunk. Строки остаются невидимыми до публикации chunk, а при ошибке или `ROLLBACK` таблица остается без изменений.
...
Добавьте `bulk_import=<table>` в запрос Manticore `/bulk` или `/json/bulk`. Тело запроса по-прежнему должно быть стандартным NDJSON, где каждая операция должна быть `insert` или `create` для привязанной таблицы:
...
Когда Manticore публикует disk chunk в целевую таблицу, строка из chunk заменяет существующую строку с тем же ID. Это также относится к ID, опубликованным в более раннем HTTP-пакете. В результате повторная отправка ранее опубликованного пакета заменяет его строки, а дубликаты внутри повторно отправленного пакета по-прежнему сохраняют первую строку. В запросе Manticore `/bulk` операция `create` (например, `{"create":{"table":"products",...}}`) ведет себя как `insert`; она не завершается ошибкой только потому, что ID уже существует.
