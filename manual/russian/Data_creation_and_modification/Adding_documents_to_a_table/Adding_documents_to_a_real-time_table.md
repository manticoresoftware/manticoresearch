...
Имена таблиц в этом режиме приводятся к каноническому нижнему регистру. И SQL-настройка, и параметр запроса Manticore `/bulk` нормализуют целевую таблицу к нижнему регистру перед авторизацией и активацией.
...
`SET bulk_import=<table>` требует чистой сессии и резервирует таблицу для записи. Поиск остается доступным, но другие сессии не могут выполнять `INSERT`, `REPLACE`, `UPDATE` или `DELETE` для зарезервированной таблицы. `BEGIN` и `START TRANSACTION` в режиме прямой записи на диск являются необязательными no-op для совместимости. `COMMIT` собирает и атомарно прикрепляет текущий disk chunk; `ROLLBACK` отбрасывает текущий пакет. Оба действия сохраняют режим и резервацию активными для следующего пакета. Автокоммит нельзя менять, пока активен этот режим. `SET bulk_import=0` или закрытие соединения отбрасывает все ожидающие изменения и освобождает резервацию.
...
* Статические сборки и macOS сейчас не поддерживаются.
