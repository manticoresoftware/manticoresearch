...
Поле/значение `ID` можно опустить, так как таблицы RT и PQ поддерживают [auto-id](../../Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md#Auto-ID). Для таблиц с числовыми ID можно также указать `0` в качестве значения `id`, чтобы принудительно сгенерировать ID автоматически. Строки с дублирующимися ID не будут перезаписаны через `INSERT`. Вместо этого для такой задачи используйте [REPLACE](../../Data_creation_and_modification/Updating_documents/REPLACE.md).

Для таблиц, созданных с [`id uuid`](../../Creating_a_table/Data_types.md#UUID-document-IDs), передавайте явный UUID в виде строки в кавычках или опускайте `id`, чтобы сгенерировать его автоматически. Явные значения должны соответствовать формату `xxxxxxxx-xxxx-Vxxx-Wxxx-xxxxxxxxxxxx`, где каждый `x` — шестнадцатеричная цифра, `V` — версия (`1`–`8`), а `W` — вариант (`8`, `9`, `a` или `b`). Буквы в шестнадцатеричных значениях в верхнем регистре допускаются и приводятся к нижнему регистру. В отличие от числовых ID, `0` не запрашивает автоматическую генерацию UUID.
...
Для таблиц, созданных с `id uuid`, передавайте JSON `id` как UUID-строку или опускайте его, чтобы сгенерировать значение автоматически:
...
Для реплицируемых таблиц указывайте свойство `cluster` в каждом документе:
...
Для всех операций записи (INSERT, REPLACE, DELETE, TRUNCATE, UPDATE) в реплицируемых таблицах необходимо:
* В SQL: Используйте формат `cluster_name:table_name` вместо одного только имени таблицы
* В JSON: Указывайте свойство `cluster` вместе со свойством `table`

Если не использовать правильный синтаксис, операция завершится ошибкой.
...
## Автоматический ID
<!-- example autoid -->
Manticore предоставляет автоматическую генерацию ID для документов, вставляемых или заменяемых в таблицу real-time или [Percolate table](../../Creating_a_table/Local_tables/Percolate_table.md). Генератор создаёт уникальное числовое значение с гарантиями, перечисленными ниже, но его не следует считать автоинкрементной последовательностью.
...
* Средняя скорость генерации авто-ID между двумя запусками сервера остается ниже примерно 16 миллионов ID в секунду

Генератор auto ID создаёт 64-битное целое число со следующим распределением:
* Биты с 0 по 23 — это счетчик, который увеличивается при каждом вызове генератора авто ID
* Биты с 24 по 55 хранят время запуска сервера в секундах, закодированное как `(unix_timestamp_at_start - 2019-05-01 00:00:00 UTC)`
* Биты с 56 по 62 хранят `server_id` (значение маскируется в диапазон 0..127)

Такая схема гарантирует, что сгенерированные ID уникальны между узлами кластера и что данные, вставляемые в разные узлы кластера, не создают коллизий. Это особенно важно при работе с реплицируемыми таблицами, поскольку так автогенерируемые ID остаются уникальными на всех узлах репликационного кластера.

Важно: 24-битный счетчик не является жестким ограничением на общее количество документов, которые можно вставить за один запуск сервера. Вы можете вставить более 16 777 216 документов после запуска; ID будут продолжать увеличиваться и оставаться уникальными для этого запущенного процесса. Правило `~16 миллионов ID в секунду` важно для уникальности при перезапусках: после перезапуска временная часть должна продвинуться достаточно далеко, чтобы вновь сгенерированные ID не пересекались с ID, созданными до перезапуска.
...
Для таблиц с числовыми ID это целочисленный публичный ID документа. Для таблиц с UUID-ID Manticore кодирует его в каноническую строку UUIDv8; клиентам виден только UUID.
...
Обратите внимание, что HTTP-эндпоинт `/bulk` не поддерживает автоматическое создание таблиц (auto schema). Эту возможность поддерживают только эндпоинт `/_bulk` (в стиле Elasticsearch) и SQL-интерфейс. HTTP-эндпоинт `/_bulk` (в стиле Elasticsearch) позволяет указывать имя таблицы вместе с именем кластера в формате `cluster_name:table_name`.
...
Для таблицы RT, объявленной с `id uuid`, `/bulk` читает UUID из `id`. `/_bulk` читает их из метаданных `_id` или из поля документа `id`. Оба endpoint'а могут опустить ID, чтобы UUID был сгенерирован автоматически.
...
Для более быстрой загрузки больших пакетов в локальную таблицу real-time Manticore Search может записывать строки `INSERT` напрямую в дисковый chunk и публиковать этот chunk при фиксации транзакции. Это позволяет не собирать пакет сначала в RAM chunk. Строки остаются невидимыми, пока chunk не опубликован, а при сбое операции или `ROLLBACK` таблица не меняется.

Поддерживаются и построчные, и колоночные таблицы, включая полнотекстовые поля, числовые атрибуты, строки, JSON, MVA/MVA64 и векторы float с KNN-индексами.

В этом режиме регистр в имени таблицы не учитывается. Например, `SET bulk_import=Products` и `bulk_import=products` выберут таблицу, сохраненную как `products`; таблицы, имена которых отличаются только регистром букв, различить нельзя.
...
Выполните `SET bulk_import=<table>` до `BEGIN` и до любых незафиксированных операций записи в этом соединении. Команда резервирует выбранную таблицу от обычных операций записи: поиск и другие bulk imports остаются доступными, а обычные операции записи блокируются до завершения всех активных импортов. Команды `BEGIN` и `START TRANSACTION` разрешены, но не обязательны и ничего не меняют в этом режиме.

`COMMIT` публикует строки, накопленные после предыдущего `COMMIT` или `ROLLBACK`, как один disk chunk. `ROLLBACK` отбрасывает эти строки. В обоих случаях пакетный импорт остается включенным, и можно начать следующий пакет. `SET bulk_import=0` отбрасывает незафиксированные строки, отключает пакетный импорт и освобождает резервирование этого соединения. Обычные операции записи возобновляются после того, как последний параллельный импорт освободит своё резервирование. Закрытие соединения делает то же самое. Пока пакетный импорт включен, изменить autocommit нельзя.

#### HTTP `/bulk`

Добавьте `bulk_import=<table>` в запрос Manticore `/bulk`. Тело запроса по-прежнему остается обычным NDJSON с разделением по строкам, и каждая операция должна быть `insert` или `create` для привязанной таблицы:
...
В теле запроса пустая строка завершает и публикует текущий пакет, а конец запроса публикует последний пакет. Manticore отправляет HTTP-ответ после обработки всех пакетов в запросе. Если более поздний пакет завершится с ошибкой, ранее опубликованные пакеты из этого запроса останутся доступны для поиска. Чтобы опубликовать весь запрос атомарно как один disk chunk, не добавляйте пустые строки. Ответ содержит один агрегированный результат `bulk` для каждого опубликованного пакета, а не отдельный результат для каждого документа.

Обычно достаточно одного запроса, как в примере выше. Если приложение намеренно отправляет несколько запросов через одно [постоянное HTTP-соединение](../../Connecting_to_the_server/HTTP.md#Persistent-connections), первый запрос выбирает таблицу. Последующие запросы `/bulk` или `/json/bulk` в этом соединении могут не повторять `bulk_import`, но должны продолжать записывать в ту же таблицу. После завершения закройте соединение или отправьте пустой запрос `/bulk?bulk_import=0`, чтобы отключить пакетный импорт и освободить резервирование этого соединения. Обычные операции записи возобновятся после завершения всех параллельных импортов.

Конечная точка поддерживает chunked transfer encoding, поэтому может обрабатывать тела больше `max_packet_size` без буферизации всего запроса.

#### Отправители с поддержкой Elasticsearch

Для отправителей, совместимых с Elasticsearch, задайте зарезервированное имя pipeline `bulk_import`, используйте bulk-действие Elasticsearch `index` и сопоставьте поле записи с `_id`. В следующих примерах поле `id` используется как стабильный ID документа.
...
Не включайте `generate_id On`: эта настройка создает UUID, которые пакетный импорт не поддерживает.
...
Для входных данных NDJSON сопоставьте исходное поле `id` с Elasticsearch `_id`, а действие и pipeline задайте в метаданных события:
...
Настройте список Filebeat `include_fields` в соответствии со схемой целевой таблицы. Целевая таблица должна существовать заранее и содержать все поля, которые отправляет клиент, например настроенное в Fluent Bit поле `event_time`. Каждый `_id` должен быть явно заданной стабильной ненулевой десятичной строкой, например `"123"`; в одном запросе ID должны быть уникальными. Запрос должен выбирать `pipeline=bulk_import`: либо как параметр всего запроса, либо в каждом действии. Все действия должны быть направлены в одну таблицу, и поддерживается только действие `index`.

Каждый flush-запрос атомарно публикуется как один disk chunk. Клиент делит непрерывный поток данных на flush-запросы в соответствии со своими настройками пакетной обработки и буферизации, поэтому вся загрузка может создать несколько disk chunks: по одному на каждый успешно обработанный flush-запрос, а не один на весь запуск клиента. Для большой первоначальной загрузки используйте крупные, но разумные flush-батчи: они уменьшают количество disk chunks и объём последующих слияний, но увеличивают задержку одного запроса и объём данных, которые придётся отправить повторно при его сбое. При публикации документы с уже существующими ID заменяются. Успешный ответ содержит один результат Elasticsearch bulk для каждого документа. Несколько запросов bulk import могут параллельно строить отдельные disk chunks для одной таблицы и публиковать их по очереди. Пока активен хотя бы один bulk import, обычные операции записи в эту таблицу остаются заблокированными.

#### Дубликаты ID документов

Внутри одного пакета прямой записи на диск первая строка для числового ID документа остается видимой, а последующие строки с тем же ID логически удаляются. Для SQL пакет состоит из строк, подготовленных перед `COMMIT` или `ROLLBACK`. Для Manticore `/bulk` каждая группа, опубликованная на пустой строке, при смене таблицы или на EOF запроса, является отдельным пакетом.

Когда Manticore публикует disk chunk в целевую таблицу, строка в этом chunk заменяет любую существующую строку с тем же ID. Это также относится к ID, опубликованным более ранним HTTP-пакетом. В результате повторная отправка уже опубликованного пакета заменяет его строки, а дубликаты внутри повторно отправленного пакета по-прежнему сохраняют первую строку. В запросе Manticore `/bulk` операция `create`, например:
...
#### Файлы staging и очистка

Manticore удаляет текущий каталог staging после обычного `COMMIT`, `ROLLBACK`, отключения режима или закрытия сессии. Сбой демона или хоста может оставить после себя заброшенный каталог staging. Более поздняя загрузка напрямую на диск создаст новый каталог с уникальным именем и не будет переиспользовать или прикреплять файлы, оставшиеся после упавшей загрузки.

Используйте следующий запрос, чтобы просмотреть все staging-записи прямой записи на диск для таблицы:
...
Результат рекурсивно перечисляет файлы и каталоги ниже staging-корня прямой записи на диск. Для каталогов и других не-regular записей размер указывается как `0`.

После того как вы убедитесь, что для таблицы не активна загрузка напрямую на диск, удалите весь staging-корень с помощью [`PURGE BULK_IMPORT`](../../Node_info_and_management/PURGE.md#PURGE-BULK_IMPORT):

```sql
PURGE BULK_IMPORT FROM TABLE products;
```

`PURGE` требует существующую локальную таблицу real-time, которая не входит в репликационный кластер. Он удаляет только staging-состояние прямой записи на диск и не меняет схему таблицы или индексированные строки. Если корень staging отсутствует, команда успешно отрабатывает как no-op. В режиме без конфигурации `DROP TABLE` также удаляет staging-корень прямой записи на диск.

#### Текущие ограничения

* Целью должна быть одна существующая, не замороженная локальная таблица real-time, которая не является участником репликационного кластера. Distributed-, sharded-, replicated-, percolate- и plain-таблицы не поддерживаются.
* SQL поддерживает только `INSERT`. Manticore `/bulk` принимает `insert` и `create`; `index`, `replace`, `update` и `delete` отклоняются.
* Каждая строка должна содержать явный числовой ненулевой ID документа. Автогенерируемые и UUID ID документов не поддерживаются.
* Статические сборки не поддерживаются.
* Исполняемый файл для текущей платформы (`indexer` в Linux или `indexer.exe` в Windows) должен находиться в том же каталоге, что и запущенный `searchd`. Manticore Search проверяет только этот соседний путь и не ищет файл в `PATH`. Для совместимости используйте исполняемый файл из той же установки, что и `searchd`. Если файл отсутствует, недоступен для чтения или не запускается, не работает только пакетный импорт; обычный запуск и обычная вставка остаются доступны.
...
Конечная точка `/bulk` поддерживает запросы 'insert', 'replace', 'delete' и 'update'. Имейте в виду, что операции можно направлять в несколько таблиц, но транзакции возможны только для одной таблицы. Если указать больше, Manticore соберет операции, направленные в одну таблицу, в одну транзакцию. При смене таблицы он зафиксирует собранные операции и начнет новую транзакцию. Пустая строка, разделяющая пакеты, также приводит к фиксации предыдущего пакета и началу новой транзакции. Запрос без операций, будь его тело пустым или содержащим только пустые строки либо строки из пробелов, является успешным no-op и возвращает пустой массив `items`.
...
Для реплицируемых таблиц указывайте свойство `cluster` в каждой операции:
...
Для реплицируемых таблиц указывайте имя кластера в имени таблицы в формате `cluster_name:table_name`:
