...
Выполните `SET bulk_import=<table>` до `BEGIN` и до любых незафиксированных операций записи в этом соединении. Команда резервирует выбранную таблицу за текущей сессией: поиск остается доступным, но другой bulk import или обычная запись в ту же таблицу отклоняется до освобождения резервирования. Команды `BEGIN` и `START TRANSACTION` разрешены, но не обязательны и ничего не меняют в этом режиме.

`COMMIT` публикует строки, накопленные после предыдущего `COMMIT` или `ROLLBACK`, как один disk chunk. `ROLLBACK` отбрасывает эти строки. В обоих случаях пакетный импорт остается включенным, и можно начать следующий пакет. `SET bulk_import=0` отбрасывает незафиксированные строки, отключает пакетный импорт и освобождает резервирование этого соединения. Запись в таблицу возобновляется после освобождения резервирования. Закрытие соединения делает то же самое. Пока пакетный импорт включен, изменить autocommit нельзя.
...
Elasticsearch-совместимый endpoint `/_bulk` не поддерживает прямой на диск `bulk_import`; используйте SQL или Manticore `/bulk`. В чистой сессии `?pipeline=bulk_import` и устаревший `?bulk_import=1` отклоняются. Другие неизвестные значения `bulk_import` игнорируются, и обычная обработка Elasticsearch bulk продолжается. Соединение с активным bulk import не может переключиться на `/_bulk`.
