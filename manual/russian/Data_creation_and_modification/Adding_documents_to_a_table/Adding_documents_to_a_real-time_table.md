...
При обычном импорте отправляйте полное тело NDJSON одним запросом, как показано выше. Закройте HTTP-соединение после завершения импорта, чтобы освободить таблицу для других операций записи.
...
Elasticsearch-совместимый эндпоинт `/_bulk` не поддерживает `bulk_import` с прямой записью на диск; используйте SQL или Manticore `/bulk`.
...
Вспомогательный загрузчик использует источник `csvpipe`. По умолчанию его команда — `/bin/cat` в Linux и `-` в Windows, где `-` читает данные из стандартного ввода индексатора. Задайте `INDEXER_RT_BULK_CSV_PIPE_COMMAND` в окружении `searchd`, чтобы переопределить только эту команду. Переопределение не выбирает другой источник или поток данных; команда должна работать с входным потоком данных платформы и передавать CSV-строки индексатору. Неподдерживаемая команда приведет к сбою вспомогательной загрузки.
