...
### Пакетная вставка с помощью indexer

Для больших append-only загрузок в локальную таблицу real-time Manticore Search может прокачивать строки `INSERT` через `indexer`, собирать один disk chunk и прикреплять его к таблице при фиксации транзакции. Это позволяет не проводить каждую строку через обычный путь вставки в real-time. Строки остаются невидимыми, пока chunk не прикреплен, а при ошибке или `ROLLBACK` таблица остается без изменений.

Используйте этот режим, когда нужно загрузить большой пакет и выгоднее сразу создать disk chunk, чем сначала строить RAM chunk. Он поддерживает как построчные, так и колонночные таблицы, включая полнотекстовые поля, числовые атрибуты, строки, JSON, MVA/MVA64 и float-векторы с KNN-индексами.

Имена таблиц для assisted mode приводятся к каноническому нижнему регистру. И SQL-настройка, и параметр запроса Manticore `/bulk` нормализуют целевую таблицу к нижнему регистру перед авторизацией и активацией.
...
`SET indexer_rt_bulk=<table>` требует чистой сессии и получает запись резервации для этой таблицы. Поиск остается доступным, но другие сессии не могут выполнять `INSERT`, `REPLACE`, `UPDATE` или `DELETE` для зарезервированной таблицы. `BEGIN` и `START TRANSACTION` в assisted mode являются необязательными no-op для совместимости. `COMMIT` собирает и атомарно прикрепляет текущий disk chunk; `ROLLBACK` отбрасывает текущий пакет. Оба действия сохраняют режим и резервацию активными для следующего пакета. Автокоммит нельзя менять, пока активен assisted mode. `SET indexer_rt_bulk=0` или закрытие соединения отбрасывает все ожидающие изменения и освобождает резервацию.

#### HTTP `/bulk`

Добавьте `indexer_rt_bulk=<table>` в запрос Manticore `/bulk` или `/json/bulk`. Тело запроса по-прежнему должно быть стандартным NDJSON, где каждая операция должна быть `insert` или `create` для привязанной таблицы:
...
Активирующему запросу не обязательно держать соединение открытым. Одноразовый запрос публикует завершенные группы перед ответом и освобождает режим и резервацию при закрытии соединения. Держите соединение постоянным только тогда, когда последующим запросам нужно унаследовать активную цель или явно отправить `indexer_rt_bulk=0`. HTTP/1.1 по умолчанию является постоянным, а для HTTP/1.0 требуется `Connection: keep-alive`.

Успешный запрос на данные публикует все завершенные группы перед ответом. Пустая строка публикует предыдущую группу, а EOF запроса публикует финальную группу. Ни один неопубликованный пакет не пересекает границу ответа; на постоянном соединении активными могут оставаться только цель и ее резервация. Более поздний запрос `/bulk` или `/json/bulk` без параметров на этом соединении наследует цель. После активации цель сессии является авторитетной; другой параметр запроса с целевым значением не переключает ее, а обычный путь bulk/DML по-прежнему отклоняет операции для другой таблицы. Активный запрос `indexer_rt_bulk=0` проходит через ту же assisted bulk-обработку, а затем отключает режим и освобождает резервацию. Он может содержать финальную группу данных; пустое или состоящее только из пробелов тело является успешным no-op, который только отключает режим.

В примере используется один процесс `curl` с `--next`, чтобы все три передачи могли переиспользовать одно соединение. Отдельные процессы `curl` создают отдельные сессии и не могут наследовать или отключать assisted mode друг друга. Общие правила соединений см. в разделе [Постоянные соединения и состояние HTTP](../../Connecting_to_the_server/HTTP.md#Persistent-connections). Закрытие соединения также освобождает резервацию. Уже активный финальный запрос данных может использовать `Connection: close`; Manticore публикует этот запрос перед завершением.

Assisted responses используют обычную оболочку bulk Manticore и содержат по одному агрегированному элементу `bulk` на каждую опубликованную группу. Конечная точка поддерживает chunked transfer encoding, поэтому может прокачивать тела больше `max_packet_size` через `indexer` без буферизации всего запроса. Неудачный запрос не публикует текущую группу и закрывает HTTP-сессию; группы, уже опубликованные на более ранних обычных границах bulk, например на пустой строке или при смене таблицы, остаются видимыми.

Elasticsearch-совместимый `/_bulk` не поддерживает assisted mode. Значение `indexer_rt_bulk` в чистой сессии `/_bulk` не активирует функцию, а `/_bulk` отклоняется, если assisted mode уже активен на этом соединении. Вывод Elasticsearch в Fluent Bit использует `/_bulk`, поэтому он продолжает работать через обычную вставку в real-time.

#### Дубликаты ID документов

Assisted mode запускает `indexer` с включенным удалением дубликатов. Внутри одного assisted-пакета первая строка для числового ID документа остается видимой, а последующие строки с тем же ID логически удаляются. Для SQL пакет состоит из строк, подготовленных перед `COMMIT` или `ROLLBACK`. Для Manticore `/bulk` каждая группа, опубликованная на пустой строке, при смене таблицы или на EOF запроса, является отдельным пакетом.

ID, который уже виден в целевой таблице, заменяется, когда позже прикрепляется assisted chunk, содержащий этот ID. Это также относится к ID, опубликованному в более ранней HTTP-группе. В результате повторная отправка ранее опубликованного пакета заменяет его строки, а дубликаты внутри повторно отправленного пакета по-прежнему сохраняют первую строку. Assisted-действие `create` ведет себя так же, как `insert`; оно не завершается ошибкой лишь потому, что ID уже существует в целевой таблице.

Поведение с удалением дубликатов доступно и для обычных сборок plain-table через опцию [`indexer --remove_dups`](../../Data_creation_and_modification/Adding_data_from_external_storages/Plain_tables_creation.md#Indexer-command-line-arguments). Без этой опции отдельный `indexer` сохраняет дублирующиеся ID документов.

#### Файлы staging и очистка

Manticore удаляет текущий каталог staging после обычного `COMMIT`, `ROLLBACK`, отключения режима или закрытия сессии. Сбой демона или хоста может оставить после себя заброшенный каталог staging. Более поздняя assisted-загрузка создаст новый каталог с уникальным именем и не будет переиспользовать или прикреплять файлы, оставшиеся после упавшей загрузки.

Используйте следующий запрос, чтобы просмотреть все assisted-staging записи для таблицы:
...
Результат рекурсивно перечисляет файлы и каталоги ниже корня assisted staging таблицы. Для каталогов и других не-regular записей размер указывается как `0`.

После того как вы убедитесь, что для таблицы не активна assisted-загрузка, удалите весь корень assisted staging с помощью [`PURGE INDEXER_RT_BULK`](../../Node_info_and_management/PURGE.md#PURGE-INDEXER_RT_BULK):

```sql
PURGE INDEXER_RT_BULK FROM TABLE products;
```

`PURGE` требует существующую локальную таблицу real-time, которая не входит в репликационный кластер. Он удаляет только состояние assisted staging и не меняет схему таблицы или индексированные строки. Если корень staging отсутствует, команда успешно отрабатывает как no-op. В режиме без конфигурации `DROP TABLE` также удаляет корень assisted staging таблицы.
...
* Статические сборки не поддерживаются. Динамические сборки Linux и Windows находят исполняемый файл `indexer` с именем платформы рядом с запущенным `searchd`; поддержка macOS сейчас не заявлена.
* Исполняемый файл `indexer` должен быть из той же сборки или установки Manticore Search, что и `searchd`. Manticore Search ищет только этот соседний исполняемый файл и не просматривает `PATH`. Если он отсутствует или не запускается, не удается только assisted insertion, а обычный запуск и обычная вставка остаются доступны.
