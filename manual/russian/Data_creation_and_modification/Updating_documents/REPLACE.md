...
`REPLACE` работает аналогично [INSERT](../../Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md), но перед вставкой новой помечает предыдущий документ с тем же ID как удалённый.
...
REPLACE INTO table [(column1, column2, ...)]
    ...
    [, (...)]
```
Колонки, явно не включённые в SQL-запрос, устанавливаются в значения по умолчанию, такие как 0 или пустая строка, в зависимости от их типа данных.
...
REPLACE INTO table
    ...
    WHERE id = <id>
```
Обратите внимание, в этом режиме можно фильтровать только по id.
...
Подробнее о `UPDATE` против частичного `REPLACE` читайте [здесь](../../Data_creation_and_modification/Updating_documents/REPLACE_vs_UPDATE.md#UPDATE-vs-partial-REPLACE).
...
* `/replace`:
  ...
  ```
  `/index` — алиас эндпоинта и работает так же.
...
* Частичная замена:
  ...
  ```
  `<table name>` может быть просто именем таблицы или в формате `cluster:table`. Это позволяет обновлять данные в рамках определённого кластера при необходимости.
...
`REPLACE` доступен для real-time и percolate таблиц. Нельзя заменять данные в простой таблице.

При выполнении `REPLACE` предыдущий документ не удаляется, а просто помечается как удалённый, поэтому размер таблицы растёт до тех пор, пока не произойдёт слияние чанков. Чтобы принудительно выполнить слияние чанков, используйте [оператор OPTIMIZE](../../Securing_and_compacting_a_table/Compacting_a_table.md).

## Bulk replace

<!-- example bulk_replace -->

Вы можете заменить несколько документов одновременно. Подробнее смотрите [bulk добавление документов](../../Data_creation_and_modification/Adding_documents_to_a_table/Adding_documents_to_a_real-time_table.md#Bulk-adding-documents).
