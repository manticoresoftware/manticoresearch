...
Удаление таблицы выполняется в 2 шага внутренне:
1. Таблица очищается (аналогично [TRUNCATE](Emptying_a_table.md))
2. Все файлы таблицы удаляются из папки таблицы. Также удаляются все внешние файлы таблицы, которые использовались таблицей (такие как формы слов, расширения или стоп-слова). Обратите внимание, что эти внешние файлы копируются в папку таблицы, когда используется `CREATE TABLE`, поэтому оригинальные файлы, указанные в `CREATE TABLE`, не будут удалены.

Удаление таблицы возможно только тогда, когда сервер работает в режиме RT. Удалить можно таблицы RT, таблицы PQ и распределенные таблицы.
...
Здесь синтаксис оператора `DROP TABLE` в SQL:
...
При удалении таблицы через SQL, добавление `IF EXISTS` может использоваться для удаления таблицы только в том случае, если она существует. Если вы попытаетесь удалить несуществующую таблицу с опцией `IF EXISTS`, ничего не произойдет.

При удалении таблицы через PHP можно добавить необязательный параметр `silent`, который работает так же, как и `IF EXISTS`.
