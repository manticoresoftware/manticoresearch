...
Начиная с версии 5.0.0, Manticore по умолчанию сохраняет все поля. Когда Manticore используется вместе с MySQL или MariaDB через SphinxSE, сохранение всех полей обычно не имеет смысла, так как оригиналы уже хранятся в MySQL/MariaDB. В таких конфигурациях рекомендуется явно отключить сохранение полей для соответствующей таблицы Manticore, установив:

```
stored_fields =
```

См. справочник по настройкам: [stored_fields](../Creating_a_table/Local_tables/Plain_and_real-time_table_settings.md#stored_fields).

Если оставить значение по умолчанию (все поля сохраняются) и затем выбрать много документов одновременно через SphinxSE, может быть превышен внутренний лимит в движке, и вы получите ошибку вида:
