...
### LENGTH()
`LENGTH(attr_mva)` функция возвращает количество элементов в наборе MVA. Работает как с 32-битными, так и с 64-битными MVA атрибутами. `LENGTH(attr_float_vector)` возвращает количество измерений (чисел с плавающей точкой) в колоночном атрибуте `float_vector`. `LENGTH(attr_json)` возвращает длину поля в JSON. Возвращаемое значение зависит от типа поля. Например, `LENGTH(json_attr.some_int)` всегда возвращает 1, а `LENGTH(json_attr.some_array)` возвращает количество элементов в массиве. `LENGTH(string_expr)` функция возвращает длину строки, полученной в результате выражения.
