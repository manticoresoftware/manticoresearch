...
Доступны только функции, перечисленные на этой странице. Функций `ASIN()`, `ACOS()`, `ATAN()`, `TAN()`, `RADIANS()`, `DEGREES()` и `PI()` нет; вызов любой из них завершится ошибкой разбора, например `P09: syntax error, unexpected '('`. Вместо них используйте эти эквиваленты:
