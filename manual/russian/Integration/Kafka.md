...
Manticore поддерживает интеграцию с [Apache Kafka](https://kafka.apache.org/) для потокового приема данных через источники Kafka и материализованные представления, что позволяет осуществлять индексацию и поиск данных в реальном времени. В настоящее время протестированы и поддерживаются версии **apache/kafka с 3.7.0 по 4.1.0**.
