...
Если имена ваших полей не соответствуют [синтаксису имен таблиц, полей и атрибутов](../Creating_a_table/Data_types.md#Table-and-field-name-syntax), допустимому в Manticore Search (например, если они содержат специальные символы), необходимо определить схему сопоставления. Например, `$keyName` — допустимый ключ в JSON, но невалидное некавычное имя поля в Manticore Search. Если попытаться использовать недопустимые имена полей без корректного сопоставления, Manticore вернет ошибку, и создание источника завершится сбоем.
