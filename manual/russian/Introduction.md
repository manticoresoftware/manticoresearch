...
#### Векторный поиск и автоматические эмбеддинги
Manticore Search может хранить векторы, переданные вашими моделями машинного обучения, или [автоматически генерировать эмбеддинги](Searching/KNN.md#Auto-Embeddings-%28Recommended%29) из текстовых полей с помощью локальных или удаленных моделей эмбеддингов. Затем можно выполнять [поиск ближайших соседей](Searching/KNN.md), чтобы реализовать поиск по сходству, рекомендации, семантический поиск, [диалоговый поиск](Searching/Conversational_search.md) и извлечение данных из текста, изображений, видео или звука.

Для длинных текстов [автоматическое разбиение на фрагменты](Searching/KNN.md#Chunking-strategies) может разделять каждый документ на фрагменты фиксированного размера, рекурсивные фрагменты или фрагменты на основе предложений и сохранять их эмбеддинги в `float_vector_array`. Поиск выполняется по каждому фрагменту независимо, при этом документ появляется в результатах один раз, а его оценка определяется ближайшим фрагментом. Стратегия `mean` вместо этого усредняет эмбеддинги фрагментов в один вектор, а стратегия по умолчанию `truncate` сохраняет прежнее поведение с одним вектором.

#### Диалоговый поиск
Manticore Search поддерживает [диалоговый поиск](Searching/Conversational_search.md) по уже существующим векторизованным таблицам. Он извлекает наиболее релевантные строки с помощью KNN-поиска, использует эти строки вместе с историей диалога как контекст для LLM и может отвечать через SQL `CALL CHAT` или HTTP JSON endpoint `/search`.
...
Manticore поддерживает [таблицы `type='sharding'`](Creating_a_table/Creating_a_sharded_table/Creating_a_sharded_table.md), которые прозрачно распределяют чтение и запись между несколькими физическими шардами на одном узле или в кластере репликации, повышая масштабируемость записи и одновременно упрощая маршрутизацию, отказоустойчивое переключение и эксплуатационное управление.

#### Аутентификация и авторизация
Manticore предоставляет встроенные [аутентификацию и авторизацию](Security/Authentication_and_authorization.md) для MySQL, HTTP/HTTPS, распределённых удалённых агентов и операций, связанных с репликацией, с пользователями, bearer-токенами и детальными правами доступа.
...
- **Диалоговый поиск и поиск с ИИ**:
  - Создавайте сценарии вопросов и ответов, а также ассистентские интерфейсы поверх уже существующего векторизованного контента.
  - Подкрепляйте ответы извлечением через KNN и историей диалога с помощью [диалогового поиска](Searching/Conversational_search.md).
