...
Особенности формата лога SQL в Manticore по сравнению с [plain форматом](../Logging/Query_logging.md#Plain-log-format) включают:
...
<!-- example plain_log -->
С форматом лога `plain` Manticore логирует все успешно выполненные поисковые запросы в простом текстовом формате. Нефуллтекстовые части запросов не логируются. JSON-запросы записываются как однострочные записи. Запросы, обработанные через [Buddy](../Installation/Manticore_Buddy.md), в этом режиме не логируются.
