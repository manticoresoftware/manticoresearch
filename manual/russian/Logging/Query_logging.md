...
Важно отметить, что Manticore ведет журнал не только запросов SELECT, но и операторов изменения данных, таких как UPDATE. Операторы UPDATE записываются в формате SQL-журнала, тогда как операции INSERT, REPLACE и DELETE не логируются. Если вам нужен полный журнал всех операций, может понадобиться дополнительное логирование на уровне приложения.

<!-- intro -->
Примеры записей журнала `sphinxql` для UPDATE:
<!-- request Example -->
```sql
...
где:
* `real-time` — это сквозное время запроса от начала до завершения. В логах запросов этому соответствует поле `real`.
* `wall-time` — это внутренняя метрика Manticore для wall-time запроса. В логах запросов этому соответствует поле `wall`, и именно это же значение используется `query_log_min_msec`. Для распределённых и многоисточниковых запросов `wall-time` может отличаться от `real-time`.
...
В отличие от формата SQL-журнала, обычный формат журнала записывает только поисковые запросы (операторы SELECT). Операторы изменения данных, такие как INSERT, REPLACE, DELETE и UPDATE, в этом формате не логируются. Операторы UPDATE записываются в формате SQL, но не в обычном формате.
