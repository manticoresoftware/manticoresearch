...
<!-- example status -->
Самый простой способ получить обзорную информацию о вашем узле Manticore — выполнить `status` в клиенте MySQL. Он отобразит информацию о различных аспектах, таких как:
* Текущая версия
* Активен ли SSL
...
* Количество задач, обрабатываемых в настоящее время
* Количество запросов, выполненных с момента запуска
...
`SHOW STATUS` — это SQL-запрос, который отображает различные полезные счетчики производительности. Счетчики IO и CPU будут доступны только если `searchd` был запущен с ключами `--iostats` и `--cpustats` соответственно (или если они были включены через `SET GLOBAL iostats/cpustats=1`).
...
Поддерживается опциональный оператор `LIKE`, позволяющий выбрать только переменные, совпадающие с определенным шаблоном. Синтаксис шаблона соответствует стандартным SQL-символам подстановки, где `%` обозначает любое количество любых символов, а `_` — один любой символ.
...
Команда `SHOW STATUS` предоставляет подробный отчет по различным метрикам производительности в Manticore, включая статистику времени запросов для insert/replace, search и update запросов. Эти данные рассчитываются за скользящие окна 1, 5 и 15 минут, показывая среднее, минимальное, максимальное и 95-й/99-й процентиль по времени выполнения запросов.

Эти метрики помогают отслеживать производительность за определённые промежутки времени, облегчая выявление тенденций в скорости ответа на запросы и возможных узких мест.

Следующие метрики включены в вывод `SHOW STATUS`:
- `*_avg`: Среднее время запроса каждого типа за последние 1, 5 и 15 минут.
- `*_min`: Кратчайшее зафиксированное время запроса для каждого типа.
- `*_max`: Самое длинное время запроса для каждого типа.
- `*_pct95`: Время, за которое выполняются 95% запросов.
- `*_pct99`: Время, за которое выполняются 99% запросов.

Эти статистики приведены отдельно для insert/replace (`insert_replace_stats_*`), search (`search_stats_*`) и update (`update_stats_*`) запросов, предоставляя детальный анализ производительности разных операций.

Если во время мониторинга не было выполнено запросов, будет отображено `N/A`.
...
`SHOW AGENT STATUS` отображает статистику [удаленных агентов](../Creating_a_table/Creating_a_distributed_table/Remote_tables.md#agent) или распределенной таблицы. Она включает такие показатели, как возраст последнего запроса, последнего ответа, количество различных видов ошибок и успешных операций и т.д. Статистика выводится для каждого агента за последние интервалы в 1, 5 и 15, каждый из которых состоит из секунд, заданных параметром [ha_period_karma](../Server_settings/Searchd.md#ha_period_karma).
...
Поддерживается опциональный оператор `LIKE`, с тем же синтаксисом, что и в `SHOW STATUS`.
