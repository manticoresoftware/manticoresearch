...
* `total_found`:
  ...
  - При использовании `HAVING` с `GROUP BY`, `total_found` отражает количество групп **после** применения фильтра `HAVING` (начиная с версии 13.15.3). Это позволяет корректно осуществлять постраничный вывод с условиями `HAVING`.
