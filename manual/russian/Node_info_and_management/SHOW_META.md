...
* `total_found`:
  ...
  - При использовании `HAVING` с `GROUP BY`, `total_found` отражает количество групп **после** применения фильтра `HAVING`. Это обеспечивает корректную пагинацию с использованием условий `HAVING`.
