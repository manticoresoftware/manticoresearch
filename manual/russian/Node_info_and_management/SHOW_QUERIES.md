...
`SHOW QUERIES` возвращает информацию обо всех текущих выполняющихся запросах. Вывод представляет собой таблицу следующей структуры:
...
- `protocol`: [Протокол соединения](../Server_settings/Searchd.md#listen), возможные значения: `sphinx`, `mysql`, `http`, `ssl`, `compressed`, `replication` или их сочетания (например, `http,ssl` или `compressed,mysql`)
