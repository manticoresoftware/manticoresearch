...
`SHOW VERSION` предоставляет подробную информацию о версиях различных компонентов экземпляра Manticore Search. Эта команда особенно полезна администраторам и разработчикам, которым необходимо проверить версию запускаемого Manticore Search, а также версии его связанных компонентов.

Вывод содержит таблицу с двумя столбцами:
- `Component`: этот столбец содержит название конкретного компонента Manticore Search.
