...
* `tid` и `tid_saved`: представляют состояние сохранения таблицы. `tid` увеличивается с каждой изменением (транзакцией). `tid_saved` показывает максимальный `tid` состояния, сохраненного в RAM части файла `<table>.ram`. Когда значения различаются, некоторые изменения существуют только в RAM и также поддерживаются binlog (если включен). Выполнение `FLUSH TABLE` или плановое периодическое сбрасывание сохраняет эти изменения. После сброса binlog очищается, и `tid_saved` представляет новое актуальное состояние.
* `query_time_*`, `exact_query_time_*`: статистика времени выполнения запросов за последние 1 минуту, 5 минут, 15 минут и с момента запуска сервера; данные представлены в виде объекта JSON, включая количество запросов и значения минимального, максимального, среднего, 95-го и 99-го перцентилей.
* `found_rows_*`: статистика найденных строк по запросам; представлена за последние 1 минуту, 5 минут, 15 минут и с момента запуска сервера; данные представлены в виде объекта JSON, включая количество запросов и значения минимального, максимального, среднего, 95-го и 99-го перцентилей.
* `command_*`: счетчики общего количества успешных выполнений определенной команды для этой таблицы.
* `search_stats_ms_*`: статистика времени выполнения (в миллисекундах) поисковых запросов. Символ * указывает временной интервал (например, 1min, 5min, 15min, total). Эти статистики рассчитываются по скользящим окнам 1, 5 и 15 минут, показывая средние, минимальные, максимальные и 95-й/99-й перцентили значений времени запросов.
* `insert_replace_stats_ms_*`: статистика времени выполнения (в миллисекундах) операций вставки и замены. Символ * указывает временной интервал (например, 1min, 5min, 15min, total). Эти статистики рассчитываются по скользящим окнам 1, 5 и 15 минут, показывая средние, минимальные, максимальные и 95-й/99-й перцентили значений времени запросов.
* `update_stats_ms_*`: статистика времени выполнения (в миллисекундах) обновлений. Символ * указывает временной интервал (например, 1min, 5min, 15min, total). Эти статистики рассчитываются по скользящим окнам 1, 5 и 15 минут, показывая средние, минимальные, максимальные и 95-й/99-й перцентили значений времени запросов.
