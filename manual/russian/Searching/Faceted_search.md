...
* необязательный параметр `mode` для каждой агрегации переопределяет унаследованный режим для этой агрегации. Поддерживаемые значения: `strict`, `auto` и `max`. `filter_mode` сохраняется как обратно совместимый псевдоним.
...
* наборы результатов фасетов `auto` и `max` могут включать маркер группы `status`. Возвращаемые значения: `available` и `unavailable`.
...
- `auto`
  ...
  - добавить маркер `status`; возвращаемые корзины имеют статус `available`
- `max`
...
- `auto`
  ...
  - и вернуть корзины цвета со `status=available`
- `max`
...
В режимах `auto` и `max` результаты SQL фасета добавляют столбец `status`. `available` означает, что корзина в настоящее время доступна для выбора, включая значения, уже выбранные в основном запросе. В режиме `max` `unavailable` означает, что корзина существует в широкой области подсчета, но недоступна в строгом/текущем наборе результатов. `max` — самый затратный режим, поэтому на больших наборах данных или запросах с большим количеством фасетов вы должны включать его только тогда, когда вам нужны широкие корзины с недоступными значениями. Например, при `size='small'` и `facet_filter_mode='max'`, результат `FACET size` может выглядеть так:
...
В режимах `auto` и `max` агрегации корзин JSON с поддержкой статуса включают поле `status`. Как и в SQL, `available` означает, что корзина в настоящее время доступна для выбора, включая значения, уже выбранные в основном запросе. В режиме `max` `unavailable` означает, что корзина существует в широкой области подсчета, но недоступна в строгом/текущем наборе результатов. `max` — самый затратный режим фасета, поэтому используйте его с осторожностью на больших наборах данных или когда запрашивается много фасетов:
...
Примечания:
- в SQL `MODE` переопределяет `facet_filter_mode` на уровне запроса для одного фасета
- в JSON `mode` или `filter_mode` переопределяет `facet_filter_mode` верхнего уровня для одной агрегации
- выбранные значения сообщаются как `status=available`, чтобы клиенты интерфейса могли рассматривать их как выбираемые значения.
