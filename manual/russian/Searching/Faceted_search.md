...
* наборы результатов фасета могут включать маркер группы `status`. Значения: `selected`, `available` и `unavailable`.
...
Встроенные режимы:
- `strict`
  - применяет все фильтры из основного запроса
- `auto`
  - применяет все фильтры из основного запроса, кроме фильтров на этом же фасете
- `max`
  - то же, что и `auto`, но также возвращает недоступные корзины с `status`
...
- `auto` = применить всё, кроме собственных фильтров этого фасета
- `max` = `auto` + недоступные корзины
- `filters` = применить только эти фильтры
- `exclude_filters` = применить всё, кроме этих фильтров

Примечание о производительности:
- `max` — самый затратный режим фасета, потому что он должен обнаруживать и помечать недоступные корзины в дополнение к обычным подсчетам фасетов
- на больших наборах данных или запросах со многими фасетами `max` может быть намного медленнее, чем `strict` или `auto`
- по умолчанию предпочитайте `strict` или `auto` и включайте `max` только тогда, когда интерфейсу действительно нужны недоступные корзины
...
- `max`
  - применяет `brand + size`
  - и также возвращает недоступные корзины цвета с `status`
- `filters=["brand"]`
...
В этом примере:
- `FACET color_id` использует фильтр `size_id=42`, но не `color_id=1`
- `FACET size_id` использует фильтр `color_id=1`, но не `size_id=42`
- в режиме `max` возвращаемые корзины также включают значения `status`
...
В режиме `max` результаты SQL-фасета добавляют столбец `status`. Этот режим также является самым затратным, поэтому на больших наборах данных или запросах с большим количеством фасетов вы должны включать его только тогда, когда вам нужны недоступные корзины в ответе. Например, с `size='small'` и `facet_filter_mode='max'`, результат `FACET size` может выглядеть так:
...
В режиме `max` корзины JSON-фасета включают поле `status`. Как и в SQL, это самый затратный режим фасета, поэтому используйте его с осторожностью на больших наборах данных или когда запрашивается много фасетов:
...
- `status=selected` в настоящее время выводится из явных фильтров значений, таких как `=` и `IN` для поля фасета.
- неподдерживаемые фильтры для того же поля, такие как диапазоны, в настоящее время не помечают корзины как `selected`.
