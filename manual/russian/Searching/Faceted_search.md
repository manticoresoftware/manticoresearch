...
* необязательный параметр верхнего уровня `facet_filter_mode` управляет тем, как все агрегации наследуют фильтры из основного запроса. Поддерживаемые значения: `strict`, `auto` и `max`.
* необязательный параметр `filter_mode` для каждой агрегации переопределяет унаследованный режим для этой агрегации. Поддерживаемые значения: `strict`, `auto` и `max`. В SQL ему соответствует ключевое слово `MODE` для отдельного фасета.
* необязательный параметр `filters` для каждой агрегации явно перечисляет, какие фильтры атрибутов основного запроса должны быть применены к этой агрегации.
* необязательный параметр `exclude_filters` для каждой агрегации явно перечисляет, какие фильтры атрибутов основного запроса не должны применяться к этой агрегации.
* необязательный параметр `zeroes` для каждой агрегации включает корзины с нулевым количеством в режиме `max`. В SQL ему соответствует ключевое слово `ZEROES` для отдельного фасета.
...
- `all filters` (только SQL) = применить всё
- `filters` = применить только эти фильтры
- `exclude_filters` = применить всё, кроме этих фильтров
...
Предложения для каждого фасета означают:
...
- `ZEROES` — в SQL-режиме `max` сохраняет корзины из более широкой области `max`, даже если видимый счетчик фасета равен `0`; в JSON ему соответствует ключ `"zeroes": true` для отдельной агрегации

Имена в SQL и JSON немного различаются:
- SQL использует опцию запроса `facet_filter_mode`, ключевое слово `MODE` для отдельного фасета и предложения `FILTERS` / `EXCLUDE FILTERS`
- JSON использует ключ верхнего уровня `facet_filter_mode`, ключи агрегации `filter_mode` / `zeroes` и ключи `filters` / `exclude_filters`

Эти предложения переопределяют область фильтрации, которая в противном случае исходила бы от `facet_filter_mode` или `MODE`. Например, `FACET color_id ALL FILTERS MODE max` все равно выдает `status`, но его подсчеты используют все фильтры основного запроса вместо широкой области по умолчанию `max`.
...
Примечания:
- в SQL `MODE` переопределяет `facet_filter_mode` на уровне запроса для одного фасета, а `ZEROES` сохраняет для него нулевые корзины в режиме `max`
- в JSON `filter_mode` переопределяет `facet_filter_mode` верхнего уровня для одной агрегации, а `zeroes: true` включает для нее то же поведение с нулевыми корзинами в режиме `max`
