...
Фасетный поиск так же важен для современного поискового приложения, как и [автозаполнение](../Searching/Autocomplete.md), [коррекция ошибок](../Searching/Spell_correction.md) и [выделение](../Searching/Highlighting.md) ключевых слов, особенно в продуктах электронной коммерции.

![Фасетный поиск](faceted.png)

Фасетный поиск оказывается полезным при работе с большими объемами данных и различными взаимосвязанными свойствами, такими как размер, цвет, производитель или другие факторы. При запросе огромных объемов данных результаты поиска часто включают множество записей, не соответствующих ожиданиям пользователя. Фасетный поиск позволяет конечному пользователю четко определить критерии, которым должны соответствовать результаты его поиска.

В Manticore Search есть оптимизация, которая сохраняет набор результатов из исходного запроса и повторно использует его для каждого вычисления фасета. Поскольку агрегирования применяются к уже вычисленному подмножеству документов, они выполняются быстро, и общее время выполнения часто может быть лишь немного длиннее, чем изначальный запрос. Фасеты могут быть добавлены к любому запросу, и фасетой может быть любое свойство или выражение. Результат фасета включает значения фасета и количество фасетов. Фасеты можно получить с помощью SQL-запроса `SELECT`, объявив их в самом конце запроса.

## Агрегации

<!-- example Aggregations -->
### SQL
Значения фасетов могут происходить из атрибута, свойства JSON в атрибуте JSON или выражения. Значения фасетов также могут иметь алиасы, но **алиас должен быть уникальным** для всех наборов результатов (набор основной выборки и наборы других фасетов). Значение фасета извлекается из агрегированного атрибута/выражения, но оно также может происходить из другого атрибута/выражения.
...
где:
* `group name` — это алиас, присвоенный агрегации
* значение `field` должно содержать имя атрибута или выражения, для которого создается фасет
* необязательный `size` указывает максимальное количество ведер (buckets), которые следует включить в результат. Если не указано, оно наследует лимит основного запроса. Более подробную информацию можно найти в разделе [Размер результата фасета](../Searching/Faceted_search.md#Size-of-facet-result).
* необязательный `sort` определяет массив атрибутов и/или дополнительных свойств, используя тот же синтаксис, что и ["параметр sort в основном запросе](../Searching/Sorting_and_ranking.md#Sorting-via-JSON).

Набор результатов будет содержать узел `aggregations` с возвращенными фасетами, где `key` — это агрегированное значение, а `doc_count` — это счетчик агрегаций.
...
{'aggregations': {u'group_brand_id': {u'buckets': [{u'doc_count': 1019,
 ...
 'hits': {'hits': [{u'_id': u'1',
                    ...
                    u'_source': {u'brand_id': 10,
                                 u'brand_name': u'Бренд Десять',
                                 u'категории': [10],
                                 u'цена': 197,
                                 u'свойство': u'Шесть',
                                 u'название': u'Продукт Восемь Один'}},
                   {u'_id': u'2',
                    u'_score': 1,
                    u'_source': {u'brand_id': 6,
                                 u'brand_name': u'Бренд Шесть',
                                 u'категории': [12, 13, 14],
                                 u'цена': 671,
                                 u'свойство': u'Четыре',
                                 u'название': u'Продукт Девять Семь'}},
                   {u'_id': u'3',
                    u'_score': 1,
                    u'_source': {u'brand_id': 3,
                                 u'brand_name': u'Бренд Три',
                                 u'категории': [13, 14, 15],
                                 u'цена': 92,
                                 u'свойство': u'Шесть',
                                 u'название': u'Продукт Пять Четыре'}},
                   {u'_id': u'4',
                    u'_score': 1,
                    u'_source': {u'brand_id': 10,
                                 u'brand_name': u'Бренд Десять',
                                 ...
                                 u'цена': 713,
                                 u'свойство': u'Пять',
                                 u'название': u'Продукт Восемь Девять'}},
                   {u'_id': u'5',
                    u'_score': 1,
                    u'_source': {u'brand_id': 7,
                                 u'brand_name': u'Бренд Семь',
                                 u'категории': [11, 12, 13],
                                 u'цена': 805,
                                 u'свойство': u'Два',
                                 u'название': u'Продукт Десять Три'}}],
          'max_score': None,
          'total': 10000},
 'профиль': None,
 'время_истечения': False,
 'время': 4}
...
```python
res = await searchApi.search({"table":"facetdemo","query":{"match_all":{}},"limit":5,"aggs":{"group_property":{"terms":{"field":"цена",}},"group_brand_id":{"terms":{"field":"brand_id"}}}})
```
...
class SearchResponse {
    ...
    hits: class SearchResponseHits {
        maxScore: null
        total: 10000
        hits: [{_id=1, _score=1, _source={price=197, brand_id=10, brand_name=Бренд Десять, categories=[10], title=Продукт Восемь Один, property=Шесть}}, {_id=2, _score=1, _source={price=671, brand_id=6, brand_name=Бренд Шесть, categories=[12, 13, 14], title=Продукт Девять Семь, property=Четыре}}, {_id=3, _score=1, _source={price=92, brand_id=3, brand_name=Бренд Три, categories=[13, 14, 15], title=Продукт Пять Четыре, property=Шесть}}, {_id=4, _score=1, _source={price=713, brand_id=10, brand_name=Бренд Десять, categories=[11], title=Продукт Восемь Девять, property=Пять}}, {_id=5, _score=1, _source={price=805, brand_id=7, brand_name=Бренд Семь, categories=[11, 12, 13], title=Продукт Десять Три, property=Два}}]
    }
    profile: null
}
```
...
Данные могут быть агрегированы по другой атрибут или выражению. Например, если документы содержат как идентификатор бренда, так и имя, мы можем вернуть в фасете имена брендов, но агрегировать идентификаторы брендов. Это можно сделать с помощью `FACET {expr1} BY {expr2}`
...
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+
|    1 |   306 |        1 | Product Ten Three   | Бренд Один  | Шесть_Десять| {"prop1":66,"prop2":91,"prop3":"Один"} | 10,11      |
|    2 |   400 |       10 | Product Three One   | Бренд Десять| Четыре_Три  | {"prop1":69,"prop2":19,"prop3":"Один"} | 13,14      |
....
|   19 |   855 |        1 | Product Seven Two   | Бренд Один  | Восемь_Семь| {"prop1":63,"prop2":78,"prop3":"Один"} | 10,11,12   |
|   20 |    31 |        9 | Product Four One    | Бренд Девять| Десять_Четыре| {"prop1":79,"prop2":42,"prop3":"Один"} | 12,13,14   |
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+
...
Если вам нужно удалить дубликаты из корзин, возвращаемых FACET, вы можете использовать `DISTINCT field_name`, где `field_name` – это поле, по которому вы хотите выполнить дедупликацию. Это также может быть `id` (что является значением по умолчанию), если вы делаете запрос FACET к распределенной таблице и не уверены, есть ли у вас уникальные id в таблицах (таблицы должны быть локальными и иметь одинаковую схему).

Если у вас есть несколько объявлений FACET в вашем запросе, `field_name` должен быть одинаковым во всех из них.

`DISTINCT` возвращает дополнительный столбец `count(distinct ...)` перед столбцом `count(*)`, позволяя вам получить оба результата без необходимости делать еще один запрос.
...
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+-------------+
|    1 |   306 |        1 | Продукт Десять Три | Бренд Один   | Шесть_Десять | {"prop1":66,"prop2":91,"prop3":"Один"} | 10,11      |           1 |
...
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+-------------+
20 строк в наборе (0.00 сек)
...
<!-- response Javascript -->
```javascript
{"took":0,"timed_out":false,"hits":{"total":10000,"hits":[{"_id": 1,"_score":1,"_source":{"price":197,"brand_id":10,"brand_name":"Бренд Десять","categories":[10],"title":"Продукт Восемь Один","property":"Шесть","price_range":0}},{"_id": 2,"_score":1,"_source":{"price":671,"brand_id":6,"brand_name":"Бренд Шесть","categories":[12,13,14],"title":"Продукт Девять Семь","property":"Четыре","price_range":3}},{"_id": 3,"_score":1,"_source":{"price":92,"brand_id":3,"brand_name":"Бренд Три","categories":[13,14,15],"title":"Продукт Пять Четыре","property":"Шесть","price_range":0}},{"_id": 4,"_score":1,"_source":{"price":713,"brand_id":10,"brand_name":"Бренд Десять","categories":[11],"title":"Продукт Восемь Девять","property":"Пять","price_range":3}},{"_id": 5,"_score":1,"_source":{"price":805,"brand_id":7,"brand_name":"Бренд Семь","categories":[11,12,13],"title":"Продукт Десять Три","property":"Два","price_range":4}},{"_id": 6,"_score":1,"_source":{"price":420,"brand_id":2,"brand_name":"Бренд Два","categories":[10,11],"title":"Продукт Два Один","property":"Шесть","price_range":2}},{"_id": 7,"_score":1,"_source":{"price":412,"brand_id":9,"brand_name":"Бренд Девять","categories":[10],"title":"Продукт Четыре Девять","property":"Восемь","price_range":2}},{"_id": 8,"_score":1,"_source":{"price":300,"brand_id":9,"brand_name":"Бренд Девять","categories":[13,14,15],"title":"Продукт Восемь Четыре","property":"Пять","price_range":1}},{"_id": 9,"_score":1,"_source":{"price":728,"brand_id":1,"brand_name":"Бренд Один","categories":[11],"title":"Продукт Девять Шесть","property":"Четыре","price_range":3}},{"_id": 10,"_score":1,"_source":{"price":622,"brand_id":3,"brand_name":"Бренд Три","categories":[10,11],"title":"Продукт Шесть Семь","property":"Два","price_range":3}},{"_id": 11,"_score":1,"_source":{"price":462,"brand_id":5,"brand_name":"Бренд Пять","categories":[10,11],"title":"Продукт Десять Два","property":"Восемь","price_range":2}},{"_id": 12,"_score":1,"_source":{"price":939,"brand_id":7,"brand_name":"Бренд Семь","categories":[12,13],"title":"Продукт Девять Семь","property":"Шесть","price_range":4}},{"_id": 13,"_score":1,"_source":{"price":948,"brand_id":8,"brand_name":"Бренд Восемь","categories":[12],"title":"Продукт Десять Один","property":"Шесть","price_range":4}},{"_id": 14,"_score":1,"_source":{"price":900,"brand_id":9,"brand_name":"Бренд Девять","categories":[12,13,14],"title":"Продукт Десять Девять","property":"Три","price_range":4}},{"_id": 15,"_score":1,"_source":{"price":224,"brand_id":3,"brand_name":"Бренд Три","categories":[13],"title":"Продукт Два Шесть","property":"Четыре","price_range":1}},{"_id": 16,"_score":1,"_source":{"price":713,"brand_id":10,"brand_name":"Бренд Десять","categories":[12],"title":"Продукт Два Четыре","property":"Шесть","price_range":3}},{"_id": 17,"_score":1,"_source":{"price":510,"brand_id":2,"brand_name":"Бренд Два","categories":[10],"title":"Продукт Десять Два","property":"Семь","price_range":2}},{"_id": 18,"_score":1,"_source":{"price":702,"brand_id":10,"brand_name":"Бренд Десять","categories":[12,13],"title":"Продукт Девять Один","property":"Три","price_range":3}},{"_id": 19,"_score":1,"_source":{"price":836,"brand_id":4,"brand_name":"Бренд Четыре","categories":[10,11,12],"title":"Продукт Четыре Пять","property":"Два","price_range":4}},{"_id": 20,"_score":1,"_source":{"price":227,"brand_id":3,"brand_name":"Бренд Три","categories":[12,13],"title":"Продукт Три Четыре","property":"Десять","price_range":1}}]}}
...
class SearchResponse {
    ...
    hits: class SearchResponseHits {
        ...
        hits: [{_id=1, _score=1, _source={price=197, brand_id=10, brand_name=Бренд Десять, categories=[10], title=Продукт Восемь Один, property=Шесть, price_range=0}}, {_id=2, _score=1, _source={price=671, brand_id=6, brand_name=Бренд Шесть, categories=[12, 13, 14], title=Продукт Девять Семь, property=Четыре, price_range=3}}, {_id=3, _score=1, _source={price=92, brand_id=3, brand_name=Бренд Три, categories=[13, 14, 15], title=Продукт Пять Четыре, property=Шесть, price_range=0}}, {_id=4, _score=1, _source={price=713, brand_id=10, brand_name=Бренд Десять, categories=[11], title=Продукт Восемь Девять, property=Пять, price_range=3}}, {_id=5, _score=1, _source={price=805, brand_id=7, brand_name=Бренд Семь, categories=[11, 12, 13], title=Продукт Десять Три, property=Два, price_range=4}}]
...
class SearchResponse {
    ...
    timedOut: false
```
    агрегации: {group_property={buckets=[{key=4, doc_count=2100}, {key=3, doc_count=1973}, {key=2, doc_count=1999}, {key=1, doc_count=2043}, {key=0, doc_count=1885}]}}
    хиты: класс SearchResponseHits {
        maxScore: null
        total: 10000
...
<!-- пример Многоуровневый -->

### Фасет по многоуровневой группировке

Фасеты могут агрегировать по многоуровневой группировке, при этом результирующий набор будет таким же, как если бы запрос выполнил многоуровневую группировку:
...
### Фасет по множеству диапазонов

Фасеты могут агрегировать по множеству диапазонов. Значения проверяются относительно диапазона ведра, где каждое ведро включает значение `from` и исключает значение `to` из диапазона.
Установка свойства `keyed` в `true` делает ответ словарём с ключами ведер, а не массивом.
...
### Фасет по множеству диапазонов дат

Фасеты могут агрегировать по множеству диапазонов дат, что аналогично обычному диапазону. Разница заключается в том, что значения `from` и `to` могут быть выражены в выражениях [Date math](../Functions/Date_and_time_functions.md#Date-math). Эта агрегация включает значение `from` и исключает значение `to` для каждого диапазона. Установка свойства `keyed` в `true` делает ответ словарём с ключами ведер, а не массивом.
...
<!-- example Ordering -->
### Упорядочивание в результате фасета

Фасеты поддерживают оператор `ORDER BY` так же, как стандартный запрос. Каждый фасет может иметь свое собственное упорядочение, и упорядочение фасета не влияет на упорядочение основного набора результатов, которое определяется оператором `ORDER BY` основного запроса. Сортировка может выполняться по имени атрибута, количеству (с использованием `COUNT(*)`, `COUNT(DISTINCT attribute_name)`), или специальной функции `FACET()`, которая предоставляет агрегированные значения данных. По умолчанию запрос с `ORDER BY COUNT(*)` будет сортироваться в порядке убывания.
...
| id   | price | brand_id | title               | brand_name  | property    | j                                     | categories |
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+
|    1 |   306 |        1 | Продукт Десять Три | Бренд Один  | Шесть_Десять| {"prop1":66,"prop2":91,"prop3":"One"} | 10,11      |
...
|   20 |    31 |        9 | Продукт Четыре Один | Бренд Девять| Десять_Четыре| {"prop1":79,"prop2":42,"prop3":"One"} | 12,13,14   |
+------+-------+----------+---------------------+-------------+-------------+---------------------------------------+------------+
...
По умолчанию каждый набор результатов фасета ограничен 20 значениями. Количество значений фасета можно контролировать с помощью оператора `LIMIT` индивидуально для каждого фасета, предоставляя либо количество значений для возврата в формате `LIMIT count`, либо с использованием смещения в формате `LIMIT offset, count`.

Максимальное количество значений фасета, которое может быть возвращено, ограничено настройкой `max_matches` запроса. Если вы хотите реализовать динамическое значение `max_matches` (ограничивая `max_matches` смещением + на страницу для повышения производительности), необходимо учитывать, что слишком низкое значение `max_matches` может повлиять на количество значений фасета. В этом случае следует использовать минимальное значение `max_matches`, которое достаточно для покрытия количества значений фасета.
 ...
 'hits': {'hits': [{u'_id': u'1',
                   ...
                   {u'_id': u'4',
                    ...
                    u'_source': {u'brand_id': 10,
                                 ...
                                 u'price': 713,
                                 u'property': u'Пять',
                                 u'title': u'Продукт Восемь Девять'}},
                   {u'_id': u'5',
                    u'_score': 1,
                    u'_source': {u'brand_id': 7,
                                 u'brand_name': u'Бренд Семь',
...
Когда вы используете SQL, поиск с фасетами возвращает несколько наборов результатов. Клиент/библиотека/коннектор MySQL, используемый **должен** поддерживать несколько наборов результатов для доступа к наборам результатов фасетов.

<!-- example Performance -->
### Performance

Внутри `FACET` - это сокращение для выполнения многозапроса, в котором первый запрос содержит основной поисковый запрос, а остальные запросы в пакете имеют каждую кластеризацию. Как и в случае многозапроса, общая оптимизация запроса может включаться для фасетного поиска, что означает, что поисковый запрос выполняется только один раз, а фасеты работают на результате поискового запроса, при этом каждый фасет добавляет лишь небольшую долю времени к общему времени запроса.


Чтобы проверить, выполнялся ли фасетный поиск в оптимизированном режиме, вы можете посмотреть в [журнал запросов](../Logging/Query_logging.md), где все зарегистрированные запросы будут содержать строку `xN`, где `N` - это количество запросов, которые выполнялись в оптимизированной группе. Альтернативно, вы можете проверить вывод оператора [SHOW META](../Node_info_and_management/SHOW_META.md), который отобразит метрику `multiplier`:
