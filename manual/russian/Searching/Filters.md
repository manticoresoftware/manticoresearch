...
Условие сравнивает атрибут или псевдоним выражения со значением. Арифметическое выражение или функцию, например `GEODIST()`, нельзя записать непосредственно в `WHERE`: `WHERE price*2 > 100` завершится ошибкой `P01: syntax error`. Вычислите выражение в списке выборки, задайте ему псевдоним и фильтруйте по этому псевдониму:

```sql
SELECT id, price*2 AS double_price FROM products WHERE double_price > 100;
```

Сравнение строк выполняется с учетом [сортировки](../Searching/Collations.md), по умолчанию это `libc_ci` (без учета регистра), поэтому `WHERE city = 'london'` также найдет `London`.
