...
### should
Запросы и фильтры, указанные в разделе `should`, должны совпадать с документами. Если в `must` или `must_not` указаны какие-либо запросы, запросы в `should` по умолчанию являются необязательными. С другой стороны, если нет ничего, кроме `should`, то хотя бы один из этих запросов должен совпасть с документом, чтобы он подошел под bool-запрос. Это эквивалент запросов `OR`. Чтобы потребовать совпадение определенного числа условий `should`, используйте [`minimum_should_match`](../Searching/Filters.md#minimum_should_match). Обратите внимание: если вы хотите сопоставить массив ([многозначный атрибут](../Creating_a_table/Data_types.md#Multi-value-integer-%28MVA%29)), можно указать атрибут несколько раз, например:
...
`minimum_should_match` управляет тем, сколько условий из `should` должно совпасть. Его можно использовать с полнотекстовыми запросами, фильтрами по атрибутам или с их комбинацией в JSON `bool`-запросе.

Значение может быть:

* целым числом, например `2`
* отрицательным целым числом, например `"-1"`, что означает, что должны совпасть все условия `should`, кроме одного
* процентом, например `"67%"`
* отрицательным процентом, например `"-25%"`, что означает, что должны совпасть все условия `should`, кроме 25%
* условным выражением, таким как `"2<-25% 9<-3"`, которое применяет правило после `<` только когда число условий `should` больше числа перед `<`

Например, следующий запрос возвращает документы, которые совпадают с любыми двумя из трех условий `should`: точной фразой `yellow dog`, точной фразой `hello world` или словом `tropical`.
...
Если присутствует `must` или `must_not`, `should` по умолчанию остается необязательным. Установка положительного значения `minimum_should_match` делает обязательным такое число условий `should` в дополнение к обязательным условиям `must` и исключающим условиям `must_not`. Если установить `minimum_should_match` в `0`, все условия `should` станут необязательными.
