...
Клауза `MATCH` позволяет выполнять полнотекстовый поиск в текстовых полях. Входная строка запроса [токенизируется](../../Creating_a_table/NLP_and_tokenization/Data_tokenization.md) с использованием тех же настроек, которые применяются к тексту при индексировании. В дополнение к токенизации входного текста, строка запроса поддерживает ряд [операторов полнотекстового поиска](../../Searching/Full_text_matching/Operators.md), которые накладывают различные правила на то, как ключевые слова должны обеспечивать допустимое совпадение.

Клаузы полнотекстового поиска могут комбинироваться с [фильтрами](../../Searching/Filters.md) атрибутов в логическом И (AND). **Отношения ИЛИ (OR) между полнотекстовыми совпадениями и фильтрами атрибутов не поддерживаются**.

Запросы с условием match всегда выполняются первыми в процессе фильтрации, за ними следуют [фильтры атрибутов](../../Searching/Filters.md). Фильтры атрибутов применяются к множеству результатов полнотекстового запроса. Запрос без клаузы match называется fullscan.

В `SELECT` может быть не более одного `MATCH()`.

Используя [синтаксис полнотекстового запроса](../../Searching/Full_text_matching/Operators.md), поиск выполняется по всем индексированным текстовым полям документа, если выражение не требует поиска в конкретном поле (например, фразовый поиск) или не ограничено операторами поля.
...
Это позволяет выполнять полнотекстовый поиск на конкретных таблицах в операции объединения. Подробнее о применении MATCH с JOIN см. в разделе [Объединение таблиц](../../Searching/Joining.md).
...
```
- `'search query'`: строка полнотекстового поиска, которая может включать различные [операторы полнотекстового поиска](../../Searching/Full_text_matching/Operators.md).
- `table_name`: (необязательно) имя таблицы, к которой применяется полнотекстовый поиск, используется в JOIN-запросах для указания таблицы, отличной от левой по умолчанию.


В операторе [SELECT](../../Searching/Full_text_matching/Basic_usage.md#SQL) используется клауза [MATCH](../../Searching/Full_text_matching/Basic_usage.md), которая должна следовать после WHERE для выполнения полнотекстового поиска. `MATCH()` принимает входную строку, в которой доступны все [операторы полнотекстового поиска](../../Searching/Full_text_matching/Operators.md).
...
Полнотекстовый поиск доступен в эндпоинте `/search` и в HTTP-клиентах. Для выполнения полнотекстовых запросов можно использовать следующие конструкции:
...
```
По умолчанию ключевые слова объединяются оператором OR. Однако это поведение можно изменить с помощью параметра "operator":
...
"operator" может иметь значение "or" или "and".

Также можно применять модификатор `boost`. Он повышает значение [IDF](../../Searching/Options.md#idf)_score слова в ранжировании путём умножения на указанный коэффициент. Модификатор не влияет непосредственно на процесс поиска.
```json
"query":
{
  "match":
...
"match_phrase" — запрос, который ищет точное совпадение всей фразы. Он подобен оператору фразы в SQL. Пример:
...
"match_all" принимает пустой объект и возвращает документы из таблицы без применения фильтров по атрибутам или полнотекстового поиска. Либо можно просто опустить клаузу `query` в запросе, что даст тот же эффект.
...
Все условия полнотекстового поиска могут комбинироваться с операторами [must](../../Searching/Filters.md#must), [must_not](../../Searching/Filters.md#must_not) и [should](../../Searching/Filters.md#should) в [JSON-запросе bool](../../Searching/Filters.md#bool-query).
