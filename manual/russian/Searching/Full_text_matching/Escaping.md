...
Это также относится к драйверам, которые используют подготовленные выражения (на стороне клиента или сервера). Manticore поддерживает серверные [подготовленные выражения](../../Connecting_to_the_server/MySQL_protocol.md#Prepared-statements) по протоколу MySQL, но `MATCH()` все равно ожидает экранированную строку запроса. Например, с подготовленными выражениями PHP PDO вам нужно добавить обратную косую черту для символа `$`:
