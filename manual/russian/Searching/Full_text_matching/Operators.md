...
Примечание: При использовании этого оператора с запросами, содержащими более 31 ключевого слова, статистика ранжирования (такая как `tf`, `idf`, `bm25`) для ключевых слов на позициях 31 и выше может учитываться некорректно. Это связано с использованием 32-битовой маски для внутреннего отслеживания появлений термина в пределах совпадения. Логика сопоставления (поиска документов) остается правильной, но оценки ранжирования могут быть искажены для очень длинных запросов.
...
Оператор `NOTNEAR` служит для отрицательного утверждения и функционирует как логическое отрицание оператора `NEAR`. Он совпадает с документом, когда присутствует левый аргумент, при условии, что правый аргумент либо отсутствует в документе, либо находится **на расстоянии более** указанного числа слов от левого аргумента.

Синтаксис: `NOTNEAR/N`, чувствителен к регистру и не допускает пробелов между ключевым словом `NOTNEAR`, символом слеша и значением расстояния.

Основные особенности:
*   **Симметричность**: Так же, как и `NEAR`, оператор `NOTNEAR` применяется независимо от порядка терминов в тексте. Совпадение исключается, если правый аргумент найден на указанном расстоянии либо **до** либо **после** левого аргумента.
*   **Порог расстояния**: Расстояние `N` представляет собой близкий диапазон (включительно). Если слова разделены `N` или меньшим количеством слов, совпадение отбрасывается. Правый аргумент должен находиться на расстоянии `N + 1` или более слов.
...
```
Операторы `SENTENCE` и `PARAGRAPH` совпадают с документом, когда оба их аргумента находятся в пределах одного предложения или одного абзаца текста соответственно. Эти аргументы могут быть ключевыми словами, фразами или экземплярами одного и того же оператора.

Порядок аргументов внутри предложения или абзаца не имеет значения. Эти операторы работают только с таблицами, построенными с включенной функцией [index_sp](../../Creating_a_table/NLP_and_tokenization/Advanced_HTML_tokenization.md#index_sp) (индексация предложений и абзацев), и в противном случае сводятся к простому оператору AND. Для информации о том, что считается предложением и абзацем, обратитесь к документации по директиве [index_sp](../../Creating_a_table/NLP_and_tokenization/Advanced_HTML_tokenization.md#index_sp).
...
Оператор ограничения `ZONE` сильно похож на оператор ограничения поля, но ограничивает совпадение определённой зоной внутри поля или списком зон. Важно отметить, что последующие подвыражения не обязательно должны совпадать в пределах одного непрерывного участка заданной зоны и могут совпадать через несколько участков. Например, запрос `(ZONE:th hello world)` совпадёт со следующим примером документа:
...
Оператор ограничения `ZONESPAN` похож на оператор `ZONE`, но требует, чтобы совпадение произошло в пределах одной непрерывной области. В приведённом выше примере `ZONESPAN:th hello world` не совпадёт с документом, так как слова "hello" и "world" не находятся в одном и том же участке.
