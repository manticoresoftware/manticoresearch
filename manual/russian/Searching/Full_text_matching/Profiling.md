...
Рассмотрим пример сложного запроса:
```sql
"hello world" @title "example program"~5 @body python -(php|perl) @* code
```
Полный смысл этого поиска таков:

* Найти слова 'hello' и 'world', расположенные рядом в любом поле документа;
* Дополнительно тот же документ должен содержать слова 'example' и 'program' в поле заголовка, при этом между ними допускается до 5 слов, но не включая 5; (например, "example PHP program" соответствует, а "example script to introduce outside data into the correct context for your program" — нет, поскольку между двумя терминами 5 или более слов)
* Более того, тот же документ должен содержать слово 'python' в теле документа, исключая 'php' или 'perl';
...
Для понимания того, как будет выполняться запрос, Manticore Search предоставляет инструменты профилирования запросов, которые позволяют изучить дерево запроса, сгенерированное выражением запроса.

<!-- example profiling -->

## Профилирование дерева запроса в SQL

Чтобы включить профилирование полнотекстового запроса с помощью SQL-запроса, его необходимо активировать перед выполнением нужного запроса:
...
Чтобы увидеть дерево запроса, выполните команду `SHOW PLAN` сразу после выполнения запроса:

```sql
SHOW PLAN;
```

Эта команда вернёт структуру выполненного запроса. Учтите, что 3 инструкции — SET profiling, сам запрос и SHOW — должны выполняться в одной сессии.
...
* `description`: поддерево запроса для данного узла, представленное строкой (в формате `SHOW PLAN`)
* `children`: дочерние узлы, если они есть
* `max_field_pos`: максимальная позиция в поле

 У узла типа keyword дополнительно присутствуют:

* `word`: преобразованное ключевое слово.
* `querypos`: позиция этого слова в запросе.
* `excluded`: слово исключено из запроса.
* `expanded`: слово добавлено в результате расширения по префиксу.
* `field_start`: слово должно появиться в начале поля.
* `field_end`: слово должно появиться в конце поля.
* `boost`: IDF слова будет умножен на это значение.
...
В некоторых случаях проверяемое дерево запроса может значительно отличаться от исходного из-за расширений и других преобразований.
...
SQL-инструкция `EXPLAIN QUERY` позволяет вывести дерево выполнения для данного полнотекстового запроса без фактического поиска в таблице.
...
`EXPLAIN QUERY ... option format=dot` позволяет вывести дерево выполнения предоставленного полнотекстового запроса в иерархическом формате, подходящем для визуализации с помощью существующих инструментов, таких как https://dreampuf.github.io/GraphvizOnline:
...
<!-- example factors -->
При использовании ранжировщика выражений возможно получить значения вычисленных факторов с помощью функции [PACKEDFACTORS()](../../Functions/Searching_and_ranking_functions.md#PACKEDFACTORS%28%29).
...
Эти значения можно использовать для понимания, почему определённые документы получают более низкие или более высокие оценки при поиске, либо для уточнения существующего выражения ранжирования.
