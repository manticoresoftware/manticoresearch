...
Расстояние нужно вычислять в списке `select` и фильтровать по его алиасу, как показано выше; `WHERE GEODIST(...) < 1000` — синтаксическая ошибка. Два метода расчета используют разные радиусы Земли, поэтому их результаты могут отличаться друг от друга и от других инструментов примерно до 0,3%; точные модели см. в [GEODIST()](../Functions/Geo_spatial_functions.md#GEODIST%28%29).
