...
Сейчас эта форма поддерживает SQL-запросы с явным `GROUP BY`. Она не поддерживает `DISTINCT`, несколько выражений значений, `OFFSET`, неявную группировку, синтаксис JSON-агрегации, `FACET`, `JOIN`, внешние `SELECT`, табличные функции, KNN/гибридные запросы и scroll. Упорядоченный псевдоним `GROUP_CONCAT()` нельзя использовать в `HAVING` или при сортировке результата. Для сортировки результата можно использовать ключи групп и обычные агрегаты, но не значения из представительной строки.
