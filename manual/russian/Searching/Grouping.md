...
Форма с сортировкой сохраняет только первые `N` значений согласно указанному порядку:

```sql
GROUP_CONCAT(expression ORDER BY sort_expression [ASC|DESC] [, ...] LIMIT N)
```

Для неё требуется явный `GROUP BY`. Результатом по-прежнему является одна строка со значениями, разделёнными запятыми. При использовании `GROUP M BY` для группы возвращаются M строк-представителей, и каждая из них содержит одну и ту же строку.
...
Сейчас эта форма поддерживается только в запросах SphinxQL с явным `GROUP BY`. Она не поддерживает `DISTINCT`, несколько выражений значений, `SEPARATOR`, `OFFSET`, неявную группировку, синтаксис агрегации JSON, `FACET`, `JOIN`, внешние запросы или табличные функции, запросы KNN/hybrid и scroll. Псевдоним упорядоченного `GROUP_CONCAT()` нельзя использовать в `HAVING` или сортировке результата. Для сортировки результата можно использовать ключи группировки и обычные агрегаты, но не значения строки-представителя.
