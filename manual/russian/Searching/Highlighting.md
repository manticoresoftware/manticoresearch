...
Подсветка позволяет вам получать выделенные текстовые фрагменты (называемые сниппетами) из документов, содержащих совпадающие ключевые слова.

Функция SQL `HIGHLIGHT()`, свойство `"highlight"` в JSON-запросах через HTTP и функция `highlight()` в PHP-клиенте все используют встроенное хранилище документов для извлечения оригинального содержания полей (включено по умолчанию).
...
При использовании SQL для подсветки результатов поиска вы получите сниппеты из различных полей, объединенные в одну строку из-за ограничений протокола MySQL. Вы можете настроить разделители конкатенации с помощью опций `field_separator` и `snippet_separator`, как указано ниже.
...
Имейте в виду, что такие параметры генерации сниппетов, как `limit`, `limit_words` и `limit_snippets`, применяются к каждому полю индивидуально по умолчанию. Вы можете изменить это поведение, используя опцию `limits_per_field`, но это может привести к нежелательным результатам. Например, в одном поле могут быть совпадающие ключевые слова, но ни один сниппет из этого поля не будет включен в набор результатов, потому что они не оценивались так высоко, как сниппеты из других полей в механизме подсветки.

Алгоритм подсветки в настоящее время приоритизирует лучшие сниппеты (с более близкими совпадениями фраз) и затем сниппеты с ключевыми словами, еще не включенными в результат. Как правило, он стремится подсветить лучшее совпадение для запроса и подсветить все ключевые слова запроса, насколько это допускается лимитами. Если совпадения не найдены в текущем поле, начало документа будет обрезано в соответствии с лимитами и возвращено по умолчанию. Чтобы вернуть пустую строку вместо этого, установите опцию `allow_empty` в 1.

Подсветка осуществляется на этапе, называемом `post limit`, что означает, что генерация сниппетов откладывается не только до тех пор, пока не будет подготовлен весь окончательный набор результатов, но и после применения клаузулы LIMIT. Например, с клаузулой LIMIT 20,10 функция `HIGHLIGHT()` будет вызываться максимум 10 раз.

## Опции подсветки

<!-- example highlighting options -->

Существует несколько дополнительных опций подсветки, которые могут быть использованы для тонкой настройки генерации сниппетов, которые общие для SQL, HTTP и PHP клиентов.

#### before_match
Строка для вставки перед совпадением ключевого слова. Макрос `%SNIPPET_ID%` может быть использован в этой строке. Первое вхождение макроса заменяется на увеличивающийся номер сниппета в пределах текущего сниппета. Нумерация начинается с 1 по умолчанию, но может быть переопределена с помощью опции `start_snippet_id`. %SNIPPET_ID% начинается заново в начале каждого нового документа. Значение по умолчанию - `<b>`.

#### after_match
Строка для вставки после совпадения ключевого слова. Значение по умолчанию - `</b>`.

#### limit
Максимальный размер сниппета в символах (кодовых точках). Значение по умолчанию - 256. Это применяется к каждому полю по умолчанию, см. `limits_per_field`.

#### limit_words
Ограничивает максимальное количество слов, которые могут быть включены в результат. Обратите внимание, что этот лимит применяется ко всем словам, а не только к совпадающим ключевым словам для подсветки. Например, если подсвечивается `Mary`, и выбран сниппет `Mary had a little lamb`, он вносит 5 слов в этот лимит, а не только 1. Значение по умолчанию - 0 (без лимита). Это применяется к каждому полю по умолчанию, см. `limits_per_field`.

#### limit_snippets
Ограничивает максимальное количество сниппетов, которые могут быть включены в результат. Значение по умолчанию - 0 (без лимита). Это применяется к каждому полю по умолчанию, см. `limits_per_field`.

#### limits_per_field
Определяет, работают ли `limit`, `limit_words` и `limit_snippets` как индивидуальные лимиты для каждого поля документа, который подсвечивается, или как глобальные лимиты для всего документа. Установка этой опции в 0 означает, что все комбинированные результаты подсветки для одного документа должны находиться в пределах указанных лимитов. Недостаток в том, что у вас могут быть несколько сниппетов, подсвеченных в одном поле, и ни одного в другом, если механизм подсветки решит, что они более релевантны. Значение по умолчанию - 1 (использовать лимиты на поле).
#### around
Количество слов, которые необходимо выбрать вокруг каждого блока совпадающего ключевого слова. Значение по умолчанию - 5.

#### use_boundaries
Определяет, следует ли дополнительно разбивать сниппеты по границам фраз, как настроено в параметрах таблицы с директивой [phrase_boundary](../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#phrase_boundary). Значение по умолчанию - 0 (не использовать границы).

#### weight_order
Указывает, следует ли сортировать извлеченные сниппеты по порядку важности (уменьшающийся вес) или по порядку появления в документе (увеличивающаяся позиция). Значение по умолчанию - 0 (не использовать порядок веса).

#### force_all_words
Игнорирует лимит длины, пока результат не включает все ключевые слова. Значение по умолчанию - 0 (не принуждать к включению всех ключевых слов).

#### start_snippet_id
Устанавливает начальное значение для макроса `%SNIPPET_ID%` (который распознается и расширяется в строках `before_match`, `after_match`). Значение по умолчанию - 1.

#### html_strip_mode
Определяет настройку режима удаления HTML. По умолчанию установлено `index`, что означает, что будут использоваться настройки таблицы. Другие значения включают `none` и `strip`, которые принудительно пропускают или применяют удаление независимо от настроек таблицы; и `retain`, который сохраняет разметку HTML и защищает ее от подсветки. Режим `retain` можно использовать только при подсветке полных документов и, следовательно, требует, чтобы не были установлены ограничения по размеру фрагментов. Разрешенные строковые значения: `none`, `strip`, `index` и `retain`.

#### allow_empty
Разрешает возвращать пустую строку в качестве результата подсветки, когда в текущем поле не удалось сгенерировать фрагменты (нет совпадений ключевых слов или нет подходящих фрагментов). По умолчанию возвращается начало исходного текста вместо пустой строки. Значение по умолчанию - 0 (не разрешать пустой результат).

#### snippet_boundary
Обеспечивает, чтобы фрагменты не пересекали границы предложения, абзаца или зоны (при использовании с таблицей, в которой включены соответствующие настройки индексации). Разрешенные значения: `sentence`, `paragraph` и `zone`.

#### emit_zones
Эмитирует HTML-тег с именем окружающей зоны перед каждым фрагментом. По умолчанию - 0 (не эмитировать имена зон).

#### force_snippets
Определяет, следует ли принудительно генерировать фрагменты, даже если ограничения позволяют подсветку всего текста. По умолчанию - 0 (не принуждать к генерации фрагментов).
...
Функция `HIGHLIGHT()` может быть использована для подсветки результатов поиска. Вот синтаксис:
...
`HIGHLIGHT()` извлекает все доступные поля с полнотекстовым индексированием из хранилища документов и подсвечивает их в соответствии с предоставленным запросом. Поддерживается синтаксис полей в запросах. Текст полей разделяется `field_separator`, который можно изменить в параметрах.
...
Необязательный второй аргумент - это строка, содержащая одно поле или список полей, разделенный запятыми. Если этот аргумент присутствует, будут извлечены и подсвечены только указанные поля. Пустая строка в качестве второго аргумента означает "извлечь все доступные поля."
...
Альтернативно, вы можете использовать второй аргумент для указания строкового атрибута или имени поля без кавычек. В этом случае предоставленная строка будет подсвечена в соответствии с предоставленным запросом, но синтаксис полей будет проигнорирован.
...
Необязательный третий аргумент - это запрос. Он используется для подсветки результатов поиска по запросу, отличному от того, который использовался для поиска.
...
Хотя `HIGHLIGHT()` предназначен для работы с хранящимися полнотекстовыми полями и строковыми атрибутами, его также можно использовать для подсветки произвольного текста. Имейте в виду, что если запрос содержит какие-либо операторы поиска по полям (например, `@title hello @body world`), в этом случае часть поля игнорируется.
...
Несколько параметров имеют значение только при генерации одной строки в качестве результата (не массива фрагментов). Это относится исключительно к функции SQL `HIGHLIGHT()`:
...
Другой способ подсветки текста - использовать оператор [CALL SNIPPETS](../Searching/Highlighting.md#CALL-SNIPPETS). Это в основном дублирует функциональность `HIGHLIGHT()`, но не может использовать встроенное хранилище документов. Однако он может загружать исходный текст из файлов.
...
Чтобы выделить все возможные поля, передайте пустой объект как свойство `highlight`.
...
В дополнение к общим вариантам выделения, несколько синонимов доступны для запросов JSON через HTTP:

#### fields
Объект `fields` содержит имена атрибутов с опциями. Он также может быть массивом имен полей (без каких-либо опций).

Обратите внимание, что по умолчанию выделение пытается выделить результаты после полнотекстового запроса. В общем случае, когда вы не указываете поля для выделения, выделение основано на вашем полнотекстовом запросе. Однако, если вы указываете поля для выделения, выделение происходит только если полнотекстовый запрос соответствует выбранным полям.
...
#### highlight_query
Опция `highlight_query` позволяет вам выделять по запросу, отличному от вашего поискового запроса. Синтаксис такой же, как и в основном `query`.
...
`pre_tags` и `post_tags` устанавливают открывающие и закрывающие теги для выделенных текстовых фрагментов. Они функционируют аналогично опциям `before_match` и `after_match`. Это необязательные параметры с значениями по умолчанию `<b>` и `</b>`.
...
#### no_match_size
`no_match_size` функционирует аналогично параметру `allow_empty`. Если установлен в 0, он действует как `allow_empty=1`, позволяя вернуть пустую строку в качестве результата выделения, когда фрагмент не может быть сгенерирован. В противном случае вернётся начало поля. Это необязательный параметр со значением по умолчанию 1.
...
<!-- example order  -->
#### order
`order` задает порядок сортировки извлеченных фрагментов. Если установлено значение `"score"`, то фрагменты сортируются по релевантности. Это опционально и работает аналогично параметру `weight_order`.
...
#### fragment_size
`fragment_size` задает максимальный размер фрагмента в символах. Он может быть глобальным или для каждого поля. Опции для каждого поля переопределяют глобальные опции. Это опционально, со значением по умолчанию 256. Он работает аналогично опции `limit`.
...
#### number_of_fragments
`number_of_fragments` ограничивает максимальное количество фрагментов в результате. Как и `fragment_size`, он может быть глобальным или для каждого поля. Это опционально, со значением по умолчанию 0 (без ограничения). Он работает аналогично опции `limit_snippets`.
...
Опции, такие как `limit`, `limit_words` и `limit_snippets`, могут быть установлены как глобальные или для каждого поля. Глобальные параметры используются как ограничения для каждого поля, если их не переопределяют параметры для каждого поля. В примере поле `title` выделено с использованием параметров по умолчанию, в то время как поле `content` использует другое ограничение.
...
#### limits_per_field
Глобальные ограничения также могут быть применены, указав `limits_per_field=0`. Установка этой опции означает, что все объединенные результаты выделения должны находиться в пределах указанных ограничений. Недостатком является то, что вы можете получить несколько выделенных фрагментов в одном поле и ни одного в другом, если движок выделения решит, что они более релевантны.
...
Утверждение `CALL SNIPPETS` строит фрагмент из предоставленных данных и запроса, используя указанные настройки таблицы. Он не может получить доступ к встроенному документу хранилища, поэтому рекомендуется использовать [функцию HIGHLIGHT()](../Searching/Highlighting.md) вместо этого.
...
#### table
`table` относится к имени таблицы, которая предоставляет настройки обработки текста для генерации фрагмента.
#### query
`query` — это полнотекстовый запрос, используемый для построения фрагментов.
#### opt_value и opt_name
`opt_value` и `opt_name` представляют собой [параметры генерации фрагментов](../Searching/Highlighting.md).
...
Большинство параметров такие же, как в функции [HIGHLIGHT()](../Searching/Highlighting.md). Однако есть несколько параметров, которые можно использовать только с `CALL SNIPPETS`.

<!-- example CALL SNIPPETS load files -->
Следующие параметры могут использоваться для выделения текста, хранящегося в отдельных файлах:

#### load_files
Этот параметр, когда он включен, рассматривает первый аргумент как имена файлов, а не как данные для извлечения фрагментов. Указанные файлы на стороне сервера будут загружены для получения данных. Будет использовано до [max_threads_per_query](../Server_settings/Searchd.md#max_threads_per_query) рабочих потоков на запрос для параллелизации работы, когда этот параметр включен. По умолчанию — 0 (без ограничений). Чтобы распределить генерацию фрагментов между удаленными агентами, вызывайте генерацию фрагментов в распределенной таблице, содержащей только одного(!) локального агента и несколько удаленных. Параметр [snippets_file_prefix](../Creating_a_table/Creating_a_distributed_table/Remote_tables.md#snippets_file_prefix) используется для генерации финального имени файла. Например, когда searchd настроен с `snippets_file_prefix = /var/data_` и `text.txt` предоставляется в качестве имени файла, фрагменты будут сгенерированы из содержимого `/var/data_text.txt`.

#### load_files_scattered
Этот параметр работает только с распределенной генерацией фрагментов с удаленными агентами. Исходные файлы для генерации фрагментов могут быть распределены между различными агентами, и главный сервер объединит все неошибочные результаты. Например, если один агент распределенной таблицы имеет `file1.txt`, другой агент имеет `file2.txt`, и вы используете `CALL SNIPPETS` с обоими этими файлами, searchd объединит результаты агентов, так что вы получите результаты как из `file1.txt`, так и из `file2.txt`. По умолчанию — 0.

Если параметр `load_files` также включен, запрос вернет ошибку, если какой-либо из файлов недоступен. В противном случае (если `load_files` не включен) он вернет пустые строки для всех отсутствующих файлов. Searchd не передает этот флаг агентам, поэтому агенты не генерируют критическую ошибку, если файл не существует. Если вы хотите быть уверены, что все исходные файлы загружены, установите оба параметра `load_files_scattered` и `load_files` в 1. Если отсутствие некоторых исходных файлов у некоторых агентов не критично, установите только `load_files_scattered` в 1.
