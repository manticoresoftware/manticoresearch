...
Обычные фильтры атрибутов работают вместе с гибридным поиском и применяются как к текстовому, так и к KNN-подзапросам. Фильтры по `weight()` и зависящим от него выражениям применяются после объединения результатов, используя итоговый текстовый вес. Текущее ограничение Manticore: эти фильтры веса нельзя использовать в деревьях фильтров с `OR`.

Например, фильтрация по обычному атрибуту:
