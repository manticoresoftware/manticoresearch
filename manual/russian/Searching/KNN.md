...
**Сколько документов возвращает KNN-поиск.** KNN-поиск возвращает набор кандидатов, а не точное число строк. Каждый дисковый фрагмент real-time таблицы добавляет до `LIMIT` × `oversampling` ближайших документов (`k` × `oversampling`, если указан устаревший параметр `k`), а `oversampling` по умолчанию равен 3. `LIMIT` по-прежнему определяет, сколько строк вы получите, но `total_found` в [SHOW META](../Node_info_and_management/SHOW_META.md) и все счетчики [FACET](../Searching/Faceted_search.md) охватывают весь набор кандидатов. Например, в таблице с 4 дисковыми фрагментами запрос `SELECT id FROM t WHERE knn(vec, 'quiet flat') LIMIT 5` вернет 5 строк с `total_found` 60 (5 × 3 × 4); после того как [OPTIMIZE](../Securing_and_compacting_a_table/Compacting_a_table.md) объединит таблицу в один фрагмент, тот же запрос покажет 15. Поэтому `total_found` в KNN-запросе не является числом релевантных документов. Чтобы поиск учитывал каждый документ, например чтобы ранжировать по сходству все документы, прошедшие ваши фильтры по атрибутам, задайте `LIMIT` (и `max_matches`, если нужно) не меньше числа документов в таблице.

**Сортировка.** Результаты KNN всегда сначала упорядочиваются по расстоянию до запроса. `ORDER BY` по другому атрибуту не переупорядочивает их: он только разрешает совпадения при равных расстояниях, при этом предупреждение не выводится. Чтобы отсортировать KNN-совпадения по атрибуту, выполните KNN-поиск в [подзапросе](../Searching/Sub-selects.md) и отсортируйте внешний запрос:
...
[Гибридные запросы](../Searching/Hybrid_search.md#Sorting) (`OPTION fusion_method='rrf'`) применяют `ORDER BY` к атрибутам. Для получения расстояния есть встроенная функция [knn_dist()](../Functions/Other_functions.md#KNN_DIST%28%29).
