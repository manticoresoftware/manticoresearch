...
Для объединения векторного поиска KNN с полнотекстовым поиском для повышения релевантности см. [Гибридный поиск](../Searching/Hybrid_search.md).
...
* `early_termination`: Включает или отключает адаптивное досрочное завершение во время обхода графа HNSW. Включено по умолчанию. Установите в `0` в SQL или `false` в JSON, чтобы отключить. Подробности см. в разделе [Досрочное завершение](../Searching/KNN.md#Early-termination).
