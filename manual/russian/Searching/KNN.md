...
> ПРИМЕЧАНИЕ: Построение графа HNSW во время сохранения RT-чанков, слияния чанков при `OPTIMIZE TABLE` / автооптимизации и перестроение KNN при `ALTER TABLE ... ADD/DROP/REBUILD` по умолчанию выполняется параллельно на многоядерных хостах; количество рабочих потоков контролируется настройкой сервера [`knn_parallel_build`](../Server_settings/Searchd.md#knn_parallel_build) (установите значение `1` для принудительного последовательного выполнения). Это влияет только на производительность при построении; синтаксис запросов и настройки времени поиска не меняются. Поскольку параллельное построение HNSW может вставлять векторы в другом порядке, результирующий граф может не быть битово идентичным последовательной сборке.
...
* `early_termination`: Включает или отключает адаптивное раннее завершение во время обхода графа HNSW. Включено по умолчанию. Установите `0` в SQL или `false` в JSON, чтобы отключить. Подробности см. в разделе [Раннее завершение](../Searching/KNN.md#Early-termination).
