...
* `early_termination`: Включает или отключает адаптивное раннее завершение во время поиска в графе HNSW. Включено по умолчанию. Установите `0` в SQL или `false` в JSON для отключения. Смотрите [Раннее завершение](#Early-termination) для деталей.
