...
- `CACHE_PATH`: необязательно. Каталог, в который скачиваются и где кэшируются локальные модели. По умолчанию это `.cache/manticore` внутри [data_dir](../Server_settings/Searchd.md#data_dir), например `<data_dir>/.cache/manticore/models--Xenova--all-MiniLM-L6-v2`, поэтому скачанная модель сохраняется после перезапусков.
...
| **Sentence Transformers** | `sentence-transformers/all-MiniLM-L6-v2` | Нет | Локальные модели на базе BERT, загружаются автоматически. Те же веса, что и в строке ONNX выше, но на CPU эмбеддинги строятся в 10–20 раз медленнее — используйте ONNX, если он есть в репозитории. См. [Выбор локальной модели эмбеддингов](../Searching/KNN.md#Choosing-a-local-embedding-model). |
...
##### Выбор локальной модели эмбеддингов

`MODEL_NAME` определяет не только качество, но и **скорость построения эмбеддингов для каждого документа на CPU — разница может быть на порядок** (см. [эмбеддинги в 14 раз быстрее с ONNX](https://manticoresearch.com/blog/onnx-embeddings-speedup/)). Репозиторий с файлом `.onnx` работает через среду выполнения ONNX Runtime; репозиторий `safetensors` с той же самой моделью работает через гораздо более медленный путь Candle. Приблизительные показатели, замеренные на небольшом инстансе с 4 vCPU и короткими документами (~300 символов):

| Модель | Формат | Размерность | Загрузка | Скорость на CPU |
|---|---|---|---|---|
| `Xenova/all-MiniLM-L6-v2` | ONNX | 384 | ~90 MB | ~30–40 док./с |
| `sentence-transformers/all-MiniLM-L6-v2` | safetensors | 384 | ~90 MB | ~2–3 док./с |
| `BAAI/bge-base-en-v1.5` | safetensors | 768 | ~420 MB | ~2–3 док./с |

* **Начните с `Xenova/all-MiniLM-L6-v2`** — модели, использованной в примерах выше. Она одинаково хорошо подходит для интерактивных запросов и массовой загрузки, а ее качества достаточно для большинства поисковых приложений.
* **Более высокая позиция в рейтинге редко оправдывает стоимость индексации на CPU.** Переход с небольшой ONNX-модели на более крупную модель `safetensors` обычно делает построение эмбеддинга для каждого документа в 10–20 раз дороже, а итоговая релевантность меняется лишь незначительно. Прежде чем переходить на «лучшую» модель, сравните ее с моделью по умолчанию на нескольких десятках собственных запросов и текстов.
* **Оцените время перед большой загрузкой.** Стоимость построения эмбеддингов растет линейно: миллисекунды на документ умножаются на число строк. При ~300 мс/док. 15 000 документов займут примерно 75 минут; при ~30 мс/док. — меньше 10 минут. Вставьте несколько сотен реальных строк во временную таблицу и экстраполируйте результат, прежде чем привязывать рабочую таблицу к модели.
* **Медленная загрузка обычно указывает на модель, а не на настройки.** Если вставки еле движутся или массовая загрузка показывает тайм-ауты и повторные попытки при построении эмбеддингов, обычная причина — модель не в формате ONNX или слишком большая модель. Переход на ONNX-репозиторий решает проблему; увеличение [`embeddings_threads`](../Server_settings/Searchd.md#embeddings_threads) не делает медленную модель быстрой и может лишить параллельные задачи рабочих потоков.
...
Документы всегда сортируются по расстоянию до вектора поиска. Любые дополнительные критерии сортировки, которые вы укажете, будут применяться после этого основного условия сортировки; см. [Сортировка результатов KNN](../Searching/KNN.md#Sorting-KNN-results). Для получения расстояния есть встроенная функция [knn_dist()](../Functions/Other_functions.md#KNN_DIST%28%29). О том, сколько документов возвращает KNN-поиск, см. [Набор кандидатов KNN](../Searching/KNN.md#KNN-candidate-set).
...
### Набор кандидатов KNN

KNN-поиск возвращает набор кандидатов, а не точное число строк. Каждый дисковый чанк RT-таблицы добавляет до `LIMIT` × `oversampling` ближайших документов (`k` × `oversampling`, если задан устаревший параметр `k`), а значение `oversampling` по умолчанию равно 3. `LIMIT` по-прежнему определяет, сколько строк вы получите, но `total_found` в [SHOW META](../Node_info_and_management/SHOW_META.md) и все счетчики [FACET](../Searching/Faceted_search.md) охватывают весь набор кандидатов. Например, для таблицы с 4 дисковыми чанками запрос `SELECT id FROM t WHERE knn(vec, 'quiet flat') LIMIT 5` вернет 5 строк с `total_found` 60 (5 × 3 × 4); после того как [OPTIMIZE](../Securing_and_compacting_a_table/Compacting_a_table.md) объединит таблицу в один чанк, тот же запрос покажет 15. Поэтому `total_found` в KNN-запросе не является числом релевантных документов. Чтобы поиск учитывал каждый документ, например чтобы ранжировать по сходству все документы, прошедшие фильтры по атрибутам, задайте `LIMIT` (и `max_matches`, если нужно) не меньше числа документов в таблице.

### Сортировка результатов KNN

Результаты KNN всегда сначала упорядочиваются по расстоянию до запроса. `ORDER BY` по другому атрибуту не переупорядочивает их: он только разрешает совпадения при равных расстояниях, и предупреждение при этом не возвращается. Чтобы отсортировать совпадения KNN по атрибуту, выполните KNN-поиск в [подзапросе](../Searching/Sub-selects.md) и отсортируйте внешний запрос:
...
[Гибридные запросы](../Searching/Hybrid_search.md#Sorting) (`OPTION fusion_method='rrf'`) применяют `ORDER BY` по атрибутам.
