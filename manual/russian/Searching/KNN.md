...
* `early_termination`: Включает или отключает адаптивное раннее завершение во время обхода графа HNSW. Включено по умолчанию. Установите `0` в SQL или `false` в JSON для отключения. Подробности см. в [Early termination](../Searching/KNN.md#Early-termination).
