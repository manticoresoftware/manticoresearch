...
### expand_blended
`0`, `off`, `1` или любая комбинация опций `blend_mode` (по умолчанию `0`). Расширяет составные ключевые слова на их составляющие варианты во время разбора запроса. При включении ключевые слова типа "well-being" расширяются до вариантов, таких как "well-being", "wellbeing", "well" и "being", которые затем группируются в поддеревья ИЛИ в дереве запроса.

Поддерживаемые значения:
* `0` или `off` - Расширение составных слов отключено (по умолчанию). Составные ключевые слова обрабатываются обычным образом без расширения.
* `1` - Расширение составных слов включено и использует настройку [blend_mode](../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#blend_mode) таблицы для определения, какие варианты генерировать.
* Любая опция (или опции) режима смешивания - Расширение составных слов включено с указанным режимом (режимами) смешивания, переопределяя настройку `blend_mode` таблицы.

Подробнее об опциях см. [blend_mode](../Creating_a_table/NLP_and_tokenization/Low-level_tokenization.md#blend_mode).

### field_weights
Именованный список целых чисел (пользовательские веса полей для ранжирования).
...
* `normalized`: Вариант BM25, idf = log((N-n+1)/n), согласно Robertson и др.
...
Во-вторых, `idf=tfidf_normalized` приводит к смещению IDF между запросами. Исторически IDF также делился на количество ключевых слов в запросе, гарантируя, что вся `sum(tf*idf)` по всем ключевым словам остается в пределах диапазона [0,1]. Однако это означало, что запросы типа `word1` и `word1 | nonmatchingword2` присваивали бы разный вес одному и тому же набору результатов, поскольку IDF как для `word1`, так и для `nonmatchingword2` делились бы на 2. Использование `OPTION idf='tfidf_unnormalized'` решает эту проблему. Имейте в виду, что факторы ранжирования BM25, BM25A, BM25F() будут соответствующим образом скорректированы при отключении этой нормализации.
...
При использовании китайской сегментации Jieba иногда может помочь использование разных режимов сегментации для токенизации документов и запроса. Полный список режимов см. в [jieba_mode](../Creating_a_table/NLP_and_tokenization/Morphology.md#jieba_mode).

### index_weights
Именованный список целых чисел. Пользовательские веса для ранжирования по таблицам.
...
### low_priority
`0` или `1` (по умолчанию `0`). Установка `low_priority=1` выполняет запрос с более низким приоритетом, перепланируя его задачи в 10 раз реже, чем другие запросы с нормальным приоритетом.
...
Если количество уникальных значений атрибута группировки меньше порога, `max_matches` будет установлен в это число. В противном случае будет использоваться значение `max_matches` по умолчанию.

Если `max_matches` был явно установлен в параметрах запроса, этот порог не действует.
...
### max_query_time
Устанавливает максимальное время выполнения поискового запроса в миллисекундах. Должно быть неотрицательным целым числом. Значение по умолчанию — 0, что означает «не ограничивать». Локальные поисковые запросы будут остановлены, как только истечет указанное время. Обратите внимание, что если вы выполняете поиск, который запрашивает несколько локальных таблиц, это ограничение применяется к каждой таблице отдельно. Имейте в виду, что это может незначительно увеличить время отклика запроса из-за накладных расходов, вызванных постоянным отслеживанием, не пора ли остановить запрос.
...
Для получения более подробной информации о каждом ранкере обратитесь к разделу [Ранжирование результатов поиска](../Searching/Sorting_and_ranking.md#Available-built-in-rankers).
...
### retry_delay
Целое число. Задержка перед повторной попыткой распределенного поиска, в миллисекундах.

### scroll

Строка. Токен прокрутки для постраничного вывода результатов с использованием [подхода пагинации Scroll](../Searching/Pagination.md#Scroll-Search-Option).
...
### token_filter
Заключенная в кавычки строка, разделенная двоеточиями: `имя_библиотеки:имя_плагина:необязательная_строка_настроек`. Фильтр токенов времени запроса создается для каждого поиска, когда полнотекстовый поиск вызывается каждой задействованной таблицей, позволяя реализовать пользовательский токенизатор, который генерирует токены в соответствии с пользовательскими правилами.
```sql
SELECT * FROM index WHERE MATCH ('yes@no') OPTION token_filter='mylib.so:blend:@'
```
### expansion_limit
Ограничивает максимальное количество расширенных ключевых слов для одного подстановочного знака, значение по умолчанию 0 означает отсутствие ограничения. Для получения дополнительных сведений обратитесь к [expansion_limit](../Server_settings/Searchd.md#expansion_limit).
...
Обратите внимание, что при выполнении полнотекстового запроса с фильтрами оптимизатор запросов решает между пересечением результатов полнотекстового дерева с результатами фильтра или использованием стандартного подхода «сначала поиск, затем фильтрация». Указание *любой* подсказки заставит демона использовать путь кода, который выполняет пересечение результатов полнотекстового дерева с результатами фильтра.

Для получения дополнительной информации о том, как работает оптимизатор запросов, обратитесь к странице [Оптимизатор на основе стоимости](../Searching/Cost_based_optimizer.md).
...
<!-- example comments -->
При использовании клиента MySQL/MariaDB убедитесь, что включили флаг `--comments`, чтобы активировать подсказки в ваших запросах.
