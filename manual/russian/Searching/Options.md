...
Вы также можете активировать режим гарантированной точности группировки/агрегации с помощью опции [accurate_aggregation](../Searching/Options.md#accurate_aggregation).

### max_query_time
Устанавливает максимальное время выполнения поискового запроса в миллисекундах. Значение должно быть неотрицательным целым числом. Значение по умолчанию — 0, что означает «без ограничений». Локальные поисковые запросы будут остановлены по истечении указанного времени. Обратите внимание, что при выполнении поиска с запросами к нескольким локальным таблицам это ограничение применяется к каждой таблице отдельно. Имейте в виду, что это может немного увеличить время отклика запроса из-за накладных расходов, вызванных постоянным отслеживанием того, время ли прекратить запрос.
...
### morphology
`none` позволяет заменять все термы запроса на их точные формы, если таблица была построена с включённой опцией [index_exact_words](../Creating_a_table/NLP_and_tokenization/Morphology.md#index_exact_words). Это полезно для предотвращения стемминга или лемматизации термов запроса.
...
`0` или `1` разрешает самостоятельное использование [отрицания](../Searching/Full_text_matching/Operators.md#Negation-operator) в запросе. По умолчанию 0. См. также соответствующую [глобальную настройку](../Server_settings/Searchd.md#not_terms_only_allowed).
...
Для подробной информации о каждом ранжировщике смотрите [Ранжирование результатов поиска](../Searching/Sorting_and_ranking.md#Available-built-in-rankers).

### rand_seed
Позволяет указать конкретное целочисленное значение зерна для запроса `ORDER BY RAND()`, например: `... OPTION rand_seed=1234`. По умолчанию для каждого запроса генерируется новое и уникальное зерно.

### retry_count
Целое число. Количество повторных попыток распределённого запроса.

### retry_delay
Целое число. Задержка между повторными попытками распределённого запроса в миллисекундах.
...
Результат одинаков в обоих случаях; выбор одного варианта может просто улучшить (или ухудшить) производительность.
...
Для группы запросов опция должна быть применена к первому запросу в группе, после чего используется при создании рабочей очереди и действует на всю группу. Эта опция эквивалентна опции [max_threads_per_query](../Server_settings/Searchd.md#max_threads_per_query), но применяется только к текущему запросу или группе запросов.

### token_filter
Заключённая в кавычки строка, разделённая двоеточиями, в формате `имя библиотеки:имя плагина:необязательная строка настроек`. Для каждого запроса создаётся токен-фильтр во время выполнения полного текста для каждой таблицы, чтобы можно было реализовать собственный токенизатор, генерирующий токены по индивидуальным правилам.
```sql
SELECT * FROM index WHERE MATCH ('yes@no') OPTION token_filter='mylib.so:blend:@'
```
### expansion_limit
Ограничивает максимальное количество расширенных ключевых слов для одного шаблона с подстановочными символами; значение по умолчанию 0 означает отсутствие ограничений. Дополнительную информацию смотрите в [expansion_limit](../Server_settings/Searchd.md#expansion_limit).

## Подсказки оптимизатора запросов

<!-- example options_force -->

В редких случаях встроенный анализатор запросов Manticore может неправильно понять запрос и определить, следует ли использовать индекс docid, вторичные индексы или столбцовый скан. Чтобы переопределить решения оптимизатора запросов, можно использовать следующие подсказки в своём запросе:

* `/*+ DocidIndex(id) */` — принудительное использование индекса docid, `/*+ NO_DocidIndex(id) */` — чтобы сообщить оптимизатору игнорировать его
* `/*+ SecondaryIndex(<attr_name1>[, <attr_nameN>]) */` — принудительное использование вторичного индекса (если доступен), `/*+ NO_SecondaryIndex(id) */` — чтобы сообщить оптимизатору игнорировать его
* `/*+ ColumnarScan(<attr_name1>[, <attr_nameN>]) */` — принудительное использование столбцового скана (если атрибут столбцовый), `/*+ NO_ColumnarScan(id) */` — чтобы сообщить оптимизатору игнорировать его

Обратите внимание, что при выполнении полнотекстового запроса с фильтрами оптимизатор решает, пересекать ли результаты полнотекстового дерева с результатами фильтра или использовать стандартный подход "найти, затем отфильтровать". Указание *любой* подсказки заставит демон использовать путь кода, который выполняет пересечение результатов полнотекстового дерева с результатами фильтра.
...
<!-- example comments -->
При использовании клиента MySQL/MariaDB обязательно добавляйте флаг `--comments` для включения подсказок в своих запросах.
