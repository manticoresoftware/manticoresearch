...
Если включена [аутентификация](../Security/Authentication_and_authorization.md), аутентифицируйте инструмент с помощью `--user` и `--password` либо `--token`. Соответствующие переменные окружения: `MANTICORE_BACKUP_USER`, `MANTICORE_BACKUP_PASSWORD` и `MANTICORE_BACKUP_TOKEN`. Bearer-токен имеет приоритет над именем пользователя и паролем. Предпочитайте переменные окружения секретам в командной строке, потому что аргументы командной строки могут быть видны другим локальным пользователям через утилиты просмотра процессов.

Аутентифицированный пользователь должен иметь `backup` на `*`. Для резервного копирования выбранных таблиц также требуется `read` на каждой выбранной таблице; для резервного копирования всех таблиц требуется неограниченный `read` на `*` (без правил `read` deny). Например:
...
| `--tables=tbl1,tbl2, ...` | Список таблиц через запятую, которые нужно включить в резервную копию. Чтобы сделать резервную копию всех таблиц, опустите этот аргумент. Все указанные таблицы должны существовать в экземпляре Manticore, из которого вы создаете копию, иначе резервное копирование завершится с ошибкой. |
| `--user=name` | Пользователь Manticore для аутентифицированных HTTP-запросов. Либо задайте `MANTICORE_BACKUP_USER`. |
| `--password=password` | Пароль для `--user`. Либо задайте `MANTICORE_BACKUP_PASSWORD`. Предпочтительнее использовать переменную окружения, чтобы пароль не отображался в списке процессов. |
| `--token=token` | Bearer-токен для аутентифицированных HTTP-запросов. Либо задайте `MANTICORE_BACKUP_TOKEN`. Токен имеет приоритет над `--user` и `--password`. |
| `--compress` | Нужно ли сжимать файлы резервной копии. По умолчанию не включено. |
...
Когда аутентификация включена, `BACKUP` требует глобальное разрешение `backup` плюс доступ к данным, которые копируются. `BACKUP TABLE <tables>` требует `backup` на `*` и `read` на каждой указанной таблице. `BACKUP` без явного списка таблиц требует `backup` на `*` и неограниченный `read` на `*`; любое правило `read` deny предотвращает полное резервное копирование. Одного разрешения `admin`, `schema` или `read` недостаточно, чтобы разрешить `BACKUP`. См. [Права доступа](../Security/Authentication_and_authorization.md#Permissions), чтобы узнать о семантике grant и deny.
...
Manticore поддерживает утилиту `mysqldump` из MySQL до версии 26.7 и утилиту `mariadb-dump` из MariaDB до версии 12.3.
...
- `-t` пропускает команды `drop`/`create` таблицы. Используйте его при переиндексации таблицы после изменения настроек полнотекстового поиска.
...
- `--replace` выполняет `replace` вместо `insert`. Используйте его при переиндексации таблицы после изменения настроек полнотекстового поиска.
