...
* `backup` - выполнять серверные операции `BACKUP`.
...
Отсутствие разрешения означает запрет по умолчанию. Действия `admin` и `backup` должны быть назначены для `*`. Действие `admin` не подразумевает `read`, `write`, `schema`, `backup` или `replication`; при необходимости назначайте эти действия явно.

`BACKUP` — это серверное операционное действие. Для `BACKUP TABLE <tables>` требуется `backup` на `*` и `read` для каждой запрошенной таблицы. Для `BACKUP` без явного списка таблиц требуется `backup` на `*` и неограниченный `read` на `*`; любое правило запрета `read` предотвращает полное резервное копирование. Команды `FREEZE` и `UNFREEZE`, поддерживаемые инструментом резервного копирования, требуют `backup` на `*` и `read` на запрошенной таблице; `FLUSH ATTRIBUTES`, `SHOW TABLES`, `SHOW STATUS` и `SHOW SETTINGS` авторизуются либо по своему обычному разрешению, либо по `backup` на `*`. Одних `admin`, `schema` или `read` недостаточно для авторизации `BACKUP`.
...
Правила сопоставляются только с запрошенным действием. Например, право `admin` не удовлетворяет `read`, `write`, `schema`, `backup` или `replication`.
