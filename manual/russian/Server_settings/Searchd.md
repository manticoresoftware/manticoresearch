...
Управляет kill dictionary для RT дисковых фрагментов. При включении, Manticore перестроит статистику слов для каждого убитого документа из хранилища документов и вычитает их на момент выполнения запроса, так что `docs`/`hits` и веса рейтинга отражают удалённые строки. По умолчанию включено в режиме `idle`.

Режимы:
- `0`: отключено, запросы используют исходную статистику словаря фрагментов.
- `realtime`: включено, убитые документы применяются сразу (повторная токенизация для каждого убитого документа).
- `flush`: включено, но обновления для каждого убитого документа откладываются до следующего flush/merge, которые перестроят статистику.
- `idle`: включено, но обновления для каждого убитого документа откладываются до момента, когда поток записи становится idle (см. `kill_dictionary_idle_timeout`).

Замечания о режиме `flush`:
- Статистика становится точной после каждого flush дискового фрагмента, а не после каждого REPLACE/DELETE.
- Задержка при записи более равномерная, чем в режиме `realtime`, но flush могут занимать больше времени, потому что статистика убитых документов применяется синхронно во время flush.
- Скорость обработки может быть аналогичной или немного хуже, чем в режиме `realtime`, в зависимости от частоты flush и данных.
...
Idle timeout для перестроения статистики kill dictionary при `kill_dictionary=idle`. Если есть ожидающие убитые документы и никакие записи не происходят в течение указанного количества секунд, Manticore перестроит статистику убитых документов и сохранит `.spks`. Установка в `-1` отключает перестроение в idle. Принимает секунды или суффиксы `ms/s/m/h/d`.
