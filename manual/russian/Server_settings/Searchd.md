...
Для сборок под другие ОС, кроме Windows, значение по умолчанию должно работать нормально, и обычно нет необходимости изменять этот параметр.
...
Управляет словарём удалённых документов (kill dictionary) для RT-сегментов на диске. При включении Manticore пересчитывает статистику слов для каждого удалённого документа из хранилища документов и вычитает её во время выполнения запроса, поэтому `docs`/`hits` и веса ранжирования отражают удалённые строки. По умолчанию включено в режиме `idle`.

Режимы:
- `0`: отключено, запросы используют сырую статистику словаря сегмента.
- `realtime`: включено, и удаления применяются немедленно (повторная токенизация при каждом удалении).
- `flush`: включено, но обновления статистики при удалениях откладываются до следующей операции сброса/слияния, которая пересчитает статистику.
- `idle`: включено, но обновления статистики при удалениях откладываются до тех пор, пока поток записи не станет неактивным (см. `kill_dictionary_idle_timeout`).

Примечания по режиму `flush`:
- Статистика становится точной после каждого сброса дискового сегмента, а не после каждого REPLACE/DELETE.
- Задержка записи более плавная, чем в режиме `realtime`, но операции сброса могут занимать больше времени, потому что статистика удалений применяется синхронно во время сброса.
- Пропускная способность может быть аналогичной или немного хуже, чем в режиме `realtime`, в зависимости от частоты сброса и набора данных.
...
Таймаут неактивности для пересчёта статистики словаря удалений при `kill_dictionary=idle`. Если есть ожидающие удаления и в течение указанного количества секунд не происходит операций записи, Manticore пересчитывает статистику удалений и сохраняет `.spks`. Установите значение `-1`, чтобы отключить пересчёт по неактивности. Принимает секунды или суффиксы `ms/s/m/h/d`.
...
Если указан номер порта, но не указан адрес, `searchd` будет прослушивать все сетевые интерфейсы. Unix-путь определяется по начальному слешу. Диапазон портов может быть установлен только для протокола репликации.
...
Суффикс `_readonly` устанавливает [режим только для чтения](../Security/Read_only.md) для слушателя и ограничивает его приемом только запросов на чтение.
...
Директив `listen` может быть несколько. `searchd` будет прослушивать клиентские соединения на всех указанных портах и сокетах. Конфигурация по умолчанию, предоставляемая в пакетах Manticore, определяет прослушивание на портах:
...
<details>
Устаревший протокол Sphinx имеет 2 фазы: обмен подтверждением соединения (handshake) и поток данных. Подтверждение соединения состоит из пакета в 4 байта от клиента и пакета в 4 байта от демона с единственной целью — клиент определяет, что удаленная сторона является настоящим демоном Sphinx, а демон определяет, что удаленная сторона является настоящим клиентом Sphinx. Основной поток данных довольно прост: пусть обе стороны объявят свои подтверждения соединения, а противоположная сторона проверит их. Этот обмен короткими пакетами подразумевает использование специального флага `TCP_NODELAY`, который отключает алгоритм Nagle для TCP и объявляет, что TCP-соединение будет выполняться как диалог небольших пакетов.
Однако, кто говорит первым в этом согласовании, строго не определено. Исторически все клиенты, использующие бинарный API, говорят первыми: отправляют подтверждение соединения, затем читают 4 байта от демона, затем отправляют запрос и читают ответ от демона.
Когда мы улучшали совместимость протокола Sphinx, мы учли следующее:

1. Обычно связь мастер-агент устанавливается от известного клиента к известному хосту на известном порту. Поэтому маловероятно, что конечная точка предоставит неверное подтверждение соединения. Таким образом, мы можем неявно предположить, что обе стороны являются корректными и действительно общаются по протоколу Sphinx.
2. Исходя из этого предположения, мы можем «склеить» подтверждение соединения с реальным запросом и отправить его в одном пакете. Если бэкенд — это устаревший демон Sphinx, он просто прочитает этот склеенный пакет как 4 байта подтверждения соединения, а затем тело запроса. Поскольку они оба пришли в одном пакете, сокет бэкенда имеет -1 RTT, а буфер фронтенда все еще работает обычным образом, несмотря на этот факт.
3. Продолжая предположение: поскольку пакет «запроса» довольно мал, а подтверждение соединения еще меньше, давайте отправим оба в начальном TCP-пакете «SYN», используя современную технику TFO (tcp-fast-open). То есть: мы подключаемся к удаленному узлу со склеенным пакетом подтверждения соединения + тела. Демон принимает соединение и сразу же имеет и подтверждение соединения, и тело в буфере сокета, так как они пришли в самом первом TCP-пакете «SYN». Это устраняет еще один RTT.
...
Для общего понимания расширения TCP Fast Open обратитесь к [Wikipedia](https://en.wikipedia.org/wiki/TCP_Fast_Open). Вкратце, оно позволяет устранить один цикл обмена пакетами TCP при установке соединения.
...
Для Linux сервер проверяет переменную `/proc/sys/net/ipv4/tcp_fastopen` и ведет себя в соответствии с ней. Бит 0 управляет клиентской стороной, бит 1 управляет слушателями. По умолчанию в системе этот параметр установлен в 1, т.е. клиенты включены, слушатели отключены.

### log

<!-- example conf log -->
Настройка log определяет имя файла журнала, в который будут записываться все события времени выполнения `searchd`. Если не указано, имя по умолчанию - 'searchd.log'.
...
Максимально допустимое количество значений на фильтр. Эта настройка используется только для внутренних проверок корректности и не влияет напрямую на использование оперативной памяти или производительность. Необязательный параметр, по умолчанию 4096.
...
Максимальное количество файлов, которые серверу разрешено открывать, называется "мягким лимитом". Обратите внимание, что обслуживание больших фрагментированных таблиц в реальном времени может потребовать установки этого лимита на высокое значение, так как каждый фрагмент на диске может занимать десяток или более файлов. Например, таблица в реальном времени с 1000 фрагментами может потребовать одновременного открытия тысяч файлов. Если в логах возникает ошибка 'Too many open files', попробуйте изменить эту опцию, так как это может помочь решить проблему.
...
Помимо прямых числовых значений, можно использовать волшебное слово 'max', чтобы установить лимит равным доступному текущему жесткому лимиту.
...
Строка версии сервера, возвращаемая по протоколу MySQL. Необязательный параметр, по умолчанию пустая строка (возвращается версия Manticore).
...
Таймаут чтения/записи сетевого запроса клиента, в секундах (или с [специальными суффиксами](../Server_settings/Special_suffixes.md)). Необязательный параметр, по умолчанию 5 секунд. `searchd` принудительно закроет соединение с клиентом, который не успевает отправить запрос или прочитать результат в течение этого таймаута.
...
Эта настройка определяет, разрешать ли запросы, содержащие только оператор [отрицания](../Searching/Full_text_matching/Operators.md#Negation-operator) полнотекстового поиска. Необязательный параметр, по умолчанию 0 (запросы только с оператором NOT завершаются ошибкой).
...
Устанавливает порог по умолчанию для уплотнения таблицы. Подробнее читайте здесь - [Количество оптимизированных дисковых чанков](../Securing_and_compacting_a_table/Compacting_a_table.md#Number-of-optimized-disk-chunks). Эта настройка может быть переопределена с помощью опции для каждого запроса [cutoff](../Securing_and_compacting_a_table/Compacting_a_table.md#Number-of-optimized-disk-chunks). Также её можно изменить динамически с помощью [SET GLOBAL](../Server_settings/Setting_variables_online.md#SET).
...
Стоимости для модели предсказания времени выполнения запроса, в наносекундах. Необязательная, значение по умолчанию: `doc=64, hit=48, skip=2048, match=64`.
...
Прерывание запросов до завершения на основе времени их выполнения (с настройкой максимального времени запроса) — это хорошая защитная сетка, но она имеет врождённый недостаток: недетерминированные (нестабильные) результаты. То есть, если вы повторите один и тот же (сложный) поисковый запрос с ограничением по времени несколько раз, ограничение будет срабатывать на разных этапах, и вы получите *разные* наборы результатов.
...
Нет двух одинаковых серверов по производительности и модели, поэтому директива `predicted_time_costs` позволяет вам настроить стоимости для приведённой выше модели. Для удобства они являются целыми числами, измеряемыми в наносекундах. (Лимит в max_predicted_time измеряется в миллисекундах, и необходимость указывать значения стоимости как 0.000128 мс вместо 128 нс несколько более подвержена ошибкам.) Не обязательно указывать все четыре стоимости сразу, так как пропущенные примут значения по умолчанию. Однако мы настоятельно рекомендуем указывать их все для удобства чтения.
...
* высокая параллельная нагрузка запросов
...
Директива `replication_connect_timeout` определяет таймаут подключения к удаленному узлу. По умолчанию значение указывается в миллисекундах, но оно может иметь [другой суффикс](../Server_settings/Special_suffixes.md). Значение по умолчанию — 1000 (1 секунда).

При подключении к удаленному узлу Manticore будет ждать успешного завершения соединения не более указанного времени. Если таймаут истек, а соединение не установлено, и включены `retries`, будет инициирована повторная попытка.
...
После установления соединения Manticore будет ждать завершения работы удаленного узла не более `replication_query_timeout`. Обратите внимание, что этот таймаут отделен от `replication_connect_timeout`, и общая возможная задержка, вызванная удаленным узлом, будет суммой обоих значений.


### replication_retry_count

Эта настройка представляет собой целое число, определяющее, сколько раз Manticore попытается подключиться и выполнить запрос к удаленному узлу во время репликации, прежде чем сообщить о фатальной ошибке запроса. Значение по умолчанию — 3.
...
Целое число, в миллисекундах. Минимальный порог реального времени для кэширования результата запроса. По умолчанию 3000, или 3 секунды. 0 означает кэшировать всё. Подробности см. в разделе [кэш запросов](../Searching/Query_cache.md). Это значение также может быть выражено с помощью временных [специальных суффиксов](../Server_settings/Special_suffixes.md), но используйте его с осторожностью и не путайте с самим названием значения, содержащим '_msec'.


### qcache_ttl_sec

Целое число, в секундах. Срок действия кэшированного набора результатов. По умолчанию 60, или 1 минута. Минимально возможное значение — 1 секунда. Подробности см. в разделе [кэш запросов](../Searching/Query_cache.md). Это значение также может быть выражено с помощью временных [специальных суффиксов](../Server_settings/Special_suffixes.md), но используйте его с осторожностью и не путайте с самим названием значения, содержащим '_sec'.
...
Ограничение (в миллисекундах), которое предотвращает запись запроса в журнал запросов. Необязательный, по умолчанию 0 (все запросы записываются в журнал запросов). Эта директива указывает, что будут записываться в журнал только запросы, время выполнения которых превышает указанный предел (это значение также может быть выражено с помощью временных [специальных суффиксов](../Server_settings/Special_suffixes.md), но используйте его с осторожностью и не путайте с самим названием значения, содержащим `_msec`).
...
Директива read_buffer_docs управляет размером буфера чтения на ключевое слово для списков документов. Для каждого вхождения ключевого слова в каждом поисковом запросе есть два связанных буфера чтения: один для списка документов и один для списка совпадений. Эта настройка позволяет управлять размером буфера списка документов.
...
Директива read_buffer_hits определяет размер буфера чтения на ключевое слово для списков совпадений в поисковых запросах. По умолчанию размер составляет 256K, минимальное значение — 8K. Для каждого вхождения ключевого слова в поисковом запросе существуют два связанных буфера чтения: один для списка документов и один для списка совпадений. Увеличение размера буфера может увеличить использование оперативной памяти на запрос, но уменьшить время ввода-вывода. Для медленных хранилищ имеет смысл использовать большие размеры буферов, в то время как для хранилищ с высокой производительностью IOPS эксперименты следует проводить в области низких значений.
...
Период проверки сброса RAM-сегментов RT-таблиц в секундах (или [специальные суффиксы](../Server_settings/Special_suffixes.md)). Необязательный параметр, по умолчанию 10 часов.

Активно обновляемые RT-таблицы, которые полностью помещаются в RAM-сегменты, все равно могут приводить к постоянно растущим бинарным журналам, влияя на использование диска и время восстановления после сбоя. С помощью этой директивы поисковый сервер выполняет периодические проверки сброса, и подходящие RAM-сегменты могут быть сохранены, что позволяет выполнить последующую очистку бинарного журнала. Подробнее см. [Бинарное журналирование](../Logging/Binary_logging.md).
...
Максимальное количество операций ввода-вывода (в секунду), которое разрешено запускать потоку слияния RT-сегментов. Необязательный параметр, по умолчанию 0 (без ограничений).

Эта директива позволяет ограничить влияние операций ввода-вывода, возникающих из-за операторов `OPTIMIZE`. Гарантируется, что все операции оптимизации RT не будут генерировать больше операций ввода-вывода на диск (IOPS) в секунду, чем установленный лимит. Ограничение rt_merge_iops может уменьшить снижение производительности поиска, вызванное слиянием.
...
Предотвращает простои `searchd` при ротации таблиц с огромными объемами данных для предварительного кэширования. Необязательный параметр, по умолчанию 1 (включить бесшовную ротацию). В системах Windows бесшовная ротация отключена по умолчанию.

Таблицы могут содержать некоторые данные, которые необходимо предварительно загрузить в оперативную память. В настоящее время файлы `.spa`, `.spb`, `.spi` и `.spm` полностью предзагружаются (они содержат данные атрибутов, данные атрибутов типа blob, таблицу ключевых слов и карту удаленных строк соответственно.) Без бесшовного переключения ротация таблицы пытается использовать как можно меньше оперативной памяти и работает следующим образом:
...
При включенном бесшовном переключении ротация работает следующим образом:
...
Бесшовное переключение достигается ценой более высокого пикового использования памяти во время ротации (поскольку данные `.spa/.spb/.spi/.spm` как старой, так и новой копии должны находиться в оперативной памяти во время предзагрузки новой копии). Среднее использование остается прежним.
...
Эта опция задает размер кэша блоков, используемого вторичными индексами. Она является необязательной, по умолчанию составляет 8 МБ. Когда вторичные индексы работают с фильтрами, содержащими много значений (например, фильтры IN()), они читают и обрабатывают метаданные блоков для этих значений.
В объединенных запросах этот процесс повторяется для каждой партии строк из левой таблицы, и каждая партия может перечитывать одни и те же метаданные в рамках одного объединенного запроса. Это может серьезно повлиять на производительность. Кэш блоков метаданных хранит эти блоки в памяти, чтобы они
...
Целое число, которое служит идентификатором сервера, используемым как seed для генерации уникального короткого UUID для узлов, являющихся частью кластера репликации. server_id должен быть уникальным для всех узлов кластера и находиться в диапазоне от 0 до 127. Если server_id не задан, он вычисляется как хэш MAC-адреса и пути к PID-файлу, или в качестве seed для короткого UUID будет использовано случайное число.
...
После построения окончательного пути к файлу сервер разворачивает все относительные директории и сравнивает конечный результат со значением `snippet_file_prefix`. Если результат не начинается с этого префикса, такой файл будет отклонён с сообщением об ошибке.
...
Также обратите внимание, что это локальная опция; она никак не влияет на агентов. Таким образом, вы можете безопасно установить префикс на главном сервере. Запросы, перенаправленные на агентов, не будут затронуты настройкой главного сервера. Однако на них повлияют собственные настройки агента.

Это может быть полезно, например, когда расположения хранилищ документов (будь то локальное хранилище или точки монтирования NAS) не согласованы между серверами.
...
> **ВНИМАНИЕ:** Если вы всё же хотите получить доступ к файлам из корня файловой системы, вы должны явно установить `snippets_file_prefix` в пустое значение (строкой `snippets_file_prefix=`), либо в корень (строкой `snippets_file_prefix=/`).
...
Максимальный размер стека для задачи (сопрограммы, один поисковый запрос может вызвать несколько задач/сопрограмм). Необязательный параметр, значение по умолчанию — 128K.
...
Определяет, удалять ли копии таблиц `.old` при успешном ротировании. Необязательный параметр, значение по умолчанию — 1 (удалять).
...
Когда запрос Manticore завершается аварийно, он может привести к падению всего сервера. При включенной функции наблюдателя `searchd` также поддерживает отдельный легковесный процесс, который отслеживает состояние основного серверного процесса и автоматически перезапускает его в случае аномального завершения. Наблюдатель включен по умолчанию.
