...
Эта настройка определяет, сколько рабочих потоков используется для построения графа HNSW во время операций, интенсивно использующих HNSW, на таблицах, содержащих атрибут `float_vector` с KNN-индексом. Несколько путей используют этот регулятор:
* **Фаза сохранения чанка (chunk-save store pass)**: когда RAM-чанк сбрасывается на диск, рабочие потоки разделяют RAM-сегменты чанка между собой и параллельно вызывают `addPoint` из hnswlib.
* **Фаза слияния чанков (chunk-merge store pass)**: `OPTIMIZE TABLE` и автооптимизация строят граф HNSW целевого чанка параллельно, разделяя диапазон живых строк из всех входных чанков между рабочими потоками.
* **ALTER KNN rebuild**: `ALTER TABLE ... ADD COLUMN`/`DROP COLUMN` для атрибута `float_vector` и `ALTER TABLE ... REBUILD KNN` перегенерируют файл `.spknn` параллельно для дисковых чанков.

Это влияет только на построение графа HNSW для таблиц с KNN-атрибутами.

Установите значение `1`, чтобы отключить параллельное построение KNN (все фазы сохранения выполняются последовательно). Более высокие значения ускоряют сохранение чанков, `OPTIMIZE` и перестроение ALTER на таблицах, где построение графа HNSW доминирует во времени выполнения операции.
...
Обратите внимание, что при [`parallel_chunk_merges`](#parallel_chunk_merges) > 1 несколько слияний могут выполняться одновременно, и каждое потребляет до `knn_parallel_build` рабочих потоков.

По умолчанию Manticore выводит значение из настройки [threads](../Server_settings/Searchd.md#threads): `max(1, min(4, threads/4))`. То есть `1` (последовательно), когда `threads` меньше 8, `2` — когда `threads` <= 11, `3` — когда `threads` <= 15, и `4` — когда 16 или больше (ограничено 4 по умолчанию). Операторы с более мощными хостами, желающие большего параллелизма, могут установить значение явно.
...
**Требования:**
`pid_file` является необязательным, если `searchd` запускается с опциями `--console`, `--nodetach` или `--systemd`, или если автоматически обнаружено управление через systemd. Во всех остальных случаях этот параметр обязателен. Он также требуется, если используется опция командной строки `--pidfile`.
