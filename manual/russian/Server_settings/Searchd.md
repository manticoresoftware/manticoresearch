...
Если вставки с автоматическим созданием эмбеддингов выполняются медленно — каждая строка занимает сотни миллисекунд, а при массовой загрузке появляются тайм-ауты и повторные попытки создания эмбеддингов, — причина почти всегда в модели, а не в числе потоков: локальные модели без файла `.onnx` работают по гораздо более медленному CPU-пути. См. [выбор локальной модели для эмбеддингов](../Searching/KNN.md#Choosing-a-local-embedding-model). Увеличение этого параметра не сделает медленную модель быстрой и может лишить параллельные поисковые запросы рабочих потоков.
...
Каталог уже должен существовать. `searchd` сам создает каталог binlog по умолчанию (в RT-режиме это `binlog/` внутри [data_dir](../Server_settings/Searchd.md#data_dir)), но не каталог, заданный через `binlog_path`. Если такого каталога нет, `searchd` не запускается и пишет в лог `FATAL: failed to open '<binlog_path>/binlog.lock': 2 'No such file or directory'`. Сначала создайте каталог или не задавайте `binlog_path`, чтобы использовать значение по умолчанию.
