...
Эта настройка полезна, когда Manticore должен явно избегать модификации хранилища таблиц, например, при обслуживании обычных таблиц с реплицированной файловой системы только для чтения. В этом режиме Manticore может обслуживать обычные таблицы из хранилища только для чтения, пропускать создание lock-файлов, если файловая система этого не позволяет, и игнорировать ожидающие артефакты `.new` обычных таблиц с предупреждением, если текущие файлы таблиц всё ещё пригодны для использования.

Эта настройка не заставляет режим RT (`data_dir`) работать на файловой системе только для чтения. `data_dir` по-прежнему требует доступного для записи хранилища.
