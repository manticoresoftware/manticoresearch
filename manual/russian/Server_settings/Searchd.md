...
Этот параметр ограничивает число CPU-потоков, которые Manticore использует при преобразовании текста в векторы. Он применяется всякий раз, когда выполняется автоматическое построение эмбеддингов: при вставке строк в таблицу, использующую `model_name`/`from`, при перестроении столбца `float_vector` с автоэмбеддингом через `ALTER TABLE`, а также когда поиск `knn(<field>, '<text>', ...)` передаёт запрос в виде текста.

Фактическое число используемых потоков также ограничено тем, сколько воркеров свободно в данный момент, поэтому на загруженном сервере потоков будет меньше, даже если лимит высокий. Используйте этот параметр, чтобы один большой пакет эмбеддингов не лишал ресурсов параллельные поисковые запросы.

По умолчанию значение равно `4`. Установите `0`, чтобы снять ограничение; в этом случае библиотека embeddings сама решит, сколько потоков использовать (по-прежнему в пределах числа свободных воркеров).

Это значение можно изменить во время работы с помощью `SET GLOBAL embeddings_threads = N` и посмотреть через `SHOW VARIABLES`. Для запросов `SELECT` с KNN его также можно переопределить для конкретного запроса с помощью `OPTION embeddings_threads = N` (см. [KNN-векторный поиск](../Searching/KNN.md#KNN-vector-search)).
