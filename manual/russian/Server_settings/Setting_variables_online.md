...
* `ro = {1 | 0}` переключает сессию в режим только чтения или обратно. В выводе `show variables` переменная отображается с именем `session_read_only`. Если включен `searchd.read_only`, сессии автоматически начинаются в режиме только чтения, и `SET ro=0` отклоняется.
