...
*   `--watchdog` (только Linux): включает внутренний [watchdog](Server_settings/Searchd.md#watchdog), переопределяя любые настройки в файле конфигурации. Примечание: `--watchdog` несовместим с режимом foreground; если используется `--console` или `--nodetach`, watchdog неявно отключается, и выдается предупреждение.

*   `--no-watchdog` (только Linux): отключает внутренний [watchdog](Server_settings/Searchd.md#watchdog). Это переопределяет любые настройки в файле конфигурации и эквивалентно установке `watchdog=0` в конфиге.

*   `--systemd` (только Linux): принуждает `searchd` работать так, будто он управляется systemd. По умолчанию `searchd` проверяет возможность сообщить systemd о своем состоянии; если коммуникация возможна, он предполагает управление через systemd. В этом режиме watchdog отключен (если не явно включен), и конфигурация `pid_file` не строго необходима. Эта опция явно принуждает режим управления через systemd.

*   `--no-systemd` (только Linux): отключает режим управления через systemd и предотвращает предположение `searchd` о любом внешнем управлении. Следовательно, процесс запускается с watchdog включенным (если не явно отключен) и записывает свой PID в файл, указанный опцией `pid_file` в конфигурации.
