...
* `--check` проверяет конфигурационный файл, подтверждает возможность запуска сервера с ним и завершается. Он выводит `OK` и завершается с кодом 0, если проверка успешна. Если проверка неудачна, он завершается с кодом 1 и выводит ту же ошибку, которую `searchd` вывел бы при обычном запуске.
