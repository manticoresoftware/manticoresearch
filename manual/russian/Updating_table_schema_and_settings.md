...
В plain mode сначала обновите конфигурацию таблицы и выполните `ALTER TABLE <table_name> RECONFIGURE`. В RT mode измените настройку с помощью `ALTER TABLE`, как описано выше. Перед созданием дампа остановите запись в таблицу или убедитесь, что документы, изменённые во время дампа и восстановления, тоже переиндексируются.
...
`-t` опускает операторы `DROP` и `CREATE TABLE`, а `--replace` заставляет дамп заменять документы с совпадающими ID. Каждый восстановленный документ индексируется с текущими полнотекстовыми настройками. Таблица должна хранить каждое полнотекстовое поле: `mysqldump` не может создавать резервные копии таблиц с не-хранимыми полями. Для реплицируемых таблиц используйте [инструкции по резервному копированию в режиме репликации](Securing_and_compacting_a_table/Backup_and_restore.md#Backup-and-restore-with-mysqldump).
