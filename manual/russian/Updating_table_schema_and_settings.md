...
* Нельзя удалить столбец (полнотекстовое поле или строковый атрибут), который используется как источник эмбеддингов (указан в `FROM`) для атрибута `float_vector`. Будет ошибка: `cannot drop column 'X': used as embedding source (FROM) by float_vector attribute 'Y'`. Сначала удалите столбец float_vector.
...
`REBUILD EMBEDDINGS` заполняет пустые столбцы `float_vector` (эмбеддинги) в существующих строках, используя настроенную модель эмбеддингов и источник `FROM`. Используйте это, когда вы добавили новый столбец `float_vector` с `MODEL_NAME` и `FROM` — существующие строки по умолчанию получают нулевой вектор; запустите `REBUILD EMBEDDINGS`, чтобы сгенерировать эмбеддинги из исходного текста (например, из полей или строковых атрибутов).

Строки, в которых вы явно устанавливаете вектор в `()` или `NULL` (явно пустой), отображаются как `NULL` и исключаются из векторного поиска; `REBUILD EMBEDDINGS` не заполняет их.

Обрабатываются только построчные атрибуты `float_vector`, у которых настроены модель и `FROM`. Столбцовые атрибуты эмбеддингов не поддерживаются. Команда обрабатывает атрибуты частями (до 1000 документов на пакет) и повторяет, пока не останется пустых эмбеддингов, поэтому новые строки, вставленные во время выполнения, могут быть обработаны в следующем проходе.

Другие запросы и операции (например, `SELECT`, `INSERT`, `REPLACE`) могут выполняться во время работы `REBUILD EMBEDDINGS`; блокировка записи индекса удерживается только во время каждого пакета обновления и окончательной перестройки KNN-индекса. Прогресс виден в `SHOW THREADS` (столбец `Info`).
