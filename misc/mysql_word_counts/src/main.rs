...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if from_indextool {
        ...
        let mut keyword_idx: Option<usize> = None;
        let mut hits_idx: Option<usize> = None;

        for line in input.lines() {
            let line = line?;
            if !in_data {
                let header = line.trim();
                if header.starts_with("keyword,docs,hits,offset") {
                    let cols: Vec<&str> = header.split(',').collect();
                    keyword_idx = cols.iter().position(|c| *c == "keyword");
                    hits_idx = cols
                        .iter()
                        .position(|c| *c == "hits_eff")
                        .or_else(|| cols.iter().position(|c| *c == "hits"));
                    in_data = true;
                }
                continue;
            }
            ...
            let keyword = match keyword_idx.and_then(|i| fields.get(i)) {
                Some(v) if !v.is_empty() => *v,
                _ => continue,
            };
            let hits = match hits_idx.and_then(|i| fields.get(i)) {
                Some(v) => *v,
                None => continue,
            };
