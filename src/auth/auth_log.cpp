...
private:
	LogLevel_e m_eMinLevel = LogLevel_e::AUTH_DISABLED;
	bool m_bLogStdout = false;
	bool m_bLogSyslog = false;
	int m_iLogFile = -1;

	void WriteEntry ( LogLevel_e eLevel, char * sBuf, int iLen );
	void LogVA ( LogLevel_e eLevel, const char * sFmt, ... );
};
...
void AuthLogger_c::Init ( const CSphString & sDaemonLogPath, const CSphConfigSection & hSearchd, bool bLogStdout, bool bLogSyslog )
{
	...
	{
		m_eMinLevel = LogLevel_e::AUTH_DISABLED;
		return;
	} else if ( sLogLevel=="info" )
	{
		m_eMinLevel = LogLevel_e::AUTH_INFO;
	} else if ( sLogLevel=="warning" )
	{
		m_eMinLevel = LogLevel_e::AUTH_WARNING;
	} else if ( sLogLevel=="error" )
	{
		m_eMinLevel = LogLevel_e::AUTH_ERROR;
	} else
	{
...
{
	LogVA ( LogLevel_e::AUTH_INFO, "user '%s' created by '%s' via %s", sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}

void AuthLogger_c::UserDeleted ( const AuthActionContext_t & tCtx, const CSphString & sTargetUser )
{
	LogVA ( LogLevel_e::AUTH_INFO, "user '%s' deleted by '%s' via %s", sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}

void AuthLogger_c::UserPasswordChanged ( const AuthActionContext_t & tCtx, const CSphString & sTargetUser )
{
	LogVA ( LogLevel_e::AUTH_INFO, "password for user '%s' changed by '%s' via %s", sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}

void AuthLogger_c::UserTokenChanged ( const AuthActionContext_t & tCtx, const CSphString & sTargetUser )
{
	LogVA ( LogLevel_e::AUTH_INFO, "bearer token for user '%s' regenerated by '%s' via %s", sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}
...
{
	LogVA ( LogLevel_e::AUTH_INFO, "granted %s on '%s' to user '%s' by '%s' via %s", sAction.cstr(), sActionTarget.cstr(), sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}
...
void AuthLogger_c::PermissionRevoked ( const AuthActionContext_t & tCtx, const CSphString & sTargetUser, const CSphString & sAction, const CSphString & sActionTarget )
{
	if ( !sAction.IsEmpty() && !sActionTarget.IsEmpty() )
		LogVA ( LogLevel_e::AUTH_INFO, "revoked %s on '%s' to user '%s' by '%s' via %s", sAction.cstr(), sActionTarget.cstr(), sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
	else if ( !sAction.IsEmpty() )
		LogVA ( LogLevel_e::AUTH_INFO, "revoked %s to user '%s' by '%s' via %s", sAction.cstr(), sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
	else if ( !sActionTarget.IsEmpty() )
		LogVA ( LogLevel_e::AUTH_INFO, "revoked on '%s' to user '%s' by '%s' via %s", sActionTarget.cstr(), sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
	else
		LogVA ( LogLevel_e::AUTH_INFO, "revoked a permission to user '%s' by '%s' via %s", sTargetUser.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
}

void AuthLogger_c::AuthSuccess ( const CSphString & sUser, AccessMethod_e eMethod, const char * sRemoteAddr )
{
	LogVA ( LogLevel_e::AUTH_INFO, "user '%s' successfully authenticated via %s from %s", sUser.cstr(), GetMethodName ( eMethod ), sRemoteAddr );
}
...
void AuthLogger_c::AuthFailure ( const CSphString & sAttemptedUser, AccessMethod_e eMethod, const char * sRemoteAddr, const char * sReason )
{
	if ( eMethod!=AccessMethod_e::HTTP_BEARER )
		LogVA ( LogLevel_e::AUTH_WARNING, "failed authentication attempt for user '%s' via %s from %s: %s", sAttemptedUser.cstr(), GetMethodName ( eMethod ), sRemoteAddr, sReason );
	else
		LogVA ( LogLevel_e::AUTH_WARNING, "failed authentication attempt for bearer '%s' via %s from %s: %s", sAttemptedUser.cstr(), GetMethodName ( eMethod ), sRemoteAddr, sReason );
}
...
{
	LogVA ( LogLevel_e::AUTH_ERROR, "user '%s' from %s denied %s on '%s'", sAttemptedUser.cstr(), sRemoteAddr.cstr(), GetActionName ( eAction ), sTarget.scstr() );
}

void AuthLogger_c::ClusterJoinOverride ( const CSphString & sClusterName )
{
	LogVA ( LogLevel_e::AUTH_CRITICAL, "node joining cluster '%s', local auth data is being overridden", sClusterName.cstr() );
}

void AuthLogger_c::ClusterBackupData ( const CSphString & sJsonData, const CSphString & sClusterName, const CSphString & sNodeName )
{
	LogVA ( LogLevel_e::AUTH_INFO, "joined cluster '%s' via node '%s' local auth data backup: %s", sClusterName.cstr(), sNodeName.cstr(), sJsonData.cstr() );
}

void AuthLogger_c::Reload ( const AuthActionContext_t & tCtx, const CSphString & sTrigger )
{
	if ( sTrigger.IsEmpty() )
		LogVA ( LogLevel_e::AUTH_INFO, "authentication data reloaded successfully by '%s' via %s", tCtx.m_sActorUser.cstr(), tCtx.m_sSource );
	else
		LogVA ( LogLevel_e::AUTH_INFO, "authentication data reloaded successfully by '%s' via %s, trigger: %s", tCtx.m_sActorUser.cstr(), tCtx.m_sSource, sTrigger.cstr() );
}
...
{
	if ( sTarget.IsEmpty() )
		LogVA ( LogLevel_e::AUTH_WARNING, "failed to %s by '%s' via %s. Error: %s", sAction.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource, sReason.cstr() );
	else
		LogVA ( LogLevel_e::AUTH_WARNING, "failed to %s on %s by '%s' via %s. Error: %s", sAction.cstr(), sTarget.cstr(), tCtx.m_sActorUser.cstr(), tCtx.m_sSource, sReason.cstr() );
}
...
void AuthLogger_c::WriteEntry ( LogLevel_e eLevel, char * sBuf, int iLen )
{
#if USE_SYSLOG
	// syslog if enabled
	if ( m_bLogSyslog )
...
void  AuthLogger_c::LogVA ( LogLevel_e eLevel, const char * sFmt, ... )
{
	...
	switch ( eLevel )
	{
		case LogLevel_e::AUTH_INFO:		sLevelBanner = "INFO"; break;
		case LogLevel_e::AUTH_WARNING:	sLevelBanner = "WARN"; break;
		case LogLevel_e::AUTH_ERROR:	sLevelBanner = "ERROR"; break;
		case LogLevel_e::AUTH_CRITICAL:	sLevelBanner = "CRITICAL"; break;
		case LogLevel_e::AUTH_DISABLED: return;
	}
