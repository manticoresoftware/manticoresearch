...
TEST ( Text, expression_parser )
{
	...
	for ( auto &dTest: dTests )
	{
		...
		ISphExprRefPtr_c pExpr ( sphExprParse ( dTest.m_sExpr, tSchema, nullptr, nullptr, sError, tExprArgs ) );
...
TEST ( Text, expression_parser_many )
{
	...
	for ( const auto & sTest : dTests )
	{
		CSphString sError;
		ExprParseArgs_t tExprArgs;
		ISphExprRefPtr_c pExpr ( sphExprParse ( sTest.cstr (), tSchema, nullptr, nullptr, sError, tExprArgs ) );
