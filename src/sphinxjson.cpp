...
void sphJsonFormat ( CSphVector<BYTE> & dOut, const BYTE * pData )
{
	...
	for ( ;; )
	{
		...
		switch ( eType )
		{
			...
			{
				char sBuf[16];
				snprintf ( sBuf, sizeof(sBuf), "%d", sphJsonLoadInt ( &p ) );
				for ( char * p = sBuf; *p; p++ )
					dOut.Add ( *p );
				break;
			}
			...
			{
				char sBuf[32];
				snprintf ( sBuf, sizeof(sBuf), "%lld", sphJsonLoadBigint ( &p ) );
				for ( char * p = sBuf; *p; p++ )
					dOut.Add ( *p );
				break;
			}
			...
			{
				char sBuf[32];
				snprintf ( sBuf, sizeof(sBuf), "%f", sphQW2D ( sphJsonLoadBigint ( &p ) ) );
				for ( char * p = sBuf; *p; p++ )
					dOut.Add ( *p );
				break;
			}
			...
			case JSON_STRING_VECTOR:
			{
				sphJsonUnpackInt ( &p ); // skip total length
