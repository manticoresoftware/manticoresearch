...
int RtSegment_t::GetMergeFactor () const
{
	return m_iRows;
}
...
const CSphFixedVector<SphDocID_t> &RtSegment_t::GetKlist() const { return m_pKlist->m_dKilled; }
...
const RtDoc_t * RtDocReader_t::UnzipDoc ()
{
	...
	m_tDoc.m_uDocID += ( SphDocID_t) uDeltaID;
	DWORD uField;
...
const RtWord_t * RtWordReader_t::UnzipWord ()
{
	...
	} else
	{
		...
		m_tWord.m_uWordID += ( SphWordID_t) uDeltaID;
	}
...
void RtIndex_t::SaveDiskDataImpl ( const char * sFilename, const SphChunkGuard_t & tGuard, const ChunkStats_t & tStats ) const
{
	...
	ARRAY_FOREACH ( i, tGuard.m_dRamChunks )
		pWordReaders.Add ( new RtWordReader_t ( tGuard.m_dRamChunks[i], m_bKeywordDict, m_iWordsCheckpoint ) );
	...
	while (true)
	{
		...
		ARRAY_FOREACH ( i, pWords )
				...
				sphDictCmpStrictly ( (const char *)pWords[i]->m_sWord+1, *pWords[i]->m_sWord, (const char *)pWord->m_sWord+1, *pWord->m_sWord )==0 ) ) )
			{
				...
				pDocReaders.Add ( new RtDocReader_t ( tGuard.m_dRamChunks[i], *pWords[i] ) );
...
struct PercolateMatchContext_t
{
	...
	PercolateMatchContext_t ( const RtSegment_t * pSeg, int iMaxCodepointLength, bool bHasMorph,
		const PercolateIndex_c * pIndex, const ISphSchema & tSchema, const SegmentReject_t & tReject )
...
static void PercolateMergeResults ( VecTraits_T<PercolateMatchContext_t *> & dMatches, PercolateMatchResult_t & tRes )
{
	if ( dMatches.IsEmpty() )
		return;
...
//		&& pQuery->m_dFilters[0].m_sAttrName=="uid"
...
//			auto uDocid = ( SphDocID_t ) pQuery->m_dFilters[0].GetValue ( i );
//			const DWORD * pRow = nullptr; //FindDocinfo ( uDocid );
...
//		GetQueries ( sFilterTags, bTagsEq, pUID, iOffset, iLimit, dQueries );
...
/*		for ( int64_t iIndexEntry = iStart; iIndexEntry!=iEnd; iIndexEntry += iStep )
		{
			...
			if ( !tCtx.m_pOverrides && tCtx.m_pFilter && !pQuery->m_iCutoff && !tCtx.m_dCalcFilter.GetLength ()
				&& !tCtx.m_dCalcSort.GetLength () && !tmMaxTimer )
			{
				// kinda fastpath
...
//		PooledAttrsToPtrAttrs ( ppSorters, iSorters, m_tMva.GetWritePtr (), m_tString.GetWritePtr ()
//								, m_bArenaProhibit );
...
/*
	...
	if ( m_pFieldFilter )
	{
		pFieldFilter = m_pFieldFilter->Clone ();
		if ( pFieldFilter
			&& pFieldFilter->Apply ( sModifiedQuery, strlen ( ( char * ) sModifiedQuery ), dFiltered, true ) )
			sModifiedQuery = dFiltered.Begin ();
	}
	...
	XQQuery_t tParsed;
	if ( !pQueryParser->ParseQuery ( tParsed, ( const char * ) sModifiedQuery, pQuery, m_pQueryTokenizer
									 , m_pQueryTokenizerJson, &m_tSchema, pDict, m_tSettings ) )
	{
	...
	*/
	...
	if ( tArgs.m_bModifySorterSchemas )
		PooledAttrsToPtrAttrs ( &dSorters[0], iSorters, m_tMva.GetWritePtr (), m_tString.GetWritePtr ()
								, m_bArenaProhibit );
