...
public:
	...
	std::vector<std::unique_ptr<columnar::Iterator_i>>	m_dFieldLenIters;
...
public:
	...
	void FinalizeDocFactors ( const CSphMatch & tMatch )
	{
		...
		// Read field lengths from per-segment columnar iterators when *_len attrs are columnar.
		// Otherwise fall back to the legacy row-locator path used for rowwise token-count attrs.
...
struct Expr_BM25F_T : public Expr_NoLocator_c
{
	...
	float Eval ( const CSphMatch & tMatch ) const final
	{
		...
		for ( int i=0; i<m_pState->m_iFields; i++ )
		{
			if ( m_pState->m_dFieldLenIters[i] )
				m_dFieldLens[i] = (int)m_pState->m_dFieldLenIters[i]->Get ( tMatch.m_tRowID );
			else if ( tLoc.m_iBitOffset>=0 )
			{
				m_dFieldLens[i] = (int)tMatch.GetAttr ( tLoc );
				tLoc.m_iBitOffset += 32;
			}
		}
...
bool RankerState_Expr_fn<NEED_PACKEDFACTORS, HANDLE_DUPES>::ExtraDataImpl ( ExtraData_e eType, void ** ppResult )
{
	...
	switch ( eType )
	{
		...
		case EXTRA_SET_COLUMNAR:
			...
			if ( auto * pColumnar = (const columnar::Columnar_i*)*ppResult )
			{
