...
$port = intval(getenv('MANTICORE_PORT') ?: '9306');
...
$insertTypes = $mysqli->prepare("INSERT INTO ps_types (id, title, i, b, ts, f, s, j, m, m64, vec) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
if (!$insertTypes) {
...
$updateTypes = $mysqli->prepare("UPDATE ps_types SET m = TO_MULTI(?), m64 = TO_MULTI(?), vec = TO_VECTOR(?) WHERE id = ?");
if (!$updateTypes) {
...
$badTypes = $mysqli->prepare("UPDATE ps_types SET m = TO_MULTI(?), m64 = TO_MULTI(?), vec = TO_VECTOR(?) WHERE id = ?");
if (!$badTypes) {
